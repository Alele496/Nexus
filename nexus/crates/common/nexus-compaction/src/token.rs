...
//!
//! - Sage chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `GrokTurn::get_num_tokens`.
//! - sage-build estimates with `bytes / 4`.
//!
...
//! (Sage chat: `ItemTokenCounter<Arc<GrokTurn>>`), so exactly one mechanism
//! is in play.
