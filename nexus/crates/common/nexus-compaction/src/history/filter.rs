...
//!
//! Behavior is byte-for-byte identical for Sage chat (`T = Arc<GrokTurn>`).
...
///   summaries so their `<nexus_user_queries>` sections can be split out.
...
/// Extracts **all** `<nexus_user_queries>...</nexus_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<nexus_user_queries>";
    let end_tag = "</nexus_user_queries>";
...
/// Extract a `<nexus_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<nexus_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<nexus_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<nexus_user_queries>\n");
    ...
    result.push_str("</nexus_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<nexus_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<nexus_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<nexus_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<nexus_user_queries>` block.
...
/// `<nexus_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<nexus_user_queries>` from earlier rounds.
...
///   would see the prior `<nexus_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<nexus_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<nexus_user_queries>"));
        assert!(out.ends_with("</nexus_user_queries>"));
        assert!(out.contains("<nexus_query>hello world</nexus_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner = "<nexus_user_queries>\n<nexus_query>first</nexus_query>\n</nexus_user_queries>";
        let inner2 = "<nexus_user_queries>\n<nexus_query>second</nexus_query>\n</nexus_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<nexus_user_queries>\n<nexus_query>Q1</nexus_query>\n</nexus_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<nexus_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<nexus_user_queries>"));
    ...
    fn separate_drops_summary_item_with_no_rest() {
        let only_queries = "<nexus_user_queries>\n<nexus_query>Q</nexus_query>\n</nexus_user_queries>";
