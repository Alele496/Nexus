//! Compacted-history assembly (sage-build's rebuild structure, generic).
//!
//! Moved from `sage-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! sage-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // sage-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn nexus_build_order_recent_before_summary() {
