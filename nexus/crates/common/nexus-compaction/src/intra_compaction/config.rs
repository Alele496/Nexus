...
///
/// - `FullReplace` (default): sage-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from sage-shell's
...
/// time (token threshold only, matching sage-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as sage-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<nexus_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "sage-4.20";
