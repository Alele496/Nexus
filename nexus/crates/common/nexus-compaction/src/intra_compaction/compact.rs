...
// The `Shared` summarizer reuses sage-build's full-replace summarization core
...
/// `FullReplace` strategy (default): sage-build's full-replace — summarize the
...
/// `<nexus_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching sage-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through sage-build's shared core.
    ...
    //     Shared with Sage chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<nexus_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<nexus_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<nexus_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): sage-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<nexus_user_queries>` preamble so
...
/// — sage-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and sage-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // sage-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // sage-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Sage chat's
...
/// the literal error messages produced by the Sage chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Sage chat sampler when the
    ...
    /// — guards the forwarding that Sage chat (`Arc<GrokTurn>`) relies on.
    #[test]
