...
use nexus_tool_protocol::{McpBlock, SessionId, ToolId, ToolOutputWire};
use nexus_tool_runtime::{ToolCallContext, ToolError, ToolStream, TypedToolOutput, terminal_only};
...
/// [`nexus_computer_hub_sdk::ToolServerBuilder`] to register them
/// with the hub.
///
...
impl McpBridge {
    ...
    /// Handlers to register with a [`nexus_computer_hub_sdk::ToolServerBuilder`].
    ///
...
#[async_trait]
impl nexus_computer_hub_sdk::ToolServerHandler for McpToolHandler {
...
mod tests {
    ...
    async fn bridge_handles_transport_error() {
        ...
        let item = stream.next().await.unwrap();
        match item {
            nexus_tool_runtime::ToolStreamItem::Terminal(Err(ref e))
                if e.kind == nexus_tool_runtime::ToolErrorKind::Execution =>
            {
                assert!(
