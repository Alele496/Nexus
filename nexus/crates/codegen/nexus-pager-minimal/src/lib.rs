//! Minimal (scrollback-native) render mode — `sage --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the
//! terminal's *native* scrollback (via `nexus_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `sage-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`nexus_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`sage-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `sage-pager`.
///
...
/// subsequent calls are ignored (see [`nexus_pager::minimal_hook`]).
pub fn install() {
    nexus_pager::minimal_hook::install(nexus_pager::minimal_hook::MinimalHooks { draw });
}
