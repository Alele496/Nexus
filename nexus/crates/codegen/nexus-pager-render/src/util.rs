...
pub use nexus_config::nexus_home;

/// Path to `$NEXUS_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    nexus_home().join("pager.toml")
}

/// User-facing label for the user sage directory (``~/.nexus`` or ``$NEXUS_HOME``).
///
/// Derived from resolved [`nexus_home()`] vs `nexus_config::default_nexus_home()`,
/// not from whether `NEXUS_HOME` is set in the environment.
pub fn display_nexus_home_prefix() -> String {
    if nexus_home() == nexus_config::default_nexus_home() {
        "~/.nexus".to_string()
    } else {
...
/// User-facing path under [`nexus_home()`], e.g. ``~/.nexus/config.toml``.
pub fn display_user_nexus_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_nexus_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`nexus_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let sage = nexus_home();
    if let Ok(rest) = path_buf.strip_prefix(&sage) {
        let prefix = display_nexus_home_prefix();
...
/// True when `path` is under user [`nexus_home()`] (not project `{cwd}/.nexus`).
pub fn is_under_user_nexus_home(path: &Path) -> bool {
    path.starts_with(nexus_home())
}
...
mod tests {
    ...
    #[test]
    fn display_nexus_home_prefix_default_install() {
        if std::env::var("NEXUS_HOME").is_ok() {
            return;
        }
        assert_eq!(display_nexus_home_prefix(), "~/.nexus");
    }

    #[test]
    fn display_user_nexus_path_joins_relative() {
        let path = display_user_nexus_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".nexus") || path.contains("$NEXUS_HOME"));
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.nexus/memory/MEMORY.md");
