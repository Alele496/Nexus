//! Detects whether sage is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` sage is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `sage-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
