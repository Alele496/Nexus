...
impl TerminalContext {
    ...
    pub fn telemetry_snapshot(&self) -> nexus_telemetry::events::TerminalTelemetry {
        ...
        nexus_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: nexus_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> nexus_shared::session::FeedbackTerminalInfo {
        use nexus_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(nexus_shared::clipboard::native_tool_name().to_owned()),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.nexus/pager.toml` and
