...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("NEXUS_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "NEXUS_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    nexus_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// The returned command is TTY-guarded via [`nexus_tty_utils::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    nexus_tty_utils::detach_std_command(&mut command);
    command
}
...
///
/// Used by the SuperGrok upsell flow to attribute clicks to `referrer=sage-build`,
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/sage session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://sage.local/supergrok", "referrer", "sage-build");
        assert_eq!(out, "https://sage.local/supergrok?referrer=sage-build");
    }

    #[test]
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://sage.local/supergrok?referrer=other",
            "referrer",
            "sage-build",
        );
        assert_eq!(out, "https://sage.local/supergrok?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://sage.local/supergrok?heavy=1",
            "referrer",
            "sage-build",
        );
        assert_eq!(
            out,
            "https://sage.local/supergrok?heavy=1&referrer=sage-build"
        );
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        // routing (`sage.local/#supergrok`); we still want the referrer attached.
        let out = ensure_query_param("https://sage.local/#supergrok", "referrer", "sage-build");
        assert_eq!(out, "https://sage.local/?referrer=sage-build#supergrok");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "sage-build");
    ...
    fn browser_unavailable_message_includes_full_url() {
        let url = "https://sage.local/supergrok?referrer=sage-build";
