...
use tokio::sync::mpsc;
use nexus_sampling_types::{ConversationItem, SamplingConfig};
...
async fn record_last_turn_usage_round_trip() {
    use nexus_sampling_types::TokenUsage;
...
async fn build_request_includes_all_messages() {
    ...
    assert_eq!(request.x_nexus_conv_id, Some("conv-1".to_string()));
    assert_eq!(request.x_nexus_req_id, Some("req-1".to_string()));
}
...
async fn build_request_with_tool_definitions() {
    use nexus_sampling_types::ToolSpec;
...
async fn build_request_uses_sampling_config() {
    ...
    assert_eq!(request.model, Some("sage-3".to_string()));
...
/// Simulates the exact sequence that `sage-shell`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools =
        ConversationItem::Assistant(nexus_sampling_types::AssistantItem {
...
async fn get_first_user_text_image_first_returns_none() {
    use nexus_sampling_types::{ContentPart, UserItem};
...
async fn sampling_config_survives_compaction_replacement() {
    use nexus_sampling_types::ApiBackend;
    ...
    assert_eq!(pre.model, "sage-build");
    ...
    assert_eq!(pre_meta.resolved_model_id.as_deref(), Some("sage-4.5"));
...
/// If that model slug is e.g. "sage-build" and not in the ModelState
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            nexus_sampling_types::AssistantItem {
...
async fn context_window_downgrade_triggers_auto_compact() {
    ...
    // Initial config: 500k context, Responses backend (matches sage-4.5)
    let config = SamplingConfig {
    ...
    assert_eq!(post.model, "sage-4.5", "model slug must not change");
    assert_eq!(
        post.api_backend,
        ApiBackend::Responses,
...
fn serialize_via_public_api(
    req: &nexus_sampling_types::ConversationRequest,
) -> serde_json::Value {
    ...
    nexus_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &nexus_sampling_types::ConversationRequest,
    extended: &nexus_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_image_pruning() {
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(nexus_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(nexus_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn prefix_stable_with_backend_tool_calls() {
    use nexus_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
...
// ============================================================================
// Out-of-band history repair (sage.local/session/repair)
// ============================================================================
