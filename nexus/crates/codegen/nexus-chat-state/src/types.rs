...
/// emitter in `sage-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (sage login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
...
/// `nexus_sampling_types::SamplingConfig` (which is secret-free).
