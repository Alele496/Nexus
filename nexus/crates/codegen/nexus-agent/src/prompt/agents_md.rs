...
//!
//! Searches from cwd to repo root, plus `~/.nexus/`. Also discovers
//! `*.md` files in rules directories: vendor-prefixed `.nexus/rules/`,
...
//! (`~/.nexus/rules/`, `~/.claude/rules/`, `~/.cursor/rules/`).
...
/// Find `*.md` files in `.nexus/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.nexus/, git repo root, and session cwd.
...
async fn read_agents_config_with_roots(
    ...
    add_discovery_root(&mut home_roots, nexus_home, true, HOME_RULES_DIRS);
    ...
    candidates
        ...
        .filter_map(|candidate| {
            ...
            let content = if candidate.is_rule {
                nexus_tools::implementations::skills::skill::extract_skill_body(&content)
            } else {
                content
            };
...
/// Used by `sage-shell` to structurally detect legacy untagged AGENTS.md
...
/// Open/close `system-reminder` (Sage) or `system_reminder` (Cursor/IDE), case-insensitive.
...
mod tests {
    ...
    async fn home_and_project_rules_have_stable_order_without_doubled_paths() {
        ...
        for path in [
            nexus_home.join(".nexus/rules/doubled.md"),
    ...
    async fn vendor_home_agents_and_rules_cells_are_independent() {
        let tmp = tempfile::tempdir().unwrap();
        let nexus_home = tmp.path().join("sage-home");
        let home = tmp.path().join("home");
        let cwd = tmp.path().join("project");
        fs::create_dir_all(&nexus_home).unwrap();
    ...
    #[tokio::test]
    async fn nested_nexus_home_keeps_project_role_in_repo_order() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        let nested = repo.join("nested");
        fs::create_dir_all(nested.join("rules")).unwrap();
        fs::create_dir_all(nested.join(".nexus/rules")).unwrap();
        init_git_repo(&repo);
        ...
        fs::write(nested.join(".nexus/rules/project.md"), "nested-project-rule").unwrap();
    ...
    #[tokio::test]
    async fn overlapping_nexus_home_and_project_root_merges_roles() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        ...
        fs::write(repo.join(".nexus/rules/project.md"), "project-sage-rule").unwrap();
        fs::write(repo.join(".claude/rules/project.md"), "project-claude-rule").unwrap();
        fs::create_dir_all(repo.join(".nexus/.nexus/rules")).unwrap();
        fs::write(repo.join(".nexus/.nexus/rules/doubled.md"), "doubled").unwrap();
        ...
        .await;
        for expected in ["home-rule", "project-sage-rule", "project-claude-rule"] {
            assert_eq!(
                configs
                    .iter()
    ...
    async fn rule_frontmatter_is_stripped_but_named_frontmatter_is_preserved() {
        ...
        for (path, body) in [
            (nexus_home.join("rules/global.md"), "custom-home-body"),
            (home.join(".claude/rules/global.md"), "claude-body"),
            (home.join(".cursor/rules/global.md"), "cursor-body"),
            (repo.join(".nexus/rules/project.md"), "sage-project-body"),
