...
//! - `Default` -- the legacy Sage prefix (built by the shell layer).
...
use nexus_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub const USER_MESNEXUS_DATE_FORMAT: &str = "%A %b %-d, %Y";
...
pub enum UserMessageTemplate {
    /// Legacy Sage prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .nexus/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `nexus_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// inside the renderer using [`USER_MESNEXUS_DATE_FORMAT`] so the producer
    ...
    /// User-scoped rule files (~/.nexus/, ~/.claude/).
    ...
    /// (`~/.nexus/projects/<encoded-cwd>/mcps`). Surfaced in
...
struct UserMessagePlaceholders<'a> {
    ...
    /// Pre-formatted using [`USER_MESNEXUS_DATE_FORMAT`]; `None` is rendered as
...
impl UserMessageContext {
    ...
    fn placeholders(&self) -> UserMessagePlaceholders<'_> {
        UserMessagePlaceholders {
            ...
            today_local: self
                .today_local
                .map(|d| d.format(USER_MESNEXUS_DATE_FORMAT).to_string()),
