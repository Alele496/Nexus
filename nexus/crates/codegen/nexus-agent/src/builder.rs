...
use std::sync::Arc;
use nexus_tools::bridge::ToolBridge;
use nexus_tools::computer::types::{AsyncFileSystem, TerminalBackend};
...
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    nexus_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.nexus/worktrees/project/fork-...-overlay`) that must stay
    ...
    memory_backend: Option<Arc<dyn nexus_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: nexus_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: nexus_tools::implementations::nexus_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn nexus_tools::implementations::lsp::LspBackend>>,
    image_gen_config: nexus_tools::implementations::nexus_build::image_gen::ImageGenConfig,
    video_gen_config: nexus_tools::implementations::nexus_build::video_gen::VideoGenConfig,
    app_builder_deployer_config:
        nexus_tools::implementations::nexus_build::deploy_app::AppBuilderDeployerConfig,
    ...
    compat: nexus_tools::types::compat::CompatConfig,
    ...
    api_key_provider: Option<nexus_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<nexus_tools::SharedAttributionCallback>,
    ...
    preloaded_skills: Option<Vec<nexus_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut nexus_tools::registry::types::ToolServerConfig) {
    use nexus_tools::implementations::nexus_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("NexusBuild:enter_plan_mode");
    let missing_exit = !existing.contains("NexusBuild:exit_plan_mode");
    let missing_ask = !existing.contains("NexusBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&nexus_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&nexus_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&nexus_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut nexus_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(nexus_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<nexus_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn nexus_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: nexus_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `NEXUS_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: nexus_tools::implementations::nexus_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn nexus_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: nexus_tools::implementations::nexus_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: nexus_tools::implementations::nexus_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: nexus_tools::implementations::nexus_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: nexus_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Without this, only auto-discovered skills (cwd/.nexus/skills, ~/.nexus/skills)
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use nexus_tools::implementations::nexus_build;
                tool_config.tools.push((&nexus_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use nexus_tools::implementations::nexus_build;
                tool_config.tools.push((&nexus_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&nexus_tools::implementations::nexus_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&nexus_tools::implementations::nexus_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&nexus_tools::implementations::nexus_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&nexus_tools::implementations::nexus_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&nexus_tools::implementations::nexus_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&nexus_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let nexus_build_ns = nexus_tools::types::tool::ToolNamespace::SageBuild.to_string();
            let mem_search_id = format!(
                "{nexus_build_ns}:{}",
                nexus_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{nexus_build_ns}:{}",
                nexus_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                nexus_tools::types::tool::ToolNamespace::SageBuild,
            );
        ...
        if task_stripped {
            use nexus_tools::types::tool::ToolNamespace;
        ...
        if let nexus_tools::implementations::nexus_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["NexusBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "NexusBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                nexus_tools::types::resources::TruncationCfg(
                    nexus_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(nexus_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(nexus_sampling_types::HostedTool::XSearch);
            }
        }
...
/// CLI naming for the shared [`nexus_tool_types::build_task_description`] builder.
const TASK_TOOL_NAMING: nexus_tool_types::TaskToolNaming<'static> = nexus_tool_types::TaskToolNaming {
...
/// CLI [`nexus_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: nexus_tool_types::SubagentToolNaming<'static> =
    nexus_tool_types::SubagentToolNaming {
...
/// shared [`nexus_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => nexus_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => nexus_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => nexus_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`nexus_tool_types::SubagentDescriptor`] and defers to
/// [`nexus_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<nexus_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            nexus_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description = nexus_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        nexus_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: nexus_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(nexus_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(nexus_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: nexus_tools::types::config_source::ConfigSource::Project {
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use nexus_tools::types::template_renderer::TemplateRenderer;
        use nexus_tools::types::tool::ToolKind;
    ...
    async fn build_pager_agent(
        ...
        use nexus_tools::computer::local::LocalTerminalBackend;
        use nexus_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                ...
                profile: AgentDefinition::default_nexus_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
            ...
            PagerFlagCase {
                ...
                profile: AgentDefinition::nexus_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
            ...
            PagerFlagCase {
                ...
                profile: AgentDefinition::nexus_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
            ...
            PagerFlagCase {
                ...
                profile: AgentDefinition::nexus_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use nexus_tools::computer::local::LocalTerminalBackend;
        use nexus_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_nexus_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use nexus_tools::computer::local::LocalTerminalBackend;
        use nexus_tools::implementations::nexus_build::ask_user_question::AskUserQuestionParams;
        use nexus_tools::notification::ToolNotificationHandle;
        use nexus_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_nexus_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "NexusBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use nexus_tools::computer::local::LocalTerminalBackend;
        use nexus_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_nexus_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("NexusBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use nexus_tools::computer::local::LocalTerminalBackend;
        use nexus_tools::implementations::nexus_build::bash::BashParams;
        use nexus_tools::notification::ToolNotificationHandle;
        use nexus_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_nexus_build();
    ...
    #[tokio::test]
    async fn claude_tool_names_map_to_nexus_equivalents() {
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use nexus_tools::computer::local::LocalTerminalBackend;
        use nexus_tools::implementations::nexus_build::web_fetch::WebFetchConfig;
        use nexus_tools::implementations::web_search::WebSearchConfig;
        use nexus_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_nexus_build();
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, nexus_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, nexus_sampling_types::HostedTool::XSearch)),
