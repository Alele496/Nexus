...
//! primitive consumed cross-crate by `sage-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.nexus/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.nexus`/`.mcp.json`/plugins would look repo-local. Drop
