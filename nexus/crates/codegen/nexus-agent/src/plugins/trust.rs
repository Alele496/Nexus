...
//!
//! Plugins from project directories (`.nexus/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.nexus/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.nexus/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.nexus/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_nexus_home() so a project's `.nexus/trusted-plugins` is never
        // read as the user trust store when neither NEXUS_HOME nor a home dir resolves.
        let Some(sage) = nexus_config::user_nexus_home() else {
            return Self {
                trusted: HashSet::new(),
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.nexus/trusted-plugins`.
    ...
    /// rewrites `~/.nexus/trusted-plugins` without the revoked entry.
