//! Agent definition types — parsed from `.nexus/agents/*.md` files.
...
use nexus_tools::implementations::codex;
use nexus_tools::implementations::nexus_build;
use nexus_tools::implementations::nexus_build_concise;
use nexus_tools::implementations::memory;
use nexus_tools::implementations::opencode;
use nexus_tools::implementations::search_tool;
use nexus_tools::implementations::use_tool;
use nexus_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard SageBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&nexus_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&nexus_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&nexus_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&nexus_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&nexus_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_nexus_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_nexus_build_toolset() -> ToolServerConfig {
    let mut tools = default_nexus_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&nexus_build::EnterPlanModeTool).into());
    tools.push((&nexus_build::ExitPlanModeTool).into());
    tools.push((&nexus_build::AskUserQuestionTool).into());
    tools.push((&nexus_build::WebSearchTool).into());
    tools.push((&nexus_build::ImageGenTool).into());
    tools.push((&nexus_build::ImageToVideoTool).into());
    tools.push((&nexus_build::ReferenceToVideoTool).into());
    tools.push((&nexus_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&nexus_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `sage-computer` (workspace/sandbox) preset.
fn nexus_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&nexus_build::ReadFileTool).into(),
        (&nexus_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&nexus_build::ListDirTool).into(),
        (&nexus_build::GrepTool).into(),
        (&nexus_build::KillTerminalCommandTool).into(),
        (&nexus_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("sage-build", workspace_nexus_build_toolset()),
        ("sage-build-concise", nexus_build_concise_toolset()),
        ("sage-build-plan", nexus_build_plan_toolset()),
        ("codex", codex_toolset()),
...
fn default_nexus_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&nexus_build::ReadFileTool).into(),
            ...
            (&nexus_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&nexus_build::SchedulerCreateTool).into(),
            (&nexus_build::SchedulerDeleteTool).into(),
            (&nexus_build::SchedulerListTool).into(),
            (&nexus_build::SendMessageTool).into(),
            (&nexus_build::CheckMailboxTool).into(),
            (&nexus_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&nexus_build::UpdateGoalTool).into(),
        ],
        behavior_preset: None,
    }
}
fn nexus_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&nexus_build_concise::BashConciseTool).into(),
            (&nexus_build_concise::ReadFileConciseTool).into(),
            (&nexus_build_concise::SearchReplaceConciseTool).into(),
            (&nexus_build::ListDirTool).into(),
            (&nexus_build::GrepTool).into(),
            kill_task_tool_config(),
            (&nexus_build::TodoWriteTool).into(),
...
pub fn nexus_build_hashline_toolset(
    hashline_tools: Vec<nexus_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<nexus_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    tools.extend([
        ...
        wait_tasks_tool_config(),
        task_tool_config(),
        (&nexus_build::WebSearchTool).into(),
        (&nexus_build::SchedulerCreateTool).into(),
        (&nexus_build::SchedulerDeleteTool).into(),
        (&nexus_build::SchedulerListTool).into(),
        (&nexus_build::SendMessageTool).into(),
...
/// Sage + plan mode toolset.
///
/// Extends the default `sage-build` toolset with plan mode tools:
...
fn nexus_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&nexus_build::ReadFileTool).into(),
            ...
            (&use_tool::UseTool).into(),
            (&nexus_build::UpdateGoalTool).into(),
            (&nexus_build::EnterPlanModeTool).into(),
            (&nexus_build::ExitPlanModeTool).into(),
            (&nexus_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            (&nexus_build::MonitorTool).into(),
            (&nexus_build::WebSearchTool).into(),
            (&nexus_build::WebFetchTool).into(),
            (&nexus_build::ImageGenTool).into(),
            (&nexus_build::ImageToVideoTool).into(),
            (&nexus_build::ReferenceToVideoTool).into(),
...
/// Sage + plan mode toolset WITHOUT subagent tools.
///
/// Same as `nexus_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn nexus_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&nexus_build::ReadFileTool).into(),
...
/// Default Sage toolset + `ask_user_question`.
///
/// Same as `default_nexus_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn nexus_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&nexus_build::ReadFileTool).into(),
...
/// - `model: sage-3-fast` → `Override("sage-3-fast")`
...
pub enum BuiltinAgentName {
    ...
    Plan,
    BrowserUse,
    #[strum(serialize = "sage-build-orchestrator")]
    SageBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::SageBuild => AgentDefinition::default_nexus_build(),
            Self::SageBuildConcise => AgentDefinition::nexus_build_concise(),
            Self::SageBuildPlan => AgentDefinition::nexus_build_plan(),
            Self::SageBuildPlanNoSubagents => AgentDefinition::nexus_build_plan_no_subagents(),
            Self::SageBuildAskUser => AgentDefinition::nexus_build_ask_user(),
            ...
            Self::SageBuildOrchestrator => AgentDefinition::nexus_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .nexus/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_nexus_build_toolset")]
    ...
    pub capability_mode: Option<nexus_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
...
pub enum AgentScope {
    /// .nexus/agents/ (project-level, highest priority)
    Project,
    /// ~/.nexus/agents/ (user-level)
    User,
    /// ~/.nexus/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_nexus_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.nexus/agent-memory/<name>/`
    User,
    /// `<project>/.nexus/agent-memory/<name>/`
    Project,
    /// `<project>/.nexus/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: nexus_config::nexus_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".nexus/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".nexus/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `nexus_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let sage = nexus_config::user_nexus_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), sage.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".nexus/agents/") || path_str.contains(".nexus\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".nexus/bundled/agents/")
            || path_str.contains(".nexus\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `sage-build*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<nexus_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["NexusBuild:read_file", "NexusBuildHashline:hashline_read"],
            [
                "NexusBuild:search_replace",
                "NexusBuildHashline:hashline_edit",
            ],
            ["NexusBuild:grep", "NexusBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_nexus_build_toolset(),
    ...
    pub fn default_nexus_build() -> Self {
        Self::base(
            BuiltinAgentName::SageBuild,
            "Nexus agent for software engineering tasks.",
        )
    }
    /// Sage Concise agent definition — concise output format for SFT/RL.
    pub fn nexus_build_concise() -> Self {
        Self {
            tool_config: nexus_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::SageBuildConcise,
                "Nexus agent with concise output format.",
            )
        }
    }
    /// Sage agent with plan mode tools.
    pub fn nexus_build_plan() -> Self {
        Self {
            tool_config: nexus_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::SageBuildPlan,
                "Nexus agent with plan mode support.",
            )
        }
    }
    /// Sage + plan mode WITHOUT subagent tools.
    pub fn nexus_build_plan_no_subagents() -> Self {
        Self {
            tool_config: nexus_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::SageBuildPlanNoSubagents,
                "Nexus agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Sage agent with the `ask_user_question` tool.
    pub fn nexus_build_ask_user() -> Self {
        Self {
            tool_config: nexus_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::SageBuildAskUser,
                "Nexus agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: nexus_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: nexus_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: nexus_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    /// Sage Orchestrator — GBL model with full SageBuild tools
    ...
    pub fn nexus_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::SageBuildOrchestrator,
                "NexusBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_nexus_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("sage-build").unwrap();
    ...
    fn nexus_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&nexus_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&nexus_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn nexus_computer_preset_is_curated_nexus_build_subset() {
        let gc = toolset_for_preset("sage-computer").unwrap();
        let gb = toolset_for_preset("sage-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = nexus_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "sage-computer tool `{}` must also ship in the sage-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "sage-computer should be a curated subset of sage-build"
        );
    }
    #[test]
    fn nexus_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("sage-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&nexus_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&nexus_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&nexus_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&nexus_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&nexus_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&nexus_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&nexus_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The sage-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the sage-build
    ...
    #[test]
    fn nexus_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("sage-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "sage-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn nexus_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("sage-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&nexus_build::LspTool).id,
            ToolConfig::from(&nexus_build::EnterPlanModeTool).id,
            ToolConfig::from(&nexus_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "sage-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_nexus_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_nexus_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::SageBuildOrchestrator => true,
            BuiltinAgentName::SageBuild
            | BuiltinAgentName::SageBuildConcise
            | BuiltinAgentName::SageBuildPlan
            | BuiltinAgentName::SageBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "sage-build-orchestrator"] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("sage-3-fast".to_string()).to_string(),
            "sage-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("sage-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.nexus/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.nexus/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_nexus_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have nexus_build tools"
        );
    }
    #[test]
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"sage-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("sage-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("sage-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"sage-3-fast\"");
    }
    #[test]
    fn test_model_override_in_frontmatter() {
        ...
        assert_eq!(
            def.model,
            ModelOverride::Override("sage-3-fast".to_string())
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!(
            { "name" : "test", "description" : "Test", "model" : "sage-code-fast-1" }
        );
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("sage-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("sage-build", BuiltinAgentName::SageBuild),
            ("sage-build-concise", BuiltinAgentName::SageBuildConcise),
            ("sage-build-ask-user", BuiltinAgentName::SageBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_nexus_build();
