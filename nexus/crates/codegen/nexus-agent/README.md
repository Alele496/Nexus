...
This crate extracts a first-class `Agent` type from `sage-shell`.
...
`sage-shell`, another in-process host, or a headless batch runner.
...
in `.sage/agents/` (project-level) or `~/.sage/agents/` (user-level).

```rust
use sage_agent::{AgentDefinition, AgentBuilder};
use sage_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".sage/agents/code-reviewer.md")?;
...
```rust
use sage_agent::discovery;

// Find all .md files in .sage/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  sage-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`sage-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `sage-agent` depends on it for tool setup.
- **`sage-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `sage-agent` (AGENTS.md
...
|---|---|---|
| `sage-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p sage-agent

# Test
cargo test -p sage-agent

# Clippy
cargo clippy -p sage-agent --fix --allow-dirty
