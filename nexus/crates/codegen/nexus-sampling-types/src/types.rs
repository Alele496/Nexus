...
pub struct ChatCompletionRequest {
    ...
    /// DeepSeek thinking mode: `{"type": "enabled"}` enables chain-of-thought.
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// DeepSeek V4 thinking configuration.
...
// Re-export ToolDefinition and FunctionTool from sage-tools.
...
pub use nexus_tools::types::definition::{FunctionTool, ToolDefinition};
...
impl ReasoningEffort {
    ...
    /// DeepSeek reasoning_effort value: maps to "high" or "max" only.
    pub fn as_deepseek_str(self) -> &'static str {
        match self {
            Self::Xhigh => "max",
            _ => "high",
        }
...
mod tests {
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(json!({ "agentType": "sage" }).as_object().unwrap()))
                .is_none()
        );
    }
