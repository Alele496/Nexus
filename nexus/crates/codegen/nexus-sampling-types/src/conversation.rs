...
use crate::types::{
    ...
    ImageUrl, MessageContent, Role, ThinkingConfig, ToolCallRequest, ToolChoice,
...
pub struct ConversationRequest {
    ...
    pub x_nexus_conv_id: Option<String>,
    pub x_nexus_req_id: Option<String>,
    pub x_nexus_session_id: Option<String>,
    pub x_nexus_turn_idx: Option<String>,
    pub x_nexus_agent_id: Option<String>,
    pub x_nexus_deployment_id: Option<String>,
    pub x_nexus_user_id: Option<String>,
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-sage-doom-loop-check` header). Empty when the
...
// ---------------------------------------------------------------------------
//
// Part of the Sage Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/sage-compaction` operate over sage-build's
...
impl nexus_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> nexus_compaction::CompactionRole {
        use nexus_compaction::CompactionRole;
        // sage-build has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // sage-build has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing sage-build history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<nexus_compaction::CompactionFileRef> {
        // sage-build `UserItem`s carry only `Text`/`Image { url }` content
        ...
        // image attachments need to survive into the `<nexus_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl nexus_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/sage-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`sage-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/sage-sampler/src/stream/chat_completions.rs])
...
impl From<ConversationRequest> for ChatCompletionRequest {
    fn from(req: ConversationRequest) -> Self {
        ...
        // DeepSeek thinking mode: when reasoning_effort is set (and not explicitly
        // "None"), use the `thinking` field (DeepSeek-native) instead of OpenAI's
        // `reasoning_effort`. Sending both causes DeepSeek to reject the request.
        // Also omit `temperature` — DeepSeek ignores it in thinking mode.
        let (thinking, reasoning_effort) = match req.reasoning_effort {
            Some(effort) if !matches!(effort, crate::ReasoningEffort::None) => {
                (Some(ThinkingConfig::enabled()), None)
            }
            _ => (None, None),
        };
        let temperature = thinking.is_none().then_some(req.temperature).flatten();
        ChatCompletionRequest {
            ...
            reasoning_effort,
            thinking,
            x_nexus_conv_id: req.x_nexus_conv_id,
            x_nexus_req_id: req.x_nexus_req_id,
            x_nexus_session_id: req.x_nexus_session_id,
            x_nexus_turn_idx: req.x_nexus_turn_idx,
            x_nexus_agent_id: req.x_nexus_agent_id,
            x_nexus_deployment_id: req.x_nexus_deployment_id,
            x_nexus_user_id: req.x_nexus_user_id,
            trace: None,
        }
    }
}
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_nexus_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_nexus_req_id = Some(req_id.into());
        self
    }
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // output without tools. The sage-shell agent does NOT use this path — a
...
/// ([crates/codegen/sage-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use nexus_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // sage-build has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_conversation_item_roundtrip() {
        ...
        let assistant = ConversationItem::assistant_with_model("Hi there!", "sage-3");
    ...
    fn test_conversation_request_to_chat_completion() {
        ...
        assert_eq!(chat_req.model, Some("sage-3".to_string()));
    ...
    fn test_conversation_request_to_responses_api() {
        ...
        assert_eq!(responses_req.model, Some("sage-3".to_string()));
    ...
    fn test_responses_api_response_to_conversation_item() {
        ...
        assert_eq!(a.model_id, Some("sage-3".to_string()));
        assert_eq!(
    ...
    fn test_assistant_with_content_and_tool_calls() {
        ...
        let assistant = AssistantItem {
            ...
            model_id: Some("sage-3".to_string()),
            model_fingerprint: None,
            reasoning_effort: None,
        };
        ...
        assert_eq!(chat_msg.tool_calls.len(), 1);
        assert_eq!(chat_msg.model_id, Some("sage-3".to_string()));
    }
    ...
    #[test]
    fn test_chat_completion_request_uses_thinking_not_reasoning_effort() {
        // When reasoning_effort is set, the conversion now emits the DeepSeek-native
        // `thinking: {"type": "enabled"}` field *instead* of `reasoning_effort`.
        // ReasoningEffort::None (meaning "explicitly disabled") still suppresses thinking.
        for (variant, expect_thinking) in [
            (crate::ReasoningEffort::None, false),
            (crate::ReasoningEffort::Minimal, true),
            (crate::ReasoningEffort::Low, true),
            (crate::ReasoningEffort::Medium, true),
            (crate::ReasoningEffort::High, true),
            (crate::ReasoningEffort::Xhigh, true),
        ] {
            ...
            // reasoning_effort must never appear at the top level – DeepSeek
            // rejects OpenAI-specific reasoning_effort.
            assert!(
                json.get("reasoning_effort").is_none(),
                "{variant:?}: reasoning_effort must be absent; got: {json:#}",
            );
            if expect_thinking {
                assert_eq!(
                    json.pointer("/thinking/type").and_then(|v| v.as_str()),
                    Some("enabled"),
                    "{variant:?}: thinking.type must be \"enabled\"; got: {json:#}",
                );
            } else {
                assert!(
                    json.get("thinking").is_none(),
                    "{variant:?}: thinking must be absent; got: {json:#}",
                );
            }
        }
    }
    ...
    fn test_btw_stripped_reasoning_produces_no_thinking_blocks() {
        ...
        // `strip_reasoning_blocks` in sage-chat-state. Here the assistant
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.nexus/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.nexus/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.nexus/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.model, Some("sage-3".to_string()));
        assert_eq!(req.temperature, Some(0.5));
        assert_eq!(req.max_output_tokens, Some(1000));
        assert_eq!(req.x_nexus_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_nexus_req_id, Some("req-456".to_string()));
    ...
    fn test_assistant_item_with_model_id() {
        ...
        .with_model_id("sage-3");

        assert_eq!(item.model_id, Some("sage-3".to_string()));
    }

    #[test]
    fn test_conversation_item_with_model_id() {
        let item = ConversationItem::assistant("Hello").with_model_id("sage-3");
        ...
        let user = ConversationItem::user("Hi").with_model_id("sage-3");
    ...
    fn responses_api_conversion_preserves_model_fingerprint() {
        ...
        assert_matches!(item, ConversationItem::Assistant(ref a) => {
            ...
            assert_eq!(a.model_id.as_deref(), Some("sage-4.5"));
    ...
    //      (earlier sage-build / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_nexus_build_shape() {
