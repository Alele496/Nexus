...
//! `nexus_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `sage-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(
            ...
            Some(".nexus/personas/concise.md")
        );
        assert_eq!(persona.model.as_deref(), Some("sage-3-fast"));
