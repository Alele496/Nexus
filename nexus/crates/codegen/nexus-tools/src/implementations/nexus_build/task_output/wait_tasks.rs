...
use crate::implementations::nexus_build::task_output::{
...
use nexus_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for WaitTasksTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            nexus_tool_types::build_wait_tasks_description(&nexus_tool_types::WaitTasksToolNaming {
...
/// [`nexus_tool_types::build_wait_tasks_description`] builder so the CLI and
...
impl nexus_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let timeout =
            crate::implementations::nexus_build::task_output::capped_wait_timeout(input.timeout_ms);
