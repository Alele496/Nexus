...
use nexus_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
/// Re-export of [`nexus_tool_types::is_not_sentinel`] for existing call sites.
pub use nexus_tool_types::is_not_sentinel;
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "NexusBuild:run_terminal_cmd" | "NexusBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
register_resource!("nexus_build", "SubagentEventSender", SubagentEventSender);
...
pub type MonitorEventBuffer = nexus_interjection_core::EventQueue<MonitorEventNotification>;

register_resource!("nexus_build", "MonitorEventBuffer", MonitorEventBuffer);
...
register_resource!("nexus_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("nexus_build", "SessionIdResource", SessionIdResource);
...
register_resource!("nexus_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("NexusBuild:run_terminal_cmd", ToolKind::Execute),
                tc("NexusBuild:read_file", ToolKind::Read),
                tc("NexusBuild:list_dir", ToolKind::List),
                tc("NexusBuild:grep", ToolKind::Search),
                tc("NexusBuild:kill_task", ToolKind::KillTaskAction),
                tc("NexusBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("NexusBuild:kill_task", ToolKind::KillTaskAction),
                tc("NexusBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("NexusBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("NexusBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
