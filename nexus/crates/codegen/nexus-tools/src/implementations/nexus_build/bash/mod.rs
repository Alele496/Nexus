...
use regex::Regex;
use nexus_config::shell::AmpersandSemantics;
...
static BASH_CAPABILITIES: LazyLock<nexus_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| nexus_tool_protocol::ToolCapabilities {
        is_read_only: false,
        tool_scope: Some(nexus_tool_protocol::ToolScope::Write),
        streaming: Some(nexus_tool_protocol::StreamingSpec {
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "nexus_build.Bash";
...
impl nexus_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<nexus_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => nexus_tool_runtime::ToolOutput::chat_completion_output(bash),
...
/// [`nexus_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `NEXUS_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("NEXUS_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in nexus_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the nexus_build
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`NEXUS_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `NEXUS_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
impl nexus_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    async fn execute(
        &self,
        mut ctx: nexus_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> nexus_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield nexus_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield nexus_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    maybe = rx.recv() => {
                        match maybe {
                            Some(ToolNotification::BashOutputChunk(chunk)) => {
                                ...
                                {
                                    yield nexus_tool_runtime::ToolStreamItem::Progress(p);
                                }
                            }
                            Some(notif) => {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield nexus_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, nexus_tool_runtime::ToolError> {
        ...
        let ampersand = nexus_config::shell::ampersand_semantics();
        if let Some(violation) = should_reject_background_op(
            ...
            return Err(nexus_tool_runtime::ToolError::invalid_arguments(message));
        }
        ...
        if input.is_background && !background_enabled {
            return Err(nexus_tool_runtime::ToolError::invalid_arguments(
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing nexus_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (nexus_build backend) and can surface an educational
...
mod tests {
    ...
    fn read_chunk_progress(p: &nexus_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
            nexus_tool_runtime::ToolProgress::Custom { subkind, payload } => {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let nexus_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let tool = BashTool;
        let mut stream = nexus_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, nexus_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                nexus_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn foreground_command_success() {
        ...
        let result = nexus_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn background_injects_python_unbuffered() {
        ...
        let _ = nexus_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn errors_when_terminal_not_in_resources() {
        ...
        let result =
            nexus_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), make_input("ls"))
                .await;
        assert!(result.is_err());
        assert!(
            result
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = nexus_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(nexus_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod versioned_background_check {
        ...
        ) -> nexus_tool_runtime::ToolCallContext {
            let mut ctx =
                nexus_tool_runtime::ToolCallContext::new(nexus_tool_protocol::ToolCallId::new_v7());
            ctx.extensions.insert(resources);
            ctx.extensions.insert(nexus_tool_runtime::BehaviorVersion(
