...
impl nexus_tool_runtime::Tool for BashTool {
    ...
    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("bash").expect("valid tool id")
    }
        ...
        let result = match backend.run(request).await {
            ...
            Err(e) => {
                ...
                return Err(nexus_tool_runtime::ToolError::execution(
                    nexus_tool_protocol::ToolId::new("run_terminal_command").expect("valid"),
                    e.to_string(),
                ));
            }
        };
...
mod tests {
    ...
    fn rt_ctx_with_call_id(
        ...
        let id = nexus_tool_protocol::ToolCallId::new(call_id).unwrap();
    ...
    async fn tool_metadata() {
        use crate::types::tool_metadata::ToolMetadata;
        let tool = BashTool;
        assert_eq!(nexus_tool_runtime::Tool::id(&tool).as_str(), "bash");
    ...
    //
    // Same shape as nexus_build/bash: the opencode tool inherits the same
