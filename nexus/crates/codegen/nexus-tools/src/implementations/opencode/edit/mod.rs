...
use crate::implementations::nexus_build::search_replace::CONTEXT_LINES;
use crate::implementations::nexus_build::search_replace::helpers::{
...
// the snake_case `params.edit.old_string` keys of the nexus_build twin resolve
...
impl nexus_tool_runtime::Tool for EditTool {
    type Args = EditInput;
    type Output = SearchReplaceOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("edit").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: EditInput,
    ) -> Result<SearchReplaceOutput, nexus_tool_runtime::ToolError> {
...
async fn ensure_parent_dirs(path: &std::path::Path) -> Result<(), nexus_tool_runtime::ToolError> {
    if let Some(parent) = path.parent()
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            nexus_tool_runtime::ToolError::execution(
                nexus_tool_protocol::ToolId::new("edit").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
// are imported from `nexus_build::search_replace::helpers` — shared across
// both the nexus_build and opencode edit tools.
...
mod tests {
    ...
    fn tool_id_and_kind() {
        ...
        let tool = EditTool;
        assert_eq!(nexus_tool_runtime::Tool::id(&tool).as_str(), "edit");
