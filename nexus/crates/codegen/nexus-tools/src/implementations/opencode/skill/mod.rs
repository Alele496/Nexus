...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.nexus/skills/ or .nexus/skills/)
${%- endif %}
</available_skills>"#;
...
impl nexus_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, nexus_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(nexus_tool_runtime::Tool::id(&tool).as_str(), "skill");
