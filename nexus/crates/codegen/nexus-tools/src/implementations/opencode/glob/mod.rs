...
impl nexus_tool_runtime::ToolOutput for GlobOutput {}
...
impl nexus_tool_runtime::Tool for GlobTool {
    type Args = GlobInput;
    type Output = GlobOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("glob").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: GlobInput,
    ) -> Result<GlobOutput, nexus_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = GlobTool;
        assert_eq!(nexus_tool_runtime::Tool::id(&tool).as_str(), "glob");
    ...
    async fn gitignore_respected() {
        ...
        nexus_test_utils::git::ensure_hermetic_git_on_path();
