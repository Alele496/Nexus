...
use nexus_tool_types::SubagentCompletedOutput;
...
use crate::implementations::nexus_build::todo::{TodoItem, TodoState};
...
pub(crate) fn typed_tool_output_preserving_cco(
    ...
    source: &impl nexus_tool_runtime::ToolOutput,
) -> nexus_tool_runtime::TypedToolOutput {
    let cco = source.chat_completion_output();
    let value = serde_json::to_value(payload).unwrap_or(serde_json::Value::Null);
    nexus_tool_runtime::TypedToolOutput::from_value(tool_id, value).with_chat_completion_output(cco)
}
...
pub struct BashOutput {
    ...
    /// Set by the nexus_build `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the nexus_build backend.
...
use nexus_tool_types::KillTaskOutput;
...
impl nexus_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<nexus_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => nexus_tool_runtime::ToolOutput::chat_completion_output(bash),
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::nexus_build::monitor::types::MonitorOutput),
    SchedulerCreate(crate::implementations::nexus_build::scheduler::create::SchedulerCreateOutput),
...
impl nexus_tool_runtime::ToolOutput for ToolOutput {
    fn chat_completion_output(&self) -> Option<nexus_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => nexus_tool_runtime::ToolOutput::chat_completion_output(bash),
            _ => None,
...
impl nexus_tool_runtime::ToolOutput for BashOutput {
    fn chat_completion_output(&self) -> Option<nexus_tool_runtime::ToolChatCompletionResponse> {
        ...
        Some(nexus_tool_runtime::ToolChatCompletionResponse {
            result: Some(nexus_tool_runtime::ToolChatCompletion {
                sender: "assistant".into(),
                message_tag: Some("raw_function_result".into()),
                code_execution_result: Some(nexus_tool_runtime::ToolCodeExecutionResult {
                    stdout,
                    stderr: String::new(),
...
impl nexus_tool_runtime::ToolOutput for GrepSearchOutput {}
impl nexus_tool_runtime::ToolOutput for ReadFileOutput {}
impl nexus_tool_runtime::ToolOutput for ListDirOutput {}
impl nexus_tool_runtime::ToolOutput for SearchReplaceOutput {}
impl nexus_tool_runtime::ToolOutput for TodoWriteOutput {}
impl nexus_tool_runtime::ToolOutput for WebSearchOutput {}
impl nexus_tool_runtime::ToolOutput for WebFetchOutput {}
impl nexus_tool_runtime::ToolOutput for SkillOutput {}
impl nexus_tool_runtime::ToolOutput for ApplyPatchOutput {}
impl nexus_tool_runtime::ToolOutput for CodexGrepFilesOutput {}
impl nexus_tool_runtime::ToolOutput for SearchToolOutput {}
impl nexus_tool_runtime::ToolOutput for EnterPlanModeOutput {}
impl nexus_tool_runtime::ToolOutput for ExitPlanModeOutput {}
impl nexus_tool_runtime::ToolOutput for AskUserQuestionOutput {}
impl nexus_tool_runtime::ToolOutput for MCPOutput {}
#[cfg(test)]
mod tests {
    use super::*;
    use crate::implementations::nexus_build::todo::{TodoPriority, TodoStatus};
    use serde_json::json;
    use nexus_tool_types::KillTaskResult;
    use nexus_tool_types::TaskOutputResult;
    ...
    fn assert_cer(
        resp: &nexus_tool_runtime::ToolChatCompletionResponse,
        stdout: &str,
        exit_code: i32,
        timed_out: bool,
    ) {
    ...
    fn bash_output_chat_completion_carries_exit_and_stdout() {
        let resp = nexus_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(
            0, b"hello\n", false,
        ))
        .unwrap();
    ...
    fn bash_output_chat_completion_empty_stdout_still_emits() {
        let resp =
            nexus_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(0, b"", false))
                .unwrap();
    ...
    fn bash_output_chat_completion_truncation_marker_and_extra() {
        ...
        let resp = nexus_tool_runtime::ToolOutput::chat_completion_output(&bash).unwrap();
        let result = resp.result.as_ref().unwrap();
    ...
    fn bash_tool_output_foreground_delegates_background_skips() {
        let resp = nexus_tool_runtime::ToolOutput::chat_completion_output(&BashToolOutput::Bash(
            sample_bash(0, b"ok", false),
        ))
        .unwrap();
        assert_cer(&resp, "ok", 0, false);
        assert!(
            nexus_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn aggregate_tool_output_bash_delegates_background_skips() {
        let resp = nexus_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Bash(
        ...
        assert!(
            nexus_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Text(
    ...
    fn bash_tool_id() -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("bash").unwrap()
    }
    #[test]
    fn into_typed_tool_output_preserves_bash_foreground_cco() {
        ...
        let dropped = nexus_tool_runtime::TypedToolOutput::from_value(typed.tool_id, expected_value);
    ...
    fn into_typed_tool_output_non_bash_cco_is_none() {
        ...
        let typed = run.into_typed_tool_output(nexus_tool_protocol::ToolId::new("text").unwrap());
