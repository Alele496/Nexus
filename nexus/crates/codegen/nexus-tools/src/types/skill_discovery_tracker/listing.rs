...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (sage build harness) ─────────────
...
///
/// `"/path/.nexus/skills/my-skill/SKILL.md"` -> `"/path/.nexus/skills/"`
...
/// Used by the budgeted (sage build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Sage build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the sage
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // sage-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: sage build harness (budgeted XML) ───────────
