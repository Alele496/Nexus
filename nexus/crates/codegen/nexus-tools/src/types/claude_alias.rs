//! Canonical external-settings tool name ↔ Sage tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`sage-hooks`) needs the
//! Sage tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`sage-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `sage` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to Sage, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Sage [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Nexus tool names this Claude tool maps to (empty when there is no direct
    /// Nexus tool — the entry then only contributes a `kind`).
    nexus: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, nexus: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        nexus,
...
const fn match_only(claude: &'static str, nexus: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        nexus,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (sage-build reads SKILL.md)
...
/// The Sage [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Sage tool names a Claude matcher term fires on.
pub fn nexus_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.nexus)
...
/// The Claude names that map to `nexus_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(nexus_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.nexus.contains(&nexus_name))
        .map(|t| t.claude)
}

/// Every distinct Sage name the table references, for the `sage-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn nexus_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.nexus.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Sage name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.nexus.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
