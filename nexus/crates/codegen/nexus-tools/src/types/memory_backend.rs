...
//!
//! `MemoryBackend` is defined in `sage-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `sage-tools`.
/// Mirrors `nexus_shell::session::memory_log::TARGET`.
