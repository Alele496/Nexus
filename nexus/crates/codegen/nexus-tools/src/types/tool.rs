...
//!
//! The tool runtime contract (`Tool` trait) lives in `nexus_tool_runtime`.
...
///
/// Serializes to snake_case (`nexus_build`, `mcp`, …) for the
...
/// (e.g. `"NexusBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
