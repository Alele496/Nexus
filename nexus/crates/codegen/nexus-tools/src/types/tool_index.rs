...
//!
//! `ToolSearchIndex` is defined in `sage-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `sage-shell`
...
/// in `sage-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `sage-shell` after MCP initialization.
#[derive(Clone)]
