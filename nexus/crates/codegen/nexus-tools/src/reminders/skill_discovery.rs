...
//! after every tool call to check for SKILL.md files in `.nexus/skills/`,
...
/// Directories that contain skill definitions (`.nexus/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".nexus", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.nexus/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.nexus/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.nexus/) are outside the git root so the walk breaks early.
