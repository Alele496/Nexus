...
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_nexus_version, get_latest_version,
    is_version_cache_fresh, try_fetch_stable_pointer, write_version_cache,
};
use nexus_shell::util::config;
use nexus_shell::util::nexus_home::{nexus_application, nexus_home};
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    println!("Nexus - v{}{}", status.current_version, channel_label);
    Ok(())
}
...
pub async fn check_update_status(update_config: &UpdateConfig) -> UpdateStatus {
    ...
    let current_version = get_installed_nexus_version();
...
pub async fn auto_update_target(update_config: &UpdateConfig) -> Option<(&'static str, String)> {
    ...
    let current = get_installed_nexus_version();
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(get_installed_nexus_version);
    if needs_update(
        &effective_current,
        &latest,
        &update_config.channel,
    ...
    let running = get_installed_nexus_version();
...
/// managed `~/.nexus/bin/sage` symlink.
///
...
fn env_installer() -> Option<&'static str> {
    if let Ok(v) = std::env::var("NEXUS_INSTALLER") {
    ...
    if std::env::var_os("NEXUS_MANAGED_BY_NPM").is_some() {
        return Some("npm");
    }
    if std::env::var_os("NEXUS_MANAGED_BY_INTERNAL").is_some() {
...
async fn run_update_subcommand(run_mode: UpdateRunMode) -> Result<Option<tokio::process::Child>> {
    ...
    match run_mode {
        ...
        UpdateRunMode::NonBlocking => {
            ...
            nexus_tools::util::detach_command(&mut cmd);
            let child = cmd.spawn()?;
            Ok(Some(child))
        }
    }
...
/// Prefer `~/.nexus/bin/sage` which always points to the latest version.
fn resolve_restart_exe() -> Result<std::path::PathBuf> {
    let canonical = nexus_application();
    if canonical.exists() {
        return Ok(canonical);
    }
...
pub fn restart_grok() -> Result<()> {
    ...
    cmd.env_clear();
    cmd.envs(std::env::vars_os().filter(|(k, _)| k != "NEXUS_AUTO_UPDATE"));
...
/// Delete `~/.nexus/models_cache.json` after a successful update.
///
...
async fn remove_stale_models_cache() {
    let cache = nexus_home().join("models_cache.json");
...
/// Remove the stale `sage-pager` symlink/binary from `~/.nexus/bin/` left by
...
/// config and writes to `~/.nexus/bin/`, so callers must isolate
/// `NEXUS_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.nexus/downloads/`, not yet
...
async fn download_verified_from_base(
    ...
    let nexus_home = nexus_home();
    let download_dir = nexus_home.join("downloads");
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    let nexus_home = nexus_home();
    let download_dir = nexus_home.join("downloads");
    let bin_dir = nexus_home.join("bin");
    ...
    regenerate_completions(&link_path, &nexus_home).await;
...
async fn regenerate_completions(binary: &std::path::Path, nexus_home: &std::path::Path) {
    // Derive $HOME independently — nexus_home may be overridden via NEXUS_HOME
    // env var, so nexus_home.parent() isn't necessarily the user's home dir.
    #[allow(deprecated)]
    let user_home = std::env::home_dir().unwrap_or_default();

    let completions: &[(&str, std::path::PathBuf)] = &[
        ("bash", nexus_home.join("completions/bash/sage.bash")),
        ("zsh", nexus_home.join("completions/zsh/_grok")),
...
///
/// When both paths share a grandparent (e.g. `~/.nexus/bin/sage` and
/// `~/.nexus/downloads/sage-0.1.203-linux-x86_64`), returns a relative path
...
///
/// Relative symlinks survive Docker bind-mounts where `~/.nexus/` is mapped
...
/// Swap `~/.nexus/bin/{sage,agent}` to point at `binary_path`. Returns the
...
/// bind-mounts of `~/.nexus/`). Windows: [`windows_replace_exe`].
///
...
async fn swap_managed_bin_links(
    ...
    let link_paths: [std::path::PathBuf; 2] = [nexus_link.clone(), agent_link];
...
async fn reconcile_agent_to_grok(bin_dir: &std::path::Path) {
    let nexus_link = bin_dir.join("sage");
    let agent_link = bin_dir.join("agent");

    let Ok(nexus_target) = tokio::fs::read_link(&nexus_link).await else {
        return;
    };
    if tokio::fs::metadata(&nexus_link).await.is_err() {
        return;
    }
    if let Ok(agent_target) = tokio::fs::read_link(&agent_link).await
        && agent_target == nexus_target
    {
        return;
    }
    match atomic_symlink_swap(&nexus_target, &agent_link).await {
        Ok(()) => tracing::info!(
            nexus_target = %nexus_target.display(),
...
async fn reconcile_agent_exe_to_grok(bin_dir: &std::path::Path) {
    ...
    match agent_exe_differs(&nexus_exe, &agent_exe).await {
        Ok(true) => {}
        Ok(false) => return,
        Err(e) => {
...
async fn agent_exe_differs(
    ...
    let nexus_len = tokio::fs::metadata(sage).await?.len();
    match tokio::fs::metadata(agent).await {
        Ok(m) if m.len() != nexus_len => return Ok(true),
        Ok(_) => {}
...
async fn gh_release_download(tag: &str, pattern: &str, dest: &std::path::Path) -> Result<()> {
    ...
    .stderr(Stdio::piped());
    nexus_tools::util::detach_command(&mut cmd);
    cmd.envs(nexus_tools::util::pager_env());
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    // ~/.nexus/downloads/ (legacy layout — skips the sage-latest indirection).
    ...
    for name in ["sage", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".nexus/downloads/") && !target_str.ends_with("sage-latest") {
...
/// inode is unlinked.
///
/// While our postinstall.js now uses versioned binaries under ~/.nexus/bin/
...
#[cfg(target_os = "macos")]
fn warn_if_other_nexus_processes_running() {
    ...
    cmd.args(["-f", "sage"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    nexus_tools::util::detach_std_command(&mut cmd);
...
pub async fn run_update(
    ...
    if !force && std::env::var_os("NEXUS_AUTO_UPDATE").is_none() {
        eprintln!("  Please restart Sage.");
    }
...
async fn refresh_deployment_config() {
    if !nexus_shell::managed_config::has_principal() {
        return;
    }
    if !nexus_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    nexus_shell::managed_config::clear_orphan();
    if !nexus_shell::config::is_managed_config_stale_for(
        &nexus_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match nexus_shell::managed_config::sync().await {
...
mod tests {
    ...
    fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
        ...
        let dest_181 = std::path::Path::new("/home/u/.nexus/downloads/sage-0.1.181-linux-x86_64");
        let dest_182 = std::path::Path::new("/home/u/.nexus/downloads/sage-0.1.182-linux-x86_64");
        ...
        assert_eq!(
            a.parent(),
            std::path::Path::new("/home/u/.nexus/downloads").into(),
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_reconcile_agent_skips_when_nexus_dangling() {
        let (_dir, bin, downloads) = managed_layout();
        std::os::unix::fs::symlink("../downloads/sage-0.2.101-macos-aarch64", bin.join("sage"))
            .unwrap();
        std::fs::write(downloads.join("sage-0.1.199-macos-aarch64"), "old").unwrap();
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_reconcile_agent_skips_when_nexus_not_symlink() {
        let (_dir, bin, downloads) = managed_layout();
    ...
    fn test_relative_symlink_target_sibling_dirs() {
        ...
        let target = std::path::Path::new("/home/alice/.nexus/downloads/sage-0.1.203");
        let link = std::path::Path::new("/home/alice/.nexus/bin/sage");
    ...
    fn test_relative_symlink_target_same_dir() {
        ...
        let target = std::path::Path::new("/home/alice/.nexus/downloads/sage-0.1.203");
        let link = std::path::Path::new("/home/alice/.nexus/downloads/sage-latest");
        let result = relative_symlink_target(target, link);
    ...
    fn test_relative_symlink_target_cross_tree_stays_absolute() {
        // /usr/local/bin/sage -> /home/alice/.nexus/downloads/sage-0.1.203
        ...
        assert_eq!(
            result,
            std::path::PathBuf::from("/home/alice/.nexus/downloads/sage-0.1.203")
        );
    }
    ...
    async fn test_relative_symlink_survives_directory_move() {
        // Simulates Docker bind-mount: create ~/.nexus/ layout at path A,
        // then move it to path B and verify the symlink still resolves.
        let dir = tempfile::tempdir().unwrap();

        // Create alice's layout
        let alice = dir.path().join("alice").join(".nexus");
        ...
        // "Bind-mount" to bob: copy the entire .nexus tree
        let bob_home = dir.path().join("bob");
        std::fs::create_dir_all(&bob_home).unwrap();
        let bob = bob_home.join(".nexus");
    ...
    //   1. NEXUS_INSTALLER (npm | internal | gh-release | gh)
    //   2. NEXUS_MANAGED_BY_NPM       → npm
    //   3. NEXUS_MANAGED_BY_INTERNAL  → internal
    ...
    impl InstallerEnvGuard {
        fn isolate() -> Self {
            const VARS: &[&str] = &[
                "NEXUS_INSTALLER",
                "NEXUS_MANAGED_BY_NPM",
                "NEXUS_MANAGED_BY_INTERNAL",
    ...
    fn test_env_installer_explicit_npm() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("NEXUS_INSTALLER", "npm") };
    ...
    fn test_env_installer_explicit_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("NEXUS_INSTALLER", "internal") };
    ...
    fn test_env_installer_explicit_gh_release() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("NEXUS_INSTALLER", "gh-release") };
    ...
    fn test_env_installer_explicit_gh_alias() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("NEXUS_INSTALLER", "gh") };
        assert_eq!(env_installer(), Some("gh-release"));
    }
    ...
    fn test_env_installer_explicit_unknown_value_returns_none() {
        ...
        // to the other env vars or to config. So `NEXUS_INSTALLER=brew`
        // disables the env-installer detection entirely.
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("NEXUS_INSTALLER", "brew") };
        // Even if MANAGED_BY_NPM is also set, the explicit var wins (and rejects).
        unsafe { std::env::set_var("NEXUS_MANAGED_BY_NPM", "1") };
        assert_eq!(
            env_installer(),
            None,
            "explicit NEXUS_INSTALLER=brew must early-return None, not fall through"
        );
    }
    ...
    fn test_env_installer_explicit_empty_returns_none() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("NEXUS_INSTALLER", "") };
        assert_eq!(env_installer(), None);
    }
    ...
    fn test_env_installer_managed_by_npm_any_value() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("NEXUS_MANAGED_BY_NPM", "") };
        assert_eq!(env_installer(), Some("npm"));
    }
    ...
    fn test_env_installer_managed_by_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("NEXUS_MANAGED_BY_INTERNAL", "1") };
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    fn test_env_installer_managed_by_npm_wins_over_npm_config_user_agent() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("NEXUS_MANAGED_BY_NPM", "1");
    ...
    fn test_env_installer_explicit_internal_wins_over_npm_managed() {
        // NEXUS_INSTALLER=internal must override an inherited MANAGED_BY_NPM.
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("NEXUS_INSTALLER", "internal");
            std::env::set_var("NEXUS_MANAGED_BY_NPM", "1");
        }
        assert_eq!(env_installer(), Some("internal"));
    }
