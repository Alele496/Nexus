...
use nexus_shell::env::NexusBuildEnvironment;
...
///
/// Constructed once from `NexusBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `NexusBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://cli-chat-proxy.nexus.local/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.nexus/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (NEXUS_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &NexusBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: nexus_shell::auth::SageComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = nexus_home().join("version.json");
...
pub use nexus_version::installed as get_installed_nexus_version;

/// Version of the managed sage binary currently on disk, read from the
/// `~/.nexus/bin/sage` symlink target (`../downloads/sage-<version>-<platform>`)
...
/// link whose target binary was deleted (e.g. manual `~/.nexus/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = nexus_shell::util::nexus_home::nexus_application();
...
/// Read the cached stable version from `~/.nexus/version.json` (sync, for display).
///
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(nexus_version::VERSION, &stable)
    })
}
...
/// `~/.nexus/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(nexus_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_update_config_default_channel_is_stable() {
        use nexus_shell::env::NexusBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&NexusBuildEnvironment::Production);
