...
//!
//! When `cli.minimum_version` is set in any config layer, Sage refuses to
//! start below that floor. With auto-update on, we install
//! `max(latest, minimum)`; otherwise the user is asked to run `sage update`.
//!
//! Set `NEXUS_TEST_VERSION` to manually exercise either path without producing
//! a real out-of-date build.

use crate::auto_update::{get_installer, run_install_script};
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_nexus_version, write_version_cache,
};
...
pub(crate) enum MinimumVersionError {
    ...
    #[error(
        "The minimum version \"{value}\" in your Sage configuration \
    ...
    #[error(
        "This version of Sage ({current}) is no longer supported. \
         Run `sage update` to install version {minimum} or later."
    )]
    ...
    /// `sage update --version X` requested a version below the floor.
    #[error(
        "Cannot install Sage {target}: the configured minimum is {minimum}. \
         Run `sage update` to install the latest allowed version."
    )]
...
/// Used by `sage update` to keep the install target at or above the pin.
...
///
/// `NEXUS_TEST_VERSION` lets devs override the running version to skip
...
mod tests {
    ...
    fn version_env_var_flows_through_to_decision() {
        let saved = std::env::var("NEXUS_TEST_VERSION").ok();

        // SAFETY: #[serial] excludes other env-touching tests.
        unsafe { std::env::set_var("NEXUS_TEST_VERSION", "0.1.50") };
        let decision =
            evaluate_minimum_version(&get_installed_nexus_version(), Some("0.1.100")).unwrap();
        assert!(matches!(
            decision,
            MinimumVersionDecision::BelowMinimum { .. }
        ));
        ...
        match saved {
            Some(v) => unsafe { std::env::set_var("NEXUS_TEST_VERSION", v) },
            None => unsafe { std::env::remove_var("NEXUS_TEST_VERSION") },
        }
    }
}
