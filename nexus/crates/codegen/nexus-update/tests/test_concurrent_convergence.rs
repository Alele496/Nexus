...
//! release:
//!
//! 1. TUI startup: `check_update_background` spawns a detached `sage update`
//!    (the Ctrl+U path now adopts this child instead of spawning a second).
//! 2. Explicit `sage update` (incl. the Ctrl+U fallback when there is no
//!    live child).
...
//!   temp-name fix, every `0.1.x` download shared one `sage-0.1.tmp`
...
use nexus_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use nexus_update::version::installed_on_disk_version;

/// Assert the active `~/.nexus/bin/sage` resolves to the expected versioned
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("sage");
    assert!(link.is_symlink(), "sage must be a symlink");
    let resolved = dunce::canonicalize(&link)
        .unwrap_or_else(|e| panic!("active sage symlink does not resolve: {e}"));
    assert_eq!(
        resolved.file_name().unwrap().to_string_lossy(),
        format!("sage-{version}-{platform}"),
        "active sage must be the expected version"
    );
    assert_eq!(
    ...
    let ran_ok = std::process::Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ran_ok, "active sage must pass the smoke-test");
}

/// Lay down a managed-install layout in the test NEXUS_HOME:
/// `bin/sage -> ../downloads/sage-<version>-<platform>` (what
...
fn fake_managed_install(version: &str) {
    ...
    let name = format!("sage-{version}-{}", host_platform());
...
// ─────────────────────────────────────────────────────────────────────────────
// Convergence: explicit `sage update` (the Ctrl+U fallback path) finds the
...
// installers that actually maintain the managed `~/.nexus/bin/sage` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.nexus/downloads cleanup), the probe must report None — otherwise
    ...
    std::fs::remove_file(
        home.join("downloads")
            .join(format!("sage-0.2.7-{platform}")),
    )
    .unwrap();
...
// shared one `sage-0.1.tmp`, so a concurrent racer could atomically rename a
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    // Pre-fix, BOTH of these wrote to downloads/sage-0.1.tmp concurrently
    ...
    for version in ["0.1.181", "0.1.182"] {
        let path = home
            .join("downloads")
            .join(format!("sage-{version}-{platform}"));
        assert_eq!(
