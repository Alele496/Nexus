...
//!
//! > `~/.nexus/bin/sage` resolves to a binary that passes the smoke-test, OR it
...
use nexus_update::auto_update::install_internal_from_base;
...
/// (`sage` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `sage` should resolve to after an install attempt.
...
/// to both `sage` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p sage-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("NEXUS_BLITZ_ITERS")
        .ok()
