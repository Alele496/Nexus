...
//! exact JSON shape produced by `sage update --check --json` for the failure
...
use nexus_update::auto_update::check_update_status;

/// Set up a fake `npm` on PATH, set `NEXUS_INSTALLER=npm` so the auto-update
