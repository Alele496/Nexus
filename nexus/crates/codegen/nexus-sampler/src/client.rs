...
use nexus_sampling_types::error::{try_parse_stream_error, user_facing_api_error_message};
...
pub use nexus_sampling_types::ApiBackend;

/// Process-level fallback for the `x-sage-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "sage-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "sage-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request `x-sage-*` headers. Optional fields are skipped when empty/`None`.
...
impl GrokRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        let mut b = builder
            .header("x-sage-conv-id", self.conv_id)
            .header("x-sage-req-id", self.req_id)
            .header("x-sage-model-override", self.model_id)
            .header("x-sage-session-id", self.session_id)
            .header("x-sage-agent-id", self.agent_id);
        if let Some(idx) = self.turn_idx {
            b = b.header("x-sage-turn-idx", idx);
        }
        if let Some(id) = self.deployment_id.filter(|s| !s.is_empty()) {
            b = b.header("x-sage-deployment-id", id);
        }
        if let Some(id) = self.user_id.filter(|s| !s.is_empty()) {
            b = b.header("x-sage-user-id", id);
        }
        b
    }
}
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = nexus_sampling_types::reported_cost_ticks(
        value
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-sage-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-sage-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<nexus_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // Add x-sage-client-version header for version gating at the proxy.
        if let Some(client_version) = config.client_version.as_ref()
            && let Ok(header_value) = HeaderValue::from_str(client_version)
        {
            headers.insert(
                HeaderName::from_static("x-sage-client-version"),
                header_value,
            );
        }
        ...
        {
            headers.insert(
                HeaderName::from_static("x-sage-deployment-id"),
                header_value,
            );
        }
        ...
        {
            headers.insert(HeaderName::from_static("x-sage-user-id"), header_value);
        }

        {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-sage-client-identifier"),
                    header_value,
                );
            }
        }
    ...
    pub async fn chat_completion(
        ...
        let x_nexus_conv_id = &payload.x_nexus_conv_id.clone().unwrap_or_default();
        let x_nexus_req_id = &payload.x_nexus_req_id.clone().unwrap_or_default();
        ...
        let nexus_headers = GrokRequestHeaders {
            ...
            req_id: x_nexus_req_id,
            model_id: &model_id,
            session_id: payload.x_nexus_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_nexus_turn_idx.as_deref(),
            agent_id: payload.x_nexus_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_nexus_deployment_id.as_deref(),
            user_id: payload.x_nexus_user_id.as_deref(),
        };
    ...
    pub async fn chat_completion_stream(
        ...
        tracing::info!(
            ...
            model = streaming_request.inner.model.as_deref().unwrap_or(""),
            stream = streaming_request.stream,
            "Sending chat/completions request"
        );
        Self::log_request_headers(&built_request, "chat/completions");

        let request_url = built_request.url().to_string();
        let response = self.http.execute(built_request).await.map_err(|e| {
            tracing::warn!("HTTP request failed ({request_url}): {e}");
    ...
    pub async fn create_response(
        ...
        let x_nexus_conv_id = request.x_nexus_conv_id.as_deref().unwrap_or_default();
        let x_nexus_req_id = request.x_nexus_req_id.as_deref().unwrap_or_default();
        ...
        let nexus_headers = GrokRequestHeaders {
            ...
            req_id: x_nexus_req_id,
            model_id: &model_id,
            session_id: request.x_nexus_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_nexus_turn_idx.as_deref(),
            agent_id: request.x_nexus_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_nexus_deployment_id.as_deref(),
            user_id: request.x_nexus_user_id.as_deref(),
        };
    ...
    /// gate that adds the opt-in `x-sage-doom-loop-check` request header, so
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_nexus_conv_id = request.x_nexus_conv_id.clone();
        let x_nexus_req_id = request.x_nexus_req_id.clone();
        let x_nexus_session_id = request.x_nexus_session_id.clone();
        let x_nexus_turn_idx = request.x_nexus_turn_idx.clone();
        let x_nexus_agent_id = request.x_nexus_agent_id.clone();
        ...
        let extra_tools = nexus_sampling_types::extra_raw_tools(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_nexus_conv_id = x_nexus_conv_id;
        wrapper.x_nexus_req_id = x_nexus_req_id;
        wrapper.x_nexus_session_id = x_nexus_session_id;
        wrapper.x_nexus_turn_idx = x_nexus_turn_idx;
        wrapper.x_nexus_agent_id = x_nexus_agent_id;
...
mod tests {
    ...
    use nexus_sampling_types::types::ChatRequestMessage;
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            x_nexus_turn_idx: None,
            x_nexus_agent_id: None,
            x_nexus_deployment_id: None,
            x_nexus_user_id: None,
            trace: None,
        };
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-XAI-token-auth".to_string(), "sage-cli".to_string());
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the sage-shell agent product.
        assert!(ua.starts_with("my-client sage-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = nexus_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "sage-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
