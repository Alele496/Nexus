...
//! it" buckets.
//!
//! `sage-sampler` is intentionally decoupled from `sage-shell`
...
/// tools live in `sage-tools`
/// (`crates/codegen/sage-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `nexus_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `nexus_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `sage-shell/src/auth/manager.rs` to match, otherwise the
