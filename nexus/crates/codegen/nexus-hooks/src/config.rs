...
pub struct HooksMap {
    ...
    /// Event names present in the JSON but not recognized by Sage.
...
pub struct RawHandler {
    ...
    /// keys (`NEXUS_HOOK_EVENT`, `NEXUS_HOOK_NAME`, `NEXUS_SESSION_ID`,
    /// `NEXUS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at load
...
pub struct HookSpec {
    ...
    ///    (`NEXUS_HOOK_EVENT`, `NEXUS_HOOK_NAME`, `NEXUS_SESSION_ID`,
    ///    `NEXUS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at
    ///    load time and a tracing warning is emitted.
    /// 2. Plugin-injected vars merged in by the plugin adapter
    ///    (`sage-agent::plugins::hooks_adapter`). The adapter sets
    ///    `NEXUS_PLUGIN_ROOT`, `CLAUDE_PLUGIN_ROOT`, `NEXUS_PLUGIN_DATA`,
    ...
    ///    (`NEXUS_HOOK_EVENT`, `NEXUS_HOOK_NAME`, `NEXUS_SESSION_ID`,
    ///    `NEXUS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`). These are
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.nexus/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.nexus/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "NEXUS_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "NEXUS_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "NEXUS_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "NEXUS_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "NEXUS_HOOKS_PARSE_TEST_MATCHER_VAR";
    ...
    fn parse_hook_file_matcher_braced_var_is_not_env_expanded() {
        ...
        let key = "NEXUS_HOOKS_PARSE_TEST_MATCHER_BRACED";
    ...
    /// User attempts to set runner-reserved keys (NEXUS_HOOK_*,
    /// NEXUS_SESSION_ID, NEXUS_WORKSPACE_ROOT, CLAUDE_PROJECT_DIR) via
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "NEXUS_HOOK_EVENT": "spoofed",
                                    "NEXUS_HOOK_NAME": "spoofed",
                                    "NEXUS_SESSION_ID": "spoofed",
                                    "NEXUS_WORKSPACE_ROOT": "/etc",
