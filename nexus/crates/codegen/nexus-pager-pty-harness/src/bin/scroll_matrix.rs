//! `scroll-matrix` — scroll validation matrix sweep for sage-pager.
//!
...
use nexus_pager_pty_harness::pager_binary;
use nexus_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against sage-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_sage-pager,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves NEXUS_SCROLL_LOG against ITS cwd (the
