//! Unified PTY harness for sage-pager.
//!
...
//!    `sage-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `NEXUS_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use nexus_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through sage's stdout. Used to reproduce the doubled-line class of bugs
    /// where sage's diff renderer never re-asserts a region it didn't write
