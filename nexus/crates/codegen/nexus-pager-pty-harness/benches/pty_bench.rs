//! `pty-bench` — PTY benchmark CLI for `sage-pager`.
//!
...
//! ```bash
//! cargo bench -p sage-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/sage-${VERSION}-linux-x86_64 \
//!   cargo bench -p sage-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for sage-pager",
    long_about = None,
)]
struct Cli {
