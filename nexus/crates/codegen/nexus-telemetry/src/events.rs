...
//!
//! These structs were extracted from `sage-shell` so they can be
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `sage wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
impl CompactionScope {
    pub fn begin(
        ...
        let percentage = nexus_token_estimation::usage_percentage_u8(tokens_used, context_window);
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: nexus_file_utils::events::types::ToolOutcome,
    pub duration_ms: u64,
...
// ---------------------------------------------------------------------------
// Pager events (called from sage-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when Sage is launched from a non-project directory).
...
pub struct ClipboardCopy {
    ...
    /// An explicit `sage wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
/// is lifted — i.e. they subscribed (externally on sage.local) and came back.
/// This is the actual conversion signal for SuperGrok Heavy subscriptions
/// attributed to Sage: the user saw the gate in Sage, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to Sage's upsell surface.
...
/// `sage-shell-manual_auth` (the `manual_auth` binding gets the `sage-shell-`
...
telemetry_event!(SuperGrokUpsellShown, "supernexus_upsell_shown");
telemetry_event!(SuperGrokUpsellClicked, "supernexus_upsell_clicked");
