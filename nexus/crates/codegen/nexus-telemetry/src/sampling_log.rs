//! Tracing layer for `target: "sampling_log"` → `~/.nexus/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `NEXUS_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "NEXUS_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = nexus_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
