...
const ENV_ENABLED: &str = "NEXUS_INSTRUMENTATION";
const ENV_LOG_PATH: &str = "NEXUS_INSTRUMENTATION_LOG";
const DEFAULT_LOG_DIR: &str = "logs";
const DEFAULT_LOG_FILE: &str = "instrumentation.log";
const DEFAULT_TRACE_FILE: &str = "instrumentation.trace.json";

pub const TARGET: &str = "xai_nexus_instrumentation";
...
fn default_log_path() -> PathBuf {
    nexus_home().join(DEFAULT_LOG_DIR).join(DEFAULT_LOG_FILE)
}
...
fn default_output_path(mode: InstrumentationMode) -> PathBuf {
    let root = nexus_home().join(DEFAULT_LOG_DIR);
    match mode {
