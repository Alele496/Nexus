...
pub enum TraceUploadReason {
    ...
    FeatureOff,
    /// No sage.local auth or deployment key.
    NoCredentials,
    /// Direct-to-bucket S3 upload.
    DirectS3,
    /// Proxy mode via sage.local auth.
    Proxy,
...
impl TraceUploadReason {
    ...
    pub fn from_upload_method(method: &Option<nexus_file_utils::UploadMethod>) -> Self {
        match method {
            Some(nexus_file_utils::UploadMethod::Proxy { .. }) => Self::Proxy,
            Some(nexus_file_utils::UploadMethod::S3 { .. }) => Self::DirectS3,
            Some(nexus_file_utils::UploadMethod::Direct { .. }) => Self::DirectGcs,
...
#[cfg(test)]
mod tests {
    use nexus_file_utils::UploadMethod;

    use super::TraceUploadReason;

    /// The `sage-shell-doom_loop_recovery` Mixpanel event's name and
