...
//! file logger that writes to `~/.nexus/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `nexus_hooks` or `nexus_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! NEXUS_HOOKS_LOG=1 sage              # enable, write to ~/.nexus/logs/hooks.log
//! NEXUS_HOOKS_LOG=/tmp/h.log sage     # write to custom path
//! NEXUS_HOOKS_LOG=0 sage              # explicitly disable
//! tail -f ~/.nexus/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "NEXUS_HOOKS_LOG";
...
///
/// Writes to `~/.nexus/logs/hooks.log` (or custom path via `NEXUS_HOOKS_LOG`).
/// Filters to hooks (`nexus_hooks`) and plugins (`nexus_agent::plugins`) targets.
/// Set `NEXUS_HOOKS_LOG=0` to disable, `NEXUS_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "nexus_hooks=debug,nexus_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || nexus_home().join("logs").join("hooks.log");
