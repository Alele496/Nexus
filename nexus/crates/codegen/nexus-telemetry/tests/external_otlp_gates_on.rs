...
use nexus_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
const SECRET_MODEL: &str = "sage-4-sk-LEAKmodel1234567890abcd";
...
fn external_stream_gates_on_end_to_end() {
    ...
    let mut cfg = external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "NEXUS_EXTERNAL_OTEL" => Some("1".into()),
    ...
    assert!(!nexus_telemetry::is_enabled());

    nexus_telemetry::log_event(nexus_telemetry::events::SessionHarness {
        session_id: "sess-gates-on".into(),
        client_identifier: Some("sage-pager".into()),
        model_id: "sage-4".into(),
        agent_name: "sage-build-plan".into(),
        permission_mode: nexus_telemetry::enums::PermissionMode::Ask,
    ...
    nexus_telemetry::log_event(nexus_telemetry::events::PromptSubmitted {
        prompt_length: 100,
        model_id: "sage-4".into(),
    ...
    nexus_telemetry::log_event(nexus_telemetry::events::ModelResponseReceived {
    ...
    nexus_telemetry::log_event(nexus_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "nexus_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.xai.nexus_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("sage-cli"),
        "service.name=sage-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("nexus_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt = col::find_event(&collected, "nexus_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "nexus_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "nexus_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "nexus_code.session.count");
