#!/usr/bin/env node
// Runs once after npm install/update. Reads the nexus binary from the
// matching per-platform optional dependency (@nexus/nexus-<platform>)
// and installs it to ~/.sage/bin/ using versioned filenames:
//
//   Unix:    nexus-<version>  +  nexus  (symlink)
//   Windows: nexus-<version>.exe  +  nexus.exe  (copy)
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.sage', 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@nexus/nexus: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@nexus/nexus-${key}`;
    try {
...
if (!version) {
    console.error('@nexus/nexus: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@nexus/nexus: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@nexus/nexus: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running nexus processes and try again.');
                return false;
            }
        }
    } else {
...
if (!platformDir) {
    console.error(`@nexus/nexus: platform package @nexus/nexus-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @nexus/nexus');
    process.exit(0);
}

installBinary('nexus', platformDir, `nexus${EXE}`);
cleanupOldVersions('nexus');

// Write installer config
const configDir = path.join(os.homedir(), '.sage');
...
// Persist the npm registry so `nexus update` and the trampoline use the same one.
const npmRegistry = process.env.SAGE_NPM_REGISTRY
    || (() => {
        try {
            const resolved = execSync(
                'npm config get @nexus:registry',
...
// Set SAGE_INSTALL_COMPLETIONS=1 to auto-generate to ~/.sage/completions.
const NEXUS_PATH = path.join(CANONICAL_DIR, `nexus${EXE}`);
if (process.env.SAGE_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.sage', 'completions');
        const bashPath = path.join(completionsDir, 'bash', 'nexus.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_nexus');
        fs.mkdirSync(path.dirname(bashPath), { recursive: true });
        fs.mkdirSync(path.dirname(zshPath), { recursive: true });
        const bashRes = spawnSync(NEXUS_PATH, ['completions', 'bash'], { encoding: 'utf8' });
        if (bashRes.status === 0) fs.writeFileSync(bashPath, bashRes.stdout);
        const zshRes = spawnSync(NEXUS_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.sage/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
    console.log('Tip: nexus completions bash > ~/.local/share/bash-completion/completions/nexus');
    console.log('     nexus completions zsh  > ~/.zsh/completions/_nexus');
}
