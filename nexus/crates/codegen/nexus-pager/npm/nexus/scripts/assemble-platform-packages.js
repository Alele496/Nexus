...
//   1. Brotli-compresses the built binary into `../nexus-<platform>/bin/<bin>.br`
...
// (`@nexus/nexus`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw nexus binary is
...
const nexusRoot = process.env.NEXUS_ROOT || path.resolve(__dirname, '..', '..', '..', '..', '..');
const npmRoot = path.resolve(__dirname, '..', '..');

const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'nexus-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `nexus-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] nexus-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'nexus',
            envVar: 'NEXUS_DARWIN_ARM64',
            defaultSource: path.join(nexusRoot, 'target', 'release', 'nexus'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'nexus',
            envVar: 'NEXUS_DARWIN_X64',
            defaultSource: path.join(nexusRoot, 'target', 'x86_64-apple-darwin', 'release', 'nexus'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'nexus',
            envVar: 'NEXUS_LINUX_X64',
            defaultSource: path.join(nexusRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'nexus'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'nexus',
            envVar: 'NEXUS_LINUX_ARM64',
            defaultSource: path.join(nexusRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'nexus'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'nexus.exe',
            envVar: 'NEXUS_WIN32_X64',
            defaultSource: path.join(nexusRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'nexus.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'nexus.exe',
            envVar: 'NEXUS_WIN32_ARM64',
            defaultSource: path.join(nexusRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'nexus.exe'),
        },
    ];
