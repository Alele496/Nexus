...
#
# Auth: SAGE_DEPLOYMENT_KEY env var (takes precedence) or ~/.sage/auth.json from `grok login`.
...
#   $env:SAGE_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex           # specific version (alt)
#   $env:SAGE_DEPLOYMENT_KEY="<key>"; irm https://x.ai/cli/install.ps1 | iex
#
...
if (-not $Version -and $env:SAGE_VERSION) {
    $Version = $env:SAGE_VERSION
}
...
if ($env:SAGE_DEPLOYMENT_KEY) {
    ...
    if ($oidcToken) {
        ...
        Write-Host 'Auth: using OIDC token from ~/.sage/auth.json.' -ForegroundColor DarkGray
    } elseif ($legacyToken) {
        $AuthSource = 'auth.json (legacy)'
        Write-Host 'Auth: using legacy token from ~/.sage/auth.json.' -ForegroundColor DarkGray
    }
}
...
$BinDir = if ($env:SAGE_BIN_DIR) { $env:SAGE_BIN_DIR } else { Join-Path $GrokDir 'bin' }

New-Item -ItemType Directory -Path $DownloadDir -Force | Out-Null
New-Item -ItemType Directory -Path $BinDir -Force | Out-Null

$Channel = if ($env:SAGE_CHANNEL) { $env:SAGE_CHANNEL } else { 'stable' }
...
if ($env:SAGE_DEPLOYMENT_KEY) {
    $ProxyUrl = if ($env:SAGE_PROXY_URL) { $env:SAGE_PROXY_URL } else { 'https://cli-chat-proxy.grok.com/v1' }
    Write-Host '  Fetching deployment config...' -ForegroundColor DarkGray
    try {
        $headers = @{ 'Authorization' = "Bearer $($env:SAGE_DEPLOYMENT_KEY)" }
