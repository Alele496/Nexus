...
2. Create a simple hook file, e.g. `~/.sage/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.sage/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.sage/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
- `SAGE_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `SAGE_HOOK_NAME` — the full configured name of this hook
- `SAGE_SESSION_ID` — the current session identifier
- `SAGE_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `SAGE_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `SAGE_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `SAGE_PLUGIN_ROOT` and `SAGE_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`SAGE_PLUGIN_ROOT` and `SAGE_PLUGIN_DATA`. These keys override any user-declared
...
{
  "type": "command",
  "command": "${HOME}/.config/sage-hooks/check.sh"
}
```
...
`${SAGE_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.sage/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.sage/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.sage/logs`).

## More Examples

See the built-in examples in the `sage-hooks` crate:

- [Safe Shell Guard](../../../sage-hooks/examples/hooks/safe-shell.json)
- [No Recursive Grep](../../../sage-hooks/examples/hooks/no-recursive-grep.json) — hard-blocks `grep -r`/`grep -R`/`rgrep` (OOM guard)
- [Session Audit Log](../../../sage-hooks/examples/hooks/session-log.json)
- [Tool Activity Logger](../../../sage-hooks/examples/hooks/tool-logger.json)

Copy them to `~/.sage/hooks/` and customize.
