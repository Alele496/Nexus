...
[models]
default = "sage-build"           # model used for new sessions
web_search = "sage-4.20-multi-agent"   # model used by the web_search tool
...
[subagents.models]
explore = "sage-build"               # route to different models
```
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "sage"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `sage`. |
...
Some configuration can be set per-project by placing files in `.sage/` within your repository:
...
| `.sage/lsp.json` | LSP server configuration |
| `.sage/sandbox.toml` | Custom sandbox profiles |
