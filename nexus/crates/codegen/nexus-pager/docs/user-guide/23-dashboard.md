...
- **`nexus dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.nexus/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Nexus · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Nexus's sibling views (
`tasks_pane`):
...
Set `NEXUS_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `nexus --bg`") would list
