...
- **`sage dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.sage/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Sage · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Sage's sibling views (
`tasks_pane`):
...
Set `SAGE_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `sage --bg`") would list
