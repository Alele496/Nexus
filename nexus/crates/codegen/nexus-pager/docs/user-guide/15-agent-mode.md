...
Agent mode runs Nexus as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`nexus -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `nexus agent` command and apply to every mode. Pass them before the mode name, for example `nexus agent --model sage-build stdio`. The `stdio` subcommand itself takes no options.

| Flag                       | Description                                                       |
| -------------------------- | ---------------------------------------------------------------- |
| `-m, --model <MODEL>`      | Set the model ID (for example, `sage-build`).                    |
...
```bash
nexus agent serve --bind 127.0.0.1:2419 --secret <token>
```

Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent generates a token and prints it at startup; you can also supply one through the `NEXUS_AGENT_SECRET` environment variable. The agent persists across reconnections, so a client can disconnect and later resume in-flight work.
...
Beyond the base ACP protocol, Nexus defines extension methods under the `x.ai/` prefix for Nexus-specific functionality. These cover:
...
| **Session Management**     | `sage.local/session/*`     | `fork`, `resolve_local_for_worktree_resume`      |
...
The tables here show representative methods in each category. The `x.ai/*` set is Nexus-specific and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
...
| `sage.local/session_notification`| Session-specific updates (diff review, retry state, auto-compact) |
| `sage.local/session/update`      | Session update (tool calls, content) |
