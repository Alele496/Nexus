...
Headless mode runs Nexus non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
Nexus processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
| `-m, --model <MODEL>`   | Model to use (e.g., `sage-build`)              |
...
On failure, Nexus emits an error object (process exit non-zero). Prompt-level
...
Nexus may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `nexus -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
nexus -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
nexus -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
nexus -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
nexus -p "What's the secret number?" --resume abc123
```
...
nexus -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
nexus --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  nexus -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
Nexus's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(nexus -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
nexus -p "Format all files" --yolo

# Run tests and fix failures
nexus -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| ------------------------------- | ------------------------------------------------------------- |
| `NEXUS_API_KEY`        | API key for authentication (required when no browser login)   |
| `NEXUS_HOME`                    | Override config directory (default: `~/.nexus`)                |
| `NEXUS_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
| `RUST_LOG`                     | Log level filter (e.g. `debug`). Headless logs to stderr.     |

For CI environments without browser access, set `NEXUS_API_KEY` with an API key from [console.x.ai](https://console.x.ai):

```bash
export NEXUS_API_KEY="xai-..."
nexus -p "Run the test suite" --yolo
```
...
- **`NEXUS_API_KEY`** — simplest for CI. See [Environment Variables](#environment-variables-for-headless) above.
- **`nexus login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`nexus login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `nexus -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug nexus -p "..." 2> debug.log`.

---

## Project Root Discovery

When Nexus starts, it discovers the project root by walking upward from `--cwd`
...
Nexus discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
Nexus stores data in `~/.nexus` (override with `NEXUS_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.nexus` read-only:

- Pre-populate `auth.json` or use `NEXUS_API_KEY`
- Session persistence fails silently (ephemeral)
- Update checks log a warning and skip

```bash
export NEXUS_API_KEY="xai-..."
export NEXUS_DISABLE_AUTOUPDATER=1
nexus -p "..." --no-auto-update
```
...
| `NEXUS_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`NEXUS_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `NEXUS_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$NEXUS_HOME/bin/nexus`).
...
- Resume: `nexus -p "continue" --resume "<id>"` or `nexus -p "continue" --continue`
