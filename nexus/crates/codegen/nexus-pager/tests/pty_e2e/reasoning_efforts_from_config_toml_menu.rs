...
async fn reasoning_efforts_from_config_toml_menu() {
    ...
    // Seed `~/.nexus/config.toml` with a per-model reasoning-effort menu.
    let nexus_home = content.home().join(".nexus");
    std::fs::create_dir_all(&nexus_home).expect("create .nexus");
    // Quote the dotted model id: bare `[model.nexus-4.5]` is TOML key-path syntax (model.nexus-4.5), not the id "sage-4.5".
    std::fs::write(
