...
use nexus_pager_pty_harness::PtyHarness;
pub(crate) use nexus_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`sage-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `sage-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `NEXUS_SCROLL_SPEED`) appended to the controller's env.
