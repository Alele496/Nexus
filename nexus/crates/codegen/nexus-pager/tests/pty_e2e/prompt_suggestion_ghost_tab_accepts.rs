...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`sage-build-0.1`, the shell's built-in default)
    ...
    env.retain(|(k, _)| k != "NEXUS_PROMPT_SUGGESTIONS");
    env.push(("NEXUS_PROMPT_SUGGESTIONS".into(), "true".into()));
