...
//!
//! Uses `sage-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.nexus/auth.json` from `NEXUS_AUTH_JSON` (path) or the
//! developer's `~/.nexus/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p sage-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use nexus_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.nexus/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("NEXUS_AUTH_JSON") {
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    let _ = std::fs::create_dir_all(&sage);
    if let Some(src) = auth_json_source() {
        let dest = sage.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.nexus/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("NEXUS_HOME".into(), sage.display().to_string()),
    ...
    // of the developer's shell. `NEXUS_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (nexus_config::
    ...
    env.push((
        "NEXUS_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("NEXUS_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (sage-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "NEXUS_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
