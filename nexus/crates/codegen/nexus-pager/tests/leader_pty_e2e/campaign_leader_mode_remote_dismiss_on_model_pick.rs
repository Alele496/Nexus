...
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared NEXUS_HOME so every spawn elects/attaches to the
    ...
    let state_path = nexus_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(nexus_home.join("config.toml")).expect("read config.toml");
    assert!(
