...
use nexus_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a sage-shell instance.
    pub is_nexus_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<nexus_announcements::RemoteAnnouncement>>
    /// Whether interactive login is required (deferred auth for `sage.local`).
    ...
    pub auth_manager: std::sync::Arc<nexus_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<nexus_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_nexus_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    let auth_manager = std::sync::Arc::new(nexus_shell::auth::AuthManager::new(
        ...
        agent_config.nexus_com_config.clone(),
    ));

    // Leader has no in-process agent; init this process's product telemetry client.
    nexus_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = nexus_shell::util::nexus_home::nexus_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a sage-shell agent
    let is_nexus_shell = resp
        .meta
        .as_ref()
...
///
/// Matches TUI startup behavior: if the first method is `sage.local`, defer auth
...
/// for a `sage.local` or `oidc` method — these are the ones that can trigger
...
mod tests {
    ...
    #[test]
    fn startup_auth_nexus_com_no_provider_needs_login_pending() {
        let methods = vec![make_auth_method("sage.local", "sage.local", None)];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
        assert_eq!(label.as_deref(), Some("sage.local"));
        assert_eq!(method_id.as_ref().unwrap().0.as_ref(), "sage.local");
        assert_eq!(mode, AuthStartMode::Pending);
    }

    #[test]
    fn startup_auth_nexus_com_with_external_provider_command() {
        let meta = serde_json::json!({ "external_provider": true });
        let methods = vec![make_auth_method("sage.local", "Acme Corp", Some(meta))];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
    ...
    #[test]
    fn startup_auth_non_nexus_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.nexus/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use nexus_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};

        let built = build_auth_methods(AuthMethodsBuildInputs {
            // enterprise-style: model has `env_key` set and the env var resolves,
            // so the shell-side predicate returns true.
            has_external_api_key: true,
            // Realistic enterprise user: no cached session token, default `sage.local`
    ...
    /// `startup_auth_nexus_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use nexus_shell::agent::auth_method::{NEXUS_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(NEXUS_COM_METHOD_ID, "Nexus", None),
            make_auth_method(XAI_API_KEY_METHOD_ID, "xai.api_key", None),
        ];
        let (needs, _, _, _) = startup_auth_metadata(&methods);
        assert!(
            needs,
            "with sage.local first, the pager must require login -- pinning \
    ...
    fn startup_auth_method_id_is_copied_not_synthesized() {
        let methods = vec![make_auth_method("sage.local", "My Login", None)];
    ...
    fn startup_auth_external_provider_false_is_pending() {
        ...
        let methods = vec![make_auth_method("sage.local", "sage.local", Some(meta))];
