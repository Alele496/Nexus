...
use nexus_telemetry::events::{SuperGrokUpsell, SuperGrokUpsellClicked};
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("supernexus_heavy") and CCP display names ("SuperGrok Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "supernexus_heavy"
}

/// URL for upgrading the subscription tier.
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://sage.local/supergrok?referrer=sage-build";

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) const UPSELL_URL_PAYG: &str = "https://sage.local?_s=usage";
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        nexus_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using Sage",
            CreditLimitCardAction::PurchaseCredits,
            nexus_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            nexus_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(nexus_telemetry::events::CreditLimitUpsellShown {
            surface: nexus_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(nexus_telemetry::events::CreditLimitUpsellShown {
        surface: nexus_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            nexus_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_supernexus_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_supernexus_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}
...
fn open_supernexus_upsell(
    ...
    log_event(nexus_telemetry::events::SuperGrokUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        ...
        QuestionOption {
            ...
            description: "Get the most out of Sage. Highest usage limits.".into(),
            preview: None,
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<nexus_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<nexus_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_supernexus_url(app: &mut AppView) -> Vec<Effect> {
    ...
    let url = app
        ...
        .unwrap_or("https://sage.local/supergrok?referrer=sage-build");
    // Funnel attribution: tag CLI-originated SuperGrok upsell clicks
    // with `referrer=sage-build`, matching the OAuth consent flow and
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "sage-build");
