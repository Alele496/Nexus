...
use super::auth::{
    dispatch_cancel_login, dispatch_login, dispatch_logout, dispatch_set_api_key_from_welcome,
    dispatch_set_api_key_from_welcome_begin, dispatch_submit_auth_code, dispatch_switch_account,
};
use super::billing::dispatch_open_supernexus_url;
use super::ctx::{
    ...
    let effects = match action {
        Action::Quit | Action::QuitConfirmed => {
            if let Some(tx) = &app.voice_cmd_tx {
                let _ = tx.try_send(nexus_voice::VoiceCommand::Shutdown);
            }
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, nexus_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(nexus_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::AnnouncementsHide => {
            ...
            .map(nexus_announcements::announcement_hide_key);
        ...
        Action::AnnouncementsOpenCta(surface) => {
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(nexus_telemetry::events::AnnouncementCtaClicked {
                    id: promo_id,
                    source: surface,
                });
                open_url_or_show(app, &url);
        ...
        Action::OpenSupergrokUrl => dispatch_open_supernexus_url(app),
        Action::OpenUrl(url) => {
        ...
        Action::SetApiKeyFromWelcomeBegin => dispatch_set_api_key_from_welcome_begin(app),
        Action::SetApiKeyFromWelcome(key) => dispatch_set_api_key_from_welcome(app, key),
        Action::CopyAuthUrl => {
...
pub(super) fn dispatch_action_result(
    ...
    use nexus_hooks_plugins_types::OutcomeStatus;
            ...
            OutcomeStatus::Success => {
                    ...
                    && modal.result_notice.is_none()
                {
                    let entry_index = match modal.last_plugins_action {
                        Some(nexus_hooks_plugins_types::PluginsAction::Uninstall { .. }) => None,
            ...
            OutcomeStatus::ConfirmationRequired => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    let confirmed_action = modal.last_plugins_action.as_ref().map(|a| {
                        ...
                        if let nexus_hooks_plugins_types::PluginsAction::Uninstall {
                            ref mut confirmed,
                            ..
                        } = action
                        {
