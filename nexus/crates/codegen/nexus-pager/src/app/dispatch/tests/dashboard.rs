...
fn voice_final_appends_to_dashboard_dispatch() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        nexus_voice::VoiceEvent::UtteranceFinal {
            text: "the build".into(),
        },
    );
    assert_eq!(
...
fn voice_dashboard_dispatch_submit_tears_down_voice() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(nexus_voice::VoiceCommand::PttRelease)
    ));
...
fn voice_target_bound_at_start_dispatch_vs_peek() {
    if !nexus_voice::AUDIO_SUPPORTED {
        return;
    }
    use crate::views::dashboard::DashboardRowId;
    use crate::views::dashboard::peek::{PeekFields, PeekPanelState};
...
/// `sage dashboard` before login: the startup hook consumes the
/// `NEXUS_OPEN_DASHBOARD_AT_STARTUP` env var and stashes
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `sage dashboard` must open the dashboard after login",
    );
    assert!(
...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    let mut app = test_app();
    seed_model(&mut app, "sage-4.5", "Nexus 4.5");
    open_dashboard(&mut app);
    app.cwd_has_git_ancestor = true;
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(nexus_shell::sampling::types::ReasoningEffort::High),
            display: "Nexus 4.5".to_string(),
        });
    ...
    assert_eq!(
        ...
        Some((
            model_id,
            Some(nexus_shell::sampling::types::ReasoningEffort::High)
        )),
        "effort must be stashed for the shell",
    );
    assert_eq!(
        agent.deferred_session_mode,
        Some(nexus_tools::types::SessionMode::Plan),
    );
...
/// `sage dashboard`, and the startup hook.
#[serial_test::serial(NEXUS_AGENT_DASHBOARD)]
#[test]
...
fn dashboard_slash_model_stages_pending_model() {
    ...
    seed_model(&mut app, "sage-4.5", "Nexus 4.5");
    open_dashboard(&mut app);
    let effects = dispatch_dashboard_dispatch_slash(&mut app, "/model sage-4.5".into());
    assert!(
        effects.is_empty(),
    ...
    assert_eq!(pending.id.0.as_ref(), "sage-4.5");
    assert_eq!(pending.display, "Nexus 4.5");
...
fn dashboard_open_reseeds_pending_model_and_mode() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: acp::ModelId::new(std::sync::Arc::from("sage-4.5")),
            effort: None,
            display: "Nexus 4.5".to_string(),
        });
...
fn dashboard_attach_subagent_lazily_replays_deferred_transcript() {
    ...
    crate::app::subagent::set_replay_nexus_home_for_tests(Some(home.path().to_path_buf()));
...
fn dashboard_overlay_cycle_non_overlay_noop_when_dashboard_disabled() {
    let _guard =
        crate::test_util::EnvVarGuard::set("NEXUS_AGENT_DASHBOARD", std::path::Path::new("0"));
    let mut app = test_app_with_agent();
    let id1 = AgentId(0);
    mark_agent_nonempty(&mut app, id1);
    let id2 = AgentId(1);
...
fn dashboard_peek_reply_with_image_sends_blocks() {
    ...
    let reply_text = {
        ...
        let img = crate::prompt_images::PastedImage {
            element_id: nexus_ratatui_textarea::ElementId::from_raw(0),
            display_number: 0,
            mime_type: "image/png".into(),
