...
fn plugin_cta_catalog_reload_empty_candidates_resets_matched_phase() {
    ...
    let response = nexus_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![nexus_hooks_plugins_types::MarketplaceScanResult {
            source_name: nexus_plugin_marketplace::OFFICIAL_SOURCE_NAME.into(),
            source_kind: "git".into(),
            source_url_or_path: nexus_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into(),
...
fn clear_default_model_persists_but_keeps_live_current() {
    ...
    let info = acp::ModelInfo::new(id.clone(), "Nexus Test".to_string());
...
fn set_default_model_resolves_known_name() {
    ...
    let info = acp::ModelInfo::new(id.clone(), "Nexus 4.5".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
fn set_default_model_idempotent_when_already_current() {
    ...
    let info = acp::ModelInfo::new(id.clone(), "Nexus Already".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
fn set_auto_dark_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Nexus Day"));
        assert!(toast.contains('\u{2713}'));
    });
}
#[test]
fn set_auto_light_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Nexus Night"));
    });
}
