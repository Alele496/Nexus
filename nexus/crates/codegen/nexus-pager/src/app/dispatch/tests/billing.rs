...
use super::*;
use nexus_shell::sampling::error::is_free_usage_exhausted_error;
...
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("supernexus_heav")));
    assert!(!is_max_tier(Some("supernexus_heavy_plus")));
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("supernexus_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("supernexus_heavy"));
}
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: nexus_telemetry::events::SuperGrokUpsell::FreeUsagePaywall,
            }
        )
    ));
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use nexus_shell::extensions::notification::{RetryState, SessionUpdate};
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: nexus_telemetry::events::SuperGrokUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
...
/// `NEXUS_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
#[serial_test::serial(NEXUS_TEST_OPEN_URL_FILE)]
#[test]
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    unsafe { std::env::set_var("NEXUS_TEST_OPEN_URL_FILE", &bad) };
    ...
    unsafe { std::env::remove_var("NEXUS_TEST_OPEN_URL_FILE") };
}
...
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    ...
    unsafe { std::env::set_var("NEXUS_TEST_OPEN_URL_FILE", &url_file) };
