...
fn cta_entry(name: &str, status: &str) -> nexus_hooks_plugins_types::MarketplacePluginEntry {
...
fn cta_outcome(
    status: nexus_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
/// Plant a Build session under the process `nexus_home()` (OnceLock-cached;
/// do not rely on setting `NEXUS_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = nexus_shell::util::nexus_home::nexus_home();
    let encoded = nexus_shell::util::nexus_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: nexus_tools::implementations::nexus_build::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    nexus_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    nexus_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
fn push_synthetic_permission(
    ...
    let request = nexus_acp_lib::AcpArgs {
