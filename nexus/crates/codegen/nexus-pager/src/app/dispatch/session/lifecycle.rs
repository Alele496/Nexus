...
/// the workspace (writes `~/.nexus/trusted_folders.toml`), mark trust resolved,
...
pub(in crate::app::dispatch) fn dispatch_trust_folder(app: &mut AppView) -> Vec<Effect> {
    if let TrustState::Pending { workspace } = &app.trust_state {
        nexus_workspace::folder_trust::grant_folder_trust(workspace);
    }
    finish_trust(app)
}
...
/// (`--resume` / `--worktree` / initial-prompt / `sage dashboard`). Extracted
...
pub(in crate::app::dispatch) fn dispatch_new_worktree_session(
    ...
    if app.models.is_empty() {
        let msg: String = "No model configured. Set up an API key in Settings (F2) \
