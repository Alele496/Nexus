...
pub enum DeferredSessionStartup {
    ...
    ForeignResume {
        tool: nexus_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = nexus_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    nexus_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = nexus_shell::session::persistence::list_summaries(Some(cwd)).await?;
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &nexus_shell::agent::config::Config,
) -> std::sync::Arc<nexus_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(nexus_shell::auth::AuthManager::new(
        &nexus_shell::util::nexus_home::nexus_home(),
        agent_config.nexus_com_config.clone(),
    ));
    auth.configure_refresher(
        agent_config.nexus_com_config.auth_provider_command.clone(),
        None,
    );
    auth
}
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if nexus_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = nexus_shell::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    if let Some(original_cwd) = nexus_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
    ...
    let agent_config = nexus_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use nexus_shell::agent::session_registry_client::SessionRegistryClient;
    use nexus_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    ...
    let storage_client = nexus_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: nexus_shell::session::restore::ProgressCallback =
...
mod tests {
    ...
    #[serial_test::serial(NEXUS_HOME)]
    #[tokio::test]
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        let home = tempfile::tempdir().expect("home tempdir");
        unsafe { std::env::set_var("NEXUS_HOME", home.path()) };
        ...
        let sessions_cwd_dir = nexus_shell::util::nexus_home::nexus_home()
            .join("sessions")
            .join(&encoded);
