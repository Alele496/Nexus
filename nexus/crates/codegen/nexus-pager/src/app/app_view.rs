...
/// [`nexus_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<nexus_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: nexus_shell::util::config::WorktreeHintMode) -> Self {
        use nexus_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        nexus_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            nexus_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`nexus_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `NEXUS_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `NEXUS_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("NEXUS_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`nexus_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => nexus_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: nexus_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`nexus_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.nexus/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`NEXUS_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    pub announcement: Option<nexus_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `NEXUS_PLUGIN_CTA`
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `sage dashboard`, the
    ...
    /// `sage.local/sessions/list` polls and `sage.local/sessions/changed` broadcasts.
    ...
    /// poll outside leader mode, so we fetch the same `sage.local/session/list` the
    ...
    /// non-selectable headers. Gated by `NEXUS_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `NEXUS_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `sage-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<nexus_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        nexus_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `nexus_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// Stamps `_meta["sage.local/session"].kind = "chat"` and omits Build agent
    ...
    /// at startup and on settings toggles from `NEXUS_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: nexus_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<nexus_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `sage wrap` tip: set after the first
    ...
    /// Login button label from `AuthMethod.name` (e.g., "sage.local", "Acme Corp").
    ...
    /// `nexus_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, nexus_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `nexus_build_access_gate`. `Some` = blocked.
    pub gate: Option<nexus_shell::auth::GateInfo>,
    ...
    pub pending_gate_verification: Option<nexus_shell::auth::GateInfo>,
    ...
    /// kill switch or `NEXUS_VOICE_MODE=0` — the STT pipeline is not started and
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &nexus_shell::util::config::RemoteSettings,
    ) -> Option<nexus_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(nexus_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &nexus_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            nexus_telemetry::session_ctx::log_event(
                nexus_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            auto_mode_gate: nexus_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: nexus_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<nexus_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && nexus_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: nexus_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(nexus_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(nexus_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `NEXUS_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<nexus_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    nexus_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
            if key!('w', CONTROL).matches(key) {
                if ctx.cwd_has_git_ancestor {
                    return InputOutcome::Action(Action::OpenNewWorktreeDialog);
                }
                // Fall back to plain new session when not in a git repo.
                return InputOutcome::Action(Action::NewSession);
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        nexus_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
///
/// Menu order: `[Import]`, New session, New worktree, Resume session, `[Changelog]`, Quit.
...
fn dispatch_menu_action(
    ...
    let new_session_idx = base;
    let worktree_idx = base + 1;
    let resume_idx = base + 2;
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                nexus_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            nexus_telemetry::session_ctx::log_event(
                                nexus_telemetry::events::SuperGrokUpsellShown {
                                    source:
                                        nexus_telemetry::events::SuperGrokUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`nexus_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use nexus_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = nexus_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                nexus_telemetry::session_ctx::log_event(
                    nexus_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: nexus_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `sage wrap` tip trigger, run at the top of every `draw`
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            nexus_telemetry::session_ctx::log_event(nexus_telemetry::events::ContextualTip {
                tip: nexus_telemetry::events::ContextualTipKind::ImageInput,
                action: nexus_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = nexus_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![nexus_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![nexus_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: nexus_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_hides_usage_for_team_users() {
        ...
        let meta = nexus_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = nexus_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_shows_usage_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&nexus_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&nexus_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(nexus_shell::auth::GateInfo {
            message: "Subscribe to use Sage".into(),
            url: Some("https://sage.local/supergrok?referrer=sage-build".into()),
            label: None,
        });
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = nexus_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(nexus_workspace::foreign_sessions::RecentForeignSession {
                tool: nexus_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![nexus_workspace::file_system::FuzzyMatchResult {
