...
use nexus_shell::sampling::types::ReasoningEffort;
...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: nexus_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open sage.local in the browser for SuperGrok subscription upsell.
    OpenSupergrokUrl,
    ...
    /// Open sage.local managed connectors, appending session teamId when set.
    ...
    /// the `sage.local/session/interjection` + `x.ai/queue/changed` rebroadcasts are
    ...
    OpenTranscriptPager,
    /// Minimal mode (`sage --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: nexus_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(nexus_hooks_plugins_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(nexus_hooks_plugins_types::PluginsAction),
    /// Execute a marketplace management action from the modal.
    ExecuteMarketplaceAction(nexus_hooks_plugins_types::MarketplaceAction),
    ...
    /// Toggle a skill enable/disable via sage.local/skills/toggle.
    ToggleSkill {
        skill_name: String,
        enabled: bool,
    },
    ...
    AnnouncementsOpenCta(nexus_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"sagenight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `sage dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.nexus/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: nexus_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    FetchSessionList {
        /// Text search pushed down to `sage.local/session/list` as `query` (chat
    ...
    /// `sage.local/sessions/list`. Only issued in leader mode while the
    /// dashboard is open.
    FetchRoster,
    /// Fetch the local on-disk session list (dormant/idle sessions) for the
    /// dashboard via `sage.local/session/list` — the non-leader fallback for the
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.nexus/config.toml`.
    ...
    /// Persist a typed setting to `~/.nexus/config.toml`. On failure,
    ...
    /// Resolve the running agent name for a session (`sage.local/session/info`).
    ...
    /// Fetch skills list from the shell (sage.local/skills/list).
    FetchSkillsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Toggle a skill via sage.local/skills/toggle (enable/disable without restart).
    ToggleSkill {
        agent_id: AgentId,
        session_id: acp::SessionId,
        skill_name: String,
    ...
    /// Fetch and display session info via sage.local/session/info.
    ShowSessionInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
        show_resolved_model: bool,
    },
    /// Fetch and display detailed context usage via sage.local/session/info.
    ShowContextInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    SendInterject {
        ...
        session_id: acp::SessionId,
        text: String,
        /// Client-minted id echoed back on the `sage.local/session/interjection`
    ...
    /// registry (`~/.nexus/active_sessions.json`).
    ...
    /// Call `sage.local/session/fork` to create a peer session that resumes
    ...
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`sage-build-0.1` when the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<nexus_workspace::session::git::RestoreDegree>,
    },
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<nexus_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    /// Leader session roster loaded via `sage.local/sessions/list`.
    RosterLoaded {
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<nexus_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<nexus_shell::util::changelog::ChangelogEntry>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<nexus_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<nexus_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<nexus_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<nexus_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<nexus_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<nexus_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    DeepSearchResults {
        results: Vec<nexus_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    /// `sage.local/session/fork` completed (no-worktree path). The pager adopts
    ...
    /// `sage.local/session/fork` failed. The placeholder agent stays in
    ...
    GateRefreshed {
        settings: Option<nexus_shell::util::config::RemoteSettings>,
    },
