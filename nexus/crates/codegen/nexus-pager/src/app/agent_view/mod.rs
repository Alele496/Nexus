...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<nexus_hooks_plugins_types::MarketplacePluginEntry>,
...
pub struct AgentView {
    ...
    pub context_state: Option<nexus_shell::session::ContextInfo>,
    ...
    /// (optimistic echo). The shell broadcasts `sage.local/session/interjection` to
    ...
    pub(crate) deferred_session_mode: Option<nexus_tools::types::SessionMode>,
    ...
    /// Running agent definition for this session (`sage.local/session/info` `agentName`).
    ...
    /// Armed when an `sage.local/session/prompt_complete` broadcast arrives for the
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(nexus_telemetry::events::CreditLimitChoice::PayAsYouGo);
            nexus_telemetry::session_ctx::log_event(
                nexus_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            nexus_telemetry::session_ctx::log_event(
                nexus_telemetry::events::SuperGrokUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use nexus_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        nexus_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = nexus_tools::util::nexus_home::nexus_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &nexus_tools::implementations::nexus_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
