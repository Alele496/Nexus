...
use super::*;
use nexus_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn format_acp_error_rate_limit_surfaces_detail_or_fallback() {
    use nexus_shell::sampling::error::{
        FREE_UNEXUS_USER_MESSAGE, RATE_LIMITED_ERROR_CODE,
        RATE_LIMITED_USER_MESNEXUS_API_KEY, RATE_LIMITED_USER_MESNEXUS_OAUTH,
    };
    ...
    let rpm_body = "You are sending requests too quickly. Please slow down, or upgrade to a Sage subscription for higher limits: https://sage.local/supergrok";
    let rpm = acp::Error::new(RATE_LIMITED_ERROR_CODE, "Rate limited")
        .data(format!("API error (status 429 Too Many Requests): {rpm_body}"));
    assert!(format_acp_error(& rpm, false).contains("sage.local/supergrok"));
    assert_eq!(format_acp_error(& rpm, true), RATE_LIMITED_USER_MESNEXUS_API_KEY);
    let empty = acp::Error::new(RATE_LIMITED_ERROR_CODE, "Rate limited");
    assert_eq!(format_acp_error(& empty, false), RATE_LIMITED_USER_MESNEXUS_OAUTH);
    assert_eq!(format_acp_error(& empty, true), RATE_LIMITED_USER_MESNEXUS_API_KEY);
    ...
    assert_eq!(format_acp_error(& free, false), FREE_UNEXUS_USER_MESSAGE);
    assert_eq!(format_acp_error(& free, true), FREE_UNEXUS_USER_MESSAGE);
    ...
    assert_eq!(format_acp_error(& free_wrapped, false), FREE_UNEXUS_USER_MESSAGE);
}
...
fn picker_keeps_conversation_with_empty_cwd_and_missing_updated_at() {
    let payload = serde_json::json!(
        ...
        "Compare GPU vendors", "source" : "conversation", "_meta" : { "sage.local/session" : {
        "kind" : "chat" } } }] }
    );
...
fn picker_keeps_old_conversation_past_cutoff() {
    let payload = serde_json::json!(
        ...
        "_meta" : { "sage.local/session" : { "kind" : "chat" } } }] }
    );
    let entries = parse_session_picker_entries(&payload);
...
/// Untitled sage.local chats must stay listed, rendered as "Untitled".
#[test]
...
fn parse_kill_outcome_reads_result_envelope() {
    use nexus_tools::types::KillOutcome;
...
fn credit_balance_prefers_current_period_end_over_billing_period_end() {
    ...
    let c = BillingConfig {
        ...
        current_period: Some(UsagePeriod {
            period_type: Some("UNEXUS_PERIOD_TYPE_WEEKLY".into()),
...
fn parse_worktree_restore_payload_full() {
    use nexus_workspace::session::git::RestoreDegree;
...
fn spawn_fake_acp_agent(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<nexus_acp_lib::AcpAgentMessage>,
) -> Arc<AtomicUsize> {
    let counter = Arc::new(AtomicUsize::new(0));
    let counter_clone = counter.clone();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let nexus_acp_lib::AcpAgentMessage::ExtNotification(args) = msg {
...
/// Redirect `NEXUS_HOME` to a tempdir for test isolation.
fn setup_nexus_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("NEXUS_HOME", tmp.path());
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use nexus_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        nexus_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        nexus_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_nexus_home_in_tempdir();
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use nexus_acp_lib::AcpAgentMessage;
    use nexus_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "x.ai/marketplace/action" => {
                        ...
                        let req: nexus_hooks_plugins_types::MarketplaceActionRequest = serde_json::from_str(
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: nexus_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            nexus_home: PathBuf::from("/path/that/must/not/be-read"),
...
async fn fetch_session_list_pushes_query_and_echoes_seq() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "sage.local/session/list");
...
fn agent_profile_names_are_valid_builtins() {
    use std::str::FromStr;
    use nexus_agent::config::BuiltinAgentName;
...
/// Default flags produce no agent profile (uses sage-build default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (sage-build already has TaskTool).
#[test]
...
/// Neutralize `NEXUS_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(NEXUS_AGENT)]` (process-global env).
fn without_nexus_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("NEXUS_AGENT", "")
}
...
#[serial_test::serial(NEXUS_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_nexus_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "sage-build-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "sage-build-plan-no-subagents");
...
/// --ask-user alone selects the sage-build-ask-user profile.
#[serial_test::serial(NEXUS_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "sage-build-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `sage-build` already has it).
#[test]
...
/// All three flags on at the runtime default produce sage-build-plan
...
fn to_meta_chat_mode_stamps_kind_and_omits_agent_profile() {
    ...
    assert_eq!(meta["sage.local/session"] ["kind"], "chat");
    assert!(
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> nexus_shell::session::SessionInfoResponse {
    use nexus_shell::session::acp_types::{ContextInfo, SessionInfoData};
    nexus_shell::session::SessionInfoResponse {
...
fn format_session_info_shows_resolved_when_enabled_and_different() {
    let info = make_session_info("sage-4.5", Some("sage-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, true);
    assert!(text.contains("Model: sage-4.5 (sage-4.3)"));
}
#[test]
fn format_session_info_hides_resolved_when_disabled() {
    let info = make_session_info("sage-4.5", Some("sage-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model: sage-4.5"));
    assert!(! text.contains("sage-4.3"));
}
#[test]
fn format_session_info_no_parens_when_resolved_matches_requested() {
    let info = make_session_info("sage-4.5", Some("sage-4.5"), 1000, 10000);
    let text = format_session_info(&info, None, true);
    assert!(text.contains("Model: sage-4.5"));
    assert!(! text.contains("(sage-4.5)"));
}
#[test]
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("sage-build", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model Hash: abc123"));
...
fn session_picker_entry_maps_to_dormant_roster_row() {
    ...
    let entry = SessionPickerEntry {
        ...
        model_id: Some("sage-4".to_string()),
        num_messages: 3,
    ...
    assert_eq!(roster.model_id.as_deref(), Some("sage-4"));
