...
//! and xAI session extension notifications (`sage.local/session_notification` and
//! replay-path `sage.local/session/update`).
...
use nexus_acp_lib::AcpClientMessage;

use super::actions::Effect;
use nexus_shell::extensions::notification::{
    SessionNotification, SessionUpdate as XaiSessionUpdate, is_reauthable_failure,
};
use nexus_shell::tools::todo::todo_item_from_plan_entry;
use nexus_workspace::permission::bash_command_splitting::BashCommandHighlights;
...
/// - `sage.local/session_notification` / `sage.local/session/update` → per-agent session updates
fn handle_ext_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    match notif.method.as_ref() {
        ...
        "sage.local/sessions/changed" => handle_sessions_changed(notif, app),
        ...
        "sage.local/session/prompt_complete" => handle_prompt_complete(notif, app),
        "sage.local/session/interjection" => handle_interjection(notif, app),
        ...
        "sage.local/announcements/update" => handle_announcements_update(notif, app),
...
/// Handle `sage.local/session/interjection` — the leader broadcasts this
...
fn handle_interjection(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse sage.local/session/interjection");
        return false;
    };
...
fn handle_ext_method(ext: nexus_acp_lib::AcpArgs<acp::ExtRequest>, app: &mut AppView) -> bool {
