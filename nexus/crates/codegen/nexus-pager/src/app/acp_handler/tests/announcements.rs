#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.nexus` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.nexus` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.nexus` reads).
    #[test]
