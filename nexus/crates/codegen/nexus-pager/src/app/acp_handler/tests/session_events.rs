#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn retry_exhausted_rate_limited_empty_reason_uses_oauth_fallback() {
        use nexus_shell::sampling::error::RATE_LIMITED_USER_MESNEXUS_OAUTH;
        ...
        match last_session_event(&scrollback) {
            Some(SessionEvent::RetryFailed { error, .. }) => {
                assert_eq!(error, RATE_LIMITED_USER_MESNEXUS_OAUTH);
            }
    ...
    fn retry_exhausted_api_key_rewrites_consumer_subscription_upsell() {
        use nexus_shell::sampling::error::RATE_LIMITED_USER_MESNEXUS_API_KEY;
        ...
        match last_session_event(&scrollback) {
            Some(SessionEvent::RetryFailed { error, .. }) => {
                assert_eq!(error, RATE_LIMITED_USER_MESNEXUS_API_KEY);
                assert!(!error.contains("sage.local/supergrok"));
            }
    ...
    fn apply_retry_state_auth_failure_pushes_reauth_prompt() {
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) from https://cli-chat-proxy.nexus.local/v1/messages: \
                          no auth context"
                    .into(),
            },
            &mut session,
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `sage logout` /
    /// `sage login` guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          run `sage logout` then `sage login`"
                    .into(),
            },
            &mut session,
            &mut scrollback, false);
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(nexus_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
