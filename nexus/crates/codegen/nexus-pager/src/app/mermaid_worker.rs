...
//! inside [`nexus_mermaid::render_checked`] is a no-op there: a panic in the
...
//!    via [`nexus_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`nexus_mermaid::RenderLimits`]) and the raster megapixel/height caps
...
pub fn render_via_subprocess(
    ...
    cmd.arg(MERMAID_RENDER_SUBCOMMAND)
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .envs(nexus_tty_utils::pager_env());
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, nexus_mermaid::MermaidError> {
...
thread_local! {
    ...
    /// `NEXUS_HOME` (whose `nexus_home()` value is cached first-write-wins, an
    /// isolation hazard under the full parallel suite — PNGs could land in the
    /// real `~/.nexus`). Thread-local, so each parallel test is independent; the
    /// `TempDir` guard lives here so the dir outlives the view. Mirrors the
    /// `subagent::REPLAY_NEXUS_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `NEXUS_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    /// hermetic, with no process-global `NEXUS_HOME` mutation. The `TempDir` lives
    ...
    /// so there is no cross-test contamination and no `nexus_home()` cache race.
