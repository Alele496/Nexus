...
use nexus_shell::sampling::types::supports_reasoning_effort_meta;
...
impl SlashCommand for ModelCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        // often contain spaces ("Nexus 4.5"); if we split on the last token
        // first, a shorter catalog entry ("Nexus") would steal the prefix and
...
mod tests {
    ...
    fn empty_query_returns_one_row_per_logical_model() {
        ...
        let (pid, pinfo) = plain_model("sage-4.5", "Nexus 4.5");
        ...
        let plain = items.iter().find(|i| i.match_text == "Nexus 4.5").unwrap();
        assert_eq!(plain.insert_text, "Nexus 4.5");
    }
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // Catalog has both "Nexus" (reasoning) and "Nexus 4.5". `/model Sage 4.5`
        // must select the full name, not treat "4.5" as an effort on "Nexus".
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("sage", "Nexus");
        let (long_id, long_info) = model_with_reasoning("sage-4.5", "Nexus 4.5");
    ...
    fn run_rejects_effort_for_non_reasoning_model() {
        ...
        state.available.insert(id, info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "Nexus 4.5 high");
