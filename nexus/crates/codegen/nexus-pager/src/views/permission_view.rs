...
use nexus_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    pub request: nexus_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `sage-shell`, NOT duplicated locally.
...
/// ```text
///  3 [ ] Tell Sage what to do differently
...
mod tests {
    ...
    fn empty_view_state(mcp_scope: Option<McpScopeState>) -> PermissionViewState {
        ...
        let perm = nexus_acp_lib::AcpArgs {
            request,
            response_tx,
        };
        PermissionViewState {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from sage\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from sage\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p sage-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/sage-pager
