...
//! loop. Recipe: `NEXUS_FPS=1 NEXUS_SCROLL_DEBUG=1 sage --resume <session>`,
//! then flip `scroll_mode` / `scroll_lines` / `invert_scroll` /
//! `scroll_speed` in `/settings` to compare variants live. For event-exact
//! capture beyond this per-frame sampling, add `NEXUS_SCROLL_LOG=1` — the
...
impl ScrollDebugHud {
    pub fn new() -> Self {
        let env_on = std::env::var("NEXUS_SCROLL_DEBUG").is_ok_and(|v| !v.is_empty() && v != "0");
        Self { enabled: env_on }
    }
...
pub struct ScrollDebugPanel {
    ...
    /// Rows left free for FPS overlays stacked above (the dev `NEXUS_FPS`
