...
use nexus_announcements::visible_announcements;
...
fn is_critical(a: &nexus_announcements::RemoteAnnouncement) -> bool {
    a.severity.as_deref() == Some("critical")
}

fn is_promo(a: &nexus_announcements::RemoteAnnouncement) -> bool {
...
fn is_live_critical(
    a: &nexus_announcements::RemoteAnnouncement,
    now: chrono::DateTime<chrono::Utc>,
) -> bool {
    is_critical(a) && !nexus_announcements::is_expired_at(a, now)
}
...
fn is_live_promo(
    ...
    is_promo(a) && !nexus_announcements::is_expired_at(a, now)
}
...
pub fn is_dismissible(a: &nexus_announcements::RemoteAnnouncement) -> bool {
...
fn is_hidden(
    ...
    is_dismissible(a) && hidden_ids.contains(&nexus_announcements::announcement_hide_key(a))
}
...
fn usable_cta(a: &nexus_announcements::RemoteAnnouncement) -> Option<(&str, &str)> {
...
pub(crate) fn usable_cta_caption(a: &nexus_announcements::RemoteAnnouncement) -> Option<&str> {
...
fn first_critical_session_announcement<'a>(
    announcements: &'a [nexus_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<&'a nexus_announcements::RemoteAnnouncement> {
...
pub fn promo_cta_target<'a>(
    announcements: &'a [nexus_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<(&'a nexus_announcements::RemoteAnnouncement, &'a str)> {
...
fn render_critical_rows(
    area: Rect,
    buf: &mut Buffer,
    ann: &nexus_announcements::RemoteAnnouncement,
    hide_hovered: bool,
) -> BannerHits {
    use unicode_width::UnicodeWidthStr;
...
mod tests {
    ...
    fn promo(id: &str, message: &str, cta: Option<(&str, &str)>) -> RemoteAnnouncement {
        RemoteAnnouncement {
            ...
            cta: cta.map(|(label, url)| nexus_announcements::AnnouncementCta {
    ...
    fn promo_cta_target_requires_usable_pair() {
        ...
        label_only.cta = Some(nexus_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: None,
            caption: None,
        });
    ...
    fn render_promo_row_button_and_hide_affordances() {
        let mut ann = promo(
            "p",
            "New promo",
            Some(("Get SuperSage", "https://sage.local")),
        );
        ann.cta.as_mut().unwrap().caption = Some("or use Ctrl+O".into());
        let anns = [ann];
        ...
        let row0 = buf_row(&buf, area, 0);
        assert!(row0.starts_with("[Get SuperSage]"), "row0={row0:?}");
        assert!(
