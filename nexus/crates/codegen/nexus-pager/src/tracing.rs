...
/// firehose (`NEXUS_DEBUG_LOG` / `NEXUS_LOG_FILE`).
pub use nexus_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `sage-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use nexus_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "nexus_shell=info,nexus_pager=trace,nexus_tools=info,nexus_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = nexus_telemetry::otel_layer::build_otel_layer(
    ...
    let instrumentation_layer = nexus_telemetry::instrumentation::layer();
    let sampling_log_layer = nexus_telemetry::sampling_log::layer();
    let hooks_log_layer = nexus_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    nexus_telemetry::debug_log::install_firehose(registry, "tui");
