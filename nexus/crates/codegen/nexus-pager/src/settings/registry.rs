...
pub struct EnumChoice {
    ...
    /// Display label shown in the chooser (e.g. `"Nexus Night"`).
...
///
/// Delegates to [`nexus_voice::canonicalize_stt_language`] so the pager and
...
pub fn canonical_voice_stt_language(value: Option<&str>) -> &'static str {
    nexus_voice::canonicalize_stt_language(value)
}
...
pub fn current_value_for(
    ...
    match key {
        ...
        // Live cache; `NEXUS_PROMPT_SUGGESTIONS` env overrides at the gate.
        ...
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = nexus_shell::models::default_model();
...
mod tests {
            ...
            match (meta.key, &meta.kind) {
                ...
                ("fork_secondary_model", SettingKind::DynamicEnum { default, .. }) => {
                    ...
                    assert_eq!(
                        ...
                        nexus_shell::models::default_model(),
    ...
    /// Spot-check the delegation to `nexus_voice::canonicalize_stt_language`
    ...
    fn voice_stt_language_settings_match_voice_crate_catalog() {
        ...
        for c in choices.iter() {
            ...
            let lang = nexus_voice::stt_language_by_code(c.canonical)
        ...
        let crate_codes: HashSet<&str> = nexus_voice::STT_LANGUAGES
            .iter()
            .map(|l| l.code)
            .collect();
        assert_eq!(
            setting_codes, crate_codes,
            "settings concrete languages must match nexus_voice::STT_LANGUAGES exactly"
        );
    }
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: nexus_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
