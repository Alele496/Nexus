...
use nexus_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
///
/// Works for every auth method: OAuth / sage.local / OIDC session tokens and
/// `XAI_API_KEY` / per-model BYOK keys.
pub fn build_voice_auth(auth_manager: Arc<nexus_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        nexus_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
