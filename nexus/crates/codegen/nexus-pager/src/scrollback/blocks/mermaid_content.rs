...
//! the generic [`CodeBlockSpan`](nexus_markdown::CodeBlockSpan) API) and
...
use nexus_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](nexus_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](nexus_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a nexus_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `SageDay` is the only light theme; every other concrete theme (and the
/// `SageNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `nexus_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::SageDay)
}
...
mod tests {
    ...
    use nexus_markdown::StreamingMarkdownRenderer;
    ...
    #[test]
    fn theme_is_dark_maps_sageday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::SageDay),
            "NexusDay is the light theme"
        );
        for dark in [
            ThemeKind::SageNight,
