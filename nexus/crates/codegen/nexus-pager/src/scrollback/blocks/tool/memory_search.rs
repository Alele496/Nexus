...
fn shorten_path(path: &str) -> &str {
    let memory_root = nexus_config::nexus_home().join("memory");
    let memory_prefix = memory_root.display().to_string();
    if let Some(rest) = path.strip_prefix(&memory_prefix) {
        let rest = rest.strip_prefix('/').or_else(|| rest.strip_prefix('\\')).unwrap_or(rest);
        if let Some(after_slash) = rest.find('/').or_else(|| rest.find('\\')) {
            return &rest[after_slash + 1..];
        }
        return rest;
    }
    // Fallback: strip to filename (handle both separator styles)
...
**File:** /root/.nexus/memory/MEMORY.md (lines 0-10)
```
## Project Conventions
...
"#;
        ...
        assert_eq!(results[0].path, "/root/.nexus/memory/MEMORY.md");
...
**File:** /root/.nexus/memory/ws/MEMORY.md (lines 1-5)
```
workspace content
```

### Result 2 (score: 0.42, source: session)
**File:** /root/.nexus/memory/ws/sessions/2026-05-01.md (lines 10-20)
```
session content
```
"#;
    ...
    fn shorten_memory_path() {
        // Paths under the configured sage memory root keep one trailing segment group.
        let memory_root = nexus_config::nexus_home().join("memory");
        let session = memory_root.join("sage-50aa78f0/sessions/2026-05-01.md");
