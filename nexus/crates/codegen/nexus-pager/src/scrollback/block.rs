...
impl RenderBlock {
    ...
    pub fn context_info(
        snapshot: nexus_shell::session::ContextInfo,
    ...
    pub fn with_hyperlinks<R>(
        &self,
        f: impl FnOnce(&[nexus_markdown::HyperlinkTarget]) -> R,
    ) -> R {
        match self {
...
mod searchable_text_tests {
    ...
    use nexus_shell::session::ContextInfo;
    ...
    fn subagent_failed_indexes_metadata_and_error() {
        ...
        assert!(text.contains("sage-test"), "got: {text:?}");
    ...
    fn context_info_indexes_model_only() {
        ...
        let block = RenderBlock::context_info(snapshot, "sage-4.5");
        // Only the model name is source text; the rest is a numeric breakdown.
        assert_eq!(block.searchable_text().as_deref(), Some("sage-4.5"));
    }
