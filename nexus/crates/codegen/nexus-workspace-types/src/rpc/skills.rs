...
//! `sage-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.nexus/skills
    Local,
    /// repo_root/.nexus/skills
    Repo,
    /// ~/.nexus/skills
    User,
    /// ~/.nexus/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.nexus/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the sage-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.nexus/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.nexus/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.nexus/plugins/infra-plugin",
            "plugin_data": "/root/.nexus/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "sage-4",
            "effort": "high",
            "user_invocable": true,
