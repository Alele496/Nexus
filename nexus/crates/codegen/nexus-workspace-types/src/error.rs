...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] nexus_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `sage-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `nexus_tools::ToolError`) happens
