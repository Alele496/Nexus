...
pub mod tips;
pub mod uname;
pub use nexus_shared::clipboard;
pub use nexus_shared::stderr::{stderr_lock, with_locked_stderr};
...
pub fn is_nexus_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_rejects_spoofed_hostname() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.nexus.local.evil.example/v1"
        ));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_v11_prefix_confusion() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.nexus.local/v11/chat/completions"
        ));
    }
    #[test]
    ...
    #[test]
    fn is_nexus_process_self_true_impossible_pid_false() {
        assert!(is_nexus_process(std::process::id()));
        assert!(!is_nexus_process(u32::MAX));
    }
}
