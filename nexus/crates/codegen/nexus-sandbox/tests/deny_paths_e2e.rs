...
fn profile_from_env() -> nexus_sandbox::ProfileName {
    nexus_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    #[cfg(target_os = "linux")]
    {
        if !nexus_sandbox::is_inside_bwrap() {
            ...
            match nexus_sandbox::bwrap_reexec_for_profile(&profile_from_env(), workspace) {
                Some(mut cmd) => {
    ...
    match scenario.as_str() {
        "block_deny" => {
            let mut sandbox = nexus_sandbox::SandboxManager::new(profile_from_env(), workspace);
...
fn run_deny_case(
    ...
    let support = nexus_sandbox::SandboxManager::support_info();
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".nexus")).expect("mkdir .nexus");
    fs::write(
        tmp.join(".nexus").join("sandbox.toml"),
