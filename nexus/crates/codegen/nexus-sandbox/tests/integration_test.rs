//! Integration tests for sage-sandbox.
//!
...
fn test_sandbox_logger() {
    use nexus_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!nexus_sandbox::should_restrict_child_network());
}
