...
//! Cache root: `~/.nexus/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = nexus_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(nexus_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    nexus_config::nexus_home().join("marketplace-cache")
}
