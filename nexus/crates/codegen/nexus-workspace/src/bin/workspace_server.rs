...
//!
//! Reads OIDC credentials from `~/.nexus/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use std::path::PathBuf;
use url::Url;
use nexus_workspace::config::WorkspaceServerMetadata;
use nexus_workspace::daemonize;
use nexus_workspace::diag_server;
use nexus_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_nexus_workspace";
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<nexus_tool_protocol::ServerId>()
        .err()
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.nexus.local/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `NEXUS_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "NEXUS_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Trust project-scoped LSP servers from `<repo>/.nexus/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "NEXUS_WORKSPACE_PROJECT_LSP_TRUSTED",
    ...
    /// Override with `NEXUS_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "NEXUS_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/xai-sage-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = nexus_computer_hub_sdk::DonatingLogLayer::new_inert();
    tracing_subscriber::registry()
        .with(env_filter)
        .with(tracing_subscriber::fmt::layer())
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("NEXUS_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match nexus_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use nexus_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("NEXUS_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized NEXUS_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if nexus_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            tracing::info!(
                ...
                "Sandbox explicitly disabled via NEXUS_SANDBOX_PROFILE=off"
            );
        } else {
            ...
            sandbox.install();
            let active = nexus_sandbox::is_active();
            ...
            tracing::info!(
                ...
                nexus_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = nexus_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("NEXUS_SESSION_ID").ok();
    ...
    nexus_workspace::init_metrics();
    let ws_handle = nexus_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = nexus_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            nexus_workspace::handle::DrainReason::Sigterm,
        )
        .await;
    ...
    nexus_computer_hub_sdk::flush_log_layer();
...
mod tests {
    ...
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("NEXUS_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["xai-workspace-server"]).unwrap();
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        ...
        assert_eq!(
            ...
            Some("https://sage.local/preview-auth")
        );
