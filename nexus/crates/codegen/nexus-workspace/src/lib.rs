...
pub use nexus_workspace_client::WorkspaceClient;
pub use nexus_workspace_types::WorkspaceEvent;
pub use nexus_hunk_tracker::HunkTrackerHandle;
...
/// environment (`NEXUS_HOME`, `HOME`, …). nextest isolates each test in its own
...
/// can clobber `NEXUS_HOME` mid-test. A single shared lock (used by every
...
mod init_metrics_tests {
    ...
    fn init_metrics_is_idempotent_and_registers_baselines() {
        ...
        assert!(has(
            "nexus_workspace_toolset_swap_rejected_total",
            &[("reason", "turn_active"), ("trigger", "update_tool_config")]
        ));
        assert!(has(
            "nexus_workspace_orphan_lost_total",
            &[("reason", "sha_mismatch")]
        ));
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "nexus_workspace_env_capture_panic_total")
        );
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "nexus_workspace_permission_timeout_total")
        );
    }
}
