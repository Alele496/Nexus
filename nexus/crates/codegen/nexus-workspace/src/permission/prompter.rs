...
use nexus_file_utils::events::{Event, EventWriter, PermissionDecision};
use nexus_mcp::servers::parse_mcp_qualified_name;
use nexus_tools::implementations::nexus_build::web_fetch::domain_from_url;

const REJECT_ONCE_LABEL: &str = "No, and tell Sage what to do differently";
...
///        `~/.nexus/config.toml` via the `Effect::PersistPermissionMode` effect
...
/// Canonical definition lives in `nexus_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `sage-mcp` can
...
/// reached `nexus_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
...
/// `"list_issues"` → `"List Issues"`, `"nexus_com_notion"` →
/// `"Nexus Com Notion"`, and `"getMyTaskList"` → `"GetMyTaskList"`.
...
pub struct McpToolPermission {
    ...
    /// (e.g. `"nexus_com_notion__notion-fetch"`).
    pub tool_name: String,
    /// Server component of a valid qualified MCP ID (e.g. `"nexus_com_notion"`).
...
mod tests {
    ...
    fn mcp_titleize_segment_handles_snake_camel_kebab() {
        ...
        assert_eq!(mcp_titleize_segment("nexus_com_notion"), "Nexus Com Notion");
