...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "sage-shell", alias = "nexus_shell")]
    Generic,
    /// Sage TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "sage-tui", alias = "nexus_tui")]
    GrokTUI,
    /// Sage Web client - identified by clientIdentifier "sage-web"
    #[serde(rename = "nexus_web")]
    GrokWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "sage-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// Sage Pager client - TUI-like terminal pager with interactive permission UI.
    ...
    ///
    /// Accepts both the hyphenated `"sage-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"nexus_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "sage-pager", alias = "nexus_pager")]
    GrokPager,
    /// Sage Desktop (Electron) client - identified by clientIdentifier "sage-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "nexus_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `sage-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "sage-shell",
            Self::GrokTUI => "sage-tui",
            Self::GrokWeb => "sage-web",
            Self::Nebula => "nebula",
            Self::Extension => "sage-code-extension",
            Self::GrokPager => "sage-pager",
            Self::Desktop => "sage-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"sage-web"`, `"sage-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("sage-web") => Self::GrokWeb,
            Some("nebula") => Self::Nebula,
            Some("sage-code-extension") => Self::Extension,
            Some("sage-desktop") => Self::Desktop,
            Some("sage-pager") => Self::GrokPager,
            _ => Self::Generic,
        }
    }
...
impl From<&nexus_tools::types::ToolInput> for AccessKind {
    fn from(input: &nexus_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.nexus/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use nexus_tools::implementations::nexus_build_hashline::edit::types::HashlineEditInput;
        use nexus_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use nexus_tools::implementations::nexus_build::bash::BashToolInput;
        use nexus_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use nexus_tools::implementations::use_tool::UseToolInput;
        use nexus_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use nexus_tools::implementations::nexus_build::monitor::types::MonitorInput;
        use nexus_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use nexus_tools::implementations::nexus_build::web_fetch::WebFetchInput;
        use nexus_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use nexus_tools::implementations::nexus_build::web_search::WebSearchInput;
        use nexus_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_nexus_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("sage-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("nexus_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
