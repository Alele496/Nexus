...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=sage-cli")
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `sage-build`)                           |
...
[subagents.models]
explore = "sage-build"              # route explore to a lighter model
```
...
[model.sage-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```
