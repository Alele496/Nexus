...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    nexus_config::MANAGED_CONFIG_FILENAME,
    nexus_config::REQUIREMENTS_FILENAME,
    nexus_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    nexus_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            nexus_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", nexus_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            nexus_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
/// config is a sage.local feature with one sage.local auth.
...
/// files (TUI tick vs `sage login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `sage setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `NEXUS_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("NEXUS_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `sage-http` classifier.
...
/// Override with `NEXUS_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("NEXUS_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** SageBuildDeployment UUID (the id
...
/// Resolve deployment key from `NEXUS_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (sage setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("NEXUS_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.nexus/`, trying the
...
/// read-only `sage setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if nexus_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(nexus_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        nexus_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(nexus_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            nexus_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(nexus_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &nexus_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<nexus_config::signed_policy::SignatureEnvelope> {
    use nexus_config::signed_policy::now_unix;
    ...
    let claim = match nexus_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &nexus_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
/// Post-login hook for `sage login` and the ACP/TUI authenticate flow: clear any
...
/// Whether a credential exists that `sage setup` could install config for.
...
/// are the non-forgeable layers. Recovery: reconnect / `sage setup`; ceasing to serve
...
fn bump_managed_rollback_floor() {
    ...
    if !nexus_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::nexus_home::nexus_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            nexus_config::bump_rollback_floor(&home);
        }
...
/// Outcome of the `sage setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.nexus`.
    Installed,
...
/// Result of `sage setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `sage setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `sage setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `sage setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `sage setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && nexus_config::signed_policy::verification_active()
...
/// Run the `sage setup` sync for the current principal. The caller must check
