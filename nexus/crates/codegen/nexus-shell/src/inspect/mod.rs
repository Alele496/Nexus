...
use nexus_tools::util::truncate::estimate_tokens;
...
/// Enterprise login-hardening policy resolved from `[nexus_com_config]`
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `NEXUS_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .nexus/config.toml files). Driven from the same resolvers used at runtime
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = nexus_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = nexus_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = nexus_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        nexus_version: nexus_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if path
        .parent()
        .is_some_and(|parent| parent == nexus_home.join("rules"))
        || has_rules_directory(file_path, ".nexus")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = nexus_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        nexus_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `nexus_agent::prompt::agents_md::read_agents_config_with_paths`
    ...
    configs
        ...
        .map(|c| {
            let file_type =
                instruction_file_type(&c.file_path, &nexus_home, imported, &extra_rule_prefixes);
            let scope = instruction_scope(&c.file_path, &nexus_home, &vendor_homes, &workspace_root);
...
async fn list_permissions(cwd: &Path) -> PermissionsReport {
    use nexus_workspace::permission::resolution;
...
/// (`[nexus_com_config]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let nexus_com_config = config
        .map(|c| c.nexus_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: nexus_com_config.api_key_auth_disabled(),
        disable_api_key_auth: nexus_com_config.disable_api_key_auth,
        force_login_team_uuid: nexus_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[nexus_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = nexus_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        nexus_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);

    let home_dir = dirs::home_dir();
    let nexus_home = nexus_config::nexus_home();

    let mut entries: Vec<HookEntry> = registry
        .all_hooks()
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&nexus_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &nexus_agent::plugins::PluginRegistry,
    skills_config: &nexus_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = nexus_agent::prompt::skills::list_skills_with_plugins(
    ...
    skills
        .into_iter()
        .map(|s| {
            let source = skill_entry_source(&s, &nexus_home);
...
/// `<nexus_home>/skills/<name>/SKILL.md` and discovered as user skills, so a
...
fn skill_entry_source(
    s: &nexus_agent::prompt::skills::SkillInfo,
    nexus_home: &Path,
) -> ConfigSource {
    use nexus_tools::implementations::skills::types::SkillScope;
    ...
    match s.scope {
        ...
        SkillScope::User => {
            if crate::builtin::is_extracted_bundled_skill(&s.name, &path, nexus_home) {
...
fn list_agents(
    ...
    let agents = nexus_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[nexus_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                nexus_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                nexus_agent::plugins::PluginScope::Project => Scope::Project,
                nexus_agent::plugins::PluginScope::User => Scope::User,
                nexus_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: nexus_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    nexus_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = nexus_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
/// `.nexus/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_nexus_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.nexus_version, r.channel);
...
mod tests {
    use super::*;
    use nexus_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use nexus_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for (vendor, path) in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.nexus"), false, &[]);
        ...
        for path in ["/repo/.nexus/rules/team.md", r"C:\repo\.nexus\rules\team.md"] {
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.nexus"), false, &[]),
                "rules"
            );
        }
        for path in [
            ...
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.nexus"), true, &[]),
                "rules"
            );
        }
        for path in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.nexus"), true, &[]);
    ...
    #[test]
    fn nexus_home_nested_in_workspace_keeps_direct_surfaces_global() {
        let nexus_home = Path::new("/repo/config");
        let workspace = Path::new("/repo");
        for path in ["/repo/config/AGENTS.md", "/repo/config/rules/global.md"] {
            assert!(matches!(
                instruction_scope(path, nexus_home, &[], workspace),
                Scope::Global
            ));
        }
        for path in [
            "/repo/config/.nexus/rules/project.md",
    ...
    #[test]
    fn workspace_scope_wins_inside_nexus_home() {
        let nexus_home = Path::new("/custom/sage");
        ...
        assert!(matches!(
            instruction_scope("/custom/sage/rules/global.md", nexus_home, &[], workspace,),
            Scope::Global
        ));
    }

    #[test]
    fn custom_nexus_home_rules_are_classified_as_rules() {
        assert_eq!(
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.nexus/requirements.toml";
    ...
    fn skill_entry_source_maps_scopes() {
        let home = Path::new("/home/u/.nexus");

        let s = skill_fixture("a", "/repo/.nexus/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.nexus/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.nexus/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::User { .. }
        ));
        ...
        let s = skill_fixture(
            "d",
            "/home/u/.nexus/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("e", "/home/u/.nexus/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    /// path `<nexus_home>/skills/<name>/SKILL.md`; a same-named skill anywhere
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.nexus/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("help", "/repo/.nexus/skills/help/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        // Bundled name in a user dir outside <nexus_home>/skills: stays user.
        let s = skill_fixture(
            "help",
        ...
        // Bundled frontmatter name in a different dir under <nexus_home>/skills:
        // not the extracted copy — stays user.
        let s = skill_fixture(
            "help",
            "/home/u/.nexus/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Non-bundled name under <nexus_home>/skills: stays user.
        let s = skill_fixture(
            "my-skill",
            "/home/u/.nexus/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.nexus dirs.
        ...
        let registry = nexus_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
