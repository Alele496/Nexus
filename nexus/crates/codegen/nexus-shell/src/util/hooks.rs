...
use nexus_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .nexus/hooks/ directories are still scanned (they hold
    ...
    // user_nexus_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .nexus.
    ...
    if let Some(ref sage) = sage {
        global.push(sage.join("hooks"));
    }

    let custom_paths: Vec<PathBuf> = sage
        .as_ref()
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".nexus").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    nexus_hooks::discovery::HookRegistry,
    Vec<nexus_hooks::error::HookError>,
) {
