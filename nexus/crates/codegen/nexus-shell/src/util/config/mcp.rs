...
use nexus_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use nexus_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `sage-config-types` (config
...
pub use nexus_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use nexus_config_types::PoolConfig;

/// First-run setup wizard state. Written once after the user completes the
/// initial configuration flow; inspected on startup to decide whether to
/// show the wizard again.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct StartupWizardConfig {
    /// Set to `true` after the user completes the first-run setup wizard.
    #[serde(default)]
...
/// Walks from cwd up to the git repo root checking `.nexus/config.toml` at each level.
/// Project-scoped `.nexus/config.toml` entries override global `~/.nexus/config.toml`
...
/// `.nexus/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.nexus/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.nexus/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.nexus/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .nexus/config.toml"
                );
...
pub fn mcp_preferences_path() -> PathBuf {
    nexus_config::nexus_home().join("mcp_preferences.json")
}
...
pub fn collect_mcp_setup_configs(
    cwd: &std::path::Path,
    plugin_registry: Option<&nexus_agent::plugins::PluginRegistry>,
    compat: &CompatConfig,
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    nexus_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.nexus/config.toml`.
...
/// Upsert an MCP server entry in `~/.nexus/config.toml`.
///
...
/// config file, e.g. a project-scoped `.nexus/config.toml`.
...
/// Delete an MCP server entry from `~/.nexus/config.toml`.
///
...
/// config file, e.g. a project-scoped `.nexus/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = nexus_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.nexus/config.toml` files.
...
// `.mcp.json` discovery moved to `sage-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use nexus_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.nexus/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::nexus_home::nexus_home().join("config.toml")
}

/// Path to the user-level config file (`~/.nexus/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.nexus/config.toml`).
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.nexus/skills", "~/.nexus/skills/special/SKILL.md"]
ignore = ["~/.nexus/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.nexus/skills", "~/.nexus/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.nexus/skills/noisy/SKILL.md"]);
    }
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${NEXUS_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
