...
use std::sync::Arc;
/// Credentials for authenticating with sage backend services.
///
...
/// User token (xAI users) sends `Bearer` + `X-XAI-Token-Auth: sage-cli`.
...
impl GrokAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your NEXUS_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `sage login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
...
impl nexus_auth::HttpAuth for GrokAuthCredentials {
