...
///
/// Uses `getrusage(RUNEXUS_SELF)` on Unix; returns 0 if sampling fails or on
...
pub use nexus_telemetry::enums::PrCreationSource;
...
pub struct SessionSignals {
    ...
    // === Bash tool patterns (nexus_build) ===
...
impl SessionSignalsHandle {
    ...
    pub fn snapshot_gcs_queue(&self, stats: &nexus_file_utils::queue::UploadQueueStats) {
        ...
        let _ = self.tx.send(SignalEvent::RecordGcsQueueSnapshot {
            ...
            orphans_cleaned: nexus_file_utils::queue::last_orphans_cleaned(),
        });
    }
...
pub(crate) fn merge_tightest_trigger(current: Option<String>, new: &[String]) -> Option<String> {
    nexus_sampling_types::doom_loop::DoomLoopSignal::tightest(
        current
            .iter()
            .map(String::as_str)
...
mod tests {
    ...
    async fn test_model_tracking() {
        ...
        // Set primary model
        handle.set_primary_model("sage-3");

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.primary_model_id, Some("sage-3".to_string()));
        assert_eq!(snapshot.models_used, vec!["sage-3".to_string()]);

        // Record additional model usage
        handle.record_model_usage("sage-4");
        handle.record_model_usage("sage-3"); // Duplicate

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.models_used.len(), 2);
        assert!(snapshot.models_used.contains(&"sage-3".to_string()));
        assert!(snapshot.models_used.contains(&"sage-4".to_string()));
    ...
    async fn test_seed_counts_restores_all_counters() {
        ...
        handle.seed_counts(
            ...
            vec!["sage-3".to_string(), "sage-4".to_string()],
        );
        ...
        handle.record_model_usage("sage-3"); // existing model
        handle.record_model_usage("sage-4.5"); // new model
        ...
        assert_eq!(snapshot.models_used.len(), 3); // sage-3 not duplicated, sage-5 added
        assert!(snapshot.models_used.contains(&"sage-4.5".to_string()));
    ...
    async fn test_restore_signals_full_round_trip() {
        ...
        handle1.record_model_usage("sage-3");
        ...
        handle1.record_assistant_message();
        handle1.record_model_usage("sage-4");
        ...
        assert!(restored.models_used.contains(&"sage-3".to_string()));
        assert!(restored.models_used.contains(&"sage-4".to_string()));
        ...
        // ITL stats must survive the restore (regression test for sage-critique bug)
        assert_eq!(
        ...
        // This is the exact scenario the sage-critique bug describes: the
        ...
        handle2.record_model_usage("sage-3"); // existing model (should dedup)
        ...
        assert_eq!(after_turn.models_used.len(), 2); // sage-3 not duplicated
