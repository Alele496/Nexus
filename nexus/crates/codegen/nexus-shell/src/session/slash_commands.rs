...
use nexus_tools::implementations::skills::skill::format_skill_name;
...
pub(super) const BUILTIN_COMMANDS: &[BuiltinCommand] = &[
    BuiltinCommand {
        ...
        argument_hint: Some("可选：需要保留的上下文说明"),
        aliases: &[],
    ...
    BuiltinCommand {
        name: "dream",
        description: "运行记忆整合（将对话日志合并为有组织的主题）",
        argument_hint: None,
        aliases: &[],
        gate: BuiltinGate::Memory,
    ...
    BuiltinCommand {
        name: "hooks-add",
        description: "添加自定义Hook文件或目录",
        argument_hint: Some("Hook文件或目录的路径"),
        aliases: &[],
        gate: BuiltinGate::Hooks,
    ...
    BuiltinCommand {
        name: "plugins",
        description: "管理插件（列表、重载、信任、添加、移除）",
    ...
    BuiltinCommand {
        name: "reload-plugins",
        description: "从磁盘重载插件（/plugins reload 的别名）",
        argument_hint: None,
        aliases: &[],
        gate: BuiltinGate::Plugins,
    ...
    BuiltinCommand {
        name: "session-info",
        description: "显示会话详情（模型、对话轮次、上下文用量）",
        argument_hint: None,
        aliases: &["status", "info"],
        gate: BuiltinGate::AlwaysOn,
...
///   matching today's Sage flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use nexus_tools::implementations::skills::skill::{
...
/// `sage-tools` so it stays identical to the pager's `LoopCommand` and the
...
mod tests {
    ...
    fn make_skill(name: &str, user_invocable: bool) -> SkillInfo {
        SkillInfo {
            name: name.to_string(),
            display_name: None,
            description: format!("名为 {name} 的技能"),
            when_to_use: None,
            short_description: Some(format!("简介: {name}")),
            author: None,
            argument_hint: None,
    ...
    fn make_scoped_skill(name: &str, scope: SkillScope) -> SkillInfo {
        SkillInfo {
            name: name.to_string(),
            display_name: None,
            description: format!("{scope:?} 技能: {name}"),
            when_to_use: None,
            short_description: Some(format!("简介: {name}")),
            author: None,
            argument_hint: None,
