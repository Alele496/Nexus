...
//! content blocks, the session calls a vision-capable Sage model
...
use nexus_chat_state::compaction_utils::{extract_real_user_queries, extract_user_query};
use nexus_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
use nexus_tools::util::truncate::truncate_middle;
...
impl ImageDescribeCache {
    ...
    pub async fn get_or_describe(
        &self,
        client: nexus_sampler::SamplingClient,
        model: &str,
        raw_bytes: &[u8],
        mime_type: &str,
        outline: Option<&str>,
...
mod tests {
    use super::*;
    use nexus_sampling_types::conversation::{ConversationItem, UserItem};
    #[test]
    fn persist_and_prepend_image_files_writes_assets_and_lists_paths() {
        ...
        assert!(
            msg.contains(&std::path::MAIN_SEPARATOR.to_string())
                && msg.contains("assets")
                && msg.contains("image-")
        );
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![nexus_sampling_types::conversation::ContentPart::Text {
                text: text.into(),
            }],
            synthetic_reason: None,
            ..Default::default()
