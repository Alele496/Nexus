...
pub enum SessionCommand {
    ...
    SetSessionModel {
        sampling_config: nexus_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// rather than the agent-level default (e.g. `sage-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-sage-model-override` header and merges
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<nexus_chat_state::ModelMetadata>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<nexus_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.nexus/hooks`,
    ...
    /// Out-of-band history repair (`sage.local/session/repair`): fix tool-pairing
    ...
    RepairHistory {
        dry_run: bool,
        respond_to:
            oneshot::Sender<anyhow::Result<nexus_chat_state::compaction_utils::HistoryRepairReport>>,
    },
    GetRewindPoints {
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, nexus_chat_state::UsageTotals)>,
    ...
    /// mid-session via the `sage.local/session/update_mcp_servers` extension method).
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<nexus_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<nexus_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<nexus_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<nexus_hooks_plugins_types::HooksListResponse>,
    },
    /// Execute a hooks management action from the pager modal.
    HooksAction {
        action: nexus_hooks_plugins_types::HooksAction,
        respond_to: oneshot::Sender<nexus_hooks_plugins_types::ActionOutcome>,
    },
    ...
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<nexus_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// tool-free model call (default `sage-build-0.1` when available via
    ...
    /// `sage-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    Interject {
        ...
        /// `sage.local/session/interjection` so the originating pager can dedup its
    ...
    TakeTurnMessages {
        respond_to: oneshot::Sender<Option<nexus_chat_state::TurnCapture>>,
    },
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<nexus_sampling_types::conversation::ConversationItem>>>,
    },
