...
/// Memory log target — matches `nexus_telemetry::memory_log::TARGET`.
...
pub fn should_flush(
    ...
    let should = nexus_token_estimation::exceeds_threshold_with_headroom(
        total_tokens,
        context_window,
...
pub fn is_duplicate(content: &str, db_path: &std::path::Path) -> bool {
    ...
    let conn = match nexus_sqlite_journal::JournalMode::for_db_path(db_path).open_readonly(db_path) {
        Ok(c) => c,
        Err(_) => {
