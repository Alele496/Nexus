//! sage-build's L5 wiring onto the shared full-replace engine
//! (`nexus_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](nexus_compaction::sample_full_replace_summary);
//! this module adapts sage-build's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](nexus_compaction::FullReplaceError).
...
use nexus_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use nexus_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for sage-build's full-replace pass.
///
...
/// else the structured sage-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the sage-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured sage-build), ignoring the shared
...
/// Map sage-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces sage-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    nexus_telemetry::session_ctx::log_event(CompactionRetryDegraded {
