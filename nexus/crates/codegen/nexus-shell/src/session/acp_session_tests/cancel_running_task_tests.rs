...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(nexus_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel::<
                nexus_acp_lib::AcpClientMessage,
            >();
            ...
            let cwd = super::support::test_cwd();
            let fs = Arc::new(
                nexus_workspace::file_system::MockFs::new(cwd.to_path_buf()),
            );
            let terminal = Arc::new(DummyTerminal {});
            let (hunk_tx, _hunk_rx) = tokio::sync::mpsc::unbounded_channel();
            let hunk_tracker_handle = nexus_hunk_tracker::HunkTrackerActor::spawn(
            ...
            agent
                .tool_bridge()
                .update_resource(
                    nexus_tools::implementations::nexus_build::task::types::CurrentPromptIdResource(
                        "running".to_string(),
                    ),
                )
                .await;
                ...
                plan_mode: Arc::new(
                    parking_lot::Mutex::new(
                        crate::session::plan_mode::PlanModeTracker::new(
                            test_temp_dir().join("test-session"),
                        ),
                    ),
                ),
                goal_enabled: false,
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![nexus_sampling_types::ToolCall {
                    id: "call-1".into(),
...
async fn cancel_propagates_to_sampler_handle_so_no_further_emission() {
    ...
    local
        .run_until(async {
            ...
            let cfg = nexus_sampler::SamplerConfig {
                api_key: Some("test-key".to_string()),
                ...
                temperature: None,
                top_p: None,
                api_backend: nexus_sampler::ApiBackend::Responses,
                auth_scheme: Default::default(),
                extra_headers: Default::default(),
                context_window: 100_000,
                client_version: None,
            ...
            let (sampler_event_tx, _sampler_event_rx) = tokio::sync::mpsc::unbounded_channel::<
                nexus_sampler::SamplingEvent,
            >();
            let sampler_handle = nexus_sampler::SamplerActor::spawn(
                cfg,
                nexus_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel::<
                nexus_acp_lib::AcpClientMessage,
            ...
            let request = ConversationRequest {
                items: vec![
                    ConversationItem::User(nexus_sampling_types::UserItem { content :
                    vec![nexus_sampling_types::ContentPart::Text { text : "hi".into(),
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use nexus_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(
                    ...
                    matches!(p, nexus_sampling_types::ContentPart::Text { text } if
