...
use nexus_acp_lib::AcpAgentGatewaySender;
use nexus_paths::AbsPathBuf;
use nexus_workspace::permission::{AccessKind, ClientType, spawn_permission_manager};
...
fn install_real_permissions(actor: &mut SessionActor) {
    let cwd = AbsPathBuf::new(std::path::PathBuf::from(actor.session_info.cwd.clone()))
        .unwrap_or_else(|_| super::support::test_cwd());
    ...
    local
        .run_until(async {
            ...
            let d = session
                .permissions
                .request(
                    AccessKind::Bash("cargo test -p sage-workspace".into()),
                    dummy_update,
                    None,
                    None,
                    None,
            ...
            assert!(
                matches!(d, nexus_workspace::permission::Decision::Allow),
            ...
            let d2 = session
                ...
                .await;
            assert!(
                !matches!(d2, nexus_workspace::permission::Decision::Allow),
    ...
    local
        .run_until(async {
            ...
            actor.permissions.set_classifier_transcript(vec![
                nexus_workspace::permission::ClassifierTurn::UserText("please run tests".into()),
            ]);
...
fn build_classifier_turns_captures_tool_use_excludes_text_and_results() {
    use nexus_workspace::permission::ClassifierTurn;
    let conv = vec![
        ...
        super::ConversationItem::assistant_tool_calls(vec![
            nexus_sampling_types::conversation::ToolCall {
