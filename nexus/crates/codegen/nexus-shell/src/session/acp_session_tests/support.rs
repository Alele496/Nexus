...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> nexus_computer_hub_sdk::ObservabilityBridge {
    nexus_computer_hub_sdk::ObservabilityBridge::new(
        None,
        nexus_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> nexus_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> nexus_agent::Agent {
    use nexus_tools::implementations::nexus_build::update_goal::UpdateGoalTool;
...
#[cfg(test)]
pub(crate) async fn test_nexus_build_agent_with_todo() -> nexus_agent::Agent {
    use nexus_tools::implementations::nexus_build::todo::TodoWriteTool;
    use nexus_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> nexus_agent::Agent {
    use nexus_tools::implementations::nexus_build::enter_plan_mode::EnterPlanModeTool;
    use nexus_tools::implementations::nexus_build::exit_plan_mode::ExitPlanModeTool;
    use nexus_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<nexus_tools::registry::types::ToolConfig>,
) -> nexus_agent::Agent {
    test_agent_from_config(
        nexus_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        nexus_agent::AgentDefinition::default_nexus_build(),
        std::sync::Arc::new(nexus_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: nexus_agent::AgentDefinition,
    backend: std::sync::Arc<dyn nexus_tools::computer::types::TerminalBackend>,
) -> nexus_agent::Agent {
    use nexus_tools::computer::local::LocalFs;
    use nexus_tools::computer::types::AsyncFileSystem;
    ...
    nexus_agent::Agent::new(
        definition,
        nexus_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        nexus_agent::ReminderPolicy::default(),
        nexus_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<nexus_acp_lib::AcpClientMessage>,
    ...
    tool_context.task_completion_reservations =
        Some(nexus_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(nexus_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        permissions: nexus_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
...
pub(crate) async fn build_actor() -> (
    ...
    let (gateway_tx, gateway_rx) =
        tokio::sync::mpsc::unbounded_channel::<nexus_acp_lib::AcpClientMessage>();
