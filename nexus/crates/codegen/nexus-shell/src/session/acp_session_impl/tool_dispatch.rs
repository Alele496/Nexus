...
/// - `file_path` — nexus_build (`search_replace`), opencode (`EditTool`,
///   `WriteTool`, `ReadTool`), codex (`read_file`), nexus_build_hashline
///   (`hashline_edit`)
/// - `path` — alternate edit/read tools
/// - `target_file` — nexus_build (`read_file`, via `#[serde(rename)]`)
///
...
/// `list_dir`=`target_directory`. Grammar lives in `nexus_chat_state`.
pub(super) fn compaction_artifact_read(
    args: &serde_json::Value,
) -> Option<nexus_chat_state::compaction_transcript::CompactionArtifact> {
    let path = str_arg(
        args,
        &["target_file", "file_path", "path", "target_directory"],
    )?;
    nexus_chat_state::compaction_transcript::classify_compaction_path(path)
}
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = nexus_tools::util::strip_redundant_session_cd(
            &command,
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(nexus_tools::types::tool::ToolKind::Execute)
        };
...
// Re-use the UTF-8-safe truncation helper from sage-sampling-types rather
...
///    [`MAX_ARGS_IN_ERROR`] bytes).  Without this, sage-shell would sanitize
