...
fn is_interruptible_wait_tool(tool_name: &str, args: &serde_json::Value) -> bool {
    match tool_name {
        ...
        | "get_terminal_command_output" => nexus_tool_types::task_output_waits_from_json(args),
...
fn interrupted_wait_tool_result_with_msg(args: &serde_json::Value, msg: &str) -> ToolRunResult {
    use nexus_tool_types::{TaskOutputOutput, TaskOutputResult};
    let task_id = args
        .get("task_ids")
...
pub(super) enum PlanEditGate {
    ...
    Allow,
    /// Sage-toolset edit outside the plan file (plan-file-only rule).
    RejectNonPlanFile,
}
...
///   `AccessKind::Edit` and is plan-file-only (same as sage edits). Deleting
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &nexus_tools::implementations::nexus_build::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
///
/// Uses `nexus_acp_lib`'s TYPED [`AcpChannelFailure`](nexus_acp_lib::AcpChannelFailure)
...
fn ext_method_no_client(err: &acp::Error) -> bool {
    matches!(
        nexus_acp_lib::acp_channel_failure(err),
        Some(nexus_acp_lib::AcpChannelFailure::SendFailed)
    )
}
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        nexus_tools::normalization::merge_tool_meta(
            &toolset,
    ...
    pub(super) async fn execute_tool_calls(
        ...
        for call in tool_calls.into_iter() {
            ...
            self.observability_bridge
                .emit(
                    nexus_tool_protocol::session_event::SessionEvent::ToolCallStarted {
                ...
                .await?
            {
                ...
                Err(tool_loop) => {
                        ...
                        crate::session::mcp_servers::parse_mcp_tool_name(&call_name)
                    {
                        ...
                        self.emit_event(nexus_file_utils::events::Event::McpToolCallCompleted {
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
                ...
                && server.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX)
            {
                let auth_rejected = match &result {
                    Err(err) => nexus_mcp::servers::is_auth_rejection_message(&err.to_string()),
                    Ok(tool_result) => {
                        tool_result.output.is_error()
                            && nexus_mcp::servers::is_auth_rejection_message(
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(nexus_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self
                        .hook_event_active(nexus_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            nexus_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            nexus_hooks::event::HookEventName::PostToolUseFailure,
                            nexus_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    nexus_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    nexus_hooks::event::HookEventName::PostToolUse,
                    nexus_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
            ...
            self.observability_bridge
                .emit(
                    nexus_tool_protocol::session_event::SessionEvent::ToolCallCompleted {
                ...
                .await;
            let (ext_file_path, ext_parameters) = if nexus_telemetry::external::is_active() {
            ...
            nexus_telemetry::session_ctx::log_event(
                nexus_telemetry::events::ToolCallCompleted {
                    tool_name: prepared.tool_name.clone(),
                    outcome: tool_outcome,
                    duration_ms,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(nexus_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                nexus_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                nexus_hooks::event::HookEventName::PreToolUse,
                None,
                nexus_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    nexus_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let nexus_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                nexus_workspace::permission::AccessKind::Read(p) => (
                    nexus_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                nexus_workspace::permission::AccessKind::Edit(p) => {
                    (nexus_telemetry::events::AccessKind::Edit, p.clone())
                }
                nexus_workspace::permission::AccessKind::Bash(cmd) => {
                    (nexus_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                nexus_workspace::permission::AccessKind::Grep { path, glob } => (
                    nexus_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                nexus_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (nexus_telemetry::events::AccessKind::Mcp, name.clone())
                }
                nexus_workspace::permission::AccessKind::WebFetch(u) => {
                    (nexus_telemetry::events::AccessKind::Web, u.clone())
                }
                nexus_workspace::permission::AccessKind::WebSearch(q) => {
                    (nexus_telemetry::events::AccessKind::Web, q.clone())
                }
            };
            ...
            nexus_telemetry::session_ctx::log_event(
                nexus_telemetry::events::PermissionPrompted {
                    tool_name: call.function.name.clone(),
            ...
            let edit_path_context = matches!(&access_kind, AccessKind::Edit(_)).then(|| {
                nexus_workspace::permission::types::EditPathContext {
            ...
            self.events.permission_resolved(
                ...
                match &decision {
                    Decision::Allow | Decision::Ask => {
                        nexus_file_utils::events::types::PermissionDecision::Allow
                    }
                    Decision::Reject(_) | Decision::PolicyDeny(_) => {
                        nexus_file_utils::events::types::PermissionDecision::Deny
                    }
                    Decision::Cancelled => {
                        nexus_file_utils::events::types::PermissionDecision::Cancelled
                    }
                    Decision::FollowupMessage(_) => {
                        nexus_file_utils::events::types::PermissionDecision::Followup
                    }
                },
                perm_start,
            );
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (nexus_telemetry::events::PermissionOutcome::Allow, None)
                }
                Decision::Reject(reason) | Decision::PolicyDeny(reason) => (
                    nexus_telemetry::events::PermissionOutcome::Deny,
                    Some(reason.to_string()),
                ),
                Decision::Cancelled => (
                    nexus_telemetry::events::PermissionOutcome::Cancelled,
                    None,
                ),
                Decision::FollowupMessage(_) => (
                    nexus_telemetry::events::PermissionOutcome::Followup,
                    None,
                ),
            };
            tracing::info_span!(
            ...
            .in_scope(|| {});
            nexus_telemetry::session_ctx::log_event(
                nexus_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        nexus_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        nexus_hooks::event::HookEventName::PermissionDenied,
                        nexus_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
    ...
    pub(super) async fn request_plan_approval(
        ...
        nexus_tools::implementations::nexus_build::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use nexus_tools::implementations::nexus_build::exit_plan_mode::{
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                nexus_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
        ...
        let (title, kind, locations, content) = match tool_call_input {
            ...
            ToolInput::ReadFile(read_file) => (
                ...
                vec![
                    acp::ToolCallLocation::new(read_file.path).line(
                        nexus_tools::normalization::norm_offset_i64(read_file.offset)
                            .map(|l| l as u32),
                    ),
                ],
                Vec::new(),
            ...
            ToolInput::WaitTasks(wait) => (
                format!(
                    ...
                    wait.task_ids.len(),
                    match wait.mode {
                        nexus_tool_types::WaitMode::WaitAny => "wait_any",
                        nexus_tool_types::WaitMode::WaitAll => "wait_all",
                    }
                ),
                acp::ToolKind::Other,
                vec![],
            ...
            ToolInput::Skill(skill) => {
                nexus_telemetry::session_ctx::log_event(
                    nexus_telemetry::events::SkillDispatched {
    ...
    async fn handle_tool_parse_error(
        ...
        call_id: &str,
        function_name: &str,
        err: nexus_tool_runtime::ToolError,
        raw_arguments: &str,
        model_id: &str,
    ) -> Result<(), acp::Error> {
        tracing::error!(
    ...
    /// `nexus_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use nexus_telemetry::enums::PrCreationSource;
        use nexus_tools::util::git_detect;
        match &result.output {
            nexus_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    nexus_telemetry::session_ctx::log_event(
                        nexus_telemetry::events::PrMerged {},
                    );
                }
            }
            nexus_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: nexus_tools::util::git_detect::PrRef,
        source: nexus_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            nexus_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                nexus_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let nexus_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                nexus_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | nexus_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            nexus_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            nexus_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`nexus_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: nexus_sampler::SamplingEvent,
    ) {
        use nexus_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::Retrying {
                ...
                if kind == nexus_sampler::SamplingErrorKind::DoomLoopDetected {
            ...
            SamplingEvent::Failed { request_id, error } => {
                nexus_telemetry::unified_log::error(
...
fn execute_tool_call_parts(
    ...
    let display = nexus_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod plan_mode_edit_gate_tests {
    ...
    use nexus_tools::types::ToolInput;
    use nexus_workspace::permission::AccessKind;
    ...
    fn search_replace(path: &str) -> ToolInput {
        use nexus_tools::implementations::nexus_build::search_replace::SearchReplaceInput;
    ...
    fn write(path: &str) -> ToolInput {
        use nexus_tools::implementations::opencode::write::WriteInput;
        ToolInput::Write(WriteInput {
            file_path: path.into(),
            content: "x".into(),
        })
    }
    /// Sage edit tools are plan-file-only while plan mode is active — the
    /// enforcement that makes plan mode read-only even under always-approve.
    #[test]
    fn nexus_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "sage tools get no markdown exception — plan file only"
        );
    }
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use nexus_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
    ...
    fn non_edit_tools_not_gated() {
        use nexus_tools::implementations::BashToolInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use nexus_tools::implementations::nexus_build::exit_plan_mode::ExitPlanModeExtResponse;
    ...
    fn ext_method_no_client_defaults_false_for_untagged_error() {
        assert!(!ext_method_no_client(&nexus_acp_lib::acp_internal_error(
...
mod wait_interrupt_tests {
    ...
    use nexus_tools::types::output::ToolOutput;
    use nexus_tool_types::TaskOutputOutput;
    ...
    async fn pending_interjection_aborts_in_flight_wait() {
        ...
        use nexus_interjection_core::PendingInterjection;
