...
fn partition_rules_by_scope(
    files: Vec<nexus_agent::prompt::agents_md::AgentConfigFile>,
    nexus_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_root: Option<&std::path::Path>,
) -> (
    Vec<nexus_agent::prompt::user_message::RuleEntry>,
    Vec<nexus_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = nexus_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use nexus_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[nexus_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
        entries.iter().map(|entry| entry.content.as_str()).collect()
    }
    #[test]
    fn partitions_custom_nexus_and_vendor_home_rules_as_user_scope() {
        let files = vec![
    ...
    #[test]
    fn nexus_home_nested_in_workspace_keeps_direct_surfaces_user_scoped() {
        let files = vec![
            file("/repo/config/AGENTS.md"),
            file("/repo/config/rules/global.md"),
            file("/repo/config/.nexus/rules/project.md"),
    ...
    #[test]
    fn nested_nexus_home_workspace_files_stay_workspace_scoped() {
        let files = vec![
            file("/custom/sage/rules/global.md"),
            file("/custom/sage/worktrees/repo/.cursor/rules/project.md"),
            file("/custom/sage/worktrees/repo/src/AGENTS.md"),
        ];
        let (workspace, user) = partition_rules_by_scope(
            files,
            Path::new("/custom/sage"),
            &[],
            Some(Path::new("/custom/sage/worktrees/repo")),
        );
        assert_eq!(paths(&user), vec!["/custom/sage/rules/global.md"]);
        assert_eq!(
            paths(&workspace),
            vec![
...
/// end for sage ordering), so a failed offload never
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(nexus_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: nexus_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use nexus_agent::prompt::agents_md::read_agents_config_with_paths;
        use nexus_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use nexus_workspace::file_system::{git_status_short, jj_status};
        use nexus_workspace::session::git::VcsKind;
    ...
    ) -> Vec<nexus_agent::prompt::user_message::McpServerEntry> {
        use nexus_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &nexus_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
        ...
        let client = nexus_sampler::SamplingClient::new(sampler_config).map_err(|e| {
