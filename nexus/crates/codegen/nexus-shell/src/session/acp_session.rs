...
use nexus_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use nexus_agent::prompt::skills::SkillsConfig;
use nexus_sampler::SamplerConfig as SamplingConfig;
use nexus_sampling_types::truncate_bytes;
...
use nexus_tools::types::compat::CompatConfig;
use nexus_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use nexus_workspace::file_system::CodebaseIndexManager;
use nexus_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use nexus_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
impl nexus_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        nexus_tools::types::resources::ManagedGatewayToolCallResponse,
        nexus_tool_runtime::ToolError,
    > {
        let auth_key = self
            .auth_manager
            .get_valid_token()
            ...
            .ok_or_else(|| nexus_tool_runtime::ToolError::unauthorized("no auth token available"))?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                nexus_tool_runtime::ToolError::unauthorized(detail)
            } else if status == reqwest::StatusCode::FORBIDDEN {
                nexus_tool_runtime::ToolError::permission_denied(detail)
            } else {
                let tool_id = nexus_tool_protocol::ToolId::new(caller)
                    .unwrap_or_else(|_| nexus_tool_protocol::ToolId::new("use_tool").expect("valid"));
                nexus_tool_runtime::ToolError::execution(tool_id, detail)
            };
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            nexus_tool_runtime::ToolError::network_error(format!(
                "Managed MCP gateway tool call failed: {}",
                e.without_url()
            ))
        }
        crate::session::managed_mcp::ManagedMcpFetchError::NoAuth => {
            nexus_tool_runtime::ToolError::unauthorized("no auth token available")
        }
    }
}
#[cfg(test)]
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, nexus_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(err.kind, nexus_tool_runtime::ToolErrorKind::PermissionDenied);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, nexus_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, nexus_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<nexus_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: nexus_chat_state::ChatStateHandle,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<nexus_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<nexus_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) doom_loop_recovery: Option<nexus_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<nexus_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<nexus_agent::Agent>,
    ...
    /// Kill-switch (`NEXUS_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<nexus_tools::implementations::nexus_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, nexus_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: nexus_agent_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<nexus_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<nexus_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<nexus_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: nexus_computer_hub_sdk::ObservabilityBridge,
    ...
    pub(crate) sampler_handle: nexus_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`nexus_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`nexus_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: nexus_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    /// Fire-and-forget — failures are logged but do not interrupt the turn.
    async fn send_after_turn_event(&self, payload: nexus_tool_protocol::turn_hook::AfterTurnPayload) {
        self.workspace_ops
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == nexus_tools::implementations::nexus_build::SCHEDULER_CREATE_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<nexus_tools::bridge::ToolBridge> {
...
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &nexus_agent::PromptContext) {
...
) -> Option<nexus_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use nexus_tools::types::output::{MCPOutput, ToolOutput};
    use nexus_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl nexus_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl nexus_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> nexus_tool_protocol::ToolId {
            nexus_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::nexus_tool_runtime::ListToolsContext,
        ) -> nexus_tool_types::ToolDescription {
            nexus_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
            _ctx: nexus_tool_runtime::ToolCallContext,
            _args: serde_json::Value,
        ) -> Result<ToolOutput, nexus_tool_runtime::ToolError> {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use nexus_tools::registry::types::ToolConfig;
    use nexus_tools::tool_taxonomy::TOOL_META_KEY;
    use nexus_workspace::permission::PermissionCommand;
            ...
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id(
                        "NexusBuild:read_file".to_string(),
                    )])
                    .await,
                );
...
mod managed_gateway_tool_tests {
    ...
    use nexus_tools::types::output::{MCPOutput, ToolOutput};
    use nexus_tools::types::tool::{ToolKind, ToolNamespace};
    use nexus_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<nexus_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
