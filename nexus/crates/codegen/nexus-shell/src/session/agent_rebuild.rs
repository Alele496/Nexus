...
//! [`nexus_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`nexus_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`nexus_agent::Agent`] owns an [`nexus_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`nexus_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use nexus_agent::config::AgentDefinition;
use nexus_agent::error::AgentBuildError;
use nexus_agent::prompt::context::PromptAudience;
use nexus_agent::prompt::skills::SkillsConfig;
use nexus_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use nexus_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use nexus_tools::implementations::nexus_build::ask_user_question::types::UserQuestionRequest;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<nexus_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<nexus_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use nexus_tools::implementations::nexus_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(nexus_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(nexus_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use nexus_tools::implementations::nexus_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            nexus_tools::computer::local::LocalTerminalBackend::new_local(
                nexus_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(nexus_tools::types::tool::ToolKind::Task)
            .expect("NexusBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("NexusBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_nexus_build())
                    .await
