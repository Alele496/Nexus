...
/// Env lane (desktop `NEXUS_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`NEXUS_CHAT_MODE`); hard-off in release builds.
...
/// Parse `sage.local/session/list` params and, under process-wide chat mode, force
...
mod tests {
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["_meta"]["sage.local/session"]["kind"], "build");
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = nexus_test_support::EnvGuard::set(
            "NEXUS_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = nexus_test_support::EnvGuard::unset("NEXUS_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = nexus_test_support::EnvGuard::set("NEXUS_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = nexus_test_support::EnvGuard::set("NEXUS_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::NEXUS_CHAT_MODE_ENV;
        let _chat_off = nexus_test_support::EnvGuard::unset(NEXUS_CHAT_MODE_ENV);
        let _desktop_off =
            nexus_test_support::EnvGuard::unset("NEXUS_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                nexus_test_support::EnvGuard::set("NEXUS_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = nexus_test_support::EnvGuard::set(NEXUS_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = nexus_test_support::EnvGuard::unset(NEXUS_CHAT_MODE_ENV);
        ...
        {
            let _on = nexus_test_support::EnvGuard::set(NEXUS_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
