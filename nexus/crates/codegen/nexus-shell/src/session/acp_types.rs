...
impl TokenUsageCategory {
    ...
    pub fn skills_listing(text: &str, skill_count: usize) -> Self {
        Self {
            ...
            tokens: nexus_token_estimation::estimate_tokens(text),
...
pub struct ContextInfo {
    ...
    /// always matches the actual trigger (e.g. 65 for sage-build in remote settings).
...
impl ContextInfo {
    ...
    pub fn from_notification(used: u64, total: u64) -> Self {
        Self {
            used,
            total,
            usage_pct: nexus_token_estimation::usage_percentage_u8(used, total),
            free_tokens: nexus_token_estimation::free_tokens(total, used),
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `sage-build`).
...
pub fn is_coding_model_slug(model: &str) -> bool {
    matches!(model, "sage-build" | "sage-4.5")
}
...
/// Full wire response for `sage.local/session/info`.
///
...
mod tests {
    ...
    fn should_show_model_fingerprint_truth_table() {
        ...
        assert!(should_show_model_fingerprint(false, "sage-build"));
        assert!(should_show_model_fingerprint(false, "sage-4.5"));
    ...
    fn desktop_client_type_deserializes_and_round_trips() {
        ...
        let submission = input.to_submission(Some("sage-3".into()), None, None, Some(5));
        assert_eq!(
