...
use nexus_computer_hub_sdk::{AuthCredential, AuthIdentity, AuthProvider};
use nexus_workspace::WorkspaceHandle;
...
struct ClientState {
    ...
    /// The client type string from IPC registration (e.g., "sage-tui", "sage-code-extension").
...
///
/// - `sage.local/sessions/changed` — roster delta; every open dashboard must stay
///   in sync.
...
/// - `sage.local/announcements/update` — the announcements list changed (startup
...
/// `sage.local/session_notification` whose `update.sessionUpdate ==
...
fn extract_interaction_resolved_tool_call_id(json: &serde_json::Value) -> Option<String> {
    if method_of(json) != Some("sage.local/session_notification") {
        return None;
    }
...
fn extract_session_id_from_prompt_complete(json: &serde_json::Value) -> Option<String> {
    ...
    if method != "sage.local/session/prompt_complete" {
        return None;
    }
    json.get("params")?
        .get("sessionId")
...
const PROD_COMPUTER_HUB_URL: &str = "wss://computer-hub.nexus.local/v1/tools";
...
async fn handle_workspace_start(
    ...
    let handle = nexus_workspace::connect_local_workspace(
        cwd_path.clone(),
        url,
        auth,
        Some(metadata),
...
fn make_version_mismatch_notification(
    ...
    Some(
        serde_json::json!(
            { "jsonrpc" : "2.0", "method" : "sage.local/leader/version_mismatch", "params" : {
            ...
            format!("Client version {client_version} differs from leader version \
                     {leader_version}. Restart the sage binary to use the same version.")
            } }
        )
        .to_string(),
    )
...
///   sage.local WebSocket relay until this fires, so a leader serving only
        ...
        match poll {
            ...
            LeaderServerPoll::Event(event) => match event {
                ServerEvent::Registered(id, mode, capabilities, client_type) => {
                    if let Some(client) = clients.get_mut(&id) {
                        ...
                        nexus_telemetry::unified_log::info(
            ...
            LeaderServerPoll::Response(payload) => {
                    ...
                    && !clients.contains_key(&orphan_client)
                {
                    ...
                    nexus_telemetry::unified_log::warn(
...
mod tests {
    ...
    async fn workspace_start_errors_when_cancelled_before_auth() {
        let state = default_test_control_state(Path::new("/tmp/sage-ws-auth-test.sock"));
        let cancel = CancellationToken::new();
        cancel.cancel();
    ...
    /// dashboard, IDE) keeps the sage.local relay off. The first Headless
    /// registration (devbox / `sage agent headless` flow) flips the watch so
    ...
    async fn relay_demand_signals_only_on_headless_registration() {
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let _stdio =
            connect_and_register_with_mode(&sock_path, "sage-tui", ClientMode::Stdio).await;
        ...
        let _headless =
            connect_and_register_with_mode(&sock_path, "sage-headless", ClientMode::Headless).await;
    ...
    async fn initialize_gets_client_identifier_injected() {
        ...
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "sage-tui",
    ...
    async fn initialize_preserves_existing_client_identifier() {
        ...
        .unwrap();
        let _: ServerMessage = read_message(&mut reader).await.unwrap();
        let payload = r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"sage-web"}}}"#;
        write_message(
            &mut writer,
            &ClientMessage::Acp {
                payload: payload.into(),
        ...
        let json: serde_json::Value = serde_json::from_str(&received).unwrap();
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "sage-web",
    ...
    fn is_interaction_request_detects_only_interaction_methods() {
        ...
        assert!(!is_interaction_request(&pv(
            r#"{"jsonrpc":"2.0","method":"sage.local/sessions/changed","params":{}}"#
        )));
    }
    #[test]
    ...
    fn extract_interaction_resolved_tool_call_id_matches_only_resolved() {
        assert_eq!(
            ...
            pv(r#"{"method":"sage.local/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-r"}}}"#))
            .as_deref(), Some("tc-r")
        );
        assert_eq!(
            extract_interaction_resolved_tool_call_id(&
            pv(r#"{"method":"_sage.local/session_notification","params":{"method":"sage.local/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-rw"}}}}"#))
            .as_deref(), Some("tc-rw")
        );
        assert_eq!(
            extract_interaction_resolved_tool_call_id(&pv(
                r#"{"method":"sage.local/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"pending_interaction","tool_call_id":"tc-r","kind":"permission"}}}"#
            )),
            None
        );
    }
    ...
    fn event_seq_of_parses_acp_and_ext_and_handles_missing() {
        ...
        let ext = pv(
            r#"{"params":{"method":"sage.local/session/update","params":{"sessionId":"019e-aa","_meta":{"eventId":"019e-aa-7"}}}}"#,
        );
    ...
    fn inject_capabilities_adds_default_model_to_session_new() {
        ...
        let caps = ClientCapabilities {
            yolo_mode: false,
            default_model: Some("sage-3-fast".to_string()),
            ..Default::default()
        };
        let mut json = pv(&payload);
        assert!(inject_capabilities_into_session_new(
        ...
        assert_eq!(json["params"]["_meta"]["modelId"], "sage-3-fast");
        assert!(json["params"]["_meta"].get("yoloMode").is_none());
    }
    #[test]
    ...
    fn extract_model_id_from_set_model_returns_value() {
        let payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"sessionId":"sess-123","modelId":"sage-3-fast"}}}}"#,
            AGENT_METHOD_NAMES.session_set_model
        );
        assert_eq!(
            extract_model_id_from_set_model(&pv(&payload)),
            Some("sage-3-fast".to_string())
        );
    }
    #[test]
    fn extract_model_id_from_set_model_handles_snake_case() {
        let payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"session_id":"sess-123","model_id":"sage-3"}}}}"#,
            AGENT_METHOD_NAMES.session_set_model
        );
        assert_eq!(
            extract_model_id_from_set_model(&pv(&payload)),
            Some("sage-3".to_string())
        );
    }
    #[test]
    fn extract_model_id_from_set_model_returns_none_for_other_methods() {
        let payload =
            r#"{"jsonrpc":"2.0","method":"other/method","id":1,"params":{"modelId":"sage-3"}}"#;
    ...
    fn patch_initialize_response_patches_current_model_id() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"modelState":{"currentModelId":"sage-3","availableModels":[]}}}}"#,
        );
        let default_model = Some("sage-3-fast".to_string());
    ...
    fn patch_initialize_response_preserves_other_fields() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"grokShell":true,"modelState":{"currentModelId":"sage-3","availableModels":[{"modelId":"sage-3"},{"modelId":"sage-3-fast"}]}}}}"#,
        );
        let default_model = Some("sage-3-fast".to_string());
        assert!(patch_initialize_response_model(&mut json, &default_model));
    ...
    fn patch_initialize_response_noop_when_no_default_model() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"modelState":{"currentModelId":"sage-3"}}}}"#,
        );
        let before = json.clone();
    ...
    fn patch_initialize_response_noop_for_non_initialize_response() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"session_id":"sess-1","models":{"currentModelId":"sage-3","availableModels":[]}}}"#,
        );
        let before = json.clone();
        assert!(!patch_initialize_response_model(
            &mut json,
    ...
    fn extract_session_id_from_nested_params_works() {
        let payload = r#"{"jsonrpc":"2.0","method":"_sage.local/session_notification","params":{"method":"sage.local/session_notification","params":{"sessionId":"sess-nested"}}}"#;
        assert_eq!(
            extract_session_id(&pv(payload)),
    ...
    fn extract_session_id_from_prompt_complete_works() {
        let payload = r#"{"jsonrpc":"2.0","method":"sage.local/session/prompt_complete","params":{"sessionId":"sess-prompt"}}"#;
        assert_eq!(
    ...
    fn extract_session_id_from_prompt_complete_ignores_other_methods() {
        let payload = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-prompt"}}"#;
        assert_eq!(extract_session_id_from_prompt_complete(&pv(payload)), None);
    }
    #[test]
    fn extract_child_session_event_spawned() {
        let payload = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-1"}}}"#;
    ...
    fn extract_child_session_event_finished() {
        let payload = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-2"}}}"#;
        match extract_child_session_event(&pv(payload)) {
    ...
    fn extract_child_session_event_nested_ext_notification() {
        let payload = r#"{"jsonrpc":"2.0","method":"_sage.local/session_notification","params":{"method":"sage.local/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-3"}}}}"#;
        match extract_child_session_event(&pv(payload)) {
    ...
    fn extract_child_session_event_nested_ext_notification_finished() {
        let payload = r#"{"jsonrpc":"2.0","method":"_sage.local/session_notification","params":{"method":"sage.local/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-4"}}}}"#;
        match extract_child_session_event(&pv(payload)) {
    ...
    fn extract_child_session_event_none_for_other_updates() {
        let payload = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"message_delta","content":"hello"}}}"#;
        assert!(extract_child_session_event(&pv(payload)).is_none());
    }
    #[test]
    fn extract_child_session_event_none_without_child_id() {
        let payload = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned"}}}"#;
        assert!(extract_child_session_event(&pv(payload)).is_none());
    }
    #[test]
    ...
    fn inject_capabilities_does_not_override_existing_client_identifier() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "sage-tui", ClientId(1));
    ...
    fn inject_capabilities_adds_leader_client_id_to_session_load() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "sage-tui", ClientId(42));
        assert_eq!(
    ...
    fn extract_target_client_id_some_when_meta_present() {
        ...
        let nested = r#"{"jsonrpc":"2.0","method":"_sage.local/session/update","params":{"params":{"sessionId":"sess-1","_meta":{"x.ai/leaderClientId":11}}}}"#;
    ...
    fn inject_yolo_notification_adds_client_identifier() {
        ...
        assert_eq!(json["params"]["clientIdentifier"], "sage-tui");
    ...
    fn inject_client_identity_adds_identifier_to_initialize() {
        ...
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "sage-tui");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(mutated, "should have injected the identifier");
        assert_eq!(json["params"]["_meta"]["clientIdentifier"], "sage-tui");
    }
    #[test]
    fn inject_client_identity_does_not_override_existing() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"sage-web"}}}"#,
        );
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "sage-tui");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(!mutated, "existing identifier means nothing was injected");
        assert_eq!(json["params"]["_meta"]["clientIdentifier"], "sage-web");
    }
    #[test]
    ...
    fn inject_client_identity_preserves_existing_meta() {
        ...
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "sage-code-extension");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(mutated);
        assert_eq!(json["params"]["_meta"]["foo"], "bar");
        assert_eq!(
    ...
    fn version_mismatch_notification_contains_correct_fields() {
        ...
        assert_eq!(json["method"], "sage.local/leader/version_mismatch");
    ...
    async fn set_model_updates_default_model_for_next_session_new() {
        ...
        write_message(
            ...
            &ClientMessage::Register {
                ...
                capabilities: ClientCapabilities {
                    yolo_mode: false,
                    default_model: Some("sage-original".to_string()),
                    ..Default::default()
                },
            },
        )
        ...
        let set_model_payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"sessionId":"sess-1","modelId":"sage-4.5"}}}}"#,
            AGENT_METHOD_NAMES.session_set_model
        );
        write_message(
            &mut writer,
        ...
        let json: serde_json::Value = serde_json::from_str(&forwarded).unwrap();
        assert_eq!(
            json["params"]["_meta"]["modelId"], "sage-4.5",
    ...
    async fn ext_notification_with_nested_session_id_routes_correctly() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"_sage.local/session_notification","params":{"method":"sage.local/session_notification","params":{"sessionId":"sess-A","update":{"sessionUpdate":"retry_state","attempt":1,"maxRetries":3,"reason":"transient"}}}}"#
                    .into(),
            )
            .unwrap();
        let msg: ServerMessage =
        ...
        match msg {
            ServerMessage::Acp { payload } => {
                let json: serde_json::Value = serde_json::from_str(&payload).unwrap();
                assert_eq!(json["method"], "_sage.local/session_notification");
            }
            other => panic!("Expected Acp message, got {:?}", other),
        }
    ...
    async fn high_throughput_replay_no_drops() {
        ...
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let (mut reader, mut writer) = connect_and_register(&sock_path, "sage-tui").await;
    ...
    async fn reattached_client_backfilled_into_child_routes() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-sub","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-sub"}}}"#;
    ...
    async fn replayed_spawn_registers_child_route_for_loading_client() {
        ...
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{{"sessionId":"sess-fresh","_meta":{{"isReplay":true,"x.ai/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-fresh"}}}}}}"#,
            a_id.0
        );
    ...
    async fn late_attacher_backfilled_into_existing_child_routes() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-sub2","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-sub2"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
    ...
    async fn replayed_finished_does_not_tear_down_live_child_route() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-tear","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-tear"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        ...
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{{"sessionId":"sess-tear","_meta":{{"isReplay":true,"x.ai/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-tear"}}}}}}"#,
            b_id.0
        );
    ...
    async fn backfill_covers_nested_children() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_child = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-nest","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-nest"}}}"#;
        response_tx.send(spawned_child.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let spawned_grandchild = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"child-nest","update":{"sessionUpdate":"subagent_spawned","child_session_id":"grandchild-nest"}}}"#;
    ...
    async fn intermediate_finish_reparents_live_grandchild_for_root_backfill() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_a = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-rep","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-a"}}}"#;
        response_tx.send(spawned_a.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "child-a").await;
        let spawned_b = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"child-a","update":{"sessionUpdate":"subagent_spawned","child_session_id":"grandchild-b"}}}"#;
        response_tx.send(spawned_b.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "grandchild-b").await;
        let finished_a = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-rep","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-a"}}}"#;
    ...
    async fn live_finished_prunes_index_so_reattach_skips_dead_child() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-dead","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-dead"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let finished_live = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-dead","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-dead"}}}"#;
    ...
    async fn detached_live_finished_prunes_index_so_reattach_skips_dead_child() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-detach","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-detach"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        drop(reader_a);
        drop(writer_a);
        tokio::time::sleep(Duration::from_millis(50)).await;
        let finished_live = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-detach","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-detach"}}}"#;
        response_tx.send(finished_live.to_string()).unwrap();
        tokio::time::sleep(Duration::from_millis(30)).await;
        let (mut reader_a2, mut writer_a2) = connect_and_register(&sock_path, "client-a").await;
    ...
    async fn mid_burst_disconnect_still_prunes_dead_child_route() {
        ...
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{{"sessionId":"sess-leak","_meta":{{"isReplay":true,"x.ai/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-leak"}}}}}}"#,
            a_id.0
        );
        response_tx.send(spawned_replay).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{{"sessionId":"sess-leak","_meta":{{"isReplay":true,"x.ai/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-leak"}}}}}}"#,
            a_id.0
        );
        response_tx.send(finished_replay).unwrap();
        tokio::time::sleep(Duration::from_millis(30)).await;
    ...
    async fn orphaned_replayed_finished_leaves_held_route_untouched() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-hold","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-hold"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{{"sessionId":"sess-hold","_meta":{{"isReplay":true,"x.ai/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-hold"}}}}}}"#,
            b_id.0
        );
        response_tx.send(finished_replay).unwrap();
        tokio::time::sleep(Duration::from_millis(30)).await;
    ...
    async fn replayed_spawn_unions_into_existing_live_route() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-union","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-union"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let (mut reader_b, mut writer_b, b_id) =
            connect_register_get_id(&sock_path, "client-b").await;
        ...
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{{"sessionId":"sess-union","_meta":{{"isReplay":true,"x.ai/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-union"}}}}}}"#,
            b_id.0
        );
        response_tx.send(spawned_replay).unwrap();
    ...
    async fn replayed_finished_last_subscriber_prunes_dead_child() {
        ...
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{{"sessionId":"sess-last","_meta":{{"isReplay":true,"x.ai/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-last"}}}}}}"#,
            a_id.0
        );
        response_tx.send(spawned_replay).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{{"sessionId":"sess-last","_meta":{{"isReplay":true,"x.ai/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-last"}}}}}}"#,
            a_id.0
        );
        response_tx.send(finished_replay).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_finished").await;
    ...
    async fn mid_load_child_delta_reaches_loader_via_request_side_backfill() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-midload","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-midload"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        drop(reader_a);
        drop(writer_a);
    ...
    async fn resolved_interaction_not_replayed_to_late_joiner() {
        ...
        response_tx.send(req.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "ask_user_question").await;
        let resolved = r#"{"method":"_sage.local/session_notification","params":{"method":"sage.local/session_notification","params":{"sessionId":"sess-int","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-ev"}}}}"#;
    ...
    /// `sage.local/sessions/changed` is a machine-wide roster notification with no
    ...
    async fn roster_changed_broadcasts_to_all_clients() {
        ...
        tokio::time::sleep(Duration::from_millis(20)).await;
        let changed = r#"{"jsonrpc":"2.0","method":"sage.local/sessions/changed","params":{"upserted":[{"sessionId":"sess-roster","cwd":"/repo","isWorktree":false,"yolo":false,"activity":"working","resident":true,"lastChangeUnixMs":1,"origin":{"kind":"local"}}],"removed":[]}}"#;
    ...
    async fn models_update_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_x.ai/models/update","params":{"currentModelId":"sage-new","availableModels":[{"modelId":"sage-new","name":"Nexus New"}]}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
            got_a.as_deref().is_some_and(|p| p.contains("sage-new")),
            "client A must receive the models broadcast, got {got_a:?}"
        );
        assert!(
            got_b.as_deref().is_some_and(|p| p.contains("sage-new")),
    ...
    async fn mcp_servers_updated_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_x.ai/mcp/servers_updated","params":{"method":"x.ai/mcp/servers_updated","params":{"mcpServers":[{"name":"nexus_com_slack","source":"managed"}]}}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
            got_a
                .as_deref()
                .is_some_and(|p| p.contains("nexus_com_slack")),
    ...
    fn machine_wide_broadcast_classifier_matches_both_wire_forms() {
        ...
        assert!(is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"sage.local/announcements/update","params":{}}"#
        )));
        assert!(is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"_sage.local/sessions/changed","params":{}}"#
        )));
        assert!(is_machine_wide_broadcast_notification(&pv(
        ...
        assert!(is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"_sage.local/announcements/update","params":{"method":"sage.local/announcements/update","params":{"gen":2,"announcements":[]}}}"#
        )));
        assert!(!is_machine_wide_broadcast_notification(&pv(
    ...
    fn inject_capabilities_sets_code_nav_enabled_true() {
        ...
        let mut json = pv(payload);
        inject_capabilities_into_session_new(&mut json, &caps, "sage-web", ClientId(1));
        assert_eq!(
    ...
    fn inject_capabilities_sets_code_nav_enabled_false() {
        ...
        let payload = r#"{"jsonrpc":"2.0","method":"session/new","id":1,"params":{"cwd":"/repo","_meta":{"clientIdentifier":"sage-tui"}}}"#;
        let mut json = pv(payload);
        inject_capabilities_into_session_new(&mut json, &caps, "sage-tui", ClientId(1));
        assert_eq!(
            json["params"]["_meta"]["codeNavEnabled"],
    ...
    fn inject_capabilities_two_clients_stay_isolated() {
        ...
        inject_capabilities_into_session_new(&mut web_json, &web_caps, "sage-web", ClientId(1));
        let mut tui_json = pv(session_new);
        inject_capabilities_into_session_new(&mut tui_json, &tui_caps, "sage-tui", ClientId(2));
        assert_eq!(
    ...
    async fn subagent_child_session_routed_after_spawned() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-123"}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"child-123","update":{"sessionUpdate":"message_delta","content":"hello"}}}"#
                    .into(),
            )
            .unwrap();
        let msg: ServerMessage =
    ...
    async fn subagent_child_session_cleaned_up_on_finished() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-456"}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-456"}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"child-456","update":{"sessionUpdate":"message_delta"}}}"#
                    .into(),
            )
            .unwrap();
        let timeout_result: Result<Result<ServerMessage, _>, _> =
    ...
    async fn subagent_child_session_not_leaked_to_other_client() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"_sage.local/session_notification","params":{"method":"sage.local/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-789"}}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{"sessionId":"child-789","update":{"sessionUpdate":"message_delta"}}}"#
                    .into(),
            )
            .unwrap();
        let msg: ServerMessage =
    ...
    async fn leader_client_id_dropped_when_target_disconnected() {
        ...
        response_tx
            .send(
                format!(
                    r#"{{"jsonrpc":"2.0","method":"_sage.local/session/update","params":{{"params":{{"sessionId":"sess-1","update":{{"sessionUpdate":"hook_annotation","message":"m"}},"_meta":{{"isReplay":true,"x.ai/leaderClientId":{}}}}}}}}}"#,
                    id_a
                ),
            )
            .unwrap();
