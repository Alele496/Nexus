//! Leader-follower IPC architecture for sage-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.nexus/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use nexus_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::NexusBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = nexus_version::VERSION;
...
/// Maximum reconnection attempts for bounded mode (headless/`sage -p`).
...
pub struct LeaderEnvUrls {
    pub nexus_ws_url: String,
    pub nexus_ws_origin: String,
}
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<NexusBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(NexusBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<NexusBuildEnvironment> {
    let environments: &[NexusBuildEnvironment] = &[NexusBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<NexusBuildEnvironment> {
    let environments: &[NexusBuildEnvironment] = &[NexusBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::nexus_home::nexus_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : nexus_version::VERSION,
                            }
                        )),
                    );
                }
...
impl From<&crate::auth::SageComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::SageComConfig) -> Self {
        Self {
            nexus_ws_url: c.nexus_ws_url.clone(),
            nexus_ws_origin: c.nexus_ws_origin.clone(),
        }
    }
}
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`sage -p` where hanging forever is unacceptable.
...
///     "sage-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
///
/// The `env_urls.nexus_ws_url` determines which leader instance to connect to.
/// Different WS URLs get different leader processes (via hashed socket paths).
///
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "sage-tui", "vscode")
...
pub async fn connect_or_spawn(
    ...
    let mut lock = LeaderLock::new(&env_urls.nexus_ws_url);
...
///
/// For a **managed install** — the running binary lives under `nexus_home`
/// (e.g. `~/.nexus/...`) — prefer the managed `~/.nexus/bin/sage` symlink. After an
/// auto-update or `sage update` atomically swaps that symlink, `current_exe()`
...
/// `nexus_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `nexus_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.nexus/bin/sage` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::nexus_home::nexus_home())
}
fn resolve_binary_with_home(nexus_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(nexus_home, std::env::current_exe().ok())
}
/// Binary file name for the managed sage install (`sage` / `sage.exe`).
fn managed_nexus_bin_name() -> &'static str {
    if cfg!(windows) { "sage.exe" } else { "sage" }
}
...
fn resolve_binary_impl(
    ...
    let managed_bin = nexus_home.join("bin").join(managed_nexus_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, nexus_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    cmd.arg("--sage-ws-url").arg(&env_urls.nexus_ws_url);
    cmd.arg("--sage-ws-origin").arg(&env_urls.nexus_ws_origin);
    ...
    let log_path = crate::util::nexus_home::nexus_home().join("leader.log");
    ...
    let leader_log = std::env::var("NEXUS_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "nexus_shell=info,nexus_acp_lib=warn,nexus_mcp=warn".into());
...
mod tests {
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            nexus_ws_url: "wss://test.invalid".into(),
            nexus_ws_origin: "https://test.invalid".into(),
        };
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("sage"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let target_v2 = bin_dir.join("sage-v2");
        std::fs::write(&target_v2, "new-binary").unwrap();
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("sage")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let new_target = bin_dir.join("sage-v2");
        std::fs::write(&new_target, "new-binary").unwrap();
        let managed = bin_dir.join("sage");
        std::os::unix::fs::symlink(&new_target, &managed).unwrap();
        let stale_target = bin_dir.join("sage-v1");
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_nexus_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_nexus_bin_name());
