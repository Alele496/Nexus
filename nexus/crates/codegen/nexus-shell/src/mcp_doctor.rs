...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = nexus_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.nexus/config.toml".to_string(),
...
/// Discover managed `nexus_com_*` servers if the user has xAI auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    let nexus_home = nexus_tools::util::nexus_home::nexus_home();
    let nexus_com_config = SageComConfig::default();
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&nexus_home, nexus_com_config));
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use nexus_mcp::rmcp::model::PaginatedRequestParams;
    match service
