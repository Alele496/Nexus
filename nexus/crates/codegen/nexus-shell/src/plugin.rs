...
use nexus_agent::plugins::discovery::PluginScope;
use nexus_agent::plugins::git_install::{self, UpdateStatus};
use nexus_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use nexus_plugin_marketplace::git::{self, SourceCacheLease};
use nexus_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.nexus/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &nexus_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `sage plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `sage plugin install ./{name}`, or add a \
                     source with `sage plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `sage plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `sage plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
pub fn marketplace_require_sha() -> bool {
    nexus_config::load_effective_config_disk_only()
        .map(|c| nexus_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| nexus_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &nexus_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.nexus/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user sage via user_nexus_home() (None when no home resolves) and
    // home separately, so removal still runs from $NEXUS_HOME when no home dir
    // exists, and never touches a cwd-relative .nexus.
    let home = dirs::home_dir();
    let sage = nexus_config::user_nexus_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref sage) = sage {
        settings_candidates.push(sage.join("settings.local.json"));
        settings_candidates.push(sage.join("settings.json"));
    }
    ...
    if let Some(ref sage) = sage {
        known_candidates.push(sage.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            ...
            "xAI Official (sage-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "xAI Official (pin: sentry@sage-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "xAI Official (sage-org/plugin-marketplace)".into(),
        ...
        assert!(
            msg.contains("  - xAI Official (sage-org/plugin-marketplace)"),
            "{msg}"
        );
    ...
    fn ambiguous_qualifier_error_lists_source_names() {
        let err = MarketplaceInstallError::AmbiguousQualifier {
            qualifier: "sage-org/plugin-marketplace".into(),
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("sage plugin install ./sentry"), "{msg}");
        assert!(msg.contains("sage plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        let err = MarketplaceInstallError::NameAmbiguous {
            name: "sentry".into(),
            candidates: vec!["xAI Official (pin: sentry@sage-org/plugin-marketplace)".into()],
        };
        let msg = err.to_string();
        assert!(
        ...
        assert!(
            msg.contains("  - xAI Official (pin: sentry@sage-org/plugin-marketplace)"),
            "{msg}"
        );
        assert!(
            msg.contains("sage plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    const OFFICIAL_URL: &str = "https://github.com/sage-org/plugin-marketplace.git";
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "xAI Official (sage-org/plugin-marketplace)".to_string(),
    ...
    fn plan_install_qualifier_ambiguous_lists_source_names() {
        let sources = [
            ...
            git_source("Mirror B", "git@github.com:sage-org/plugin-marketplace.git"),
        ];
        let err = plan_install(
            &sources,
            "sentry",
            Some("sage-org/plugin-marketplace"),
            |_| Ok(Vec::new()),
        )
        .expect_err("two sources share the owner/repo");
        match err {
            MarketplaceInstallError::AmbiguousQualifier { qualifier, sources } => {
                assert_eq!(qualifier, "sage-org/plugin-marketplace");
                assert_eq!(
                    sources,
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> nexus_workspace::permission::resolution::MarketplaceAllowlist {
        nexus_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = nexus_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        let name = resolve_qualified_source_name_with(&sources, "sage-org/plugin-marketplace")
