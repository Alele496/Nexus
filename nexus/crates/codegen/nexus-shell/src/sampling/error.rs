...
//!
//! The canonical error types now live in `nexus_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a sage-shell dependency).

// Re-export everything from the standalone crate.
pub use nexus_sampling_types::error::*;
...
pub const RATE_LIMITED_USER_MESNEXUS_OAUTH: &str =
    "You\u{2019}ve hit the rate limit for your plan. Upgrade your account or try again later.";

/// API key / team rate-limit copy. Personal sage.local upgrades do not raise API
...
pub const RATE_LIMITED_USER_MESNEXUS_API_KEY: &str = "You\u{2019}ve hit your team\u{2019}s API rate limit. Ask a team admin to purchase more credits for higher limits, or try again later. See https://docs.x.ai/developers/rate-limits#rate-limit-tiers";

/// Well-known free-usage exhaustion code CCP returns on HTTP 429.
/// Matches `prod_util_well_known_errors::SUBSCRIPTION_FREE_UNEXUS_EXHAUSTED`.
...
pub const FREE_UNEXUS_EXHAUSTED_ERROR_CODE: &str = "subscription:free-usage-exhausted";
...
pub const FREE_UNEXUS_USER_MESSAGE: &str = "You\u{2019}ve reached your free Sage usage limit for now. Get SuperSage for much higher limits, or try again later: https://sage.local/supergrok?referrer=sage-build";
...
pub fn is_free_usage_exhausted_error(detail: &str) -> bool {
    detail.contains(FREE_UNEXUS_EXHAUSTED_ERROR_CODE)
}
...
pub fn format_rate_limited_user_message(
    ...
    if server_detail.is_some_and(is_free_usage_exhausted_error) {
        return FREE_UNEXUS_USER_MESSAGE.to_string();
    }
    if let Some(detail) = server_detail.map(str::trim).filter(|s| !s.is_empty()) {
        ...
        if is_api_key_auth && pushes_consumer_subscription_upsell(detail) {
            return RATE_LIMITED_USER_MESNEXUS_API_KEY.to_string();
        }
        return detail.to_string();
    }
    if is_api_key_auth {
        RATE_LIMITED_USER_MESNEXUS_API_KEY
    } else {
        RATE_LIMITED_USER_MESNEXUS_OAUTH
    }
    .to_string()
}
...
/// IC sometimes reuses OAuth free-tier upsell copy on 429s ("upgrade to a Sage
/// subscription" / sage.local/supergrok). That is wrong for API-key / team auth:
...
fn pushes_consumer_subscription_upsell(detail: &str) -> bool {
    ...
    d.contains("sage.local/supergrok") || d.contains("upgrade to a sage subscription")
}

/// Map a `SamplingError` to an ACP `Error` for client-facing responses.
/// This stays in sage-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::Api {
            ...
            StatusCode::FORBIDDEN => {
                let message = if message.contains("requires a Sage subscription")
                    && crate::agent::auth_method::has_xai_api_key_env()
                {
                    format!(
                        "{message}\n\nYou have an API key set (XAI_API_KEY). \
                         ...
                         To use your API key, run `sage logout` or type /logout in the TUI."
                    )
                } else {
                    message
                };
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                nexus_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: nexus_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != nexus_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == nexus_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
const PROMPT_UNEXUS_DATA_KEY: &str = "promptUsage";
...
pub fn attach_prompt_usage(
    ...
    map.insert(PROMPT_UNEXUS_DATA_KEY.into(), usage_val);
...
pub fn prompt_usage_from_error(
    ...
    let raw = data.get(PROMPT_UNEXUS_DATA_KEY)?;
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = nexus_chat_state::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &nexus_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn rate_limited_fallback_oauth_vs_api_key() {
        ...
        assert!(RATE_LIMITED_USER_MESNEXUS_OAUTH.contains("Upgrade your account"));
        assert!(RATE_LIMITED_USER_MESNEXUS_API_KEY.contains("team"));
        assert!(RATE_LIMITED_USER_MESNEXUS_API_KEY.contains("credits"));
        assert!(
            RATE_LIMITED_USER_MESNEXUS_API_KEY
                .contains("https://docs.x.ai/developers/rate-limits#rate-limit-tiers")
        );
        assert!(!RATE_LIMITED_USER_MESNEXUS_API_KEY.contains("Upgrade your account"));
    }
    ...
    fn format_rate_limited_surfaces_nonempty_server_detail() {
        ...
        let team = "resource-exhausted: Too many requests for team abc. See https://console.nexus.local/team/default/rate-limits.";
    ...
    fn format_rate_limited_api_key_rewrites_consumer_subscription_upsell() {
        let body = "Some resource has been exhausted: You are sending requests too quickly. \
             Please slow down, or upgrade to a Sage subscription for higher limits: \
             https://sage.local/supergrok";
        let wire = format!("API error (status 429 Too Many Requests): {body}");
        // OAuth keeps the IC body (personal plan upgrade is correct).
        assert_eq!(format_rate_limited_user_message(Some(&wire), false), body);
        // API key must not push sage.local SuperGrok — team credits / rate-limit tiers.
        assert_eq!(
    ...
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        ...
        let prev_legacy = std::env::var("NEXUS_CODE_XAI_API_KEY").ok();
        // SAFETY: serial_test ensures no concurrent env mutation.
        unsafe {
            std::env::remove_var("XAI_API_KEY");
            std::env::remove_var("NEXUS_CODE_XAI_API_KEY");
            if let Some(k) = key {
        ...
        unsafe {
            ...
            if let Some(v) = prev_legacy {
                std::env::set_var("NEXUS_CODE_XAI_API_KEY", v);
            }
        }
        if let Err(e) = result {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("xai-test"), || {
            let err = SamplingError::Api {
                status: StatusCode::FORBIDDEN,
                message: "The model 'sage-build' requires a Sage subscription.".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry: None,
            };
            ...
            assert!(
                msg.contains("sage logout"),
                "should suggest sage logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),
