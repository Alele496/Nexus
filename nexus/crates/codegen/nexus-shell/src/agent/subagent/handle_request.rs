...
pub(crate) async fn handle_subagent_request(
    ...
    {
        use nexus_tool_types::SubagentIsolationMode;
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(nexus_agent::config::IsolationMode::Worktree)
        {
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = nexus_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        if effective_runtime.isolation != nexus_tool_types::SubagentIsolationMode::None
    ...
    } else if effective_runtime.isolation != nexus_tool_types::SubagentIsolationMode::None
    {
        ...
        let creation_mode: nexus_fast_worktree::CreationMode = ctx.worktree_type.into();
        let btrfs_delegate = crate::session::worktree::btrfs_delegate_from_env();
        match tokio::task::spawn_blocking(move || {
                let mut builder = nexus_fast_worktree::WorktreeBuilder::new(
                        &source_clone,
                        &dest,
                    )
                    .working_tree_mode(
                        nexus_fast_worktree::WorkingTreeMode::PreserveWorkingTree,
                    )
                    .creation_mode(creation_mode)
                    .worktree_kind(nexus_fast_worktree::WorktreeKind::Subagent)
    ...
    {
        let reminder = nexus_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = nexus_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
            nexus_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let initial_child_tokens = nexus_chat_state::estimate_conversation_tokens(
        &forked_conversation,
    );
    ...
    let attribution_callback: Option<nexus_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    let yolo_policy_block = nexus_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        let memory_tools: Vec<nexus_tools::registry::types::ToolConfig> = vec![
            (& nexus_build::ReadFileTool).into(), (& nexus_build::SearchReplaceTool)
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = nexus_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == nexus_hooks::event::HookEventName::Stop {
                            s.event = nexus_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if is_plugin_agent {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    nexus_agent::config::McpServerRef::Named(name) => {
                    ...
                    nexus_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            nexus_agent::prompt::skills::list_skills_with_plugins(
                    Some(&parent_cwd_str),
    ...
    nexus_telemetry::session_ctx::log_event(nexus_telemetry::events::SubagentLaunched {
        subagent_id: request.id.clone(),
    ...
    let spawn_result = session::spawn_session_on_thread(
            ...
            nexus_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
            nexus_chat_state::CompactionMode::Summary,
            ...
            std::sync::Arc::new(
                parking_lot::Mutex::new(
                    nexus_workspace::file_system::CodebaseIndexManager::new(),
                ),
            ),
            false,
            subagent_fs_watch,
            ...
            if inherit_skills {
                ...
                nexus_agent::prompt::skills::SkillsConfig::default()
            },
            ...
            ctx.yolo_mode
                || matches!(
                    ...
                    nexus_agent::config::PermissionMode::BypassPermissions
                ),
            false,
            None,
            ...
            Vec::new(),
            nexus_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        nexus_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        nexus_telemetry::events::Outcome::Cancelled
    } else {
        nexus_telemetry::events::Outcome::Error
    };
    nexus_telemetry::session_ctx::log_event(nexus_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
