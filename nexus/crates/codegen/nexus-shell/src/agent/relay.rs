...
//! connection to the sage.local relay server with automatic reconnection.
...
/// Config for the sage.local WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a sage.local first-party session
    ...
    pub(crate) fn for_session(
        ...
        Some(Self {
            ws_url: ctx.nexus_ws_url.clone(),
            ws_origin: ctx.nexus_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to sage.local with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `sage login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from Sage WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(nexus_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                nexus_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        let cfg = crate::auth::SageComConfig::default();
