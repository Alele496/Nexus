...
fn jwt_tier_claim_maps_free_and_paid() {
    ...
    assert_eq!(
        ...
        Some("supernexus_heavy")
    );
    assert_eq!(
        jwt_tier_claim(&jwt_with_tier(6)).as_deref(),
        Some("supernexus_lite")
    );
...
fn jwt_claim_matches_user_subscription_tier_known_pairs() {
    let cases = [
        ...
        ("supernexus_heavy", "SuperGrokPro"),
        ("supernexus_lite", "SuperGrokLite"),
    ];
...
mod hunk_tracking_mode {
    ...
    use nexus_hunk_tracker::TrackingMode;
    #[test]
...
fn harness_pair(id: &str) -> Vec<nexus_sampling_types::conversation::ConversationItem> {
    use nexus_sampling_types::ToolCall;
    use nexus_sampling_types::conversation::ConversationItem;
    vec![
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_nexus_build() {
    let prev = std::env::var("NEXUS_AGENT").ok();
    unsafe {
        std::env::remove_var("NEXUS_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("NEXUS_AGENT", v) }
    }
}
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = nexus_agent::AgentDefinition::from_json(&serde_json::json!(
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_nexus_build_family() {
    let prev = std::env::var("NEXUS_AGENT").ok();
    unsafe {
        std::env::remove_var("NEXUS_AGENT");
    }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use nexus_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".nexus").join("agents");
    ...
    assert_eq!(
        def.model,
        nexus_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("NEXUS_AGENT", v) },
        None => unsafe { std::env::remove_var("NEXUS_AGENT") },
    }
}
#[test]
...
#[test]
fn null_agent_type_falls_back_to_session_default_nexus_build_plan() {
    assert_eq!(
...
#[test]
fn null_agent_type_falls_back_to_session_default_nexus_build() {
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use nexus_tools::computer::local::{LocalFs, LocalTerminalBackend};
    use nexus_tools::notification::ToolNotificationHandle;
    use nexus_tools::registry::types::SessionContext;
    let tmp = tempfile::tempdir().unwrap();
    ...
    let builder = nexus_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: nexus_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: nexus_tools::implementations::nexus_build::image_gen::ImageGenConfig::default(),
        video_gen_config: nexus_tools::implementations::nexus_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: nexus_tools::implementations::nexus_build::deploy_app::AppBuilderDeployerConfig::default(),
        api_key_provider: None,
        auth_provider: None,
        attribution_callback: None,
        system_reminder_tag: nexus_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = nexus_hunk_tracker::HunkTrackerActor::spawn(
        "test".to_string(),
        std::path::PathBuf::from("/tmp"),
        hunk_event_tx,
        nexus_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        max_turns: None,
        hunk_tracker_handle,
        chat_state_handle: nexus_chat_state::ChatStateHandle::noop(),
        ...
        permission_handle: nexus_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        ...
        hook_registry: None,
        workspace_ops: nexus_workspace::WorkspaceOps::for_test(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use nexus_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use nexus_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let _env = EnvGuard::set("NEXUS_HOME", nexus_home.path());
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<nexus_acp_lib::AcpClientMessage>,
) -> Option<crate::agent::roster::RosterChanged> {
    let mut found = None;
    while let Ok(msg) = rx.try_recv() {
        if let nexus_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            if found.is_none()
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::NEXUS_COM_METHOD_ID,
...
async fn auth_type_session_based_with_current_returns_session_token() {
    ...
    assert_eq!(agent.auth_type(), nexus_chat_state::AuthType::SessionToken,);
}
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), nexus_chat_state::AuthType::ApiKey,);
}
...
/// Minimal agent whose `nexus_com_config` engages the api-key kill switch
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.nexus_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    ...
    use nexus_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("NEXUS_DISABLE_API_KEY_AUTH");
...
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{NEXUS_COM_METHOD_ID, XAI_API_KEY_ENV_VAR};
    use nexus_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("NEXUS_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(XAI_API_KEY_ENV_VAR, "test-deployment-key");
...
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_nexus_com_without_credentials() {
    use crate::agent::auth_method::{
        NEXUS_COM_METHOD_ID, LEGACY_XAI_API_KEY_ENV_VAR, XAI_API_KEY_ENV_VAR,
    };
    use nexus_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("NEXUS_DISABLE_API_KEY_AUTH");
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use nexus_tools::implementations::nexus_build::video_gen::{
...
async fn prepare_image_gen_config_fails_open_without_auth() {
    use nexus_tools::implementations::nexus_build::image_gen::ImageGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
async fn prepare_video_gen_config_sends_client_identifier_header() {
    use nexus_tools::implementations::nexus_build::video_gen::VideoGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = nexus_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = nexus_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(nexus_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<nexus_workspace::permission::PermissionEvent>();
    agent
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::NEXUS_DISABLE_CUSTOM_BRIDGE_ENV);
...
fn build_agent_with_gateway_rx() -> (
    MvpAgent,
    tokio::sync::mpsc::UnboundedReceiver<nexus_acp_lib::AcpClientMessage>,
) {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, SageComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
...
fn write_project_subagent_definitions(cwd: &std::path::Path) {
    let roles = cwd.join(".nexus/roles");
    let personas = cwd.join(".nexus/personas");
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                nexus_subagent_resolution::config::SubagentRole {
...
/// End-to-end gate wiring: project `.nexus/roles` / `personas` alone must drive
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    let _env = EnvGuard::set("NEXUS_HOME", home.path());
    let _sim = EnvGuard::set(nexus_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("NEXUS_FOLDER_TRUST");
    let repo = tempfile::tempdir().unwrap();
    git2::Repository::init(repo.path()).unwrap();
    write_project_subagent_definitions(repo.path());
    run_local_for_bridge_test(|| async {
...
async fn answer_folder_trust_request(
    gw_rx: &mut tokio::sync::mpsc::UnboundedReceiver<nexus_acp_lib::AcpClientMessage>,
    outcome: &str,
) -> serde_json::Value {
    ...
    let nexus_acp_lib::AcpClientMessage::ExtMethod(args) = msg else {
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use nexus_test_support::EnvGuard;
    use nexus_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("NEXUS_HOME", home.path());
    let _sim = EnvGuard::set(nexus_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("NEXUS_FOLDER_TRUST");
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, nexus_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(nexus_acp_lib::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use nexus_test_support::EnvGuard;
    use nexus_hooks_plugins_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("NEXUS_HOME", home.path());
    let _sim = EnvGuard::set(nexus_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("NEXUS_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(nexus_acp_lib::AcpClientMessage::ExtMethod(_)))
            ),
...
fn ann(id: &str) -> nexus_announcements::RemoteAnnouncement {
    nexus_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<nexus_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = nexus_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = nexus_announcements::RemoteAnnouncement {
...
async fn emit_announcements_gate_emits_updates_baseline_and_bumps_gen() {
    ...
    let recv_gen =
        |rx: &mut tokio::sync::mpsc::UnboundedReceiver<nexus_acp_lib::AcpClientMessage>| {
            ...
            let nexus_acp_lib::AcpClientMessage::ExtNotification(args) = msg else {
