...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    use nexus_workspace::permission::types::{
    ...
    local
        .run_until(async {
            ...
            let cwd = nexus_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                nexus_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    nexus_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                nexus_workspace::permission::AccessKind::Read(Some(".env".into())),
                nexus_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        nexus_workspace::permission::Decision::PolicyDeny(_)
                    ),
