...
//! two ways:
//!
//!   - request/response `sage.local/sessions/list` → `{ "sessions": [RosterEntry, …] }`
//!   - broadcast notification `sage.local/sessions/changed` →
...
/// Response payload for `sage.local/sessions/list`.
#[derive(Serialize, Deserialize, Clone, Debug, Default)]
pub struct RosterListResponse {
    pub sessions: Vec<RosterEntry>,
}

/// Params payload for the `sage.local/sessions/changed` broadcast notification.
#[derive(Serialize, Deserialize, Clone, Debug, Default)]
...
pub const SESSIONS_LIST_METHOD: &str = "sage.local/sessions/list";
pub const SESSIONS_CHANGED_METHOD: &str = "sage.local/sessions/changed";
