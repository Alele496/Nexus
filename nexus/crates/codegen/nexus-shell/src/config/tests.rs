...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "NEXUS_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$NEXUS_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${NEXUS_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$NEXUS_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$NEXUS_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the NEXUS_MEMORY env var.
...
/// Run `f` with NEXUS_MEMORY explicitly unset.
fn without_nexus_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("NEXUS_MEMORY", None, f)
}
/// Run `f` with NEXUS_MEMORY set to a specific value.
fn with_nexus_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("NEXUS_MEMORY", Some(value), f)
}
#[test]
fn memory_config_default_disabled() {
    without_nexus_memory(|| {
...
fn memory_config_env_var_zero_does_not_enable() {
    with_nexus_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "NEXUS_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_nexus_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "NEXUS_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_nexus_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "NEXUS_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_nexus_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "NEXUS_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_nexus_memory(
        ...
        || {
            ...
            let mem = MemoryConfig::resolve(true, false, &config, None);
            assert!(
                mem.enabled, "CLI --experimental-memory should override NEXUS_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_nexus_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "--no-memory should override --experimental-memory, NEXUS_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_nexus_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(! mem.enabled, "--no-memory should override NEXUS_MEMORY=1");
        },
    );
}
#[test]
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$NEXUS_TEST_USER $NEXUS_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the NEXUS_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with NEXUS_SUBAGENTS explicitly unset.
fn without_nexus_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("NEXUS_SUBAGENTS", None, f)
}
/// Run `f` with NEXUS_SUBAGENTS set to a specific value.
fn with_nexus_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("NEXUS_SUBAGENTS", Some(value), f)
}
#[test]
fn subagents_config_default_enabled() {
    without_nexus_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_nexus_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(! sa.enabled, "NEXUS_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_nexus_subagents(
        ...
        || {
            ...
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(
                ! sa.enabled, "NEXUS_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_nexus_subagents(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let sa = SubagentsConfig::resolve(true, &config);
            assert!(sa.enabled, "--subagents CLI flag should override NEXUS_SUBAGENTS=0");
        },
    );
}
#[test]
    ...
    with_nexus_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(sa.enabled, "NEXUS_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "sage-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        "NEXUS_MANAGED_MCPS_ENABLED",
        managed_mcps,
        || with_env_var_opt("NEXUS_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "NEXUS_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "NEXUS_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "NEXUS_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("NEXUS_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_nexus_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_nexus_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "NEXUS_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("NEXUS_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
fn without_nexus_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    with_tools_env(None, None, f)
}
fn with_nexus_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    with_tools_env(Some(value), None, f)
}
#[test]
fn tools_config_default_disabled() {
    without_nexus_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_nexus_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "NEXUS_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".nexus/prompts/impl.md"
        "#;
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".nexus/prompts/impl.md"));
}
#[test]
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".nexus/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".nexus").join("roles");
...
fn personas_parse_from_toml() {
    let toml_str = r#"
            ...
            instructions_file = ".nexus/personas/concise.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".nexus/personas/concise.md"));
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".nexus").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".nexus").join("bundled");
    std::fs::create_dir_all(workspace.join(".nexus").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".nexus").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".nexus").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".nexus").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".nexus/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".nexus/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".nexus/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".nexus/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".nexus/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".nexus/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
    ...
    std::fs::remove_file(home.join(".nexus/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".nexus/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.nexus/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
fn enterprise_two_file_merge_routes_deployment_key_to_proxy() {
    for k in [
        ...
        "NEXUS_TRACE_UPLOAD_ENDPOINT_URL",
    ] {
...
cli_chat_proxy_base_url = "https://cli-chat-proxy.nexus.local/v1"
...
"#,
    ...
    assert_eq!(
        ...
        "https://cli-chat-proxy.nexus.local/v1/deployment/config"
    );
...
fn validate_hooks_path_rejects_outside_nexus_home() {
    ...
    assert!(
        msg.contains("must be under ~/.nexus/"),
        "should mention ~/.nexus/ restriction, got: {msg}"
    );
}
#[test]
...
#[test]
fn validate_hooks_path_accepts_nexus_hooks_subdir() {
    let nexus_home = crate::util::nexus_home::nexus_home();
    let valid_path = nexus_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.nexus/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use nexus_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
fn simulate_release_build() -> nexus_test_support::EnvGuard {
    nexus_test_support::EnvGuard::set(nexus_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
fn base_resolver_without_project_cwd_keeps_project_files_out() {
    let tmp = tempfile::tempdir().unwrap();
    write_subagent_definitions(&tmp.path().join(".nexus"), &[("project", "Project")]);
    let base = SubagentsConfig::resolve_base_with_sources(
        false,
...
#[test]
fn explicit_nexus_root_is_the_only_user_source() {
    let tmp = tempfile::tempdir().unwrap();
    let ambient = tmp.path().join("ambient-home/.nexus");
...
/// verdicts untouched). NEXUS_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores NEXUS_HOME even on panic). No user-global
/// `$NEXUS_HOME/config.toml` is seeded: `nexus_home()` is `OnceLock`-cached,
...
/// NEXUS_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use nexus_agent::plugins::{TrustStore, discover_plugins};
    use nexus_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("NEXUS_HOME", home.path());
    let _flag = EnvGuard::unset("NEXUS_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. NEXUS_HOME-isolated (empty
/// store); NEXUS_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
