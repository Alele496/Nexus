...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.nexus/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** sage process
    /// sharing the same `~/.nexus` (the writer may also be this process;
...
/// Watches `~/.nexus/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.nexus/config.toml`, `.mcp.json`,
...
/// `<cwd>/.nexus/` (catches `<cwd>/.nexus/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.nexus/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let nexus_home_buf = nexus_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(nexus_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(nexus_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(nexus_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(nexus_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %nexus_home.display(),
                    error = %e,
        ...
        // includes `<cwd>/.nexus/config.toml` so the loop above
        // watches `<cwd>/.nexus/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.nexus/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.nexus/config.toml` trigger a
    ...
    /// `<cwd>/.nexus/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.nexus/` does not yet
/// exist at session-open time, the `.nexus/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.nexus/` followed by a
/// write to `<cwd>/.nexus/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    if let Err(e) = debouncer
        .watcher()
        .watch(&nexus_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .nexus directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let nexus_dir = cwd.join(".nexus");
    if let Err(e) = debouncer.watcher().unwatch(&nexus_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .nexus directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.nexus/` hasn't
...
/// Watches skill directories (`~/.nexus/skills/`, `<repo>/.nexus/skills/`, etc.)
...
/// recursively: `nexus_home` (`~/.nexus`, or `$NEXUS_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.nexus` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, nexus_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, nexus_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".nexus", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, nexus_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(nexus_home) {
        return true;
    }
...
/// `~/.nexus` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](nexus_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = nexus_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &nexus_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &nexus_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only nexus_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_nexus_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let nexus_home = home.join(".nexus");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &nexus_home, Some(home));

        // nexus_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&nexus_home));
        ...
        assert!(!g(&home.join("repo").join(".nexus")));
    }

    /// Regression for the ~/.nexus inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.nexus/worktrees/`) must not
    ...
    /// A write to `<nexus_home>/models_cache.json` must surface as
    ...
    /// A write to `<cwd>/.nexus/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let nexus_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_grok = cwd.path().join(".nexus");
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.nexus/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.nexus/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".nexus");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
