...
//!
//! Fetches the authenticated user's Sage billing configuration
...
/// (e.g. `UNEXUS_PERIOD_TYPE_WEEKLY`); kept so callers can distinguish weekly
/// vs monthly cycles.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Current billing configuration for Sage coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `SageBuildBillingConfig` fields
...
/// `GetSageBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// absent (legacy `GetSageBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Top-level response (primarily from `GET /rest/sage/credits` + auto-topup-rule).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
///
/// Keeps history to a count + the most recent period so `~/.nexus/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Billing data requires auth with sage.local. Run `sage login` to authenticate.",
    )?;
    ...
    let credits_resp = crate::http::shared_client()
        ...
        .header(
            "X-XAI-Token-Auth",
            crate::auth::SageComConfig::default().token_header,
        )
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Auto top-up data requires auth with sage.local. Run `sage login` to authenticate.",
    )?;
    ...
    fn billing_config_deserializes_credits_config_shape() {
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "currentPeriod": {
                    "type": "UNEXUS_PERIOD_TYPE_WEEKLY",
                ...
                "productUsage": [
                    {"product": "PRODUCT_NEXUS_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
        ...
        assert_eq!(
            ...
            Some("UNEXUS_PERIOD_TYPE_WEEKLY")
        );
