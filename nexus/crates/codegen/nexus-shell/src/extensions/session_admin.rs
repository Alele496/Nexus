...
//! lifecycle:
//!
//! - `sage.local/session/rename`                  rename a session locally + remote
//! - `sage.local/session/delete`                  delete a session locally + remote
//! - `sage.local/session/update_mcp_servers`      mid-session MCP server swap
//! - `sage.local/session/fork`                    fork a session into a new one
...
use nexus_telemetry::id::agent_id;

#[tracing::instrument(skip_all, fields(method = %args.method))]
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "sage.local/session/rename" => handle_session_rename(agent, args).await,
        "sage.local/session/delete" => handle_session_delete(agent, args).await,
        "sage.local/session/update_mcp_servers" => handle_update_mcp_servers(agent, args).await,
        "sage.local/session/fork" => handle_session_fork(agent, args).await,
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `sage sessions delete <id>` CLI path.
...
/// hot-reload watcher when `<cwd>/.nexus/config.toml`,
...
/// Hot-reload the model catalog from `~/.nexus/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `x.ai/commands/list` (the pull used by sage-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .nexus/plugins under it, plus
