...
pub fn loaded_plugin_to_info(plugin: &nexus_agent::plugins::LoadedPlugin) -> PluginInfo {
    use nexus_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &nexus_agent::plugins::PluginOrigin) -> PluginOrigin {
    use nexus_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
        AgentOrigin::CliOverride => PluginOrigin::CliOverride,
        AgentOrigin::ProjectSage => PluginOrigin::ProjectSage,
        AgentOrigin::ProjectClaude => PluginOrigin::ProjectClaude,
        AgentOrigin::UserSage => PluginOrigin::UserSage,
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "x.ai/plugins/action" => {
            let req: nexus_hooks_plugins_types::PluginsActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    use super::*;
    use nexus_agent::plugins::PluginOrigin as AgentOrigin;
    use nexus_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> nexus_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        nexus_agent::plugins::LoadedPlugin {
