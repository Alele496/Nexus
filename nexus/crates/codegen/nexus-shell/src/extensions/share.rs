...
async fn upload_share_data_to_gcs(
    ...
    if let Err(e) = nexus_file_utils::gcs::upload_bytes_signed(
...
fn require_xai_auth_for_share(
    ...
    super::auth_gate::require_xai_auth(
        ...
        "Share session is disabled. Run `sage login` to authenticate.",
    )
}
...
mod tests {
    ...
    #[test]
    fn share_rejects_non_xai_auth_with_actionable_nexus_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            SageComConfig::default(),
