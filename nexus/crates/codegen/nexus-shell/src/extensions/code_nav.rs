...
//! using the sage-codebase-graph index.
//!
//! ## Extension Methods
//!
//! | Method | Description |
...
fn to_code_nav_ext_response(resp: nexus_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for sage-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
