...
fn discover_auto_sources(cwd: &str, skills: &[SkillInfo]) -> Vec<(String, usize)> {
    ...
    for subdir in &subdirs {
        try_add_source(nexus_home.join(subdir), None);
    }
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "sage.local/skills/add" => {
            ...
            .await
            {
                nexus_telemetry::session_ctx::log_event(
                    nexus_telemetry::events::SkillAdded {
                        added_count: 0,
                        total_skills: 0,
                        success: false,
                    },
            ...
            nexus_telemetry::session_ctx::log_event(nexus_telemetry::events::SkillAdded {
        ...
        "sage.local/skills/remove" => {
            ...
            {
                nexus_telemetry::session_ctx::log_event(
                    nexus_telemetry::events::SkillRemoved { success: false },
                );
            ...
            nexus_telemetry::session_ctx::log_event(nexus_telemetry::events::SkillRemoved {
                success: true,
            });
