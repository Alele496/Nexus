// claude_import.rs
// Scans Claude settings and generates TOML patches for .nexus/config.toml.
//
...
use nexus_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use nexus_workspace::permission::rules::parse_permission_rule;
use nexus_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
pub enum ImportScope {
    /// User-level: writes to `~/.nexus/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.nexus/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.nexus/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.nexus/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.nexus/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.nexus/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.nexus/config.toml`.
///
...
pub fn mark_claude_imported() -> anyhow::Result<()> {
    let path = crate::util::nexus_home::nexus_home().join("config.toml");
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.nexus/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.nexus/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        let global_path = crate::util::nexus_home::nexus_home().join("config.toml");
        ...
        // Hooks are written separately to ~/.nexus/hooks/imported-from-claude.json.
        let hooks_dir = crate::util::nexus_home::nexus_home().join("hooks");
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".nexus").join("config.toml");
        ...
        let hooks_dir = project_root.join(".nexus").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .nexus/hooks/ JSON files in apply_hooks_to_dir,
...
/// (Claude-compatible). The native hooks loader scans `.nexus/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .nexus/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    impl Drop for MarkerGuard {
        fn drop(&mut self) {
            ...
            unsafe { std::env::remove_var("_NEXUS_CLAUDE_MARKER_OVERRIDE") };
        }
    }
    ...
    fn gate_load_claude_env_returns_empty_when_marker_set() {
        ...
        refresh_marker_cache(true);
        let dir = tempfile::tempdir().unwrap();
        let env = nexus_workspace::permission::claude_settings::load_claude_env_with_project(
            dir.path(),
            true,
        );
        assert!(
    ...
    fn discover_hook_source_paths_skips_cursor_when_disabled() {
        ...
        refresh_marker_cache(false);
        let dir = tempfile::tempdir().unwrap();
        let mut compat = nexus_tools::types::compat::CompatConfig::default();
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.nexus) don't interfere.
        let has_probe = |reg: &nexus_hooks::discovery::HookRegistry| {
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `NEXUS_HOME` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        unsafe { std::env::set_var("_NEXUS_CLAUDE_MARKER_OVERRIDE", "1") };
        let dir = tempfile::tempdir().unwrap();
        ...
        // managed settings, and the developer's real `~/.nexus/config.toml`.
        // We can't isolate `nexus_home()` because it's `OnceLock`-cached.
        // Instead, assert on rule *provenance*: no rule should originate from
        // our tempdir's `.claude/settings.json`. The dev's real ~/.nexus
        // config rules (if any) are out of scope for this test.
        let resolved =
            nexus_workspace::permission::resolution::resolve_permissions_with_provenance(
                dir.path(),
            )
            .await;
        if let Some(r) = resolved {
            let tempdir_claude = claude_dir.join("settings.json");
            use nexus_workspace::permission::types::RequirementSource;
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.nexus/config.toml` may legitimately have the marker
        // set during local testing, and we can't override `nexus_home()`
