...
use nexus_sampler::SamplerConfig;
use nexus_tools::implementations::nexus_build;
use nexus_tools::registry::types::ToolConfig;

/// Production sage-build foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production sage-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `NEXUS_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `NEXUS_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> nexus_tools::implementations::nexus_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("NEXUS_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| nexus_config::env_bool("NEXUS_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `nexus_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `SageBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `SageBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "NexusBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "NexusBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"NexusBuild:read_file"));
        assert!(ids.contains(&"NexusBuild:search_replace"));
        assert!(ids.contains(&"NexusBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"NexusBuildHashline:hashline_read"));
        assert!(ids.contains(&"NexusBuildHashline:hashline_edit"));
        assert!(ids.contains(&"NexusBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            nexus_agent::config::AgentDefinition::plan(),
            nexus_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "NexusBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"NexusBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"NexusBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"NexusBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production sage-build must set the 10h foreground ceiling"
        );
    }
