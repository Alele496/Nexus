...
/// Resolved absolute path to bash. On Unix uses the `nexus_config` shell
/// resolution cascade (`$NEXUS_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        nexus_config::shell::unix_shell_path(nexus_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `sage-tools`.
pub use nexus_tools::util::pager_env;
...
/// sage-pager which renders its own UI and doesn't need raw ANSI codes).
///
