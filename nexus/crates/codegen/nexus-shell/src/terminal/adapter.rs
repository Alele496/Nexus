//! AcpTerminalAdapter: implements `sage-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates sage-tools' `TerminalBackend` trait into ACP protocol calls:
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (sage-desktop) spawns with `shell: true`
...
/// Wraps sage-shell's ACP gateway to satisfy sage-tools' TerminalBackend.
