...
/// after we stop waiting. Mirrors the bound the `sage-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("NEXUS_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for nexus_acp_lib::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
...
mod tests {
    ...
    #[cfg_attr(windows, ignore = "Unix signals unavailable on Windows")]
    #[tokio::test]
    ...
    #[cfg_attr(windows, ignore = "Unix process group signals unavailable on Windows")]
    #[tokio::test]
