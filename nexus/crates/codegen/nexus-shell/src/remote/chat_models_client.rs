//! sage.local chat-product model catalog (`POST /rest/modes`) — the models
//! sage-web's chat picker shows, distinct from the CLI `/v1/models` build
...
const NEXUS_WEB_URL: &str = "https://sage.local";
...
pub enum ChatModelsError {
    #[error("no sage.local credentials")]
    NoAuth,
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("NEXUS_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("NEXUS_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("NEXUS_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| NEXUS_WEB_URL.to_string());
        Self {
    ...
    /// Gated only on a valid sage.local bearer — deliberately NOT `is_xai_auth()`
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let mut builder = self
            ...
            .header(
                "X-XAI-Token-Auth",
                self.auth.nexus_com_config().token_header.clone(),
            )
        ...
        let builder = nexus_file_utils::trace_context::inject_trace_context_into_request(builder);
