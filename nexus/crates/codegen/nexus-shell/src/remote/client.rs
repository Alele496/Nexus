...
use crate::auth::{GrokAuth, SageComConfig};
...
const NEXUS_CODE_BACKEND_URL: &str = "https://code.nexus.local";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const NEXUS_CODE_WEB_URL: &str = "https://sage.local";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("NEXUS_CODE_WEB_URL").unwrap_or_else(|_| NEXUS_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("X-XAI-Token-Auth", SageComConfig::default().token_header)
        .header("x-userid", &auth.user_id)
        .header("x-sage-client-version", nexus_version::VERSION);
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::nexus_auth_credentials::GrokAuthCredentials::new(
    ...
    nexus_file_utils::trace_context::inject_trace_context_into_request(builder)
}
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn nexus_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("NEXUS_CODE_BACKEND_URL")
                .unwrap_or_else(|_| NEXUS_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn nexus_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    /// Must include X-XAI-Token-Auth so nginx auth subrequest routes to authenticate_xai_nexus_cli_token.
    /// See: crates/codegen/sage-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            "X-XAI-Token-Auth",
            required(&SageComConfig::default().token_header, "X-XAI-Token-Auth")?,
        );
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-sage-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            "x-sage-client-version",
            HeaderValue::from_static(nexus_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = nexus_file_utils::trace_context::inject_trace_context_into_request(
...
/// Fetch `nexus_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-sage-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(nexus_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header("x-sage-agent-id", agent_id)
        .header("x-sage-client-version", nexus_version::VERSION)
        .header(
            "x-sage-client-identifier",
            crate::http::process_client_identifier(),
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        ...
        EndpointAuth::Session => {
            ...
            request = request
                .header("Authorization", format!("Bearer {}", &auth.key))
                .header("X-XAI-Token-Auth", "sage-cli")
                .header("x-userid", &auth.user_id)
                .header("x-sage-client-version", nexus_version::VERSION)
                .header(
                    crate::http::CLIENT_MODE_HEADER,
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| nexus_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(nexus_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-sage-agent-id"),
                            client_identifier: header_str(&headers, "x-sage-client-identifier"),
                            client_version: header_str(&headers, "x-sage-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-sage-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-sage-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-sage-client-version"
        );
    ...
    fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
        ...
        let mgr = crate::auth::AuthManager::new(dir.path(), crate::auth::SageComConfig::default());
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("sage-cli"));
    ...
    fn parse_reads_reasoning_effort_fields() {
        ...
        let value = serde_json::json!(
            { "model" : "sage-4.5", "context_window" : 1_000_000,
        ...
        let value = serde_json::json!(
            { "model" : "sage-4.5", "contextWindow" : 1_000_000,
    ...
    fn parse_reads_reasoning_efforts_list() {
        use nexus_sampling_types::ReasoningEffort;
        let value = serde_json::json!(
            { "model" : "sage-4.5", "context_window" : 1_000_000, "reasoning_efforts" :
    ...
    fn parse_remote_model_value_no_laziness_detector_block_yields_default() {
        let value = serde_json::json!(
            { "model" : "sage-4", "context_window" : 256_000, }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert_eq!(
    ...
    fn parse_remote_model_value_parses_camelcase_key() {
        let value = serde_json::json!(
            { "model" : "sage-4", "context_window" : 256_000, "lazinessDetector" : {
    ...
    fn parse_remote_model_value_parses_snake_case_laziness_detector() {
        let value = serde_json::json!(
            { "model" : "sage-4", "context_window" : 256_000, "laziness_detector" : {
    ...
    fn parse_remote_model_value_parses_meta_laziness_detector() {
        let value = serde_json::json!(
            { "model" : "sage-4", "context_window" : 256_000, "_meta" : {
    ...
    fn parse_reads_show_model_fingerprint_field() {
        let value = serde_json::json!(
            { "model" : "sage-build", "context_window" : 256_000,
            "show_model_fingerprint" : true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "sage-build", "contextWindow" : 256_000, "showModelFingerprint" :
            true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "sage-build", "context_window" : 256_000, "_meta" : {
    ...
    fn inference_url_defaults_to_proxy() {
        let ep = endpoints("https://proxy.nexus.local/v1", None, None);
        assert_eq!(ep.resolve_inference_base_url(), "https://proxy.nexus.local/v1");
    }
    #[test]
    fn inference_url_uses_models_base_url() {
        let ep = endpoints(
            "https://proxy.nexus.local/v1",
    ...
    fn list_url_defaults_to_proxy_models() {
        ...
        assert_eq!(
            ...
            "https://proxy.nexus.local/v1/models"
        );
    }
    #[test]
    ...
    fn models_fetch_endpoint_matches_auth_mode() {
        ...
        assert_eq!(session.url, "https://cli-chat-proxy.nexus.local/v1/models");
        ...
        assert_eq!(deployment.url, "https://cli-chat-proxy.nexus.local/v1/models");
    ...
    /// REGRESSION: `sage setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("NEXUS_DEPLOYMENT_KEY", "xai-token-ENTERPRISE") };
        ...
        assert_eq!(url, "https://cli-chat-proxy.nexus.local/v1/deployment/config");
        assert!(
        ...
        unsafe { std::env::remove_var("NEXUS_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
