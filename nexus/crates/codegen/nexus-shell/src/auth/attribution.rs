...
//!
//! 1. [`nexus_telemetry::unified_log::warn`] for the local
//!    `~/.nexus/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`nexus_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`nexus_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`nexus_sampler::SamplerConfig::attribution_callback`] at every
...
use nexus_sampler::{Auth401AttributionCallback, SamplingConsumer};
use nexus_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`nexus_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `nexus_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn nexus_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `nexus_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `sage-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `sage-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.nexus/logs/unified.jsonl) + scrubbed
...
fn compute_attribution_payload(
    ...
    // `nexus_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.nexus/auth.json` leaks in.
    ...
    /// `ShellAttribution` implements `nexus_tools::Auth401AttributionCallback`
