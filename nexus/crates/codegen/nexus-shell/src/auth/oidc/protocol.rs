...
use super::super::config::{ForceLoginTeam, SageComConfig, OAuth2ProviderConfig, OidcAuthConfig};
...
pub fn is_configured(config: &SageComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &SageComConfig) -> Option<ForceLoginTeam> {
...
mod tests {
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "sage-cli:access".into()],
            audience: None,
        };
        ...
        assert!(url.contains("referrer=sage-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("sage-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.x.ai/authorize".into(),
            token_endpoint: "https://auth.x.ai/token".into(),
        ...
        assert!(url.contains("referrer=sage-desktop"));
        assert!(!url.contains("referrer=sage-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!(
            ...
            "offline_access sage-cli:access api:access", "principal_type" : "Team",
