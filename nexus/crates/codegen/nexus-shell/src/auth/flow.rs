...
use crate::auth::{AuthManager, GrokAuth, SageComConfig, parse_output};
use crate::util::nexus_home;
...
/// session.
fn is_cached_credential_compatible(auth: &GrokAuth, nexus_com_config: &SageComConfig) -> bool {
    let expected_issuer = nexus_com_config
        .oidc
        .as_ref()
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(nexus_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("NEXUS_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `NEXUS_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `nexus_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("NEXUS_LOGIN_DEVICE_FLOW");
...
async fn run_external_auth_provider(
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.nexus_com_config());
...
pub async fn run_auth_flow_with_stderr_bridge(
    auth_manager: &Arc<AuthManager>,
    nexus_com_config: &SageComConfig,
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = nexus_com_config.oidc.is_some(),
        has_oauth2 = nexus_com_config.oauth2.is_some(),
        has_external_auth = nexus_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, nexus_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, nexus_com_config)
        }) {
            nexus_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = nexus_com_config.auth_provider_command {
    ...
    // `sage login --devbox` uses run_devbox_login and is not gated here.
    if !nexus_com_config.blocks_automatic_oidc()
    ...
    // --device-auth / NEXUS_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(nexus_com_config) {
        return crate::auth::oidc::run_login_flow(nexus_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = nexus_com_config.oauth2 {
    ...
    // No OIDC or OAuth2 configured — try API key fallback.
    tracing::info!(
        "auth: no OAuth2/OIDC configured, falling back to API key auth"
    );
    match try_api_key_auth(auth_manager).await {
        Ok((auth, did_auth)) => return Ok((auth, did_auth)),
        Err(e) => {
            tracing::error!(
                error = %e,
                "auth: no OAuth2 configuration and no API key available"
            );
            anyhow::bail!(
                "No authentication method available. Set NEXUS_API_KEY environment variable \
                 or add api_key to your model config in config.toml."
            )
        }
    }
}

/// API key fallback: when no OIDC/OAuth2 is configured, try to authenticate
/// using an API key from environment variable or stored auth.json.
async fn try_api_key_auth(
    auth_manager: &Arc<AuthManager>,
) -> anyhow::Result<(GrokAuth, bool)> {
    // Try NEXUS_API_KEY env var first, then fall back to auth.json.
    let api_key = match crate::agent::auth_method::read_xai_api_key_env() {
        Ok(key) => {
            tracing::info!("auth: using API key from environment variable");
            Some(key)
        }
        Err(_) => {
            // Read API key from auth.json (may have been stored by setup wizard).
            let nexus_home = crate::util::nexus_home::nexus_home();
            crate::auth::storage::read_api_key(&nexus_home)
        }
    };

    let api_key = match api_key {
        Some(key) => key,
        None => anyhow::bail!("No API key found in environment or auth.json"),
    };

    let auth = GrokAuth {
        key: api_key,
        auth_mode: super::AuthMode::ApiKey,
        ..Default::default()
    };

    // Persist to auth.json so subsequent starts can find it.
    if let Err(e) = crate::auth::storage::store_api_key(
        &crate::util::nexus_home::nexus_home(),
        &auth.key,
    ) {
        tracing::warn!(error = %e, "auth: failed to persist API key to auth.json");
    }

    // Store in-memory so auth() fast-path finds it.
    let stored = auth_manager
        .save_without_enrichment(auth)
        .await
        .map_err(|e| anyhow::anyhow!("Failed to save API key credentials: {e}"))?;

    tracing::info!("auth: API key auth fallback succeeded");
    Ok((stored, true))
}
...
pub async fn try_ensure_fresh_auth(nexus_com_config: &SageComConfig) -> Option<GrokAuth> {
    let nexus_home = nexus_home::nexus_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&nexus_home, nexus_com_config.clone()));

    // auth() handles cached-valid (fast path), OIDC refresh, external
    // binary -- all through refresh_chain (single mutation point).
    auth_manager.configure_refresher(nexus_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    if let Some(auth) = try_ensure_fresh_auth(nexus_com_config).await {
        return Some(auth);
    }
    let nexus_home = nexus_home::nexus_home();
    let auth_manager = Arc::new(AuthManager::new(&nexus_home, nexus_com_config.clone()));
    ...
    mint_session_noninteractive(&auth_manager, nexus_com_config).await
}
...
async fn mint_session_noninteractive(
    ...
    if let Some(cmd) = nexus_com_config.auth_provider_command.as_deref() {
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(nexus_com_config).await)
    } else {
        ensure_authenticated(nexus_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `sage login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `NEXUS_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `sage login` on an
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.nexus_com_config, login_override).await {
        if config.nexus_com_config.oauth2.is_none() {
            // No OIDC and no oauth2 here, so `--oauth` can't help.
            anyhow::bail!("Sign-in is not available for this deployment. Set NEXUS_API_KEY instead.");
        }
        let nexus_home = nexus_home::nexus_home();
        let auth_manager = Arc::new(AuthManager::new(&nexus_home, config.nexus_com_config.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.nexus_com_config) {
            eprintln!(
    ...
    // config was actually applied; `sage setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `NEXUS_API_KEY` / `XAI_API_KEY` (legacy) env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        nexus_telemetry::external::set_identity(
            nexus_telemetry::external::IdentityAttrs::default(),
        );
        nexus_telemetry::external::flush();
...
/// `sage logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let nexus_home = nexus_home::nexus_home();
    let auth_manager = AuthManager::new(&nexus_home, config.nexus_com_config.clone());
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!("You are authenticated via NEXUS_API_KEY (environment variable).");
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("NEXUS_API_KEY is still set and will be used for authentication.");
    }
    Ok(())
}
...
mod tests {
    ...
    /// Run `f` with `NEXUS_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("NEXUS_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("NEXUS_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("NEXUS_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A sage.local first-party (x.ai-issuer) OIDC session — `is_xai_auth()` true.
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), SageComConfig::default());
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only nexus_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), SageComConfig::default()));
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), SageComConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("NEXUS_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("NEXUS_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        // NEXUS_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into Sage: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `sage login` passes `on_stderr=None`; stderr must be inherited so
