...
///
/// On success, persists credentials to `~/.nexus/auth.json` and returns
...
pub(crate) mod tests {
    ...
    fn auth_manager_with_nexus_home(
        nexus_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(nexus_home, SageComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let nexus_home = temp_dir.path().join(".nexus");
        std::fs::create_dir_all(&nexus_home).unwrap();
        let auth_manager = auth_manager_with_nexus_home(&nexus_home, "http://127.0.0.1:9");
    ...
    fn assert_build_auth_rejected(cfg: SageComConfig, token_principal: &str, expected_err: &str) {
        ...
        std::fs::create_dir_all(&nexus_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&nexus_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
        ...
        assert!(
            !nexus_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        ...
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_nexus_home(temp_dir.path(), "http://127.0.0.1:9");
