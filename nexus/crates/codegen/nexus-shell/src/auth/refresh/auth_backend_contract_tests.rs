...
use nexus_telemetry::events::{AuthTokenKind, ManualAuthReason};
...
async fn auth_backend_contract_token_responses_map_to_outcomes() {
    ...
    for (name, status, body, expect) in cases {
        ...
        let auth_manager = Arc::new(
            AuthManager::new(dir.path(), SageComConfig::default()).with_proxy_base_url(&base_url),
        );
...
async fn auth_backend_contract_dead_token_emits_typed_manual_auth_event() {
    ...
    for (source, want_surface) in [
        ...
        let dir = tempfile::tempdir().unwrap();
        let auth_manager = Arc::new(
            AuthManager::new(dir.path(), SageComConfig::default()).with_proxy_base_url(&url),
        );
    ...
    let ok_manager = Arc::new(
        AuthManager::new(ok_dir.path(), SageComConfig::default()).with_proxy_base_url(&ok_url),
    );
