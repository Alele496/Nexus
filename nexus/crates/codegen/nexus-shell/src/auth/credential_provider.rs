...
use crate::util::nexus_auth_credentials::GrokAuthCredentials;
...
fn api_key_id_for(auth: Option<&crate::auth::GrokAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| nexus_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub(crate) fn embedding_session_credentials(
...
) -> nexus_memory::EndpointScopedCredentials {
    ...
    nexus_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
...
pub fn build_storage_client_for_proxy(
...
) -> nexus_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn nexus_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        nexus_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(nexus_version::VERSION, client_identifier)
...
impl nexus_file_utils::storage_client::Auth401AttributionCallback for StorageClientAttributionBridge {
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        nexus_telemetry::external::set_identity(
            nexus_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
pub fn build_default_otel_layer_config() -> nexus_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let nexus_com_config = crate::auth::SageComConfig::default();
    let exporter = nexus_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = nexus_com_config.token_header.clone();
    let nexus_home = crate::util::nexus_home::nexus_home();
    let bootstrap = Arc::new(AuthManager::new(&nexus_home, nexus_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    nexus_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use std::sync::Mutex;
    use nexus_auth::AuthCredentialProvider;
    /// Serializes tests that pin `NEXUS_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `NEXUS_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("NEXUS_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("NEXUS_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("NEXUS_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("NEXUS_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: nexus_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use nexus_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
