...
/// auth.json scope key for plain API key auth (desktop login, `sage login --api-key`).
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from sage-desktop login or `sage login --api-key`)
    ApiKey,
}
...
pub struct GrokAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_nexus_code_access: Option<bool>,
...
impl GrokAuth {
    ...
    /// `true` when this auth can access sage.local managed MCP connectors.
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `sage login --legacy`
...
/// Early-invalidation buffer. Override with `NEXUS_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("NEXUS_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
