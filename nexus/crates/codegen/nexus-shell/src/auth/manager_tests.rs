...
/// `has_nexus_code_access`. The `&mut GrokAuth` signature already
...
fn apply_user_info_enrichment_preserves_token_fields() {
    let mut disk = GrokAuth {
        ...
        has_nexus_code_access: Some(true),
    ...
    assert_eq!(disk.has_nexus_code_access, snapshot.has_nexus_code_access);
...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    let _legacy = EnvGuard::unset("NEXUS_CODE_XAI_API_KEY");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), SageComConfig::default()));
    mgr.hot_swap(GrokAuth {
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use nexus_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use nexus_telemetry::events::{AuthTokenKind, ManualAuthSurface};
