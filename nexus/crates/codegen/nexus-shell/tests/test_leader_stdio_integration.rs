...
use nexus_shell::cpu_profile::ControlErrorCode;
...
async fn setup_control_test_server(
    temp: &TempDir,
) -> (std::path::PathBuf, nexus_shell::leader::ServerHandle) {
...
async fn test_session_new_valid_default_model_injected() {
    ...
    let meta = &json["params"]["_meta"];
    assert_eq!(meta["modelId"], "sage-3-fast");
...
/// `sage.local/session_notification` (carrying the `ModelChanged` update) out
...
async fn test_set_model_broadcasts_to_session_subscribers() {
    ...
    invoker
        .send(format!(
            r#"{{"jsonrpc":"2.0","id":42,"method":"session/setModel","params":{{"sessionId":"{}","modelId":"sage-4"}}}}"#,
            shared_sid
        ))
        .unwrap();
    let received = acp_rx.recv().await.unwrap();
    ...
    assert_eq!(json["params"]["modelId"], "sage-4");
    ...
    let broadcast = format!(
        r#"{{"jsonrpc":"2.0","method":"sage.local/session_notification","params":{{"sessionId":"{}","update":{{"sessionUpdate":"model_changed","model_id":"sage-4","reasoning_effort":"high"}}}}}}"#,
        shared_sid
    );
    response_tx.send(broadcast.clone()).unwrap();
    let response = format!(
        r#"{{"jsonrpc":"2.0","result":{{"meta":{{"model":"sage-4"}}}},"id":"{}"}}"#,
        setmodel_ns_id
    );
    response_tx.send(response).unwrap();
    ...
    assert_eq!(inv2["result"]["meta"]["model"], "sage-4");
...
async fn test_session_ownership_cleanup_on_disconnect() {
    use nexus_shell::leader::run_leader_server;
    ...
    tokio::spawn(async move {
        ...
        let (shutdown_tx, _shutdown_rx) =
            tokio::sync::watch::channel(nexus_shell::leader::ShutdownReason::Manual);
...
async fn test_raw_registration_handshake_not_ready_then_ready() {
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            watch::channel(nexus_shell::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
/// Integration test: a connected client receives `sage.local/leader/version_mismatch`
...
async fn test_version_mismatch_notification_sent_to_client() {
    ...
    use tokio::sync::{mpsc, watch};
    use tokio_util::sync::CancellationToken;
    use nexus_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, run_leader_server};
...
async fn test_auto_update_shutdown_reason_reaches_client() {
    use nexus_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, ShutdownReason};
    ...
    tokio::time::timeout(
        ...
        disconnect_rx.wait_for(|r| *r != nexus_shell::leader::DisconnectReason::Connected),
    )
    .await
    ...
    assert_eq!(
        *disconnect_rx.borrow(),
        nexus_shell::leader::DisconnectReason::LeaderShutdown
    );
}
...
///
/// This binary does not sandbox NEXUS_HOME, so on a dev machine these entries
/// land in the real `~/.nexus` log — accepted: the server already writes
...
fn orphan_log_count(request_id: &str) -> usize {
    let Some(bytes) = nexus_telemetry::unified_log::snapshot_log() else {
        return 0;
    };
...
async fn wait_for_client_disconnected_log(client_id: u64) -> bool {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let seen = nexus_telemetry::unified_log::snapshot_log().is_some_and(|bytes| {
...
async fn test_hung_agent_leaves_transport_healthy_and_forwards_cancel() {
    ...
    assert_eq!(
        *client.disconnect_reason().borrow(),
        nexus_shell::leader::DisconnectReason::Connected,
