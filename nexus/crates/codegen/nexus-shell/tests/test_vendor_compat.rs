...
//! `.nexus`, `.cursor`, and `.claude` vendor dirs, spawns a real `sage agent
//! stdio` process against the mock inference server (toggling the
//! `NEXUS_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
//! and asserts on the full inference request bodies:
//!
//! - the Sage-native skill is always present regardless of toggles
...
//! ```bash
//! cargo test -p sage-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_NEXUS_SKILL: &str = "ZZ_NEXUS_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".nexus", "sage-skill", MARKER_NEXUS_SKILL);
...
/// Defaults (all vendors on): sage + cursor-vendor + claude-vendor skills present; the
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_NEXUS_SKILL),
            "sage-skill must always be present"
        );
        assert!(
...
/// `NEXUS_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; sage stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("NEXUS_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_NEXUS_SKILL),
            "sage-skill always present"
        );
        assert!(
            !body.contains(MARKER_CURSOR_SKILL),
...
/// `NEXUS_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; sage stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("NEXUS_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_NEXUS_SKILL),
            "sage-skill always present"
        );
...
/// `NEXUS_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("NEXUS_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `NEXUS_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("NEXUS_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `NEXUS_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("NEXUS_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `NEXUS_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("NEXUS_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("NEXUS_CURSOR_SKILLS_ENABLED", "false"),
            ("NEXUS_CURSOR_RULES_ENABLED", "false"),
            ("NEXUS_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_NEXUS_SKILL), "sage always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("NEXUS_CLAUDE_SKILLS_ENABLED", "false"),
            ("NEXUS_CLAUDE_RULES_ENABLED", "false"),
            ("NEXUS_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
...
/// All vendor compat OFF: only sage-native skill survives.
#[tokio::test]
#[ignore] // requires pre-built binary
