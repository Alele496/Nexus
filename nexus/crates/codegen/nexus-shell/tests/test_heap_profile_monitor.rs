...
use nexus_shell::auth::{AuthManager, AuthMode, GrokAuth, SageComConfig};
use nexus_shell::heap_profile::{
...
use nexus_test_support::{EnvGuard, MockInferenceServer};
use nexus_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = SageComConfig::default().auth_scope();
    let auth = GrokAuth {
        key: token.to_owned(),
...
fn assert_storage_auth(uploads: &[nexus_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), SageComConfig::default()));
