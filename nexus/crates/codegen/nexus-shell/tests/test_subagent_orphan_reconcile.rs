...
//!
//! This test spawns a real `sage agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p sage-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // GrokStdioClient sets HOME=<temp>; the binary uses <HOME>/.nexus as NEXUS_HOME.
        let nexus_home = shared_home.path().join(".nexus");
        let session_dir = locate_session_dir(&nexus_home, session_id.0.as_ref());
