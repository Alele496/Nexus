...
//!
//! Runs the built sage binary against the mock inference server with a
//! caller-owned `$NEXUS_HOME`, then inspects `~/.nexus/debug/`:
...
//!   `~/.nexus/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `NEXUS_LOG_SAMPLING`/`NEXUS_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `NEXUS_DEBUG_LOG=1`) writes a per-session
//!   `~/.nexus/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.nexus/debug/` files).
//! - `NEXUS_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p sage-shell --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$NEXUS_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".nexus").join("debug")
}

/// List firehose `*.txt` files under `~/.nexus/debug` (excluding the `latest.txt`
...
/// Build a headless `sage -p` command with a pinned `$NEXUS_HOME` so the firehose
/// lands under `<home>/.nexus/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    nexus_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home);
    // Pin the home location and drop inherited firehose toggles for determinism.
    cmd.env("NEXUS_HOME", home.join(".nexus"));
    cmd.env_remove("NEXUS_DEBUG_LOG");
    cmd.env_remove("NEXUS_LOG_FILE");
    cmd.env_remove("NEXUS_LOG_SAMPLING");
    cmd.env_remove("NEXUS_HOOKS_LOG");
    cmd
}
...
/// headless `sage -p` client is near-silent, so its lazily-opened firehose may
...
async fn debug_flag_enables_firehose_without_crashing() {
    ...
    assert_headless_success(&result, "sage --debug headless", Some(&server));
...
async fn no_debug_flag_writes_no_debug_dir() {
    ...
    assert_headless_success(&result, "sage headless (no --debug)", Some(&server));
    assert!(
...
/// A live `agent` session writes `~/.nexus/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let home = TempDir::new().expect("create temp home");
        let nexus_home = home.path().join(".nexus");
        let nexus_home_str = nexus_home.to_string_lossy().into_owned();

        let client = GrokStdioClient::spawn_with_home_and_env(
            &server,
            workdir.path(),
            home,
            &[("NEXUS_DEBUG_LOG", "1"), ("NEXUS_HOME", &nexus_home_str)],
        )
        .await;
        ...
        let session_file = nexus_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.nexus/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = nexus_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `NEXUS_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `NEXUS_LOG_SAMPLING`/`NEXUS_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // clears NEXUS_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.nexus/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // NEXUS_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = nexus_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.nexus/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn debug_file_flag_writes_single_file_and_bypasses_routing() {
    ...
    assert_headless_success(&result, "sage --debug-file", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        explicit.exists(),
...
/// `NEXUS_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn nexus_log_file_explicit_path_is_written() {
    let server = MockInferenceServer::start()
        .await
        .expect("start mock server");
    let workdir = git_workdir();
    ...
    cmd.env("NEXUS_LOG_FILE", &custom);
    let result = run_headless_with_cmd(cmd).await;

    assert_headless_success(&result, "sage NEXUS_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit NEXUS_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "NEXUS_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
