//! Integration test for `_sage.local/session/update` notifications.
//!
...
use nexus_shell::session::info::Info as SessionInfo;
use nexus_shell::session::persistence::default_model_id;
use nexus_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};
...
/// must survive `updates.jsonl` and reload as a replayable `_sage.local/session/update`.
#[tokio::test]
