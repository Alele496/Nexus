...
use nexus_sqlite_journal::JournalMode;
...
impl WorktreeDb {
    /// Open (or create) the DB at `nexus_home/worktrees.db`.
    pub fn open(nexus_home: &Path) -> Result<Self> {
        Self::open_at(&nexus_home.join("worktrees.db"))
    }
    ...
    /// Open the default DB at `~/.nexus/worktrees.db`.
    ///
    /// Discovers sage home via `$NEXUS_HOME`, falling back to the canonicalized
    /// `$HOME/.nexus` (matching `nexus_config::nexus_home`).
...
#[cfg(test)]
static NEXUS_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`NEXUS_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `NEXUS_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `NEXUS_HOME` before `_lock` releases, so the env is correct
...
impl SageHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = NEXUS_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("sage-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via NEXUS_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until NEXUS_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("NEXUS_HOME");
        unsafe { std::env::set_var("NEXUS_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
