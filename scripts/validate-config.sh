...
if [ -f "$SYS_ROOT/AGENTS.md" ]; then
    ...
    red "MISSING — grok-build needs this as project rules entry point"
    ERRORS=$((ERRORS + 1))
fi

# ---- .grok/config.toml ----
echo -n "[2/8] .grok/config.toml ... "
if [ -f "$SYS_ROOT/.grok/config.toml" ]; then
    # Check TOML is parseable (basic check with grep for required sections)
    if grep -q '\[subagents\]' "$SYS_ROOT/.grok/config.toml" && \
       grep -q '\[subagents.roles.developer\]' "$SYS_ROOT/.grok/config.toml" && \
       grep -q '\[subagents.roles.reviewer\]' "$SYS_ROOT/.grok/config.toml" && \
       grep -q '\[subagents.roles.operator\]' "$SYS_ROOT/.grok/config.toml" && \
       grep -q '\[subagents.roles.advisor\]' "$SYS_ROOT/.grok/config.toml"; then
...
# ---- .grok/agents/ ----
echo -n "[3/8] .grok/agents/ ... "
AGENT_COUNT=0
for agent in developer reviewer operator advisor; do
    if [ -f "$SYS_ROOT/.grok/agents/$agent.md" ]; then
...
# ---- .grok/skills/ ----
echo -n "[4/8] .grok/skills/ ... "
SKILL_COUNT=0
for skill in dispatch ship council-review; do
    if [ -f "$SYS_ROOT/.grok/skills/$skill/SKILL.md" ]; then
        # Check YAML frontmatter
        if head -1 "$SYS_ROOT/.grok/skills/$skill/SKILL.md" | grep -q '^---$'; then
...
# ---- .grok/hooks/ ----
echo -n "[5/8] .grok/hooks/ ... "
if [ -f "$SYS_ROOT/.grok/hooks/safety-gates.json" ]; then
    if grep -q '"PreToolUse"' "$SYS_ROOT/.grok/hooks/safety-gates.json" && \
       grep -q '"matcher"' "$SYS_ROOT/.grok/hooks/safety-gates.json"; then
...
# ---- .grok/rules/ ----
echo -n "[6/8] .grok/rules/ ... "
if [ -f "$SYS_ROOT/.grok/rules/project-conventions.md" ]; then
    green "OK"
else
...
for agent in developer reviewer operator advisor; do
    if grep -q "agents/$agent.md" "$SYS_ROOT/.grok/config.toml" 2>/dev/null; then
        if [ ! -f "$SYS_ROOT/.grok/agents/$agent.md" ]; then
...
if grep -q 'security-reviewer' "$SYS_ROOT/.grok/config.toml" && \
   grep -q 'perf-reviewer' "$SYS_ROOT/.grok/config.toml" && \
   grep -q 'readability-reviewer' "$SYS_ROOT/.grok/config.toml"; then
    :
else
