...
//! - Session resume
//!
//! Each test spawns a real `sage agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p sage-shell --test test_agent_type_invariant -- --ignored
//! ```
...
/// - `model-a`: no agent_type (→ "sage-build")
/// - `model-b`: no agent_type (→ "sage-build")
...
/// `sage-build` harness. The system prompt sent to the LLM should contain
/// the sage-build identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_nexus_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Nexus") || sys_prompt.contains("sage"),
            "default model should use sage-build harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_grok =
            original_sys_prompt.contains("Nexus") || original_sys_prompt.contains("sage");
        let resumed_has_grok =
            resumed_sys_prompt.contains("Nexus") || resumed_sys_prompt.contains("sage");
        assert_eq!(
            original_has_grok,
            resumed_has_grok,
            "resumed session should use the same harness as the original\n\
             original identity markers: sage={original_has_grok}\n\
             resumed identity markers: sage={resumed_has_grok}\n\
...
/// default to `"sage-build"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_nexus_build() {
    with_local_set(|| async {
            ...
            assert!(
                sys_prompt.contains("Nexus") || sys_prompt.contains("sage"),
                "model without agent_type should default to sage-build harness\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
}
/// The `NEXUS_AGENT` escape hatch should override the model's agent_type.
/// Setting `NEXUS_AGENT=sage-build` with an alternate-agent model should use
/// sage-build harness.
#[tokio::test]
#[ignore]
async fn test_nexus_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            nexus_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("NEXUS_AGENT", "sage-build");
            let mut child = cmd.spawn().expect("spawn sage");
            ...
            let incoming = nexus_acp_lib::LineBufferedRead::spawn_local(incoming);
            ...
            assert!(
                sys_prompt.contains("Nexus") || sys_prompt.contains("sage"),
                "NEXUS_AGENT=sage-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
