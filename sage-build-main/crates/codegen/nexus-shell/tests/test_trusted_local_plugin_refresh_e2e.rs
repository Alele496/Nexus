...
//! 4. Smoke-validate session JSON under `$NEXUS_HOME/sessions/` after exit.
//!
//! Requires a built `sage` binary (`NEXUS_BINARY` or cargo-built pager) for the
...
use nexus_agent::plugins::SharedPluginRegistryHandle;
use nexus_agent::plugins::discovery::DiscoveryConfig;
use nexus_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _nexus_guard = EnvVarGuard::set("NEXUS_HOME", &nexus_home);
    ...
    let mut registry = InstallRegistry::empty(nexus_home.join("installed-plugins"));
    ...
    // nexus_home() caches the first NEXUS_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        nexus_agent::plugins::PluginScope::CliOverride
    );
...
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/NEXUS_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via nexus_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _nexus_guard = EnvVarGuard::set("NEXUS_HOME", &nexus_home);
    ...
    // Smoke: session storage under NEXUS_HOME/sessions has JSON artifacts.
