...
use nexus_sampler::RetryPolicy;
use nexus_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
...
use nexus_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = nexus_test_support::git_workdir();
    ...
    let mut cmd = tokio::process::Command::new(nexus_test_support::nexus_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    nexus_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home.path());
    cmd.env("NEXUS_HOME", nexus_home);
    // Don't attach to a developer's ambient leader; spawn fresh against the mock.
    cmd.env_remove("NEXUS_LEADER_SOCKET");

    let result = nexus_test_support::run_headless_with_cmd(cmd).await;
    nexus_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
