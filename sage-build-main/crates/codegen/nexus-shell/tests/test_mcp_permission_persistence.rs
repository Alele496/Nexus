...
use nexus_acp_lib::{AcpAgentGatewaySender, AcpClientMessage};
...
/// Shared `NEXUS_HOME` for the entire test binary. The `OnceLock` in
/// `sage-config` only allows one value per process, so all tests share
...
async fn dont_ask_policy_denies_without_prompting() {
    use nexus_workspace::permission::types::{PermissionConfig, PromptPolicy};
