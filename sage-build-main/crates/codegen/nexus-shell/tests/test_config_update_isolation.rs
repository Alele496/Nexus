...
/// All tests share this as NEXUS_HOME (the `OnceLock` in sage-config
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("NEXUS_HOME", &path) };
        path
    })
}
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = nexus_shell::config::load_effective_config().unwrap();
    let effective_cfg = nexus_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    nexus_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = nexus_shell::util::config::load_config_from_toml(&user_toml);
    ...
    assert_eq!(user_cfg.models.default.as_deref(), Some("sage-3"));
}
