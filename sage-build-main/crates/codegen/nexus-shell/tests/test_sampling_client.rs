...
use nexus_shell::sampling::{
    ApiBackend, Client, ConversationItem, ConversationRequest, ConversationToolChoice,
    SamplingError, ToolCall, ToolSpec, rs,
};
use nexus_shell::session::storage::JsonlStorageAdapter;
use nexus_test_support::sse::responses_api_reasoning_and_text_events;
use nexus_test_support::{MockInferenceServer, ScriptedResponse, SseEvent};
...
async fn test_chat_completions_streaming_tool_calls() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        ScriptedResponse::sse(chat_completion_tool_call_stream(tool_calls, "sage-test")),
    );
    let client = create_test_client(&server.url(), ApiBackend::ChatCompletions);
...
/// Upgrade path, sage-build / Responses API: a legacy session whose
...
async fn responses_upgrade_roundtrips_reconstructed_reasoning_as_typed_input() {
    // 1. Seed a legacy sage-build chat_history.jsonl (inline reasoning
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy sage-build reasoning","encrypted":"ENC_BLOB_xyz","id":"rs_grokbuild_legacy"},"model_id":"sage-build"}"#,
            "\n",
        ),
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        Some("legacy sage-build reasoning")
    );
}
...
async fn messages_upgrade_emits_reconstructed_reasoning_as_thinking_block() {
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy anthropic thinking","encrypted":"SIGNATURE_abc","id":""},"model_id":"sage-4.5"}"#,
            "\n",
        ),
    )
    .unwrap();
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use nexus_shell::sampling::rs::ResponseStreamEvent;
        match event {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        if let ResponseStreamEvent::ResponseCompleted(completed) = event {
            for output in completed.response.output {
                use nexus_shell::sampling::rs::OutputItem;
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use nexus_shell::sampling::rs::OutputMessageContent;
...
async fn test_request_includes_headers() {
    ...
    assert_eq!(request.header("x-sage-conv-id"), Some("conv-12345"));
    assert_eq!(request.header("x-sage-req-id"), Some("req-67890"));
}
...
/// `SamplerConfig::doom_loop_recovery` puts `x-sage-doom-loop-check` on the
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    use nexus_sampling_types::doom_loop::{DOOM_LOOP_CHECK_EVENT_TYPE, SAMPLE_CHECK_EVENT_DATA};
    ...
    assert_eq!(logged.header("x-sage-doom-loop-check"), Some("true"));
}
...
async fn test_doom_loop_check_disabled_sends_no_header_and_drops_check_frames() {
    ...
    let logged = server.requests().pop().unwrap();
    assert!(logged.path.contains("/responses"));
    assert_eq!(logged.header("x-sage-doom-loop-check"), None);
}
...
async fn test_responses_backend_hits_responses_endpoint_not_chat_completions() {
    ...
    // The session-level dispatch in `sage-sampler` selects the
