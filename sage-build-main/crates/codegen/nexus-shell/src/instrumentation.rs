//! Shim — see `nexus_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `nexus_shell::instrumentation_timer!`). Keeping the macro here
...
pub use nexus_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `nexus_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `nexus_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `nexus_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `nexus_telemetry::instrumentation`.
#[macro_export]
