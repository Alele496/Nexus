...
//!
//! Repairs via [`nexus_chat_state::compaction_utils::repair_history`]. Resident
...
use nexus_chat_state::compaction_utils::HistoryRepairReport;
...
/// back atomically. `nexus_root` is injectable for tests.
async fn repair_on_disk(nexus_root: &std::path::Path, session_id: &str, dry_run: bool) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &nexus_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(nexus_root.to_path_buf());
    ...
    let report = nexus_chat_state::compaction_utils::repair_history(&mut chat_history);
