...
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `nexus_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.nexus_com_config.nexus_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_xai_api_key(
            agent_config.nexus_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    ...
    ///
    /// Uses `NEXUS_AUTH_PATH` (not `NEXUS_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("NEXUS_AUTH"),
            EnvGuard::set("NEXUS_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("NEXUS_DEPLOYMENT_KEY"),
            EnvGuard::unset("NEXUS_WS_ORIGIN"),
            EnvGuard::unset("NEXUS_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("NEXUS_AUTH", &json);
