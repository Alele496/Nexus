//! Tracks open TUI sessions in `~/.nexus/active_sessions.json` for crash
...
pub fn try_unregister(session_id: &acp::SessionId) -> io::Result<bool> {
    try_unregister_in(&crate::util::nexus_home::nexus_home(), session_id)
}

/// Remove entries with dead PIDs and return them.
pub fn collect_crashed() -> io::Result<Vec<ActiveSession>> {
    collect_crashed_in(&crate::util::nexus_home::nexus_home())
}
