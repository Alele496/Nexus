...
/// [`nexus_config::env_bool`] parser (`NEXUS_TOOLS_FIND_BFS` /
/// `NEXUS_TOOLS_GREP_UGREP`, plus the `NEXUS_FIND_BFS` / `NEXUS_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.nexus/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`nexus_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = nexus_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = nexus_config::env_bool(primary).or_else(|| nexus_config::env_bool(alias));
    ...
    (
        resolve("NEXUS_TOOLS_FIND_BFS", "NEXUS_FIND_BFS", "find_bfs"),
        resolve("NEXUS_TOOLS_GREP_UGREP", "NEXUS_GREP_UGREP", "grep_ugrep"),
    )
}
...
const ENV_LOGIN_SHELL_CAPTURE: &str = "NEXUS_LOGIN_ENV";
...
/// the shared [`nexus_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "NEXUS_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`NEXUS_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            nexus_tools::implementations::nexus_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`NEXUS_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        nexus_tools::implementations::nexus_build::ask_user_question::response_timeout_env_secs(),
...
) -> nexus_tools::implementations::nexus_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use nexus_tools::implementations::nexus_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = nexus_tools::implementations::nexus_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes NEXUS_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
