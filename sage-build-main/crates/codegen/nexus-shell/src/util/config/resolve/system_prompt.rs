pub const ENV_SYSTEM_PROMPT_LABEL: &str = "NEXUS_SYSTEM_PROMPT_LABEL";

pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = nexus_agent::DEFAULT_SYSTEM_PROMPT_LABEL;

/// Resolve system-prompt identity label.
/// Precedence: env → config per-model → `[agent]` → GB per-model → GB global →
/// `"Nexus"`. Empty/whitespace falls through.
///
...
mod system_prompt_label_tests {
    ...
    /// Serialize access to `NEXUS_SYSTEM_PROMPT_LABEL` and clear it for tier tests.
