pub mod config;
pub mod nexus_auth_credentials;
pub mod hooks;

// The foundation utilities live in `sage-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `nexus_shell::util::…` users compile
// unchanged.
pub use nexus_shell_base::util::*;
...
pub(crate) fn is_user_instruction_path(
    ...
    let nexus_rules = nexus_home.join("rules");
    let is_exact_home_surface = parent
        .is_some_and(|parent| parent == nexus_home || parent == nexus_rules)
...
mod is_user_instruction_path_tests {
    ...
    #[test]
    fn nexus_home_named_file_nested_in_workspace_is_user_scoped() {
    ...
    #[test]
    fn workspace_descendants_under_nexus_home_stay_project_scoped() {
        assert!(!is_user_instruction_path(
            Path::new("/custom/sage/worktrees/repo/src/AGENTS.md"),
