...
/// prediction.
pub(crate) const DEFAULT_SUGGEST_MODEL: &str = "sage-build-0.1";
...
/// (`sage-build-0.1`) is API-key-only and excluded from OAuth catalogs, so
...
/// `NEXUS_PROMPT_SUGGESTIONS_MODEL` keeps working for models the catalog does
...
const MESNEXUS_CAP_CHARS: usize = 1_500;
...
fn transcript_line(role: &str, text: &str) -> Option<String> {
    ...
    let mut text = text;
    if text.len() > MESNEXUS_CAP_CHARS {
        let cut = floor_char_boundary(text, MESNEXUS_CAP_CHARS);
        text = &text[..cut];
    }
...
mod tests {
    ...
    fn effective_model_default_requires_catalog() {
        ...
        // OAuth catalogs exclude sage-build-0.1 → skip the request entirely,
    ...
    fn effective_model_env_pin_bypasses_catalog_guard() {
        // NEXUS_PROMPT_SUGGESTIONS_MODEL is the explicit escape hatch: used
    ...
    fn transcript_budget_keeps_newest_messages() {
        let filler = "b".repeat(MESNEXUS_CAP_CHARS);
        ...
        let t = build_transcript(&conv).unwrap();
        assert!(t.len() <= TRANSCRIPT_BUDGET_CHARS + MESNEXUS_CAP_CHARS + 64);
