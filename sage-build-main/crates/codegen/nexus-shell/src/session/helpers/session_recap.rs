...
use nexus_chat_state::{compaction_utils, estimate_conversation_tokens, estimate_item_tokens};
...
///    (sage/SGLang via ChatCompletions/Responses) keeps reasoning VERBATIM so
...
pub(crate) fn build_recap_items(
    ...
    let mut items = if strip_reasoning {
        nexus_chat_state::compaction_utils::strip_reasoning_blocks(conversation)
    } else {
        conversation
    };
...
/// `max_prompt_length` for current `sage-build` / `sage-4.5` product backends
...
/// that unlikely for normal sage-build sessions).
///
/// * Fast path — if the whole snapshot already fits, returns
///   `build_recap_items(...)` verbatim (keeps the sage prefix KV cache warm;
...
pub(crate) fn budget_recap_items(
    ...
    // verbatim path keeps the sage prefix cache warm.
...
mod tests {
    ...
    fn main_turn_count_counts_real_users_only() {
        use std::sync::Arc;
        use nexus_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
    ...
    fn mk_tool_call(id: &str, args: &str) -> nexus_sampling_types::ToolCall {
        use std::sync::Arc;
    ...
    fn budget_over_budget_strips_reasoning_even_on_grok() {
        ...
        // sage backend => strip_reasoning=false, but the over-budget branch must
    ...
    fn budget_fast_path_keeps_reasoning_on_grok() {
        ...
        // Fits under a large window on sage (strip_reasoning=false) => verbatim,
        // reasoning kept so the prefix KV cache stays warm.
        let out = budget_recap_items(conv, "system-reminder", false, 256_000);
        assert!(
            out.iter()
                .any(|i| matches!(i, ConversationItem::Reasoning(_))),
            "fits path on sage must keep reasoning verbatim"
        );
    }
