...
mod interjection_broadcast_tests {
    ...
    /// `handle_interjection` depends on: method `sage.local/session/interjection`
            ...
            .run_until(async {
                ...
                while let Ok(msg) = gateway_rx.try_recv() {
                    if let nexus_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                        && args.request.method.as_ref() == "sage.local/session/interjection"
                    {
                        payload =
                ...
                let payload = payload.expect("an sage.local/session/interjection broadcast");
                assert_eq!(
