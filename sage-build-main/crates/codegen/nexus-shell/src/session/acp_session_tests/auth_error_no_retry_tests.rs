...
/// attribution.
fn auth_error() -> nexus_sampler::SamplingErrorInfo {
    nexus_sampler::SamplingErrorInfo {
        kind: nexus_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: nexus_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        nexus_chat_state::AuthType::SessionToken => "cached_token",
        nexus_chat_state::AuthType::ApiKey => "xai.api_key",
    };
...
async fn make_actor_with_method_and_credentials(
    ...
    actor
        .chat_state_handle
        .update_credentials(nexus_chat_state::Credentials {
...
fn model_not_found_error() -> nexus_sampler::SamplingErrorInfo {
    nexus_sampler::SamplingErrorInfo {
            kind: nexus_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model sage-build does not exist or your team does not have access".into(),
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must mention `sage logout`, got: {msg}"
            );
            assert!(
                msg.contains("sage login"),
                "hint must mention `sage login`, got: {msg}"
            );
            assert!(
...
fn unauthorized_401_error() -> nexus_sampler::SamplingErrorInfo {
    nexus_sampler::SamplingErrorInfo {
            kind: nexus_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.nexus.local/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_xai_token_auth=sage-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
