...
async fn rebuild_reinjects_task_completion_resource_identity() {
    use nexus_tools::reminders::task_completion::{
...
/// Local skill is always listable, so it renders into the sage markdown skill
/// catalog when the pending baseline is drained for a different agent.
fn regression_skill() -> nexus_tools::implementations::skills::types::SkillInfo {
...
/// Regression: a zero-turn agent rebuild INTO a sage/Default agent
/// must re-inject the baseline skill `<system-reminder>`. `initialize()`
/// is otherwise the only place skills are surfaced for the sage agent, so
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "reminder must carry the sage skill catalog header:\n{text}",
            );
            assert!(
