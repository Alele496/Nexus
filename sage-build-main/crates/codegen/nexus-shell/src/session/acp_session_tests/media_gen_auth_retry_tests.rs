...
use nexus_tools::types::output::{ToolOutput, ToolRunResult};

fn succeeding_am() -> Arc<AuthManager> {
    let dir = tempfile::tempdir().unwrap();
    let am = Arc::new(AuthManager::new(dir.path(), SageComConfig::default()));
...
fn ok_result(text: &str) -> Result<ToolRunResult, nexus_tool_runtime::ToolError> {
    Ok(ToolRunResult {
...
fn err(msg: &str) -> Result<ToolRunResult, nexus_tool_runtime::ToolError> {
...
fn http_err(status: u16, msg: &str) -> Result<ToolRunResult, nexus_tool_runtime::ToolError> {
    Err(
        nexus_tool_runtime::ToolError::new(nexus_tool_runtime::ToolErrorKind::Custom, msg.to_owned())
            .with_details(
...
fn is_auth_tool_error_classification() {
    ...
    let cases: Vec<(bool, nexus_tool_runtime::ToolError)> = vec![
        ...
        (
            true,
            nexus_tool_runtime::ToolError::new(
        ...
        // the inference path's gate in sage-sampling-types/src/error.rs:
        ...
        (
            true,
            nexus_tool_runtime::ToolError::invalid_arguments("response: invalid api key for project"),
        ),
        // Fallback path: OAuth 2.0 `invalid_token` payload (RFC 6749)
        // surfaced as raw JSON without a structured status code.
        (
            true,
            nexus_tool_runtime::ToolError::invalid_arguments(r#"{"error":"invalid_token"}"#),
        ),
        // Fallback path: case-insensitive "unauthorized" anywhere in
        // the message body.
        (
            true,
            nexus_tool_runtime::ToolError::invalid_arguments("UNAUTHORIZED"),
        ),
        // Negative: transport failure must not trigger a token refresh.
        (
            false,
            nexus_tool_runtime::ToolError::invalid_arguments("Image generation timed out after 60s"),
        ),
        // Negative: structural not-found error; not a network response.
        (
            false,
            nexus_tool_runtime::ToolError::not_found(
                nexus_tool_protocol::ToolId::new("image_gen").expect("valid"),
        ...
        (
            false,
            nexus_tool_runtime::ToolError::invalid_arguments("request id req_401abc failed"),
        ),
    ];
