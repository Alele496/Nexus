...
fn test_json_round_trip() {
    let ctx = nexus_agent::PromptContext {
        ..Default::default()
    };

    let json = serde_json::to_string_pretty(&ctx).unwrap();
    let loaded: nexus_agent::PromptContext = serde_json::from_str(&json).unwrap();
...
/// depend on `nexus_home()` and `SessionInfo` path encoding).
#[test]
fn test_json_round_trip_via_filesystem() {
    ...
    let ctx = nexus_agent::PromptContext::default();
    ...
    let loaded: nexus_agent::PromptContext = serde_json::from_str(&read_json).unwrap();
...
fn test_system_prompt_is_plain_text_not_json() {
    let prompt = "You are a Sage subagent.";
...
fn test_canonical_artifacts_coexist() {
    ...
    let read_ctx: nexus_agent::PromptContext = serde_json::from_str(
...
fn test_system_prompt_matches_chat_history_system_message() {
    ...
    let system_prompt = "You are a Sage subagent.\n\n<tool_calling>\n...";
...
fn test_corrupt_json_returns_error() {
    ...
    let result: Result<nexus_agent::PromptContext, _> = serde_json::from_str(&json);
...
fn fake_prompt_path() -> std::path::PathBuf {
    std::path::PathBuf::from("/tmp/sage-test-home/sessions/cwd/sid/prompts/prompt_0.txt")
}
...
fn build_truncated_preserves_small_query_truncates_context() {
    ...
    assert!(
        ...
        "sage ordering: query block first"
    );
...
//
// `nexus_home()` is a process-wide `OnceLock`, so the real async method is
