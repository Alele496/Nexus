...
//!
//! Tests mutate `NEXUS_GOAL_CLASSIFIER` so they carry `serial`.
...
use nexus_tools::implementations::nexus_build::update_goal::UpdateGoalInput;
...
async fn answer_summarizer(
    ...
    req: Box<nexus_tools::implementations::nexus_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
fn configure_goal_actor(
    ...
    actor.tool_context.cwd =
        nexus_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
