...
//!
//! Used by both the ACP `sage.local/session/list` handler and the `sage sessions`
...
pub async fn fetch_merged(
    ...
    let repo_urls_fut = async {
        cwd.map(|c| {
            nexus_workspace::session::git::resolve_normalized_remote_urls(std::path::Path::new(
                c,
            ))
        })
        .unwrap_or_default()
