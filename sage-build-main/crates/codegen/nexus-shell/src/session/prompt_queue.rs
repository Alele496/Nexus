...
//!
//! Canonical definitions live in `nexus_prompt_queue`; this re-export keeps every existing
//! `crate::session::prompt_queue::*` and cross-crate `nexus_shell::session::prompt_queue::*`
//! path resolving without edits.

pub use nexus_prompt_queue::{QueueChanged, QueueEntryMeta, QueueEntryWire};
...
mod tests {
    ...
    fn queue_entry_wire_round_trips_last_editor() {
        let entry = QueueEntryWire {
            ...
            version: 3,
            owner: Some("sage-tui".to_string()),
            last_editor: Some("sage-vscode".to_string()),
            kind: "prompt".to_string(),
            text: "hello".to_string(),
            position: 0,
        };
        let json = serde_json::to_value(&entry).unwrap();
        assert_eq!(json["lastEditor"], "sage-vscode");
