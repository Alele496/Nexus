//! Session-level fs-watch policy over [`nexus_fsnotify`].
//!
//! Mechanism (OS watch, coalesce, refcount) lives in `nexus_fsnotify`. This module
...
use nexus_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> nexus_workspace::file_system::FileIndexDelta {
    use nexus_workspace::file_system::FileIndexDelta;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = nexus_workspace::session::git::get_branch(&self.cwd).await;
        let commit = nexus_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = nexus_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = nexus_workspace::session::git::GitHeadChanged {
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: nexus_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root =
                nexus_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || nexus_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = nexus_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(nexus_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: nexus_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {
