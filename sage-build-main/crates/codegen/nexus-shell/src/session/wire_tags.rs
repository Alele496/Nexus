...
pub(crate) static USER_MESNEXUS_CHUNK: LazyLock<String> = LazyLock::new(|| {
...
/// leading `{`). Built from [`USER_MESNEXUS_CHUNK`] so the literal isn't hand-
/// maintained; the quoted key means it can't false-match the bare discriminant
/// escaped inside user content.
pub(crate) static USER_MESNEXUS_CHUNK_PREFIX: LazyLock<String> =
    LazyLock::new(|| format!(r#""sessionUpdate":"{}""#, *USER_MESNEXUS_CHUNK));
...
mod tests {
    ...
    fn derived_discriminants_match_wire_format() {
        assert_eq!(USER_MESNEXUS_CHUNK.as_str(), "user_message_chunk");
        assert_eq!(
        ...
        assert_eq!(
            USER_MESNEXUS_CHUNK_PREFIX.as_str(),
