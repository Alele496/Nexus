...
pub struct ParsedPrompt {
    ...
    /// Sage mode may include editor open/focus metadata; the compat mode does not.
...
impl ParsedPrompt {
    ...
    ///
    /// Layout:
    /// - **Sage mode:** `<user_query>` + `<skill_information>` + context
...
///
/// When `is_cursor` is false, produces original Sage-format output:
...
/// Sage-format resource links: `<focused_files>` / `<open_files>` with
...
mod tests {
    ...
    /// Shorthand: render + assemble for sage mode.
    fn render_grok(
        message: &str,
        embedded: Vec<String>,
    ...
    #[test]
    fn test_nexus_render_plain_message() {
        let result = render_grok("hello", vec![], vec![], &[], false);
        assert_eq!(result, "<user_query>\nhello\n</user_query>");
    }
    #[test]
    fn test_nexus_render_with_attachments_uses_system_reminder_wrapper() {
        ...
        assert!(
            ...
            "Nexus should start with <user_query>, got: {result}"
        );
    }
    #[test]
    fn test_nexus_render_user_query_first() {
        ...
        assert!(
            uq_pos < rr_pos,
            "Nexus: <user_query> ({uq_pos}) should come before resource links ({rr_pos})\ngot: {result}"
        );
        assert!(result.contains("<system-reminder>"));
    }
    #[test]
    fn test_nexus_render_resource_links_use_focused_files_format() {
        let links = vec![
