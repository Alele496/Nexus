...
use nexus_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
use nexus_mcp::servers as inner;
...
pub fn build_config_resolved_event(
    ...
    cwd: &Path,
) -> nexus_file_utils::events::Event {
    let disabled: Vec<String> = crate::util::config::disabled_mcp_server_names(cwd)
        .into_iter()
        .collect();
    let servers = configs
        .iter()
        .map(|c| nexus_file_utils::events::McpConfigServer {
        ...
        .collect();
    nexus_file_utils::events::Event::McpConfigResolved { servers, disabled }
}
