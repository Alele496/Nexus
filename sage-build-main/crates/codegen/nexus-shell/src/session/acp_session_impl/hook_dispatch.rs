...
) -> nexus_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> nexus_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use nexus_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> nexus_hooks::runner::RunContext<'_> {
        nexus_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[nexus_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, nexus_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: nexus_hooks::event::HookEventName,
        payload: nexus_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            nexus_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                nexus_hooks::result::HookRunResult::Success {
                    ...
                    hook_name,
                    elapsed,
                    nexus_telemetry::events::HookOutcome::Success,
                ),
                nexus_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    nexus_telemetry::events::HookOutcome::Error,
                ),
                nexus_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            nexus_telemetry::session_ctx::log_event(nexus_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: nexus_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
