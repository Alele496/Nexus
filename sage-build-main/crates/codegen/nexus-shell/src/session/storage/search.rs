...
//! other concurrently running sage processes (which may wipe or downgrade
...
use crate::session::wire_tags::{REWIND_MARKER, USER_MESNEXUS_CHUNK};
...
/// User-configurable overrides via `~/.nexus/settings.json` are deferred
/// to a follow-up.
...
///
/// TODO: When multiple sage processes run concurrently, they each have
...
fn search_db_path(root_dir: &Path) -> PathBuf {
    ...
    nexus_sqlite_journal::JournalMode::for_db_path(&path).effective_db_path(&path)
}
...
fn collect_all_indexable_content_single_pass(updates_path: &Path) -> io::Result<(String, u64)> {
    ...
    for line_result in reader.lines() {
        ...
        // ("_sage.local/session/update"). Dispatch on source first, then tag.
        if !is_xai {
            // ── ACP content events ──────────────────────────────────
            match tag {
                Some(t) if t == *USER_MESNEXUS_CHUNK => {
                    flush_assistant(&mut current_assistant, &mut assistant_texts);
...
fn collect_delta_content(updates_path: &Path, offset: u64) -> io::Result<DeltaResult> {
    ...
    for line_result in reader.lines() {
        ...
        match tag {
            ...
            Some(t) if !is_xai && t == *USER_MESNEXUS_CHUNK => {
                flush_assistant(&mut current_assistant, &mut assistant_texts);
                if let Ok(peek) = serde_json::from_str::<UserContentPeek<'_>>(raw_params)
                    && let Some(content) = peek.update.content
                    && content.content_type == Some("text")
