...
//! `sage-file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use nexus_file_utils::events::tracker::EventTracker;
pub(crate) use nexus_file_utils::events::types::{
...
) -> Option<nexus_sampling_types::PriorTurnInterrupt> {
    use nexus_sampling_types::PriorTurnInterrupt;
    match category {
...
/// committing it would silently run sage-build flavor. Distinct from
...
/// (`sage-file-utils`). The two enums are intentional mirrors and live in
