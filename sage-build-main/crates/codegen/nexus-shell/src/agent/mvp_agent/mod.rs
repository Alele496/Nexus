...
use nexus_telemetry::id::{agent_id, agent_instance_id};
use nexus_telemetry::session_ctx::log_event;
use nexus_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use nexus_workspace::permission::{ClientType, PermissionEvent};
...
use nexus_workspace::session::git::GitDiscoveryResult;
use nexus_hunk_tracker::HunkTrackerActor;
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        ...
        "SuperGrokPro" => jwt_claim == "supernexus_heavy",
        "SuperGrokLite" => jwt_claim == "supernexus_lite",
        _ => false,
    }
}
...
/// `_meta["sage.local/session"].kind` → [`SessionKind`]; absent/unknown/malformed → `Build`.
fn parse_session_kind(
    meta: Option<&acp::Meta>,
) -> crate::session::unified_list::SessionKind {
    use crate::session::unified_list::SessionKind;
    use serde::Deserialize;
    meta.and_then(|m| m.get("sage.local/session"))
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a nexus_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[nexus_announcements::RemoteAnnouncement]>,
    last_emitted: &[nexus_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<nexus_announcements::RemoteAnnouncement>> {
    let current = nexus_announcements::filter_expired_at(
...
/// Override with `NEXUS_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("NEXUS_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
pub struct MvpAgent {
    ...
    /// sage.local chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// list. Set by `enforce_nexus_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.nexus/`).
    ///
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<nexus_codebase_graph::IndexManagerHandle>>,
    >,
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        nexus_tools::implementations::nexus_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: nexus_tools::implementations::nexus_build::task::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: nexus_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled sage-desktop's first
    ...
    workspace_ops: RefCell<Option<nexus_workspace::WorkspaceOps>>,
    ...
    /// Announcements list last actually emitted via `sage.local/announcements/update`
    ...
    last_emitted_announcements: RefCell<Vec<nexus_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &nexus_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<nexus_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !nexus_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = nexus_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `sage-build` →
/// `sage-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `sage-build` →
/// `sage-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use nexus_agent::config::is_strict_harness_agent_type;
    match (
...
use nexus_chat_state::conversation_util::replace_or_insert_system_head;
...
///
/// Headers injected:
///  - `x-sage-client-version` -- required by the proxy's version-gate check.
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-sage-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
    ...
    headers
        .entry("x-sage-client-identifier".to_string())
...
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<nexus_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(nexus_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<nexus_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    /// - xAI updates (`"_sage.local/session/update"`) → `ExtNotification`.
    ///
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<nexus_acp_lib::AcpResult<()>>,
        >,
        mark_replay: bool,
        ...
        let is_xai = method == "_sage.local/session/update";
        if is_xai {
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<nexus_acp_lib::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = nexus_tools::types::TaskSnapshot {
    ...
    pub(super) async fn enforce_nexus_code_access(&self, auth: &crate::auth::GrokAuth) {
        if !auth.is_xai_auth() {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings nexus_build_access_gate)"
            );
            ...
            .map(|auth| {
                let gate = if !self.tier_allowed.get() && gate.is_none() {
                    ...
                    Some(crate::auth::GateInfo {
                        message,
                        url: Some(
                            "https://sage.local/supergrok?referrer=sage-build".to_string(),
                        ),
    ...
    /// cached_token, sage.local/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.nexus/bundled/` and the manifest.
...
/// absent — matching the `nexus_build_access_gate` flag's server-side default.
///
/// Used by both `enforce_nexus_code_access` (initial login gate) and
