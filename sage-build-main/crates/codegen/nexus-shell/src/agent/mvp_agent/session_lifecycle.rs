...
impl MvpAgent {
    ...
    /// session end — a terminal/explicit close (`sage.local/session/close`). It must
    ...
    /// (`sage.local/session/close`). Finalizes the cloud replica (genuine session
    ...
    /// `sage.local/sessions/changed` notification with the session in `removed` so
    ...
    /// `sage.local/sessions/changed` notification with the current entry in
    ...
    /// Emit an `sage.local/sessions/changed` upsert for a resident session with an
    ...
    /// Fan an `sage.local/sessions/changed` delta out to every attached client.
    ///
    ...
    /// the `sage.local/sessions/changed` special-case in `leader/server.rs`).
