...
use nexus_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
struct Inner {
    ...
    gateway: RwLock<Option<nexus_acp_lib::AcpAgentGatewaySender>>,
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&nexus_home, SageComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: nexus_acp_lib::AcpAgentGatewaySender) {
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<nexus_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<nexus_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    /// Hot-reload the catalog from `~/.nexus/models_cache.json` after an
    ...
    /// When another sage process sharing `~/.nexus` (a `--no-leader` run, a
    ...
    /// `ModelsCacheManager` path is fixed to `nexus_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::nexus_home::nexus_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.nexus_version.as_deref() != Some(nexus_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            nexus_version: Some(nexus_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "Nexus" both route to `sage-build`)
...
fn resolve_prefetch_env(nexus_com_config: Option<SageComConfig>) -> Option<PrefetchEnv> {
    let nexus_home = crate::util::nexus_home::nexus_home();
    let auth_manager = AuthManager::new(&nexus_home, nexus_com_config.unwrap_or_default());
...
pub fn start_early_prefetch(nexus_com_config: Option<SageComConfig>) -> Option<EarlyPrefetchHandle> {
    let env = resolve_prefetch_env(nexus_com_config)?;
    Some(spawn_prefetch_thread(env))
}
...
mod tests {
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use nexus_sampling_types::ReasoningEffortOption;
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        ...
        let _unset_legacy = EnvGuard::unset("NEXUS_CODE_XAI_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            ...
            make_entry_config_with_id(Some("sage-build"), "sage-build", Some("Nexus")),
