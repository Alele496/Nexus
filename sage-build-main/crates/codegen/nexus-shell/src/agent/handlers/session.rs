...
/// Router for sage.local/session/* and sage.local/session_summaries/* methods.
pub async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
) -> Result<acp::ExtResponse, acp::Error> {
    match args.method.as_ref() {
        "sage.local/session/info" => handle_session_info(agent, args).await,
        "sage.local/session/close" => handle_session_close(agent, args).await,
        "sage.local/session/list" => handle_session_list(agent, args).await,
        "sage.local/sessions/list" => handle_roster_list(agent, args).await,
        m if m.starts_with("sage.local/session_summaries/") => {
...
/// `sage.local/sessions/list` — the FleetView roster. Returns every
...
/// `sage.local/sessions/changed` broadcast.
...
async fn handle_session_close(
    ...
    if existed {
        ...
        tracing::info!(session_id = %req.session_id, "session closed via sage.local/session/close");
    } else {
...
async fn handle_session_summaries(
    ...
    match args.method.as_ref() {
        "sage.local/session_summaries/session_list" => {
        ...
        "sage.local/session_summaries/workspace_list" => {
        ...
        "sage.local/session_summaries/workspace_list_recent" => {
