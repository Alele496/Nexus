...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    if cfg.remote_settings.is_none()
        && let Some(handle) =
            crate::agent::models::start_early_prefetch(Some(cfg.nexus_com_config.clone()))
    {
        match handle.join() {
            Ok(result) => {
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        crate::builtin::extract_bundled_files(&nexus_home);

        crate::extensions::marketplace::purge_default_skills_installs(&nexus_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&nexus_home);
        }
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    let nexus_auth = auth_manager.current().filter(|a| a.is_xai_auth());
    let user_id = nexus_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = nexus_auth.as_ref().and_then(|a| a.team_id.clone());
