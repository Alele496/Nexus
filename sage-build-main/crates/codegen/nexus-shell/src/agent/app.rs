...
/// or live subagent). The second signal covers relay-driven (sage.local
...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(nexus_file_utils::trace_context::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = nexus_agent::prompt::workspace_user::optional_workspace_user_dir();
...
/// ([`nexus_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    nexus_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
...
pub async fn run_stdio_agent(
    ...
    nexus_telemetry::unified_log::set_version(nexus_version::VERSION);
    ...
    nexus_file_utils::queue::cleanup_orphaned_uploads(
        &nexus_home::nexus_home(),
        nexus_file_utils::queue::DEFAULT_MAX_AGE,
    );

    // Log the client that launched us (set by sage-desktop when spawning `sage agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("NEXUS_CLIENT_VERSION") {
        crate::unified_log::info(
            "NEXUS_CLIENT_VERSION",
            None,
    ...
    // blocking stdin reads (see `nexus_acp_lib::spawn_stdin_line_reader`): on
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `sage agent stdio` is a local/interactive entrypoint (spawned by
            // sage-desktop), so it needs the gate like the leader and pager paths;
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    nexus_telemetry::unified_log::set_version(nexus_version::VERSION);
    // `sage agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `sage agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a sage.local session. \
        Run `sage login` to sign in, or use `sage agent stdio` for API-key access.";
    ...
    let ctx = &agent_config.nexus_com_config;
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'sage login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `sage login`."),
        }
    } else if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&nexus_home::nexus_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
    ...
    // Capture the sage build URL for the first-connection callback
    let nexus_code_url = format!("{}/build", ctx.nexus_ws_origin);
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open Sage: {} (press Enter to open in browser)",
                nexus_code_url
            );
            eprintln!();
            let url_for_open = nexus_code_url.clone();
...
/// Start the leader's sage.local relay connection according to the start policy,
/// returning the slot where the [`RelayHandle`](crate::agent::relay::RelayHandle)
/// is parked once the connection task is running.
///
/// * `relay_on_demand == false` (default — explicit `sage agent leader`
...
///   duplication of mirroring every agent message to sage.local.
///
...
/// When a sage.local session is present, the leader connects to the websocket relay
...
/// * `relay_on_demand` - If true, defer the sage.local relay WebSocket until the
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: nexus_version::VERSION.to_string(),
    })
    ...
    local_set
        .run_until(async move {
            ...
            // Bridge websocket messages to agent (from sage.local relay)
            ...
            // Start (or arm) the sage.local relay. Eager by default — a bare
            // `sage agent leader` (devbox / systemd) has no local IPC clients
            ...
            let relay_handle_slot = if let Some(relay_config) = relay_config {
                ...
                info!("Relay disabled: no sage.local session token (BYOK / local-only leader)");
                Rc::new(std::cell::RefCell::new(None))
            };
            ...
            let auth_scope = agent_config.nexus_com_config.auth_scope();
            // Gated on user_nexus_home() so a cwd-relative .nexus/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = nexus_config::user_nexus_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.nexus/models_cache.json
                            // (another sage process fetched a fresher /v1/models
...
mod tests {
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::SageComConfig {
            nexus_ws_url: format!("ws://{addr}"),
            nexus_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
    ...
    /// `sage agent leader` (devbox/systemd — no local IPC clients,
    /// `relay_on_demand == false`) must connect the sage.local relay eagerly.
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `sage agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
