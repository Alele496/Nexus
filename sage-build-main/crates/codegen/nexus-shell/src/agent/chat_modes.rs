//! sage.local chat-product model catalog: caches `/rest/modes` and maps modes to
...
/// ~54 min, matching sage-web's refetch cadence.
...
pub const NEXUS_CHAT_MODE_ENV: &str = "NEXUS_CHAT_MODE";
...
impl ChatModesManager {
    ...
    /// The active sage.local identity, or `None` when unauthenticated. Modes are
...
/// Maps sage.local modes → `SessionModelState`: keeps only `available` modes,
...
mod tests {
    ...
    fn name_falls_back_to_id_when_title_blank() {
        ...
        let state = modes_to_model_state(&resp);
        assert_eq!(state.available_models[0].name, "sage-4.5");
    }
}
