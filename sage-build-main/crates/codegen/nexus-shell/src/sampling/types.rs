// Re-export all types from the standalone sage-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use nexus_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `sage-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `sage-sampler` crate can reference them without a circular
// dep on `sage-shell`.

// Tests for the types now live in sage-sampling-types crate.

use nexus_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `sage-sampling-types` or
/// `sage-tools`) so neither of those crates needs to depend on
