...
const MAX_MESNEXUS_SIZE: u32 = 64 * 1024 * 1024; // 64MB

#[derive(Debug, thiserror::Error)]
pub enum ProtocolError {
    #[error("IO error: {0}")]
    Io(#[from] io::Error),
    #[error("Message too large: {0} bytes (max: {MAX_MESNEXUS_SIZE})")]
    MessageTooLarge(u32),
...
pub enum ClientMode {
    /// Headless mode (sage agent, sage agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (sage agent stdio, sage -p) - uses local IPC.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `sage update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `sage update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `sage update` just installed; the leader uses
...
mod tests {
    ...
    async fn rejects_oversized_messages() {
        ...
        client
            .write_all(&(MAX_MESNEXUS_SIZE + 1).to_be_bytes())
            .await
            .unwrap();

        match read_frame(&mut server).await {
            Err(ProtocolError::MessageTooLarge(size)) => {
                assert_eq!(size, MAX_MESNEXUS_SIZE + 1);
            }
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["sage-a".into(), "sage-b".into()],
            pid: 4242,
        };
