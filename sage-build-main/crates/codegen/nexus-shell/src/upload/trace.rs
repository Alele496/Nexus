...
use url::Url;
use nexus_file_utils::queue::{EnqueueOutcome, TraceExportSource, UploadQueue, UploadRetryPolicy};
use nexus_workspace::permission::PermissionEvent;
...
pub(crate) fn spawn_trace_upload<T: Serialize + Send + 'static>(
    ...
    tokio::spawn(async move {
        ...
        let ok = nexus_file_utils::gcs::upload_bytes(
            &gcs_config,
            &object_path,
            &bytes,
            "application/json",
...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        nexus_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        nexus_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = nexus_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: nexus_sandbox::is_active(),
    })
}
...
fn classify_workspace(cwd: &str) -> String {
    ...
    if path.ancestors().any(|p| p.join(".git").exists()) {
        "git".to_owned()
    } else if nexus_file_utils::workspace_classifier::is_project_dir(path) {
        "project".to_owned()
    } else {
...
pub(crate) async fn upload_subagent_metadata(
    ...
    if let Err(e) =
        nexus_file_utils::gcs::upload_bytes(&config, &gcs_path, &json, "application/json").await
    {
        tracing::warn!(
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&nexus_agent::plugins::PluginRegistry>,
) {
...
use nexus_file_utils::gcs::upload_bytes;
...
pub(crate) async fn upload_artifact_to_gcs(
    ...
    match upload_bytes(&config, gcs_path, content, content_type).await {
        ...
        Err(e) => {
            let status_code = e
                .downcast_ref::<nexus_file_utils::storage_client::HttpUploadError>()
                .map(|e| e.status_code);
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        nexus_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(nexus_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
pub(crate) async fn upload_turn_messages(
    ctx: &PromptTraceContext,
    _capture: nexus_chat_state::TurnCapture,
    _wait: UploadWait,
) -> bool {
    super::manifest::skip_artifact(
        &ctx.artifact_tracker,
...
pub(crate) fn build_chat_history_session_state(
    _messages: &[nexus_sampling_types::conversation::ConversationItem],
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_attribution(
        &self,
    ) -> Option<Arc<dyn nexus_file_utils::storage_client::Auth401AttributionCallback>> {
        nexus_file_utils::gcs::StorageConfig::proxy_attribution(&self.with_auth())
    }
    fn proxy_credentials(&self) -> Option<Arc<dyn nexus_auth::AuthCredentialProvider>> {
        nexus_file_utils::gcs::StorageConfig::proxy_credentials(&self.with_auth())
    }
    fn proxy_http_client(&self) -> Option<reqwest::Client> {
        nexus_file_utils::gcs::StorageConfig::proxy_http_client(&self.with_auth())
    }
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    nexus_workspace::recovery::run_startup_recovery(&nexus_home, &queue).await;
                tracing::info!(?report, "startup spill recovery complete");
                queue.cleanup_orphans(nexus_file_utils::queue::DEFAULT_MAX_AGE);
            }
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    nexus_workspace::recovery::purge_spilled_items(&nexus_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
...
pub(crate) fn spawn_purge_stale_upload_scratch() {
    {
        ...
        let dir = crate::util::nexus_home::nexus_home()
            .join("upload_queue")
            .join("scratch");
...
pub(crate) fn spawn_upload_queue(
    ...
    let queue = UploadQueue::spawn(nexus_home, resolver, UploadRetryPolicy::default());
...
pub(crate) async fn upload_trace_artifact_blocking(
    ...
    let queue_result = if let Some(queue) = &ctx.upload_queue {
            ...
            .await
        {
            ...
            Err(e)
                if e.downcast_ref::<nexus_file_utils::queue::QueueClosed>()
                    .is_some() =>
            {
                tracing::debug!(
...
mod tests {
    ...
    fn dynamic_resolver_refreshes_proxy_token() {
        ...
        let nexus_com_config = SageComConfig::default();
        let scope = nexus_com_config.auth_scope();
    ...
    async fn resolve_async_falls_back_when_no_refresher() {
        ...
        let auth_json = serde_json::to_string_pretty(&store).unwrap();
        std::fs::write(dir.path().join("auth.json"), &auth_json).unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), nexus_com_config));
        let resolver = DynamicResolver {
            auth_manager,
            base_config: TraceExportConfig {
                bucket_url: None,
    ...
    fn dynamic_resolver_supplies_proxy_credentials_and_attribution() {
        use crate::session::repo_changes::UploadMethod;
        use nexus_file_utils::queue::TraceExportSource;
        let dir = tempfile::tempdir().unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
    ...
    fn home_project_dir() -> Option<tempfile::TempDir> {
        ...
        if !nexus_file_utils::workspace_classifier::is_project_dir(&home.join("probe")) {
            return None;
        }
