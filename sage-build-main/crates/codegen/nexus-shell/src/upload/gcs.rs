...
//! `StorageClient` constructed inside `nexus_file_utils::gcs::*` helpers.
//!
...
//! to an `nexus_file_utils::gcs::*` helper.
...
/// shell-side adapter that lets `nexus_file_utils::gcs::*` helpers wire
...
///
/// ```ignore
/// nexus_file_utils::gcs::upload_bytes(
...
/// `NEXUS_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("NEXUS_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match nexus_file_utils::gcs::upload_bytes(
