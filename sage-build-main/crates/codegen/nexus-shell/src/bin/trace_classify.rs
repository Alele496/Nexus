...
use clap::Parser;
use nexus_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// shell uses (`$NEXUS_HOME` or `~/.nexus`). Exposed primarily for
...
mod tests {
    ...
    #[test]
    fn cli_nexus_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.nexus_home, Some(PathBuf::from("/tmp/scratch-sage")));
    }
