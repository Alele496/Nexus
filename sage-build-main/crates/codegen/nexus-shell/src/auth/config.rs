...
/// Default scopes for the xAI OAuth2 provider. Includes `sage-cli:access`
...
pub enum PreferredAuthMethod {
    ...
    ApiKey,
    /// OIDC / OAuth2 session (`cached_token`, interactive `sage.local` / `oidc`,
    /// including devbox-minted OIDC).
    Oidc,
}
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct SageComConfig {
    ...
    /// Login button label (env: `NEXUS_AUTH_PROVIDER_LABEL`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// refresh works. Env: `NEXUS_AUTH_TOKEN_TTL`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// can't bypass the deployment's IdP login. Env: `NEXUS_DISABLE_API_KEY_AUTH`.
...
/// Customer OIDC Identity Provider configuration (`[nexus_com_config.oidc]`).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// OAuth2 provider configuration (`NEXUS_OAUTH2_ISSUER` / `NEXUS_OAUTH2_CLIENT_ID`).
///
...
const DEFAULT_OAUTH2_REFERRER: &str = "sage-build";
/// Returns `true` when `NEXUS_LOCAL_AUTH=1` is set,
/// indicating the local accounts-app should be used as the OAuth2 issuer.
pub fn use_local_auth() -> bool {
    std::env::var("NEXUS_LOCAL_AUTH")
        .map(|v| !v.is_empty() && v != "0")
        .unwrap_or(false)
}
/// Returns the active xAI OAuth2 issuer — the local-dev issuer when
/// `NEXUS_LOCAL_AUTH=1` is set, otherwise the production issuer.
...
/// auth.json scope key used by the pre-OIDC `sage login --legacy` flow.
...
impl SageComConfig {
    ...
    /// `NEXUS_DISABLE_API_KEY_AUTH` env lockdown is sticky: because the env value
    ...
    /// pin is fail-closed. Explicit `sage login --devbox` / `--api-key` bypass
    ...
    pub fn auth_scope(&self) -> String {
        if let Some(ref oidc) = self.oidc {
            ...
            // No OIDC or OAuth2 configured — API key mode; scope is a
            // stable key for per-method credential storage under auth.json.
            "api_key".to_string()
        }
    }
}
impl OAuth2ProviderConfig {
    ...
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("NEXUS_OAUTH2_ISSUER").ok()?;
        let client_id = std::env::var("NEXUS_OAUTH2_CLIENT_ID").ok()?;
        let principal_type = std::env::var("NEXUS_OAUTH2_PRINCIPAL_TYPE").ok();
        let principal_id = std::env::var("NEXUS_OAUTH2_PRINCIPAL_ID").ok();
        ...
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("NEXUS_OAUTH2_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or(default_scopes),
            principal_type,
            principal_id,
            referrer: Some(
                std::env::var("NEXUS_OAUTH2_REFERRER")
...
impl Default for SageComConfig {
    fn default() -> Self {
        let oidc = OidcAuthConfig::from_env();
        // Only enable OAuth2/OIDC when the user explicitly configures them via
        // env vars. No hardcoded default — Sage authenticates via API key unless
        // an org explicitly wires in their own IdP through NEXUS_OIDC_* or
        // NEXUS_OAUTH2_* environment variables.
        ...
        Self {
            nexus_ws_origin: std::env::var("NEXUS_WS_ORIGIN")
                .unwrap_or_else(|_| PROD_WS_ORIGIN.to_owned()),
            nexus_ws_url: std::env::var("NEXUS_WS_URL")
                .unwrap_or_else(|_| PROD_RELAY_WS_URL.to_owned()),
            token_header: "sage-cli".to_owned(),
            oidc,
            oauth2,
            auth_provider_command: std::env::var("NEXUS_AUTH_PROVIDER_COMMAND").ok(),
            auth_provider_label: std::env::var("NEXUS_AUTH_PROVIDER_LABEL").ok(),
            auth_token_ttl: std::env::var("NEXUS_AUTH_TOKEN_TTL")
                .ok()
                .and_then(|v| v.parse().ok()),
            disable_api_key_auth: std::env::var("NEXUS_DISABLE_API_KEY_AUTH")
                .ok()
...
/// Parse a boolean env-var value for sage's on/off flags. A bare presence
/// enables the flag, but the common falsy spellings (`0`, `false`, `off`,
/// `no`, empty) count as disabled — so e.g. `NEXUS_DISABLE_API_KEY_AUTH=false`
...
/// True when the admin has set `NEXUS_DISABLE_API_KEY_AUTH` to a truthy value in
...
fn env_lockdown_forced() -> bool {
    std::env::var("NEXUS_DISABLE_API_KEY_AUTH")
        .ok()
        .is_some_and(|v| env_flag_enabled(&v))
}
impl OidcAuthConfig {
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("NEXUS_OIDC_ISSUER").ok()?;
        let client_id = std::env::var("NEXUS_OIDC_CLIENT_ID").ok()?;
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("NEXUS_OIDC_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or_else(|_| default_oidc_scopes()),
            audience: std::env::var("NEXUS_OIDC_AUDIENCE").ok(),
        })
    }
}
#[cfg(test)]
mod tests {
    ...
    fn preferred_method_deserializes_from_toml() {
        let cfg: SageComConfig = toml::from_str(
            r#"
        ...
        let cfg: SageComConfig = toml::from_str("").expect("parse empty");
