...
use nexus_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        nexus_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
        ...
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.nexus_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<GrokAuth> {
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            nexus_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
...
mod tests {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), SageComConfig::default()));
        (dir, m)
    }
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.nexus_com_config().auth_scope();
        let fresh = GrokAuth {
