...
/// Format: https://sage.local/build/{sessionId}
pub fn build_share_url(session_id: &str) -> String {
    let base_url =
        std::env::var("NEXUS_CODE_WEB_URL").unwrap_or_else(|_| "https://sage.local".to_string());
        ...
        Some("initialize") => {
            ...
            if let Some(request_id) = id {
                ...
                *initialized = true;
                ...
                let upsert = json!({
                    "jsonrpc": "2.0",
                    "method": "_sage.local/session/upsert",
                    "params": {
                        "sessionId": session_id,
                        "cwd": cwd,
                    }
...
mod tests {
    ...
    fn test_build_share_url_default() {
        ...
        assert_eq!(url, "https://sage.local/build/test-session-123");
    }
    ...
    fn test_build_share_url_with_uuid() {
        ...
        assert_eq!(
            url,
            "https://sage.local/build/01937d8a-1234-7abc-9def-0123456789ab"
        );
    }
