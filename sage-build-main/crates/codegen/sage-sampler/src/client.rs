...
use sage_sampling_types::error::{try_parse_stream_error, user_facing_api_error_message};
...
pub use sage_sampling_types::ApiBackend;
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = sage_sampling_types::reported_cost_ticks(
        value
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<sage_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub async fn chat_completion(
        ...
        let x_sage_conv_id = &payload.x_sage_conv_id.clone().unwrap_or_default();
        let x_sage_req_id = &payload.x_sage_req_id.clone().unwrap_or_default();
        ...
        let sage_headers = GrokRequestHeaders {
            conv_id: x_sage_conv_id,
            req_id: x_sage_req_id,
            model_id: &model_id,
            session_id: payload.x_sage_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_sage_turn_idx.as_deref(),
            agent_id: payload.x_sage_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_sage_deployment_id.as_deref(),
            user_id: payload.x_sage_user_id.as_deref(),
        };
        let http_request = sage_headers
    ...
    pub async fn create_response(
        ...
        let x_sage_conv_id = request.x_sage_conv_id.as_deref().unwrap_or_default();
        let x_sage_req_id = request.x_sage_req_id.as_deref().unwrap_or_default();
        ...
        let sage_headers = GrokRequestHeaders {
            ...
            req_id: x_sage_req_id,
            model_id: &model_id,
            session_id: request.x_sage_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_sage_turn_idx.as_deref(),
            agent_id: request.x_sage_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_sage_deployment_id.as_deref(),
            user_id: request.x_sage_user_id.as_deref(),
        };
        ...
        sage_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = sage_headers
    ...
    pub async fn create_response_stream(
        ...
        let mut http_request = sage_headers
            .apply(self.post(self.endpoint("responses")))
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_sage_conv_id = request.x_sage_conv_id.clone();
        let x_sage_req_id = request.x_sage_req_id.clone();
        let x_sage_session_id = request.x_sage_session_id.clone();
        let x_sage_turn_idx = request.x_sage_turn_idx.clone();
        let x_sage_agent_id = request.x_sage_agent_id.clone();
        ...
        let extra_tools = sage_sampling_types::extra_raw_tools(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_sage_conv_id = x_sage_conv_id;
        wrapper.x_sage_req_id = x_sage_req_id;
        wrapper.x_sage_session_id = x_sage_session_id;
        wrapper.x_sage_turn_idx = x_sage_turn_idx;
        wrapper.x_sage_agent_id = x_sage_agent_id;
...
mod tests {
    ...
    use sage_sampling_types::types::ChatRequestMessage;
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            x_sage_conv_id: None,
            x_sage_req_id: None,
            x_sage_session_id: None,
            x_sage_turn_idx: None,
            x_sage_agent_id: None,
            x_sage_deployment_id: None,
            x_sage_user_id: None,
            trace: None,
        };
    ...
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = sage_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
