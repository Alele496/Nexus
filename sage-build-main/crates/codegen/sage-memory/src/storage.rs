...
//! `~/.sage/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.

use std::path::{Path, PathBuf};

use sage_tools::util::sage_home::sage_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `sage_agent::config::MemoryScope` (agent memory dir).
...
/// `~/.sage/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.sage/memory/xai-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.sage/memory/`
    global_dir: PathBuf,
    /// `~/.sage/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.sage/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| sage_home().join("memory"));
    ...
    pub fn total_chunk_count(&self) -> usize {
        ...
        // mounts (SIGBUS); see sage_sqlite_journal::JournalMode::open_readonly.
        sage_sqlite_journal::JournalMode::for_db_path(&db_path)
    ...
    ///
    /// File path: `~/.sage/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
