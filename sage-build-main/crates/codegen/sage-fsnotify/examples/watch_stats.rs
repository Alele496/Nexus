...
//! startup latency, under either strategy (`SAGE_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
...
//! SAGE_FSNOTIFY_PER_DIR=0 cargo run --release -p sage-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! SAGE_FSNOTIFY_PER_DIR=1 cargo run --release -p sage-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
use sage_fsnotify::{FsConfig, FsEventSource};
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("SAGE_FSNOTIFY_PER_DIR").unwrap_or_default();
