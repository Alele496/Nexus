...
```bash
# Run with workspace sandbox (read everywhere, write to CWD + temp dirs + ~/.sage/)
sage --sandbox workspace

# Read-only mode (read everywhere, write only to ~/.sage/ + temp dirs)
sage --sandbox read-only

# Most restrictive profile (read CWD + system paths, write CWD + temp dirs + ~/.sage/, no child network)
sage --sandbox strict
```
...
| `workspace`           | Everywhere         | CWD + `~/.sage/` + `/tmp` + `/var/tmp`         | Allowed       | Normal development                |
| `devbox`              | Everywhere         | All top-level dirs except `/data`              | Allowed       | Disposable dev VMs                |
| `read-only`           | Everywhere         | `~/.sage/` + `/tmp` + `/var/tmp`               | Blocked¹      | Exploration, code review          |
| `strict`              | CWD + system paths | CWD + `~/.sage/` + `/tmp` + `/var/tmp`         | Blocked¹      | Untrusted code                    |
...
**workspace** -- The recommended profile for everyday development. The agent can read any file on the system (for understanding dependencies, system libraries, etc.) but can only write to the current working directory, `~/.sage/`, and temp directories (`/tmp`, `/var/tmp`, plus the macOS temp dirs). Network access is allowed for tools like `web_search` and MCP servers.
...
**read-only** -- Use when you want the agent to analyze code without modifying your project files. The agent can read everything but can only write to `~/.sage/` (needed for session persistence) and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

**strict** -- The most restrictive profile, for reviewing untrusted code. The agent can only read files within the current working directory and essential system paths. Writes are limited to CWD, `~/.sage/`, and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

---

## Custom Profiles

Create custom sandbox profiles in `~/.sage/sandbox.toml` (global) or `.sage/sandbox.toml` (per-project):
...
The sandbox is applied to the **entire sage process** at startup using kernel primitives -- not per-command wrapping. This means all tool operations are covered:
...
for the life of the session**. When you resume it (`sage --resume <id>`,
`sage --continue`, or `sage -r`), Grok restores that same profile automatically —
...
1. An explicit `--sandbox <profile>` flag or `SAGE_SANDBOX` environment variable
...
Sandbox events are logged to `~/.sage/sandbox-events.jsonl` for debugging. Events include:
