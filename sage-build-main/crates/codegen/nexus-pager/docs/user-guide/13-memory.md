...
3. `SAGE_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
|----------|-------|-------------|
| `~/.sage/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.sage/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.sage/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |
...
Ask Grok to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.sage/memory/MEMORY.md` for cross-project preferences:
...
Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.sage/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.
...
You can edit memory files directly under `~/.sage/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
```
Memory saved to ~/.sage/memory/MEMORY.md
```
...
The `sage memory` command manages memory from the shell. It has one subcommand, `clear`:
...
sage memory clear --workspace

# Clear the global MEMORY.md
sage memory clear --global

# Clear both workspace and global memory
sage memory clear --all

# Skip the confirmation prompt (-y is the short form)
sage memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.sage/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.sage/memory/` for external edits and reindex |
...
By default, Grok watches `~/.sage/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `sage inspect` output.
2. Check the flag: `sage --experimental-memory` or `SAGE_MEMORY=1`.
3. Check for `--no-memory` or `SAGE_MEMORY=0` overriding your config.
...
```bash
ls ~/.sage/memory/
cat ~/.sage/memory/MEMORY.md
$EDITOR ~/.sage/memory/MEMORY.md
```
