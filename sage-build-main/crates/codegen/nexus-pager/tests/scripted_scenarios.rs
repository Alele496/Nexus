...
//!
//! These tests exercise the real `sage-pager` binary through a PTY using
...
/// enterprise deploy report: with `NEXUS_GOAL=1`, `/goal` must
...
/// explicitly off (`NEXUS_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `NEXUS_FOLDER_TRUST=1` and a git repo that
...
/// also pins `NEXUS_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
