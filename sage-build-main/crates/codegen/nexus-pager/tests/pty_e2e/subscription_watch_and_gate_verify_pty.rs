...
///
/// Proto `prod_auth.SubscriptionTier`: 5 = `supernexus_heavy` = live
...
/// Like [`seed_fake_oauth`], but under the `NEXUS_LOCAL_AUTH` dev issuer
...
/// `NEXUS_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
...
fn spawn_subscription_pager(
    ...
    env_refs.push(("NEXUS_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("NEXUS_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("NEXUS_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
