...
pub(crate) use nexus_pager_pty_harness::{
...
pub(crate) fn contextual_hints_env(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.push(("NEXUS_CONTEXTUAL_HINTS".into(), "1".into()));
    env
}
...
// `nexus_pager_pty_harness::flows` (re-exported above).
...
/// would never fetch `/v1/settings`. Spawns WITHOUT `NEXUS_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    env_refs.push(("NEXUS_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// (`NEXUS_TEST_VERSION`) and an explicit `NEXUS_FOLDER_TRUST` — `1` when `feature_on`,
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/NEXUS_HOME
...
pub(crate) fn trust_env(content: &ContentController, feature_on: bool) -> Vec<(String, String)> {
    ...
    // auto-trust.
    env.push(("NEXUS_TEST_VERSION".into(), "0.0.0-sim".into()));
    // Set NEXUS_FOLDER_TRUST explicitly: the default is on, so `0` is the opt-out
    // that exercises the feature-off path rather than relying on an absent var.
    let folder_trust = if feature_on { "1" } else { "0" };
    env.push(("NEXUS_FOLDER_TRUST".into(), folder_trust.into()));
    env
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".nexus")
        .join(nexus_workspace::trust::TRUST_FILE_NAME);
    let store = nexus_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&nexus_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `nexus_pager_pty_harness::flows`
// (re-exported above).
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    std::fs::create_dir_all(&nexus_home).expect("create fake NEXUS_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(nexus_home.join("config.toml"), config).expect("write config.toml");
}
...
/// Seed `~/.nexus/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{NEXUS_HOME|HOME}/.nexus/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let nexus_home = content.home().join(".nexus");
    std::fs::create_dir_all(&nexus_home).expect("create .nexus");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let nexus_home = content.home().join(".nexus");
    std::fs::create_dir_all(&nexus_home).expect("create .nexus");
    // Minimal opt-in only — matches load_config's `{NEXUS_HOME|HOME}/.nexus/config.toml`.
...
pub(crate) fn mouse_toggle_env(content: &ContentController) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    env.push(("NEXUS_MOUSE_REPORTING_TOGGLE".into(), "true".into()));
    env
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "NEXUS_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        nexus_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
/// `NEXUS_HOME`, returning the exit code (`None` if it never exited within
...
pub(crate) fn run_wrap_driving(
    ...
    let mut env: Vec<(&str, &str)> = vec![("NEXUS_HOME", &home_str), ("NO_COLOR", "1")];
...
/// Dump an asciinema cast of `harness` into `$NEXUS_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("NEXUS_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use nexus_pager_pty_harness::host_clipboard::{
...
pub(crate) use nexus_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
