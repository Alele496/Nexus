...
//!
//! The stack is `tmux -> nvim :terminal -> sage`: nvim/tmux repaint sage's
//! pane out-of-band (no sage PTY resize), and sage's diff renderer only
...
//! into the virtual terminal, bypassing sage) and then check whether sage
//! heals it. `NVIM` is set in sage's env so it sees the embedded-editor
//! context the fix keys off (mirroring a real nvim `:terminal`).
//!
//! The fix: sage forces a full clear+repaint on `FocusGained` in
//! editor/multiplexer contexts, so the injected row is gone after refocus.
//! The final assertion guards that heal.

use super::common::*;

/// Unique sentinel that sage would never render on its own.
...
async fn out_of_band_stale_row_heals_on_focus_gained() {
    ...
    env.push(("NVIM".into(), "/tmp/sage-pty-harness-fake-nvim.sock".into()));
    ...
    // virtual screen at col 1 — the static left margin sage's diff renderer
    ...
    // sage must not self-heal out-of-band content via ordinary diff redraws:
    // it only rewrites cells whose own model changed, so this row is stranded.
    h.update(Duration::from_millis(300));
    assert!(
        h.contains_text(STALE_MARKER),
        "stale row should survive a normal redraw (sage's diff renderer doesn't own it)\nscreen:\n{}",
        h.screen_contents()
    );

    // A FocusGained (CSI I) forces a full clear+repaint that re-asserts sage's
