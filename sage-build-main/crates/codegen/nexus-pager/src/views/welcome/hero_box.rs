...
const HERO_SUBTITLE: &str = "感谢使用 Nexus，输入 /feedback 反馈意见！";
...
pub(super) fn compute_hero_box(
    ...
    announcement: Option<&nexus_announcements::RemoteAnnouncement>,
    expanded: bool,
...
managed devices and accounts. Report security incidents";
    ...
    fn upgrade_cta_reserves_rows_and_returns_button_rect() {
        ...
        let a = ann(None, Some("Nexus 4.5 is here. Upgrade now."));
        ...
        let mut pinned = ann(None, Some("Nexus 4.5 is here. Upgrade now."));
        pinned.dismissible = Some(false);
        pinned.cta = Some(nexus_announcements::AnnouncementCta {
        ...
        let mut dismissible = ann(None, Some("Nexus 4.5 is here. Upgrade now."));
        dismissible.cta = Some(nexus_announcements::AnnouncementCta {
            label: Some("Upgrade Account".into()),
            url: Some("https://sage.local".into()),
            caption: Some("or use Ctrl+O".into()),
        });
