...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a nexus_announcements::RemoteAnnouncement>,
...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Sage** VERSION+channel **Beta** (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | Sage Beta [channel] (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Sage Beta**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    let channel = nexus_update::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", nexus_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
            spans.push(Span::styled(
                " Beta",
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub foreign_resume_hint: Option<&'a nexus_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [nexus_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a nexus_shell::auth::GateInfo>,
    ...
    let mut result = match params.auth_state {
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Nexus is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that Sage
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Nexus may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &nexus_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn render_welcome_done(
    ...
    let menu_items: &[(&str, &str)] = if !p.has_access {
        ...
        let (key_enter, key_w, key_s, key_q, key_i_with_x) = (
            "Enter",
            "ctrl+w",
            "ctrl+s",
            if in_vscode_family { "ctrl+d" } else { "ctrl+q" },
        ...
        let mut items: Vec<(&str, &str)> = Vec::with_capacity(6);
        ...
        items.push((key_enter, "新建会话"));
        items.push((key_w, "新建工作区 (需 Git 仓库)"));
...
fn masked_auth_token_view(input: &str, cursor_byte: usize, width: usize) -> (String, usize) {
    ...
    let buffer =
        nexus_ratatui_textarea::EditBuffer::from_parts(masked.display.as_str(), masked.cursor_byte);
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        ...
        for (tool, label) in [
            ...
            let hint = nexus_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn command_auth_arm_shows_url_and_waiting() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.x.ai/oauth2/authorize?client_id=sage";
    ...
    fn long_ann() -> nexus_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = nexus_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
