...
pub struct CreditBalance {
    ...
    /// `UNEXUS_PERIOD_TYPE_WEEKLY`). Drives the "Weekly/Monthly limit" label.
...
mod tests {
    ...
    fn summary_pay_as_you_go_enabled_renders_used_of_limit() {
        let b = CreditBalance {
            ...
            period_type: Some("UNEXUS_PERIOD_TYPE_MONTHLY".into()),
    ...
    fn usage_label_from_period_type() {
        assert_eq!(
            bal_period(0.0, "UNEXUS_PERIOD_TYPE_WEEKLY").usage_label(),
            "Weekly limit"
        );
        assert_eq!(
            bal_period(0.0, "UNEXUS_PERIOD_TYPE_MONTHLY").usage_label(),
            "Monthly limit"
        );
        // Unknown / unspecified / absent → falls back to "Usage".
        assert_eq!(
            bal_period(0.0, "UNEXUS_PERIOD_TYPE_UNSPECIFIED").usage_label(),
            "Usage"
        );
    ...
    fn summary_uses_period_label() {
        let weekly = bal_period(25.0, "UNEXUS_PERIOD_TYPE_WEEKLY");
        assert_eq!(format_usage_summary(&weekly, None), "Weekly limit: 25%");
        let monthly = bal_period(25.0, "UNEXUS_PERIOD_TYPE_MONTHLY");
        assert_eq!(format_usage_summary(&monthly, None), "Monthly limit: 25%");
    }

    #[test]
    fn warning_uses_period_label() {
        let weekly = bal_period(92.0, "UNEXUS_PERIOD_TYPE_WEEKLY");
        assert_eq!(
    ...
    fn summary_floors_usage_percent() {
        ...
        let almost = bal_period(99.994, "UNEXUS_PERIOD_TYPE_WEEKLY");
        assert_eq!(format_usage_summary(&almost, None), "Weekly limit: 99%");
        // A true 100% still shows 100%.
        let full = bal_period(100.0, "UNEXUS_PERIOD_TYPE_WEEKLY");
