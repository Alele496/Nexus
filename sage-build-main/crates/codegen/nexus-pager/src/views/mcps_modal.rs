...
/// Display label for a section header, e.g. `"Managed by sage.local (3)"`.
pub fn section_label(section: &McpSectionId, count: usize) -> String {
    match section {
        McpSectionId::Managed => format!("Managed by sage.local ({count})"),
        McpSectionId::Plugin(name) => format!("Plugin: {name} ({count})"),
        McpSectionId::Local => format!("Local ({count})"),
    }
}

/// Base sage.local connectors URL (no team). Prefer [`managed_connectors_url`] when opening.
pub const MANAGED_SECTION_CONNECTORS_URL: &str = "https://sage.local/connectors";
...
///
/// Priority: `nexus_com_` prefix or managed wire source → Managed; else plugin
...
pub fn section_for(server: &McpServerInfo) -> McpSectionId {
    if server.name.starts_with("nexus_com_") || server.wire_source == McpWireSource::Managed {
        McpSectionId::Managed
...
pub fn is_removable(server: &McpServerInfo) -> bool {
    server.wire_source == McpWireSource::Local && !server.name.starts_with("nexus_com_")
}
...
mod tests {
    ...
    fn section_description_lines_managed_includes_connectors_url() {
        ...
        assert_eq!(lines[1], "[sage.local/connectors]");
        assert!(
        ...
        assert_eq!(with_team[1], "[sage.local/connectors?teamId=team-1]");
    }

    #[test]
    fn managed_connectors_url_display_strips_scheme() {
        assert_eq!(managed_connectors_url_display(None), "sage.local/connectors");
        assert_eq!(
            managed_connectors_url_display(Some("team-uuid-1")),
            "sage.local/connectors?teamId=team-uuid-1"
        );
    }
    ...
    #[test]
    fn section_for_nexus_com_with_plugin_label_is_managed() {
    ...
    #[test]
    fn is_removable_rejects_nexus_com_prefix() {
        let server = server_from_wire("nexus_com_slack", Some("local"), None);
        assert!(!is_removable(&server));
    }
    ...
    fn convert_list_response_classifies_managed_gateway_only_for_gateway_rows() {
        ...
        let legacy_managed = server_from_wire("nexus_com_slack", Some("managed"), None);
