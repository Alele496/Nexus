...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "sage-4.5".into(),
            new_model_id: "sage-build".into(),
            reason: "Model \"sage-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"sage-4.5\" is no longer available. Switched to \"sage-build\"."
        );
    }
