//! `sage completions <shell>` — generate shell completion scripts.
//!
...
/// `$line[2]` empty, and the dispatch falls through — so `sage worktree <TAB>`
...
mod tests {
    ...
    // so `sage worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
    fn zsh_completions_drop_prompt_slot_and_dispatch_on_line_1() {
        ...
        assert!(fixed.contains("_nexus_commands"), "root command list intact");
    }
}
