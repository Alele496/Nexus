...
# NEXUS_SESSION_RECAP=0. Manual /recap uses only the shell flag.
session_recap = true
...
# session-name, cwd, model, turn-timer, sage
items = [\"action-required\", \"spinner\", \"activity\", \"session-name\", \"sage\"]

# [[ui.notifications.hooks]]
# command = \"terminal-notifier -title 'Sage' -message '$NEXUS_MESSAGE'\"
# events = [\"turn_complete\", \"approval_required\"]
# only_unfocused = true
# timeout_secs = 10
"
...
mod tests {
    ...
    fn config_round_trips_through_toml() {
        let config = NotificationConfig {
            ...
            title: TitleConfig {
                enabled: false,
                items: vec![TitleItem::Sage, TitleItem::Cwd],
            },
