...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("NEXUS_EVENT", event_str)
        .env("NEXUS_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("NEXUS_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'NEXUS_EVENT=%s\\nNEXUS_MESSAGE=%s\\nNEXUS_SESSION_ID=%s\\n' \
             \"$NEXUS_EVENT\" \"$NEXUS_MESSAGE\" \"$NEXUS_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("NEXUS_EVENT=Turn complete"),
            "missing NEXUS_EVENT: {content}"
        );
        assert!(
            content.contains("NEXUS_MESSAGE=hello world"),
            "missing NEXUS_MESSAGE: {content}"
        );
        assert!(
            content.contains("NEXUS_SESSION_ID=sess-42"),
            "missing NEXUS_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("NEXUS_SESSION_ID"),
            "NEXUS_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("NEXUS_EVENT=Turn complete"));
        assert!(content.contains("NEXUS_MESSAGE=test body payload"));
        assert!(content.contains("NEXUS_SESSION_ID=test-session-123"));
    }
}
