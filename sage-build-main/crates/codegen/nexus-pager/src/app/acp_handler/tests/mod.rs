...
use nexus_shell::extensions::notification::RetryState;
use nexus_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> nexus_shell::extensions::notification::ImageCompressedEntry {
    nexus_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn interjection_broadcast(
    ...
    acp::ExtNotification::new(
        "sage.local/session/interjection",
        std::sync::Arc::from(
...
pub(super) fn make_exit_plan_ext(
    ...
    nexus_acp_lib::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<nexus_acp_lib::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(nexus_acp_lib::AcpArgs {
...
pub(super) fn announcements_update_notif(
    r#gen: u64,
    announcements: &[nexus_announcements::RemoteAnnouncement],
) -> acp::ExtNotification {
    acp::ExtNotification::new(
...
/// Build an `sage.local/session/prompt_complete` ext-notification for `session_id`.
pub(super) fn prompt_complete_ext(session_id: &str) -> acp::ExtNotification {
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("sage.local/session/prompt_complete", std::sync::Arc::from(raw))
}
...
/// Build an `sage.local/session/prompt_complete` ext-notification with an explicit
...
/// Build an `sage.local/session/prompt_complete` ext-notification carrying a
...
/// Build a durable `TurnCompleted` update on the `sage.local/session/update` rail,
...
/// `sage.local/session/update` rail, optionally stamped `isReplay`.
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use nexus_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
/// Build an `sage.local/session/interjection` ext-notification (no id).
pub(super) fn interjection_ext(session_id: &str, text: &str) -> acp::ExtNotification {
    interjection_ext_with_id(session_id, text, None)
}
/// Build an `sage.local/session/interjection` ext-notification with an optional
...
pub(super) fn interjection_ext_with_id(
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("sage.local/session/interjection", std::sync::Arc::from(raw))
}
...
/// Build an `ExtNotification` envelope for `sage.local/session_notification`.
...
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(nexus_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
/// Shared temp `NEXUS_HOME` for disk-replay tests. `nexus_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `NEXUS_HOME`
...
/// Runs `f` with a thread-local sage home override so disk replay tests do not
/// depend on process-wide `nexus_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_nexus_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_nexus_home_for_tests(None);
    out
}
...
pub(super) fn dispatch_goal_update(
    ...
    handle(
        AcpClientMessage::ExtNotification(nexus_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("sage.local/session_notification", raw.into()),
            response_tx: tx,
        }),
        app,
    )
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(nexus_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
}
/// Build an `sage.local/session_notification` carrying
...
pub(super) fn interaction_resolved_ext(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("sage.local/session_notification", std::sync::Arc::from(raw))
}
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = nexus_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_replayed_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("sage.local/session/update", std::sync::Arc::from(raw))
}
...
pub(super) fn send_late_bg_detection(app: &mut AppView, tc_id: &str) {
    use serde_json::json;
    use nexus_tools::types::output::{BashOutput, ToolOutput};
    let agent = app.agents.get_mut(&AgentId(0)).unwrap();
    let meta = crate::acp::meta::NotificationMeta::default();
    let bash = BashOutput {
...
pub(super) fn task_completed_notif(
    ...
    use nexus_tools::types::TaskSnapshot;
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: nexus_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use nexus_shell::extensions::mcp::{
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = nexus_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "nexus_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName" : "nexus_com_linear", "tools" : [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("x.ai/mcp/tools_changed", std::sync::Arc::from(raw))
}
