...
/// Show session info: fetch via sage.local/session/info and display in scrollback.
///
...
pub(super) fn dispatch_show_privacy_info(app: &mut AppView) -> Vec<Effect> {
    ...
    lines.push("  - [features] telemetry / NEXUS_TELEMETRY_ENABLED");
    lines.push("  - [telemetry] trace_upload / NEXUS_TELEMETRY_TRACE_UPLOAD");
    lines.push("  - NEXUS_EXTERNAL_OTEL / OTEL_*");
    lines.push("");
...
/// (persists via ACP ext-request, NOT `~/.nexus/config.toml`).
...
/// Show context info: fetch via sage.local/session/info and display rich breakdown.
///
...
///
/// When the remote settings `nexus_build_usage_redirect_url` flag is set (delivered via
