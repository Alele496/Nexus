...
pub(super) fn dispatch_task_result(result: TaskResult, app: &mut AppView) -> Vec<Effect> {
    match result {
        ...
        TaskResult::SessionFailed { agent_id, error } => {
            ...
            // A fresh session-create failure always leaves a zombie: no
            // session_id, no forked_from.  Remove it, surface the error,
            // and return the user to a usable state (fallback agent or
            // welcome screen).
        ...
        TaskResult::ForeignResumeCwdCanonicalized {
                ...
                && let Some(canonical_cwd) = accepted_cwd
            {
                vec![Effect::DetectForeignResumeHint {
                    ...
                    nexus_home: nexus_tools::util::nexus_home::nexus_home(),
                    launch_token,
                }]
            } else {
                vec![]
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets =
                nexus_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
