...
//!
//! - Request/response `sage.local/sessions/list` → [`RosterListResponse`].
//! - Broadcast notification `sage.local/sessions/changed` → [`RosterChanged`].
//!
...
/// Response to `sage.local/sessions/list`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Broadcast payload for `sage.local/sessions/changed`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Parse an `sage.local/sessions/list` ext-response body into a [`RosterListResponse`].
///
/// The agent serializes the response through
/// `ExtMethodResult::success(..).to_ext_response()` (see
/// `sage-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
/// `sage.local/sessions/changed` broadcast. Mirrors how `Effect::FetchSessionList`
/// unwraps `result` for `sage.local/session/list`.
...
mod tests {
    ...
    fn agent_entry() -> nexus_shell::agent::roster::RosterEntry {
        use nexus_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        // Exact wire bytes the agent emits for `sage.local/sessions/list`.
        ...
        assert_eq!(e.model_id.as_deref(), Some("sage-4"));
        assert!(e.yolo);
    ...
    /// `sage.local/sessions/changed` round-trip: serialize the agent's `RosterChanged`
