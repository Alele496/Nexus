...
//! `SAGE_OSC52_SINK` / `LC_SAGE_OSC52_SINK`). Do not generalize this pattern
//! to untrusted multiplexers without an explicit allowlist.

use base64::Engine as _;
use sage_pager_render::clipboard::{ImageData, osc52_sink_active};
...
/// Successful host image frame: `SAGE_WRAP_IMG\n<mime>\n<base64>`.
pub const MAGIC_IMG: &str = "SAGE_WRAP_IMG";

/// Host has no image (`SAGE_WRAP_NONE`; not a prefix of [`MAGIC_IMG`]).
pub const MAGIC_NONE: &str = "SAGE_WRAP_NONE";
...
mod tests {
    ...
    fn garbage_is_not_wrap_paste() {
        ...
        assert_eq!(try_decode_wrap_host_image_paste("SAGE_WRAP_IM"), None);
    }

    #[test]
    fn malformed_img_frame_consumed_not_text() {
        assert_eq!(
            try_decode_wrap_host_image_paste("SAGE_WRAP_IMG"),
            Some(WrapImagePaste::NoImage)
        );
        assert_eq!(
            try_decode_wrap_host_image_paste("SAGE_WRAP_IMG\nbad"),
    ...
    fn oversized_b64_rejected_before_decode() {
        ...
        let payload = format!("SAGE_WRAP_IMG\nimage/png\n{huge}");
        assert_eq!(
