...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("SAGE_EVENT", event_str)
        .env("SAGE_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("SAGE_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'SAGE_EVENT=%s\\nSAGE_MESSAGE=%s\\nSAGE_SESSION_ID=%s\\n' \
             \"$SAGE_EVENT\" \"$SAGE_MESSAGE\" \"$SAGE_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("SAGE_EVENT=Turn complete"),
            "missing SAGE_EVENT: {content}"
        );
        assert!(
            content.contains("SAGE_MESSAGE=hello world"),
            "missing SAGE_MESSAGE: {content}"
        );
        assert!(
            content.contains("SAGE_SESSION_ID=sess-42"),
            "missing SAGE_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("SAGE_SESSION_ID"),
            "SAGE_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("SAGE_EVENT=Turn complete"));
        assert!(content.contains("SAGE_MESSAGE=test body payload"));
        assert!(content.contains("SAGE_SESSION_ID=test-session-123"));
    }
}
