...
use crate::util::display_user_sage_path;
...
Examples:
  ...
  # Add to the project config (./.sage/config.toml) instead of ~/.sage/config.toml
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.sage/config.toml`, available in all your projects
    User,
    /// `./.sage/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Config to write to: user (~/.sage/config.toml) or project (./.sage/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = sage_shell::util::config::load_mcp_server_configs_with_project(&cwd);
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    sage_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => sage_shell::util::config::user_config_path(),
        McpScope::Project => {
            sage_shell::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn scope_display(scope: McpScope, path: &Path) -> String {
    match scope {
        McpScope::User => display_user_sage_path("config.toml"),
...
fn select_remove_site(
    ...
    use sage_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    sage_shell::util::config::user_config_path(),
                )
            })
        })
}
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        sage_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    let (scope, path) = match select_remove_site(user_defined, find_project_site(), requested_scope)
    {
        ...
        Err(RemoveError::Ambiguous { project_path }) => {
            ...
            eprintln!("  user: {}", display_user_sage_path("config.toml"));
    ...
    // ancestor .sage/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = sage_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        sage_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = sage_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.sage/config.toml");
