...
use sage_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    // When available, add field: startup_announcements: Option<Vec<sage_announcements::RemoteAnnouncement>>
    ...
    pub auth_manager: std::sync::Arc<sage_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<sage_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_sage_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    sage_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = sage_shell::util::sage_home::sage_home().join("config.toml");
...
mod tests {
    ...
    #[test]
    fn startup_auth_sage_com_no_provider_needs_login_pending() {
    ...
    #[test]
    fn startup_auth_sage_com_with_external_provider_command() {
    ...
    #[test]
    fn startup_auth_non_sage_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.sage/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use sage_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    /// `startup_auth_sage_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use sage_shell::agent::auth_method::{SAGE_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(SAGE_COM_METHOD_ID, "Sage", None),
