...
use sage_tools::types::output::{ReadFileOutput, SearchToolOutput, WebFetchOutput};
use sage_tools::util::strip_redundant_session_cd;
...
impl AcpUpdateTracker {
    ...
    fn handle_user_message(
        ...
        let mut block = if let Some(dt) = display_override {
            ...
            let skill_display =
                sage_tools::implementations::skills::skill::extract_skill_display_text(&text);
...
fn user_message_hidden_from_scrollback(
    ...
    if let Some(pid) = meta.prompt_id.as_deref()
        && sage_shell::session::PromptOrigin::from_prompt_id(pid)
...
fn extract_edit_error(tc: &acp::ToolCall) -> String {
    use sage_tools::types::output::SearchReplaceOutput;
    if let Some(ref raw) = tc.raw_output
...
fn extract_listdir_content(raw: &Option<serde_json::Value>) -> Option<String> {
    let val = raw.as_ref()?;
    match serde_json::from_value::<ToolOutput>(val.clone()) {
        Ok(ToolOutput::ListDir(sage_tools::types::output::ListDirOutput::Content(c))) => {
            Some(c.content)
        }
        _ => None,
    }
...
fn extract_use_tool_output(raw: &Option<serde_json::Value>) -> Option<String> {
    ...
    if let Ok(output) = serde_json::from_value::<ToolOutput>(val.clone()) {
        let text = match output {
            ToolOutput::MCP(mcp) => {
                use sage_tools::types::output::MCPOutputDetails;
                match mcp.output() {
...
mod tests {
    ...
    fn test_search_tool_call_flow() {
        use sage_tools::types::output::{GrepFileMatch, GrepLineMatch, GrepSearchOutput};
        let mut tracker = AcpUpdateTracker::new();
    ...
    #[test]
    fn is_task_tool_recognizes_sage_build_variant() {
...
[CI] IN_PROGRESS=5 SUCCESS=38\n\
    ...
    fn video_tool_variants_use_typed_path_not_generic_scrape() {
        ...
        let cases: &[(&str, ToolOutput)] = &[
            (
                "ImageToVideo",
                ToolOutput::ImageToVideo(sage_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
            (
                "ReferenceToVideo",
                ToolOutput::ReferenceToVideo(sage_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
        ];
    ...
    fn media_gen_ref_skips_uploaded_only_video() {
        let output =
            ToolOutput::ImageToVideo(sage_tools::types::output::MediaGenOutput::uploaded(
    ...
    fn tier_restricted_media_shows_upsell_text_not_error() {
        ...
        let output = ToolOutput::Text(sage_tools::types::output::TextOutput::from(upsell));
        let tc = acp::ToolCall::new(
