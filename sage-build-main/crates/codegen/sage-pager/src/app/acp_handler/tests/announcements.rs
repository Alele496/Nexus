#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.sage` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.sage` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.sage` reads).
    #[test]
