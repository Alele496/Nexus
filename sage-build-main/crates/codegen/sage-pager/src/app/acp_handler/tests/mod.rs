...
use sage_shell::extensions::notification::RetryState;
use sage_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> sage_shell::extensions::notification::ImageCompressedEntry {
    sage_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn make_exit_plan_ext(
    ...
    sage_acp_lib::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<sage_acp_lib::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(sage_acp_lib::AcpArgs {
...
pub(super) fn announcements_update_notif(
    r#gen: u64,
    announcements: &[sage_announcements::RemoteAnnouncement],
) -> acp::ExtNotification {
    acp::ExtNotification::new(
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use sage_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(sage_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
/// Shared temp `SAGE_HOME` for disk-replay tests. `sage_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `SAGE_HOME`
...
/// depend on process-wide `sage_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_sage_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_sage_home_for_tests(None);
    out
}
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(sage_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = sage_workspace::session::git::GitHeadChanged {
...
pub(super) fn send_late_bg_detection(app: &mut AppView, tc_id: &str) {
    use serde_json::json;
    use sage_tools::types::output::{BashOutput, ToolOutput};
    let agent = app.agents.get_mut(&AgentId(0)).unwrap();
    let meta = crate::acp::meta::NotificationMeta::default();
    let bash = BashOutput {
...
pub(super) fn task_completed_notif(
    ...
    use sage_tools::types::TaskSnapshot;
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: sage_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use sage_shell::extensions::mcp::{
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = sage_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "sage_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName" : "sage_com_linear", "tools" : [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("x.ai/mcp/tools_changed", std::sync::Arc::from(raw))
}
