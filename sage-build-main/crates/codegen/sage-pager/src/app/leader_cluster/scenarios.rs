...
#[test]
#[ignore = "leader-cluster: needs single-process isolation (process-global env + sage_home OnceLock in the shared lib test binary); run: cargo test -p sage-pager --lib -- app::leader_cluster --ignored --test-threads=1"]
...
/// Locate `<sage_home>/sessions/<enc-cwd>/<sid>/updates.jsonl` without the
...
fn find_session_updates_file(sid: &str) -> Option<PathBuf> {
    let sessions = effective_sage_home().join("sessions");
