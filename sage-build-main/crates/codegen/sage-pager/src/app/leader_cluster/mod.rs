...
//!
//! Env sandboxing follows this crate's `serial(SAGE_HOME)` idiom; note
//! `sage_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_sage_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `XAI_API_KEY`,
//! `SAGE_LEADER_SOCKET`, `SAGE_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `sage_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
...
/// The sage home the agent actually persisted under: `sage_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_sage_home() -> PathBuf {
    sage_config::sage_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same SAGE_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(SAGE_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = sage_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("SAGE_HOME", sage_home.path()),
            crate::test_util::EnvVarGuard::set("SAGE_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("SAGE_XAI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XAI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("SAGE_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("SAGE_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("SAGE_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (sage_version), not
        ...
        generation_tasks.push(tokio::task::spawn_local(async move {
            let _ = run_leader_server(
                ...
                sage_shell::agent::activity::AgentActivity::default(),
                tokio::sync::watch::channel(true).1,
                tokio::sync::watch::channel(false).0,
                tokio::sync::watch::channel(sage_shell::leader::ShutdownReason::Manual).0,
                None,
                control_state,
            )
            .await;
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same SAGE_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `SAGE_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    sage_ws_url: String::new(),
                    sage_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
