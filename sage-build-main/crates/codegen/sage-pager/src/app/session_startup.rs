...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain Sage session whose first prompt resumes a foreign tool session.
    ForeignResume {
        tool: sage_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
/// Build `sage.local/session/fork` params shared by TUI effects and headless.
///
...
pub fn fork_session_params(
    ...
    let source_cwd = sage_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = sage_shell::util::sage_home::sage_home().join("sessions");
    let encoded = sage_shell::util::sage_home::encode_cwd_dirname(&cwd_str);
...
/// Parse `newSessionId` from an `sage.local/session/fork` ACP response body.
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    sage_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
pub struct MaterializeCtx {
    ...
    /// Process-wide flag: resume targets are sage.local conversations, not
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = sage_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'sage' to start a new session."
        )
    })?;
    Ok((first.info.id.to_string(), first.display_title_opt()))
}
/// `AuthManager` for direct sage.local calls made outside the agent (pre-ACP
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &sage_shell::agent::config::Config,
) -> std::sync::Arc<sage_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(sage_shell::auth::AuthManager::new(
        &sage_shell::util::sage_home::sage_home(),
        agent_config.sage_com_config.clone(),
    ));
    auth.configure_refresher(
        agent_config.sage_com_config.auth_provider_command.clone(),
        None,
    );
    auth
}
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if sage_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = sage_shell::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    if let Some(original_cwd) = sage_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
    ...
    let agent_config = sage_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use sage_shell::agent::session_registry_client::SessionRegistryClient;
    use sage_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    use sage_shell::session::restore::restore_session_with_storage;
    use sage_shell::util::sage_home::sage_home;
    ...
    let storage_client = sage_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: sage_shell::session::restore::ProgressCallback =
...
mod tests {
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["sage"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["sage", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["sage", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_alone_errors() {
        let err = parse(&["sage", "--fork-session"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["sage", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["sage", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["sage"])).chat_mode);
    }
