...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<sage_hooks_plugins_types::MarketplacePluginEntry>,
...
pub struct AgentView {
    ...
    pub context_state: Option<sage_shell::session::ContextInfo>,
    ...
    pub(crate) deferred_session_mode: Option<sage_tools::types::SessionMode>,
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(sage_telemetry::events::CreditLimitChoice::PayAsYouGo);
            sage_telemetry::session_ctx::log_event(
                sage_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            sage_telemetry::session_ctx::log_event(
                sage_telemetry::events::SuperGrokUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use sage_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        sage_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = sage_tools::util::sage_home::sage_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &sage_tools::implementations::sage_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
