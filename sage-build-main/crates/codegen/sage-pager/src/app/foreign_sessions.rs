...
use sage_workspace::foreign_sessions::{
...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, sage_home: &Path) -> [PathBuf; 2] {
        ...
        [
            sage_home
                ...
                .join(skill)
                .join("SKILL.md"),
            sage_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
    ...
    Fut: Future<Output = bool>,
{
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(sage_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, sage_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, sage_home, metadata_exists).await;
...
pub(crate) fn scan_effect(
    ...
    (compat.claude || compat.codex || compat.cursor).then(|| Effect::ScanForeignSessions {
        cwd: cwd.to_path_buf(),
        compat,
        sage_home: sage_home.to_path_buf(),
        coordinator,
        seq,
    })
}
...
mod tests {
    ...
    use sage_workspace::foreign_sessions::ForeignSessionSource;
