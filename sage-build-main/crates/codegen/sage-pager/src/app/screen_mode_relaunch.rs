...
pub(crate) const SAGE_SCREEN_MODE_ENV: &str = "SAGE_SCREEN_MODE";
...
pub(crate) fn build_screen_mode_relaunch_args(
    ...
    // Keep a CLI mode flag for hand-pasted resume hints that omit SAGE_SCREEN_MODE.
    if want_minimal {
...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!("{SAGE_SCREEN_MODE_ENV}={mode} sage {flag} --resume {session_id}")
}
...
pub(crate) fn exec_screen_mode_relaunch(session_id: &str, want_minimal: bool) -> io::Result<()> {
    ...
    cmd.env(SAGE_SCREEN_MODE_ENV, screen_mode_env_value(want_minimal));
...
/// Parse a [`SAGE_SCREEN_MODE_ENV`] or config `[ui] screen_mode` value
...
/// Consume the one-shot screen-mode override env (see [`SAGE_SCREEN_MODE_ENV`]).
///
...
pub(crate) fn take_screen_mode_env_override() -> Option<super::ScreenMode> {
    let raw = std::env::var_os(SAGE_SCREEN_MODE_ENV);
    if raw.is_some() {
        ...
        unsafe { std::env::remove_var(SAGE_SCREEN_MODE_ENV) };
    }
...
mod tests {
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        // Recovery command must carry SAGE_SCREEN_MODE so following the
        ...
        assert_eq!(
            ...
            "SAGE_SCREEN_MODE=fullscreen sage --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "SAGE_SCREEN_MODE=minimal sage --minimal --resume abc-sid"
        );
    }
