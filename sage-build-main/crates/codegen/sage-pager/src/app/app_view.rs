...
/// [`sage_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<sage_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: sage_shell::util::config::WorktreeHintMode) -> Self {
        use sage_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        sage_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            sage_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`sage_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `SAGE_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `SAGE_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("SAGE_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`sage_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => sage_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: sage_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`sage_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.sage/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`SAGE_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `SAGE_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<sage_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<sage_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `SAGE_PLUGIN_CTA`
    ...
    /// non-selectable headers. Gated by `SAGE_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `SAGE_CANCEL_REWIND` env /
    ...
    pub session_picker_content_results:
        Option<Vec<sage_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        sage_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `sage_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `SAGE_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: sage_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<sage_shell::util::config::ContextualHintsRemote>,
    ...
    /// `sage_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, sage_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `sage_build_access_gate`. `Some` = blocked.
    pub gate: Option<sage_shell::auth::GateInfo>,
    ...
    pub pending_gate_verification: Option<sage_shell::auth::GateInfo>,
    ...
    /// `~/.sage/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `SAGE_VOICE_MODE=0` — the STT pipeline is not started and
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &sage_shell::util::config::RemoteSettings,
    ) -> Option<sage_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(sage_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &sage_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            sage_telemetry::session_ctx::log_event(
                sage_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: sage_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: sage_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: sage_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<sage_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && sage_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: sage_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(sage_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(sage_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `SAGE_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<sage_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    sage_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        sage_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                sage_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            sage_telemetry::session_ctx::log_event(
                                sage_telemetry::events::SuperGrokUpsellShown {
                                    source:
                                        sage_telemetry::events::SuperGrokUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`sage_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use sage_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = sage_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                sage_telemetry::session_ctx::log_event(
                    sage_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: sage_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            sage_telemetry::session_ctx::log_event(sage_telemetry::events::ContextualTip {
                tip: sage_telemetry::events::ContextualTipKind::ImageInput,
                action: sage_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = sage_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![sage_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![sage_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: sage_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_hides_usage_for_team_users() {
        ...
        let meta = sage_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = sage_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_shows_usage_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&sage_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&sage_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(sage_shell::auth::GateInfo {
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = sage_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(sage_workspace::foreign_sessions::RecentForeignSession {
                tool: sage_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![sage_workspace::file_system::FuzzyMatchResult {
