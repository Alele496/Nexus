...
pub(super) fn dispatch_show_privacy_info(app: &mut AppView) -> Vec<Effect> {
    ...
    lines.push("  - [features] telemetry / SAGE_TELEMETRY_ENABLED");
    lines.push("  - [telemetry] trace_upload / SAGE_TELEMETRY_TRACE_UPLOAD");
    lines.push("  - SAGE_EXTERNAL_OTEL / OTEL_*");
    lines.push("");
...
/// (persists via ACP ext-request, NOT `~/.sage/config.toml`).
...
///
/// When the remote settings `sage_build_usage_redirect_url` flag is set (delivered via
