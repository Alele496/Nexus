...
/// Start (if needed) and submit the initial prompt from `sage "<prompt>"`.
///
...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sage_telemetry::events::ContextualTip {
            tip: sage_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sage_telemetry::events::ContextualTip {
            tip: sage_telemetry::events::ContextualTipKind::SmallScreen,
            action: sage_telemetry::events::ContextualTipAction::Shown,
        });
    }
}

/// Show the one-shot "Over SSH? Run `sage wrap ssh <host>` locally…" hint at
...
pub(in crate::app) fn show_ssh_wrap_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sage_telemetry::events::ContextualTip {
            tip: sage_telemetry::events::ContextualTipKind::SshWrap,
            action: sage_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_plan_nudge(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sage_telemetry::events::ContextualTip {
            tip: sage_telemetry::events::ContextualTipKind::PlanMode,
            action: sage_telemetry::events::ContextualTipAction::Shown,
        });
    }
    vec![]
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sage_telemetry::events::ContextualTip {
            tip: sage_telemetry::events::ContextualTipKind::WordSelect,
            action: sage_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
pub(super) fn dispatch_accept_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    log_event(sage_telemetry::events::ContextualTip {
        tip: sage_telemetry::events::ContextualTipKind::WordSelect,
        action: sage_telemetry::events::ContextualTipAction::Accepted,
    });
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(sage_telemetry::events::ContextualTip {
            tip: sage_telemetry::events::ContextualTipKind::SendNow,
            action: sage_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_send_prompt_inner(
            ...
            .is_restricted(invocation.token)
    {
        ...
        // another question modal already up, `open_supersage_upsell` would
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        let exec_result = {
            ...
            if let Some(invocation) = parse_invocation(trimmed) {
                ...
                {
                    use sage_telemetry::events::{PagerCommandSource, PagerSlashCommand};
                    use sage_telemetry::session_ctx::log_event;
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        ...
        let free_usage_blocked = agent.session.free_usage_blocked
            || result
                .as_ref()
                .err()
                .is_some_and(|e| sage_shell::sampling::error::is_free_usage_exhausted_error(e));
