...
fn voice_on_welcome_creates_session_and_records() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(sage_voice::VoiceCommand::PttPress)
    ));
}
#[test]
...
fn worktree_session_preserves_subdirectory_offset() {
    ...
    let id = AgentId(0);
    let worktree_root = PathBuf::from("/home/user/.sage/worktrees/repo/pager-123");
...
/// trust. SAGE_HOME-isolated so the write hits a temp store, not the real one.
...
fn bg_task_killed_no_op_for_unknown_session() {
    ...
    let effects = dispatch(
        Action::TaskComplete(TaskResult::BgTaskKilled {
            ...
            outcome: Some(sage_tools::types::KillOutcome::AlreadyExited),
        }),
        &mut app,
    );
    assert!(effects.is_empty());
...
fn cycle_mode_pre_session_blocked_by_policy_pin() {
    ...
    app.yolo_policy_block = Some(POLICY_WARNING);
    {
        ...
        agent.deferred_session_mode = Some(sage_tools::types::SessionMode::Plan);
    }
