...
fn test_app() -> AppView {
    ...
    AppView {
        ...
        auth_methods: vec![acp::AuthMethod::Agent(acp::AuthMethodAgent::new(
            acp::AuthMethodId::new("sage.local"),
            "Sage".to_string(),
        ))],
...
fn cta_entry(name: &str, status: &str) -> sage_hooks_plugins_types::MarketplacePluginEntry {
...
fn cta_outcome(
    status: sage_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
/// Plant a Build session under the process `sage_home()` (OnceLock-cached;
/// do not rely on setting `SAGE_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = sage_shell::util::sage_home::sage_home();
    let encoded = sage_shell::util::sage_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: sage_tools::implementations::sage_build::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    sage_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    sage_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
fn push_synthetic_permission(
    ...
    let request = sage_acp_lib::AcpArgs {
