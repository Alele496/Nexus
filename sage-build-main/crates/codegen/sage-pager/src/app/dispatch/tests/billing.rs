...
use super::*;
use sage_shell::sampling::error::is_free_usage_exhausted_error;
...
fn is_max_tier_positive_match() {
    assert!(is_max_tier(Some("supersage_heavy")));
...
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("supersage_heav")));
    assert!(!is_max_tier(Some("supersage_heavy_plus")));
...
fn upsell_non_max_upgrade_url_is_supergrok() {
    ...
    assert!(url.contains("referrer=sage-build"), "got: {url}");
}
...
fn is_credit_limit_error_matches_legacy_403_and_pool_402() {
    ...
    assert!(is_credit_limit_error(
        None,
        "API error (status 402 Payment Required): Sage usage balance exhausted"
    ));
    assert!(is_credit_limit_error(
        None,
        "status 403: run out of credits"
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("supersage_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("supersage_heavy"));
}
...
fn free_usage_upsell_shows_two_options_with_exact_labels() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: sage_telemetry::events::SuperGrokUpsell::FreeUsagePaywall,
            }
        )
    ));
    ...
    let expected = [
        ...
        (
            ...
            "Get the most out of Sage. Highest usage limits.",
            Some(UPSELL_URL_UPGRADE),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use sage_shell::extensions::notification::{RetryState, SessionUpdate};
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: sage_telemetry::events::SuperGrokUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
...
/// `SAGE_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
#[serial_test::serial(SAGE_TEST_OPEN_URL_FILE)]
#[test]
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "sage-open-url-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("SAGE_TEST_OPEN_URL_FILE", &bad) };
    ...
    unsafe { std::env::remove_var("SAGE_TEST_OPEN_URL_FILE") };
}
...
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    let url_file =
        std::env::temp_dir().join(format!("sage-open-url-ok-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("SAGE_TEST_OPEN_URL_FILE", &url_file) };
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "sage-open-url-upsell-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("SAGE_TEST_OPEN_URL_FILE", &bad) };
