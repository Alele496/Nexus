...
use sage_telemetry::events::{SuperGrokUpsell, SuperGrokUpsellClicked};
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("supersage_heavy") and CCP display names ("SuperGrok Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "supersage_heavy"
}
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        sage_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            sage_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            sage_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(sage_telemetry::events::CreditLimitUpsellShown {
            surface: sage_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(sage_telemetry::events::CreditLimitUpsellShown {
        surface: sage_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            sage_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_supersage_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_supersage_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}
...
fn open_supersage_upsell(
    ...
    log_event(sage_telemetry::events::SuperGrokUpsellShown {
        source,
        auth_method,
    });
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<sage_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<sage_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_supersage_url(app: &mut AppView) -> Vec<Effect> {
