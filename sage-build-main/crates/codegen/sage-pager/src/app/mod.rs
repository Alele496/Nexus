...
pub use sage_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
/// (`[ui] mouse_reporting_toggle` / `SAGE_MOUSE_REPORTING_TOGGLE`). Seeded once
...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    sage_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn voice_mode_config_value() -> Option<bool> {
    sage_shell::config::load_effective_config()
        .ok()
        .and_then(|cfg| {
            cfg.get("features")
                .and_then(|f| f.get("voice_mode"))
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use sage_shell::agent::config::{BoolFlag, ConfigSource};
    let resolved = BoolFlag::env("SAGE_VOICE_MODE")
...
///
/// Uses [`sage_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub fn resolve_use_leader(
    ...
    _remote_settings: Option<&sage_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<sage_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<sage_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<sage_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    sage_tty_utils::redirect_native_stderr();
    ...
    let sage_com_config =
        match sage_shell::agent::config::Config::new_from_toml_cfg(&raw_config) {
            Ok(c) => c.sage_com_config,
            Err(e) => {
                tracing::warn!(
                    error = % e, "failed to parse config for auth refresh, using defaults"
                );
                sage_shell::auth::SageComConfig::default()
            }
        };
    let refreshed_auth = sage_shell::auth::try_ensure_fresh_auth(&sage_com_config).await;
    let early_prefetch =
        sage_shell::agent::models::start_early_prefetch_with_auth(refreshed_auth);
    ...
    sage_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(sage_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = sage_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("SAGE_HUNK_TRACKER").ok();
    ...
    let launch_auto = sage_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    cancel.cancel();
    sage_tty_utils::global_process_scope().kill_all();
...
fn disable_mouse_paste_raw() {
    sage_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(sage_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    sage_crash_handler::enable_terminal_escape_restore();
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        sage_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(sage_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                sage_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = sage_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let term = sage_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
    ...
    .inspect_err(|_| {
        ...
        sage_crash_handler::disable_terminal_escape_restore();
    })
}
...
mod tests {
    ...
    fn restore_runs_teardown_even_when_writer_failed() {
        ...
        let terminal = sage_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = sage_shell::util::config::RemoteSettings {
