...
use super::*;
use sage_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use sage_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use sage_workspace::session::git::RestoreDegree;
...
fn spawn_fake_acp_agent(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<sage_acp_lib::AcpAgentMessage>,
) -> Arc<AtomicUsize> {
    let counter = Arc::new(AtomicUsize::new(0));
    let counter_clone = counter.clone();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let sage_acp_lib::AcpAgentMessage::ExtNotification(args) = msg {
...
/// Redirect `SAGE_HOME` to a tempdir for test isolation.
fn setup_sage_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("SAGE_HOME", tmp.path());
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use sage_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        sage_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        sage_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_sage_home_in_tempdir();
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use sage_acp_lib::AcpAgentMessage;
    use sage_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "x.ai/marketplace/action" => {
                        ...
                        let req: sage_hooks_plugins_types::MarketplaceActionRequest = serde_json::from_str(
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: sage_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            sage_home: PathBuf::from("/path/that/must/not/be-read"),
...
/// Neutralize `SAGE_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(SAGE_AGENT)]` (process-global env).
fn without_sage_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("SAGE_AGENT", "")
}
...
#[serial_test::serial(SAGE_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_sage_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> sage_shell::session::SessionInfoResponse {
    use sage_shell::session::acp_types::{ContextInfo, SessionInfoData};
    sage_shell::session::SessionInfoResponse {
