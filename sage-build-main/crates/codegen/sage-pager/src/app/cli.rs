...
See ~/.sage/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// `SAGE_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.sage/config.toml` or when the `SAGE_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
pub struct HeadlessArgs {
    ...
    pub sage_ws_origin: Option<String>,
    #[arg(long = "sage-ws-url")]
    pub sage_ws_url: Option<String>,
}
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "SAGE_AGENT_SECRET")]
...
fn version_with_channel() -> &'static str {
    ...
    V.get_or_init(|| {
        let label = sage_update::channel_label();
        sage_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), label)
    })
}
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.sage/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// persists per-segment markdown to grep. Sets `SAGE_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            sage_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    #[arg(long, env = "SAGE_SANDBOX", value_name = "PROFILE")]
    ...
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.sage/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.sage/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "SAGE_LOG_SAMPLING", hide = true)]
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`SAGE_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                sage_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                sage_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<sage_sandbox::ProfileName>().ok()
                    != s.parse::<sage_sandbox::ProfileName>().ok() =>
            {
...
mod tests {
    ...
    #[test]
    fn bare_sage_has_no_initial_prompt() {
        let args = PagerArgs::try_parse_from(["sage"]).expect("bare sage parses");
