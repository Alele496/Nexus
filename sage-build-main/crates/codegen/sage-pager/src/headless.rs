...
//! `spawn_sage_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use sage_acp_lib::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use sage_shell::agent::auth_method::AuthMethodKind;
use sage_shell::agent::config::Config as AgentConfig;
use sage_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use sage_shell::util::config as cli_config;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::spawn_sage_shell;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<sage_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use sage_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<sage_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = sage_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut sage_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    sage_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !sage_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !sage_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &sage_tools::types::compat::CompatConfig::default());
...
pub async fn run_single_turn(
    ...
    sage_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = sage_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = sage_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        sage_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_sage_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("SAGE_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = sage_shell::active_sessions::register(
            sage_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(sage_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(sage_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.sage can't hang exit.
        let _ = sage_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = sage_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    notif: &sage_acp_lib::AcpArgsBox<acp::ExtNotification>,
    format: OutputFormat,
) -> ExtEvent {
...
mod tests {
    ...
    use super::*;
    use sage_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            sage_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> sage_acp_lib::AcpArgsBox<acp::ExtNotification> {
    ...
    fn headless_session_update_unknown_method_is_none() {
        ...
        let raw = serde_json::value::to_raw_value(&payload).unwrap();
        let (tx, _rx) = tokio::sync::oneshot::channel();
        let notif = sage_acp_lib::AcpArgs {
