...
/// firehose (`SAGE_DEBUG_LOG` / `SAGE_LOG_FILE`).
pub use sage_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
...
pub use sage_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "sage_shell=info,sage_pager=trace,sage_tools=info,sage_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = sage_telemetry::otel_layer::build_otel_layer(
    ...
    let instrumentation_layer = sage_telemetry::instrumentation::layer();
    let sampling_log_layer = sage_telemetry::sampling_log::layer();
    let hooks_log_layer = sage_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    sage_telemetry::debug_log::install_firehose(registry, "tui");
