...
//! the generic [`CodeBlockSpan`](sage_markdown::CodeBlockSpan) API) and
...
use sage_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](sage_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](sage_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a sage_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
/// dark. The render worker maps this to `sage_mermaid::MermaidTheme`; it
...
mod tests {
    ...
    use sage_markdown::StreamingMarkdownRenderer;
