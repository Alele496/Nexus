...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("Sage Test", 4); // cursor between "Sage" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
fn string_editor_uses_canonical_edits_policy_and_live_validation() {
    ...
    let mut state = editor_render_fixture("Sage Tes", "Sage Tes".len());
    ...
    assert_eq!(state.editing_buffer(), Some("Sage Test"));
...
fn string_editor_paste_sanitizes_validates_and_consumes_rejected_text() {
    let mut state = editor_render_fixture("Sage Tst", "Sage T".len());
