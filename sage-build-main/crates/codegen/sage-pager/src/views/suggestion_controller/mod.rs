...
pub struct SuggestionController {
    ...
    /// `SAGE_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `SAGE_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `x.ai/suggest` request.
    /// Resolved at construction from `SAGE_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: sage_config::env_bool("SAGE_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: sage_config::env_bool("SAGE_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("SAGE_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
