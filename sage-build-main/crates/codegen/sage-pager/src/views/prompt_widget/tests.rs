#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/sage-test-image.png"));
        ...
        assert!(
            !full.contains("/tmp/sage-test-image.png"),
            "source path must not appear in the buffer chip: {full:?}"
        );
        assert_eq!(
            pw.images[0].source_path.as_deref(),
            Some(std::path::Path::new("/tmp/sage-test-image.png")),
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/sage-test-image.png"),
    ...
    fn accept_inside_command_with_args_absorbs_existing_separator() {
        ...
        pw.textarea.insert_str("/mod sage-4");
        pw.textarea.set_cursor(3);
        pw.refresh_slash(&models);
    ...
    fn accept_arg_completion_replaces_arg_range() {
        ...
        models.available.insert(
            model_id.clone(),
            agent_client_protocol::ModelInfo::new(model_id, "Sage 4.5".to_string()),
        );
        ...
        // Accept arg completion → should replace "gr" with "Sage 4.5".
        pw.accept_slash_completion(&models);
        let text = pw.textarea.text().to_string();
        assert!(
            text.contains("Sage 4.5"),
    ...
    fn test_image() -> PastedImage {
        PastedImage {
            element_id: sage_ratatui_textarea::ElementId::from_raw(0), // overwritten by insert_image
            display_number: 0,
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> sage_workspace::file_system::FuzzyMatchResult {
        sage_workspace::file_system::FuzzyMatchResult {
