...
/// Env override for the whole feature: `SAGE_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "SAGE_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call:
/// `SAGE_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "SAGE_PROMPT_SUGGESTIONS_MODEL";
...
pub struct PromptSuggestionController {
    ...
    /// `SAGE_PROMPT_SUGGESTIONS` env var, falling back to the persisted
...
pub fn resolve_enabled() -> bool {
    ...
    ENV_OVERRIDE
        .get_or_init(|| sage_config::env_bool(PROMPT_SUGGESTIONS_ENV))
