...
pub enum ActiveModal {
    ...
    SessionPicker {
        ...
        content_results: Option<Vec<sage_shell::extensions::session_search::SearchSessionHit>>,
...
fn fit_docs_ask_sage_tip(docs_path: &str, width: usize) -> String {
...
pub fn render_doc_picker_overlay(
    ...
    if let Some(tip_rect) = tip_area {
        let docs_path = crate::util::display_user_sage_path(DOCS_USER_GUIDE_REL);
        let tip_line = fit_docs_ask_sage_tip(&docs_path, tip_rect.width as usize);
...
mod doc_picker_tip_tests {
    use super::{
        ActiveModal, DOCS_USER_GUIDE_REL, fit_docs_ask_sage_tip, howto_list_modal,
    ...
    fn fit_docs_tip_prefers_path_and_never_overflows() {
        let path = crate::util::display_user_sage_path(DOCS_USER_GUIDE_REL);
        let long =
        ...
        assert_eq!(fit_docs_ask_sage_tip(&path, long.width()), long);
        assert_eq!(fit_docs_ask_sage_tip(&path, short.width()), short);
        assert_eq!(fit_docs_ask_sage_tip(&path, path_only.width()), path_only);
        for w in [5usize, 12, 20, 30] {
            let line = fit_docs_ask_sage_tip(&path, w);
