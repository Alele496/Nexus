...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // reads this (see `sage_pager_render::clipboard::osc52_sink_active`) to
    ...
    // these two without clearing it. The canonical `SAGE_OSC52_SINK` is
    ...
    cmd.env("SAGE_OSC52_SINK", "1");
    cmd.env("LC_SAGE_OSC52_SINK", "1");
