...
use sage_shell::auth::{AuthManager, try_ensure_fresh_auth};
use sage_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // If the user has previously run the interactive `sage` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.sage_com_config).await;
    ...
    let client = sage_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = sage_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                None,
                limit,
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use sage_shell::session::merge::REMOTE_TIMEOUT;
            use sage_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = sage_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
