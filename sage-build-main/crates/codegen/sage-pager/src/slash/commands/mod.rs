...
mod tests {
    ...
    fn sample_models() -> ModelState {
        ...
        models.available.insert(
            id_fast.clone(),
            acp::ModelInfo::new(id_fast.clone(), "Sage 4.5".to_string()),
        );
        let id_pro = acp::ModelId::new(Arc::from("sage-4.3"));
        models.available.insert(
            id_pro.clone(),
            acp::ModelInfo::new(id_pro.clone(), "Sage 4.3".to_string()),
        );
    ...
    fn model_suggest_args_returns_available_models() {
        ...
        assert!(
            items
                .iter()
                .any(|i| i.display.starts_with("Sage 4.5") && i.insert_text == "Sage 4.5")
        );
        assert!(
            items
                .iter()
                .any(|i| i.display == "Sage 4.3" && i.insert_text == "Sage 4.3")
        );
    }
    #[test]
    ...
    fn usage_manage_returns_open_url() {
        match run_usage("manage") {
            CommandResult::Action(Action::OpenUrl(url)) => {
                assert_eq!(url, "https://sage.local/?_s=usage");
            }
