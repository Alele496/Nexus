...
use sage_shell::sampling::types::supports_reasoning_effort_meta;
...
impl SlashCommand for ModelCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        // often contain spaces ("Sage 4.5"); if we split on the last token
        // first, a shorter catalog entry ("Sage") would steal the prefix and
        ...
        // level (e.g. `none` on sage-4.5) surfaces the effort error with the
...
mod tests {
    ...
    fn empty_query_returns_one_row_per_logical_model() {
        ...
        let (pid, pinfo) = plain_model("sage-4.5", "Sage 4.5");
        ...
        let plain = items.iter().find(|i| i.match_text == "Sage 4.5").unwrap();
        assert_eq!(plain.insert_text, "Sage 4.5");
    }
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // Catalog has both "Sage" (reasoning) and "Sage 4.5". `/model Sage 4.5`
        // must select the full name, not treat "4.5" as an effort on "Sage".
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("sage", "Sage");
        let (long_id, long_info) = model_with_reasoning("sage-4.5", "Sage 4.5");
        ...
        match result {
            ...
            other => panic!("expected SetDefaultModel(Sage 4.5), got {other:?}"),
        }
    }
    ...
    fn run_rejects_effort_for_non_reasoning_model() {
        let mut state = ModelState::default();
        let (id, info) = plain_model("sage-4.5", "Sage 4.5");
        state.available.insert(id, info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "Sage 4.5 high");
