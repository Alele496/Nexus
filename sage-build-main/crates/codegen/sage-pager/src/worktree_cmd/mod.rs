...
use sage_fast_worktree::WorktreeRecord;
...
pub async fn run(args: WorktreeArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    let spawned = crate::acp::spawn::spawn_sage_shell(agent_config.clone(), &cancel, None).await?;
...
async fn dispatch(command: WorktreeCommand, tx: &sage_acp_lib::AcpAgentTx) -> Result<()> {
    match command {
...
async fn cmd_show(tx: &sage_acp_lib::AcpAgentTx, id_or_path: &str) -> Result<()> {
...
async fn cmd_db(tx: &sage_acp_lib::AcpAgentTx, command: WorktreeDbCommand) -> Result<()> {
    match command {
...
mod tests {
    ...
    fn ext_envelope_unwraps_success_result() {
        let json = r#"{"result": {"path": "/home/user/.sage/worktrees.db"}, "error": null}"#;
        #[derive(serde::Deserialize)]
        struct PathResp {
            path: String,
        }
        ...
        assert_eq!(inner.path, "/home/user/.sage/worktrees.db");
    }
