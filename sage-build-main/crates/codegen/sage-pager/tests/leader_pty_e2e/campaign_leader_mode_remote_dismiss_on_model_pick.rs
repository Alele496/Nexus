...
#[ignore = "PTY e2e; run with cargo test -p sage-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared SAGE_HOME so every spawn elects/attaches to the
    // same leader (mirrors `LeaderCluster`).
    let sage_home = content.home().join(".sage");
    std::fs::create_dir_all(&sage_home).expect("create SAGE_HOME");
    std::fs::write(
    ...
    let state_path = sage_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(sage_home.join("config.toml")).expect("read config.toml");
    assert!(
