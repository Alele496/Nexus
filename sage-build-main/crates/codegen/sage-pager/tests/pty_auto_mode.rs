...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.sage/auth.json` from `SAGE_AUTH_JSON` (path) or the
//! developer's `~/.sage/auth.json` so the pager skips device-login when
...
use sage_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.sage/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("SAGE_AUTH_JSON") {
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    if let Some(src) = auth_json_source() {
        ...
        eprintln!("pty_auto_mode: no ~/.sage/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("SAGE_HOME".into(), sage.display().to_string()),
    ...
    // of the developer's shell. `SAGE_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (sage_config::
    ...
    env.push((
        "SAGE_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("SAGE_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "SAGE_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
