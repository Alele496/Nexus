...
use sage_pager_pty_harness::StyledLine;
...
/// Seed a user-invocable skill under `dir/.sage/skills`. The workspace must
...
fn seed_test_skill(dir: &Path) {
    let skill_dir = dir.join(".sage").join("skills").join("test-skill");
