...
fn spawn_with_announcements(content: &ContentController, override_json: &str) -> PtyHarness {
    ...
    env.push((
        "SAGE_ANNOUNCEMENTS_OVERRIDE".into(),
...
/// safe-open path — observed via the `SAGE_TEST_OPEN_URL_FILE` seam, never a
...
async fn promo_cta_click_opens_link_and_hide_roundtrip() {
    ...
    let extra_env = [
        ("SAGE_TEST_OPEN_URL_FILE", url_file_str.as_str()),
...
/// Pinned (`dismissible:false`) promo seeded via `SAGE_ANNOUNCEMENTS_OVERRIDE`.
...
/// `SAGE_TEST_OPEN_URL_FILE` seam + a `TERM_PROGRAM` pin for OSC 8).
