...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("sage-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.sage/config.toml` with a per-model reasoning-effort menu.
    let sage_home = content.home().join(".sage");
    std::fs::create_dir_all(&sage_home).expect("create .sage");
    // Quote the dotted model id: bare `[model.sage-4.5]` is TOML key-path syntax (model.sage-4.5), not the id "sage-4.5".
    std::fs::write(
