...
///
/// Proto `prod_auth.SubscriptionTier`: 5 = `supersage_heavy` = live
...
/// Like [`seed_fake_oauth`], but under the `SAGE_LOCAL_AUTH` dev issuer
...
/// `SAGE_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let sage_home = content.home().join(".sage");
    std::fs::create_dir_all(&sage_home).expect("create temp .sage");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    env_refs.push(("SAGE_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let content = ContentController::start_with_models(vec![MockModel::new("sage-3")])
        .await
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("SAGE_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn startup_gate_shows_paywall_for_free_user_after_live_check() {
    ...
    content.server().set_settings(json!({
        ...
        "gate_url": "https://sage.local/supergrok?referrer=sage-build",
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("SAGE_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
