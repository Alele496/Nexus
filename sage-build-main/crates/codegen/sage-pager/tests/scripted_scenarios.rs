...
/// enterprise deploy report: with `SAGE_GOAL=1`, `/goal` must
...
/// explicitly off (`SAGE_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `SAGE_FOLDER_TRUST=1` and a git repo that
...
/// also pins `SAGE_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
