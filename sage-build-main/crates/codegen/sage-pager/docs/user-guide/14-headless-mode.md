...
By default, each `sage -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
sage -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
sage -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
sage -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
sage -p "What's the secret number?" --resume abc123
```
...
sage -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
sage --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  sage -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
RESULT=$(sage -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
sage -p "Format all files" --yolo

# Run tests and fix failures
sage -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| `SAGE_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
...
sage -p "Run the test suite" --yolo
```
...
- **`sage login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`sage login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `sage -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug sage -p "..." 2> debug.log`.
...
Grok stores data in `~/.sage` (override with `SAGE_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.sage` read-only:
...
```bash
export XAI_API_KEY="xai-..."
export SAGE_DISABLE_AUTOUPDATER=1
sage -p "..." --no-auto-update
```
...
| `SAGE_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`SAGE_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `SAGE_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$SAGE_HOME/bin/sage`).
...
- Resume: `sage -p "continue" --resume "<id>"` or `sage -p "continue" --continue`
