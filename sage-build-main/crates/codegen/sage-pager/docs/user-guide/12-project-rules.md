...
| Location | Notes |
|----------|-------|
| `$SAGE_HOME/rules/` (default `~/.sage/rules/`) | Always scanned; applies to all projects |
...
1. **Home rules**: `$SAGE_HOME`, then enabled `~/.claude/` and `~/.cursor/` sources
...
As top-level instruction files, Grok discovers only the recognized filenames listed under [Supported File Names](#supported-file-names) — not custom names such as `AGENTS.local.md` or `notes.md`. (Inside a rules directory such as `.sage/rules/`, every `*.md` file is loaded regardless of name.)
...
| Path | Purpose |
|------|---------|
| `.sage/config.toml` | Project-scoped MCP servers, plugins, and permission rules (other settings load only from `~/.sage/config.toml`) |
...
Use `sage inspect` to see all loaded project instructions:
...
5. **Version control your rules.** Commit AGENTS.md to the repository so the whole team benefits. User-specific overrides belong in `~/.sage/` (global rules).
