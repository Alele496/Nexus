...
| `~/.sage/skills/`, `~/.sage/commands/` | User | Lowest | Personal skills for all projects |
...
Grok deduplicates skills by name -- a higher-priority location overrides a lower one. Grok also scans `.agents/skills/` (and `commands/`) at each tier (alongside `.sage/`) and walks every directory between your working directory and the repo root.
...
Grok scans the Claude and Cursor skill directories by default. To stop scanning a vendor, set its `skills` cell to `false` under `[compat.cursor]` or `[compat.claude]` in `~/.sage/config.toml`, or set the `SAGE_CURSOR_SKILLS_ENABLED` or `SAGE_CLAUDE_SKILLS_ENABLED` environment variable to `false`. See [Configuration](05-configuration.md#harness-compatibility) for details. Grok always filters out known vendor-shipped default skills (such as Cursor's `shell`, `canvas`, and `statusline`), regardless of these settings.

### Additional Skill Directories

Add directories, exclude paths, or disable individual skills via `[skills]` in `~/.sage/config.toml`:
...
3. **Creates the skill directory.** Grok creates the `<scope>/.sage/skills/<name>/` directory, plus `scripts/` or `references/` subdirectories when the skill needs them.
...
- **Project** (`<repo_root>/.sage/skills/<name>/`) -- available only in this repository and shareable with teammates through version control. Grok recommends this scope inside a git repository.
- **User** (`~/.sage/skills/<name>/`) -- available across all your projects.
...
To browse your skills, type `/` to open the slash-command menu. Grok lists every built-in command and skill and filters them as you type. To list skills from the command line instead, run `sage inspect` (see [Viewing Skill Details](#viewing-skill-details)).
...
/user:commit         # The "commit" skill from ~/.sage/skills/
```
...
Run `sage inspect` to see every skill Grok discovers, along with the rest of your configuration:

```bash
sage inspect          # Human-readable summary
sage inspect --json   # Machine-readable report
```
...
Grok ships with built-in skills and extracts them to `~/.sage/skills/` on startup -- among them `/create-skill`, `/help`, and `/check-work`. Bundled skills behave like user skills, and a same-named skill in a higher-priority location (local or repo) overrides the bundled copy; `sage inspect` labels the extracted copies `bundled` so they stay distinguishable from skills you authored yourself. (A plugin skill of the same name does not override it; it stays available under its qualified `plugin:name` form.)

Skills can also come from plugins. When you install a plugin that includes skills, they appear alongside your user and project skills. `sage inspect` labels each plugin-provided skill with its source as `plugin: <name>`.
...
5. **Version-control project skills.** Commit `.sage/skills/` to your repository so the whole team benefits. User skills in `~/.sage/skills/` stay personal and unshared.
