...
//!
//! > After any install attempt, `$BIN_DIR/sage` resolves to a binary that runs,
...
//!
//! The installer lives in the sibling `sage-pager` crate; it is resolved by
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../sage-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
const INSTALLER_BLOCK_START: &str = "# >>> sage installer >>>";
...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".nexus").join("downloads");
    let bin = home.join(".nexus").join("bin");
    std::fs::create_dir_all(&downloads).unwrap();
    std::fs::create_dir_all(&bin).unwrap();
    let prev = downloads.join(format!("sage-{platform}"));
    ...
    std::os::unix::fs::symlink(format!("../downloads/sage-{platform}"), &link).unwrap();
    dunce::canonicalize(&prev).unwrap()
}

/// Re-resolve `$BIN_DIR/sage` from disk and re-run it: the active sage must
/// always execute, and never be a `.tmp`/partial file.
fn assert_active_nexus_runs(home: &Path) {
    let link = home.join(".nexus").join("bin").join("sage");
    assert!(link.is_symlink(), "sage must remain a symlink");
    let resolved =
        dunce::canonicalize(&link).unwrap_or_else(|e| panic!("sage symlink dangles: {e}"));
    let name = resolved.file_name().unwrap().to_string_lossy().to_string();
    assert!(
        !name.contains(".tmp"),
        "active sage must not be a temp file: {name}"
    );
    let ok = Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ok, "active sage must run: {}", resolved.display());
}
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("NEXUS_BIN_DIR", home.join(".nexus").join("bin"))
        .env("NEXUS_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
...
fn assert_single_installer_block(path: &Path, preserved: Option<&str>) {
    ...
    assert_eq!(
        n,
        1,
        "{} must contain exactly one sage installer block, got {n}:\n{body}",
        path.display()
    );
...
fn run_shell_rc_case(case: &ShellRcCase) {
    ...
    assert_active_nexus_runs(&home_path);
}

#[test]
fn install_sh_blitz_keeps_nexus_runnable_under_corruption() {
    ...
    for (mode, expect_ok) in cases {
        ...
        // sage always runs (new good binary on success, previous-good on
        // rejection).
        assert_active_nexus_runs(home.path());
    }
}
