...
/// Sage state directory — always writable (`$NEXUS_HOME` or `~/.nexus`).
pub(crate) fn nexus_home() -> PathBuf {
...
/// Writable directory paths for the read-only profile (minimal: just ~/.nexus + temp).
/// Device files are handled separately via `allow_file` in `to_capability_set_with_config`.
