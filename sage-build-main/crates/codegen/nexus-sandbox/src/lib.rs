...
/// Zero-permission placeholder (file or dir) under `nexus_home` used by bwrap bind-over.
///
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::nexus_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    /// Create a temp workspace whose `.nexus/sandbox.toml` contains `toml_body`.
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        ...
        let args: Vec<String> = cmd
            ...
            .map(|a| a.to_string_lossy().to_string())
            .collect();
        let blocked_dir = paths::nexus_home()
