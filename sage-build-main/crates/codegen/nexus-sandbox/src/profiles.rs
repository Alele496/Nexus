...
//! `off`. Custom profiles via `~/.nexus/sandbox.toml` or `.nexus/sandbox.toml`.
...
/// Load sandbox config from `~/.nexus/sandbox.toml` and `.nexus/sandbox.toml`.
///
...
impl ProfileName {
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // directories like ~/.nexus/ that may not exist on first run.
