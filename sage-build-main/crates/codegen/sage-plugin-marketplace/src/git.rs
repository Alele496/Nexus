...
//! Cache root: `~/.sage/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = sage_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(sage_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    sage_config::sage_home().join("marketplace-cache")
}
...
mod tests {
    ...
    fn cache_hash_is_deterministic() {
        let url = "https://github.com/sage-org/sage-plugin-marketplace.git";
