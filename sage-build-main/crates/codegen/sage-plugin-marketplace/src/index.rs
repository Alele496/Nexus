...
//!
//! Catalog file lookup, in order: `.sage-plugin/marketplace.json` (preferred),
//! `.sage-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.sage-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.sage-plugin/plugin.json`
...
mod tests {
    ...
    #[test]
    fn load_index_sage_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".sage-plugin", "sage"), (".claude-plugin", "claude")] {
