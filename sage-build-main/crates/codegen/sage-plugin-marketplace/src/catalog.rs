...
//! `.sage-plugin/plugin-index.json` (preferred), then
...
use sage_hooks_plugins_types::PluginComponents;
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_sage_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".sage-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_sage_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".sage-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".sage-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
