...
//!    the lookup order — `.sage-plugin/marketplace.json` is preferred), use it.
...
fn scan_single_plugin(plugin_dir: &Path, relative_path: &str) -> MarketplaceEntry {
    ...
    let manifest_result = sage_agent::plugins::manifest::load_manifest(plugin_dir);
    let manifest = match &manifest_result {
        Ok(sage_agent::plugins::manifest::ManifestLoadResult::Found(m)) => Some(m.as_ref()),
        _ => None,
    };
...
mod tests {
    ...
    fn url_sourced_entry_carries_keywords() {
        let dir = tempfile::tempdir().unwrap();
        let sage_dir = dir.path().join(".sage-plugin");
        std::fs::create_dir_all(&sage_dir).unwrap();
        std::fs::write(
            sage_dir.join("marketplace.json"),
            r#"{
                "name": "kw-marketplace",
                "plugins": [{
                    "name": "remote-plugin",
    ...
    #[test]
    fn sage_plugin_dir_index_drives_scan_end_to_end() {
        let dir = tempfile::tempdir().unwrap();
        ...
        std::fs::write(
            ...
            r#"{
                ...
                "plugins": [{
                    "name": "sage-plugin",
                    "description": "From the .sage-plugin index",
                    "category": "design",
    ...
    fn write_sage_file(dir: &Path, file: &str, content: &str) {
        let sage_dir = dir.join(".sage-plugin");
        std::fs::create_dir_all(&sage_dir).unwrap();
        std::fs::write(sage_dir.join(file), content).unwrap();
    }
    ...
    fn url_entry_gets_components_when_sha_matches() {
        ...
        write_sage_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn url_entry_without_pinned_sha_gets_no_components() {
        let dir = tempfile::tempdir().unwrap();
        write_sage_file(dir.path(), "marketplace.json", &url_marketplace_index(""));
        write_sage_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn malformed_catalog_degrades_to_no_components() {
        ...
        write_sage_file(dir.path(), "plugin-index.json", "not json");
