...
pub enum SkillScope {
    /// cwd/.sage/skills
    Local,
    /// repo_root/.sage/skills
    Repo,
    /// ~/.sage/skills
    User,
    /// ~/.sage/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.sage/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.sage/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.sage/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.sage/plugins/infra-plugin",
            "plugin_data": "/root/.sage/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "sage-4",
            "effort": "high",
            "user_invocable": true,
