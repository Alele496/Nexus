...
//!
//! Credentials are stored in `$NEXUS_HOME/mcp_credentials.json`, keyed by a
...
///
/// Local helper (not shell-base): `sage-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$NEXUS_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$NEXUS_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$NEXUS_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user sage home (set $NEXUS_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$NEXUS_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(nexus_config::user_nexus_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$NEXUS_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("sage-mcp-credentials-test");
