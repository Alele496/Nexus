...
const PRODUCTION_ENDPOINTS: SageBuildEndpoints = SageBuildEndpoints {
    cli_chat_proxy_base_url: "https://cli-chat-proxy.sage.local/v1",
    asset_server_url: "https://assets.sage.local",
    relay_ws_url: "wss://code.sage.local/ws/code-agent",
...
pub enum SageBuildEnvironment {
    #[default]
    Production,
}
impl SageBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        SageBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            SageBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, SageBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            SageBuildEnvironment::Production => "SAGE_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> SageBuildEndpoints {
        match self {
            SageBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// `SAGE_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for SageBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SageBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            SageBuildEnvironment::Production.env_prefix(),
            "SAGE_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "XAI_SAGE_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            SageBuildEnvironment::Production.relay_ws_url(),
            SageBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            SageBuildEnvironment::from_flags(false, false),
            SageBuildEnvironment::Production
        );
    }
}
