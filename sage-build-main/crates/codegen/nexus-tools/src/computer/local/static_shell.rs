...
pub use nexus_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__NEXUS_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    nexus_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = nexus_config::shell::detect_unix_shell_kind();
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias nexus_alias_probe='echo ALIAS_OK'\nnexus_fn_probe() { echo FN_OK; }\n",
            "nexus_alias_probe && nexus_fn_probe",
        )
        .await;
