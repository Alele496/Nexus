//! LSP server configuration from `.nexus/lsp.json`.
...
/// This is the canonical merge function — both session startup and `sage inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::nexus_home::nexus_home().join("lsp.json");
    let project_path = cwd.join(".nexus").join("lsp.json");
...
/// Load LSP server configs from `~/.nexus/lsp.json` and `<cwd>/.nexus/lsp.json`.
