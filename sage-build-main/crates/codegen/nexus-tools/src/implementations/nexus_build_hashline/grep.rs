...
use crate::implementations::nexus_build::grep::{GrepSearchInput, GrepTool, OutputMode};
...
impl nexus_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = nexus_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(nexus_tool_runtime::Cwd(cwd));
        let mut result = nexus_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                nexus_tool_runtime::ToolError::execution(
                    nexus_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        assert_eq!(nexus_tool_runtime::Tool::id(&tool).as_str(), "hashline_grep");
