...
impl nexus_tool_runtime::Tool for CodexListDirTool {
    type Args = CodexListDirInput;
    type Output = ListDirOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("list_dir").expect("valid tool id")
    }
    ...
    ) -> Result<ListDirOutput, nexus_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn tool_lists_directory() {
        ...
        let ctx = nexus_tool_runtime::ToolCallContext::default();
    ...
    async fn empty_directory_returns_success() {
        ...
        let output = nexus_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        match output {
            ListDirOutput::Content(content) => {
