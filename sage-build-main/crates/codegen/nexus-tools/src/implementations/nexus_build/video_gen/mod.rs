...
pub use nexus_tools_api::slash_commands::{
...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> nexus_file_utils::s3::S3StaticCredentials {
        nexus_file_utils::s3::S3StaticCredentials {
...
impl VideoGenClient {
    pub fn new(
        ...
        api_key_provider: Option<SharedApiKeyProvider>,
    ) -> Result<Self, nexus_tool_runtime::ToolError> {
        ...
        extra_headers.into_iter().try_for_each(|(key, value)| {
            ...
            Ok::<(), nexus_tool_runtime::ToolError>(())
        })?;
    ...
    pub async fn generate_with_images(
    ...
    ) -> Result<VideoOutcome, nexus_tool_runtime::ToolError> {
        ...
        if !status.is_success() {
            ...
            return Err(nexus_tool_runtime::ToolError::new(
                nexus_tool_runtime::ToolErrorKind::Custom,
                ...
                "done" => {
                    ...
                    return match presigned {
                        ...
                        None if video_url.is_empty() => {
                            Err(nexus_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn download_video(&self, url: &str) -> Result<Vec<u8>, nexus_tool_runtime::ToolError> {
        let response = self.download_http.get(url).send().await.map_err(|e| {
            nexus_tool_runtime::ToolError::invalid_arguments(format!("Failed to download video: {e}"))
        })?;
    ...
    async fn finish_zdr_video(
        ...
        let config = self.zdr_video_output_s3.as_ref().ok_or_else(|| {
            nexus_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, nexus_tool_runtime::ToolError> {
        ...
        let upload_url = nexus_file_utils::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_and_download(
        ...
        urls: &ZdrPresignedUrls,
        request_id: &str,
    ) -> Result<Vec<u8>, nexus_tool_runtime::ToolError> {
        let get_url = self
    ...
    async fn presign_zdr_get_url(
        ...
        let url = nexus_file_utils::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
async fn resolve_image_reference(value: &str) -> Result<String, nexus_tool_runtime::ToolError> {
    ...
    if value.starts_with("data:image/") {
        let comma = value.find(',').ok_or_else(|| {
            nexus_tool_runtime::ToolError::invalid_arguments("malformed data URL in image reference")
        })?;
    ...
    let (_w, _h, mime) =
        crate::util::image_validate::validate_image_bytes(&raw_bytes).map_err(|e| {
            nexus_tool_runtime::ToolError::invalid_arguments(format!("invalid image reference: {e}"))
        })?;
...
fn validate_one_of(
    ...
    Err(nexus_tool_runtime::ToolError::invalid_arguments(format!(
        "`{field}` must be one of: {}. Got {value}.",
        allowed.join(", ")
    )))
}

fn validate_imagine_duration(duration: Option<u32>) -> Result<(), nexus_tool_runtime::ToolError> {
...
) -> Result<(VideoGenClient, std::path::PathBuf), nexus_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    video_bytes: &[u8],
) -> Result<std::path::PathBuf, nexus_tool_runtime::ToolError> {
...
) -> Result<MediaGenOutput, nexus_tool_runtime::ToolError> {
    match outcome {
...
impl nexus_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
impl nexus_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
mod tests {
    ...
    async fn image_to_video_rejects_bad_duration() {
        ...
        let err = nexus_tool_runtime::Tool::run(
            &tool,
