...
//!
//! - **`sage-tools`** — tool blocks on a oneshot, formats the result.
//! - **`sage-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`sage-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `sage-tools`.
...
register_resource!("nexus_build", "UserQuestionSender", UserQuestionSender);
