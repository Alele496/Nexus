...
impl nexus_tool_runtime::ToolOutput for LspToolOutput {}
...
impl nexus_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, nexus_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    nexus_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.nexus/lsp.json or <cwd>/.nexus/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
