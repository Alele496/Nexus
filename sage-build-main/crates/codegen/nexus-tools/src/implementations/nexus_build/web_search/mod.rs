...
impl nexus_tool_runtime::Tool for WebSearchTool {
    type Args = WebSearchInput;
    type Output = WebSearchOutput;

    fn id(&self) -> nexus_tool_protocol::ToolId {
        nexus_tool_protocol::ToolId::new("web_search").expect("valid tool id")
    }
    ...
    ) -> Result<WebSearchOutput, nexus_tool_runtime::ToolError> {
        ...
        let (content, citations) = client
            ...
            .map_err(|e| {
                nexus_tool_runtime::ToolError::execution(
                    nexus_tool_protocol::ToolId::new("web_search").expect("valid"),
                    e.to_string(),
                )
            })?;
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(nexus_tool_runtime::Tool::id(&tool).as_str(), "web_search");
        assert!(
