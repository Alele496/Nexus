...
///
/// `skills` is the standard layout (`.nexus/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Sage sessions.
...
/// name (e.g. `~/.nexus/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Sage.
...
/// `cwd` (exclusive). At each directory, checks for `.nexus/skills/`,
...
/// visits known config roots (`.nexus`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.nexus` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".nexus", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.nexus is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.nexus/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.nexus/ → kept (user content).
        let nexus_shell = tmp.path().join(".nexus").join("skills").join("shell");
        std::fs::create_dir_all(&nexus_shell).unwrap();
        std::fs::write(
            nexus_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (nexus_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.nexus/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .nexus skill in an intermediate dir.
        ...
        .unwrap();
        let nexus_skill = sub.join(".nexus").join("skills").join("sage-dyn");
        std::fs::create_dir_all(&nexus_skill).unwrap();
        std::fs::write(nexus_skill.join("SKILL.md"), "---\nname: sage-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"sage-dyn"),
            "sage-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only sage-dyn discovered.
        ...
        assert!(
            names_off.contains(&"sage-dyn"),
            "sage-dyn missing: {names_off:?}"
        );
        assert!(
