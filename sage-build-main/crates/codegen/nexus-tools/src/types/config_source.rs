...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.nexus/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.nexus/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.nexus/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.nexus/ or ~/.claude/.
    User { path: PathBuf },
