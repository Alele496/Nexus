...
//!
//! Searches `.nexus/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.nexus/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.nexus/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".nexus/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.nexus/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.nexus/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), sage.as_deref())
}
...
fn all_subagents_with_home(
    ...
    home: Option<&Path>,
    nexus_home: Option<&Path>,
) -> Vec<SubagentEntry> {
    let discovered = discover_with_home(cwd, home, nexus_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.nexus/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.nexus/agents/` walking from `cwd` up to repo root
/// 2. `~/.nexus/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.nexus/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user sage agents, `.claude`
/// compat agents, then bundled. `.nexus` dirs resolve from `nexus_home`
/// (NEXUS_HOME-aware) plus the legacy literal `~/.nexus` when NEXUS_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    nexus_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.nexus, included only when it differs from nexus_home
    // (i.e. NEXUS_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_grok = home
        .map(|h| h.join(".nexus"))
        .filter(|legacy| nexus_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = nexus_home {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let sage = nexus_config::user_nexus_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), sage.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, nexus_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let sage = nexus_config::user_nexus_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), sage.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, nexus_home);
...
///
/// Project-level `.nexus/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let sage = nexus_config::user_nexus_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), sage.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, nexus_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, nexus_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the Sage
...
/// Load project agent definitions from every `.nexus/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectSage,
            PluginScope::User => PluginOrigin::UserSage,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_nexus_when_nexus_home_differs() {
        let home = Path::new("/home/u");
        let sage = Path::new("/custom/grokhome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(sage))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&sage.join("agents")));
        assert!(paths.contains(&home.join(".nexus").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&sage.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".nexus").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_nexus_home_is_dot_grok() {
        let home = Path::new("/home/u");
        let sage = home.join(".nexus");
        let count = user_agent_dirs(Some(home), Some(&sage))
            .into_iter()
            .filter(|(p, _)| *p == sage.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.nexus/agents when nexus_home == ~/.nexus"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_nexus_build() {
        let def = by_name("sage-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "sage-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".nexus").join("agents");
        let agents_dir_2 = inner_dir.join(".nexus").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".nexus").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".nexus")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".nexus")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".nexus").join("agents");
        let bundled_dir = home.join(".nexus").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".nexus")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".nexus")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".nexus").join("agents");
        let bundled_dir = home.join(".nexus").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "sage-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "sage-build.md",
            "sage-build",
        ...
        let def = by_name_in_cwd("sage-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "sage-build");
        assert_eq!(def.description, "Custom sage-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .nexus/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("sage-build-orchestrator")
            .expect("from_str must resolve sage-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::SageBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "sage-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("sage-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find sage-build-orchestrator");
        assert_eq!(def.name, "sage-build-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.nexus/agents/) agent named "explore" should NOT shadow
