...
//!
//! Extracted from `sage-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `sage-chat-state::compaction_utils::strip_system_tags` which
...
/// `sage-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use nexus_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(nexus_sampling_types::synthesized_reasoning_item(text))
    }
