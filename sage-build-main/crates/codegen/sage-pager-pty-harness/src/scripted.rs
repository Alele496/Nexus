//! Scripted TUI scenario runner for sage-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$SAGE_HOME`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&sage_home)
                .with_context(|| format!("create scenario SAGE_HOME {}", sage_home.display()))?;
            fs::write(sage_home.join("config.toml"), config_toml)
