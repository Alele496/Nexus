...
pub use sage_test_support::mock_server::LogEntry;
pub use sage_test_support::mock_server::MockModelEntry as MockModel;
pub use sage_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use sage_test_support::sse;
pub use sage_test_support::{
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.sage
    ...
    ///
    /// Mirrors `sage_test_support::env::test_env_cmd_tokio`.
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit SAGE_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // SAGE_HOME is set in the test runner's env).
            ("SAGE_HOME".into(), sage_home),
            ("SAGE_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("SAGE_XAI_API_BASE_URL".into(), self.url()),
            ...
            ("SAGE_PROMPT_SUGGESTIONS".into(), "false".into()),
            // Compatibility set_turns remains request-FIFO, so retries stay off.
            ("SAGE_MAX_RETRIES".into(), "0".into()),
        ]
    }
...
mod tests {
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("SAGE_HOME").as_deref(),
            content.home().join(".sage").to_str()
        );
        assert_eq!(get("SAGE_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("SAGE_XAI_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("XAI_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("SAGE_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("SAGE_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("SAGE_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("SAGE_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("SAGE_MAX_RETRIES").as_deref(), Some("0"));
