...
//! elected leader) plus a fixed leader socket beneath its `SAGE_HOME`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `SAGE_HOME`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared SAGE_HOME => one leader; the socket lives beneath it so
        // every client (sharing the same env) elects/attaches to the same one.
        let sage_home = content.home().join(".sage");
        std::fs::create_dir_all(&sage_home).context("create sage home")?;
        let socket = sage_home.join("leader-e2e.sock");
    ...
    /// The cluster's sessions root: `SAGE_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".sage").join("sessions")
    }
...
mod tests {
    ...
    fn envelope(update_json: &str) -> String {
        format!(
            r#"{{"timestamp":1,"method":"_sage.local/session/update","params":{{"sessionId":"s","update":{update_json}}}}}"#
        )
    }
