...
impl InferenceRequestKind {
    fn classify(headers: &HeaderMap, body: &Value) -> Self {
        if nonempty_header(headers, "x-sage-turn-idx").is_some() {
            return Self::Foreground;
        }
        if nonempty_header(headers, "x-sage-req-id").is_some() {
...
impl InferenceOverrides {
    ...
    pub(crate) fn classify(
        ...
        let fingerprint =
            nonempty_header(headers, "x-sage-req-id").map(|request_id| ModelCallFingerprint {
                endpoint,
                kind,
