...
fn local_nexus_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("sage-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_nexus_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
...
/// Resolve sage binary: `NEXUS_BINARY` env (CI) or a locally built `sage-pager` binary.
pub fn nexus_binary() -> PathBuf {
    if let Ok(path) = std::env::var("NEXUS_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "NEXUS_BINARY does not exist: {}", p.display());
        return p;
    }
    ...
    let binary = local_nexus_binary_path();
    ensure_local_nexus_binary(&binary);
    binary
}
...
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        ...
        // without an explicit NEXUS_HOME every spawned child shares the real
        // `%USERPROFILE%\.nexus` — test 1's models_cache.json (which embeds
        ...
        .env("NEXUS_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("NEXUS_XAI_API_BASE_URL", mock_url)
        ...
        .env("NEXUS_DISABLE_AUTOUPDATER", "1");
}
