...
//! Shared test utilities for sage-build crates: mock inference server, SSE
...
//! - [`GrokStdioClient`] — ACP client that drives `sage agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `sage agent --leader stdio` (unix)
//! - [`run_headless`] — Run `sage -p` against the mock server and capture output
//! - [`git_workdir`] — Create a temp directory with git repo (forces libgit2 init)
//! - [`nexus_binary`] — Resolve the sage binary path (NEXUS_BINARY env or cargo_bin)
...
pub use env::{EnvGuard, git_workdir, nexus_binary};
pub use headless::{
