//! ACP stdio clients for testing sage sessions end-to-end: the typed
...
/// Spawn `sage agent stdio` with the canonical hermetic test env: the sandbox
...
/// kill-list so a test can still set e.g. `NEXUS_DEBUG_LOG=1` explicitly.
...
/// Drives `sage agent stdio` via the ACP protocol over pipes.
///
...
/// Drives `sage agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
