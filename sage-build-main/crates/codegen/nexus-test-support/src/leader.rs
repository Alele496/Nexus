...
use nexus_acp_lib::LineBufferedRead;

use crate::env::nexus_binary;
...
/// (version-skew) test. Falls back to [`nexus_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "NEXUS_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`nexus_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "NEXUS_BINARY_CLIENT";
...
/// (`NEXUS_BINARY_LEADER`, else the shared [`nexus_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`NEXUS_BINARY_CLIENT`,
/// else the shared [`nexus_binary`] resolution).
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&nexus_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export NEXUS_* vars
            // (e.g. NEXUS_LEADER_SOCKET pointing at a REAL leader on this
            ...
            .env("NEXUS_LEADER_SOCKET", home.join(".nexus").join("leader.sock"))
            .env("NEXUS_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("NEXUS_XAI_API_BASE_URL", server.url())
            .env("XAI_API_KEY", "test-key-for-ci")
            .env("NEXUS_TELEMETRY_ENABLED", "false")
            .env("NEXUS_FEEDBACK_ENABLED", "false")
            .env("NEXUS_TRACE_UPLOAD", "false")
            .env("NEXUS_INSTRUMENTATION", "disabled")
            // Inherited by the spawned leader, whose stderr goes to
            // ~/.nexus/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "nexus_shell=debug");
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".nexus").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".nexus").join("leader.log")).unwrap_or_default()
}
