...
//! `sage-workspace`. Wire types live in
//! `nexus_workspace_types::rpc`; this crate adds the connected-state
...
use nexus_computer_hub_sdk::harness::ToolHarness;
use nexus_workspace_types::rpc::agents_md::{AgentConfigFile, DiscoverAgentsMdReq};
use nexus_workspace_types::rpc::code_nav::{
...
use nexus_workspace_types::rpc::fs::{
...
use nexus_workspace_types::rpc::git::{
...
use nexus_workspace_types::rpc::hunks::{
...
use nexus_workspace_types::rpc::search::{
    ContentSearchData, ContentSearchRequest, FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq,
    FuzzyStatusReq,
};
use nexus_workspace_types::rpc::session::{
    BeginPromptReq, EndPromptReq, FileRewindResponse, RewindToReq,
};
use nexus_workspace_types::rpc::skills::{DiscoverPluginsReq, DiscoverSkillsReq, SkillInfo};
...
use nexus_workspace_types::rpc::worktree::{
...
use nexus_workspace_types::rpc::{RpcEnvelope, RpcError, WORKSPACE_RPC_TOOL_ID, WorkspaceRpc};
use nexus_tool_runtime::{ToolCallContext, ToolStreamItem, TypedToolOutput};
...
pub async fn consume_stream_terminal(
    stream: &mut nexus_tool_runtime::ToolStream<TypedToolOutput>,
) -> Result<TypedToolOutput, nexus_tool_runtime::ToolError> {
    loop {
...
/// Check whether a [`ToolError`](nexus_tool_runtime::ToolError) indicates
...
pub fn is_transport_fatal(err: &nexus_tool_runtime::ToolError) -> bool {
    match err.kind {
        nexus_tool_runtime::ToolErrorKind::NetworkError => true,
        nexus_tool_runtime::ToolErrorKind::Custom => err
            .details
            .as_ref()
...
impl WorkspaceClient {
    ...
    pub async fn rpc_raw(
        ...
        let tool_id = nexus_tool_protocol::ToolId::new(WORKSPACE_RPC_TOOL_ID)
...
mod tests {
    ...
    use nexus_computer_hub_sdk::harness::LocalRegistry;
    use nexus_workspace_types::rpc::skills::SkillScope;
    use nexus_tool_protocol::{SessionId, ToolId};
    use nexus_tool_runtime::{Tool, ToolError};
    ...
    impl nexus_tool_runtime::ToolOutput for RawOut {}
    #[derive(Debug)]
    ...
    impl Tool for FakeWorkspaceRpc {
        ...
        fn description(&self, _ctx: &::nexus_tool_runtime::ListToolsContext) -> ToolDescription {
        ...
        async fn run(&self, _ctx: ToolCallContext, args: Self::Args) -> Result<RawOut, ToolError> {
            ...
            match args.method.as_str() {
                ...
                "workspace.discover_skills" => ok(serde_json::json!(
                    ...
                    "path" : "/workspace/.nexus/skills/my-skill/SKILL.md", "scope"
                    : "local", }]
                )),
    ...
    async fn consume_stream_terminal_returns_ok() {
        ...
        let mut stream = nexus_tool_runtime::terminal_only(Ok(typed));
        assert_eq!(
    ...
    async fn consume_stream_terminal_returns_err() {
        let mut stream: nexus_tool_runtime::ToolStream<TypedToolOutput> =
            nexus_tool_runtime::terminal_only(Err(ToolError::network_error("oops")));
    ...
    async fn consume_stream_terminal_exhausted_stream_is_network_error() {
        ...
        let mut stream = nexus_tool_runtime::terminal_only::<TypedToolOutput>(Ok(typed));
