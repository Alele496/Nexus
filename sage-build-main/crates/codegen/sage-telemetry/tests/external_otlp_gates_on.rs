...
use sage_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
fn external_stream_gates_on_end_to_end() {
    ...
    let mut cfg = external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "SAGE_EXTERNAL_OTEL" => Some("1".into()),
    ...
    assert!(!sage_telemetry::is_enabled());

    sage_telemetry::log_event(sage_telemetry::events::SessionHarness {
        session_id: "sess-gates-on".into(),
        client_identifier: Some("sage-pager".into()),
        model_id: "sage-4".into(),
        agent_name: "sage-build-plan".into(),
        permission_mode: sage_telemetry::enums::PermissionMode::Ask,
    ...
    sage_telemetry::log_event(sage_telemetry::events::PromptSubmitted {
        prompt_length: 100,
        model_id: "sage-4".into(),
    ...
    sage_telemetry::log_event(sage_telemetry::events::ModelResponseReceived {
    ...
    sage_telemetry::log_event(sage_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "sage_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.xai.sage_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
    ...
    let prompt = col::find_event(&collected, "sage_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "sage_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "sage_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "sage_code.session.count");
