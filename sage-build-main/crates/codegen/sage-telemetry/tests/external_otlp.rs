...
fn external_stream_end_to_end() {
    ...
    let mut cfg = sage_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
    ...
    cfg.client = sage_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    sage_telemetry::external::init(Some(cfg));
    assert!(sage_telemetry::external::is_active());
    ...
    sage_telemetry::log_event(sage_telemetry::events::SessionNew {
    ...
    for expected in [
        "sage_code.session_start",
        "sage_code.user_prompt",
        "sage_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "sage_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "sage_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "sage_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "sage_code.token.usage"));
    assert_eq!(
    ...
    sage_telemetry::external::shutdown();
    assert!(
