...
//! file logger that writes to `~/.sage/logs/memory.log`.
//!
//! ## When to use
//!
...
//! SAGE_MEMORY_LOG=0 sage                # disable even when enabled
//! tail -f ~/.sage/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "SAGE_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.sage/logs/memory.log`. Filters to `xai_memory=trace`.
    /// Set `SAGE_MEMORY_LOG=0` to disable, `SAGE_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || sage_home().join("logs").join("memory.log");
