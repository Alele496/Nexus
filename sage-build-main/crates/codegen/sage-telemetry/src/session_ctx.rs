...
//!
//! Extracted from `sage-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `sage-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `sage-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "sage-shell-",
            EmitterOrigin::Workspace => "sage-workspace-",
        }
    }
}
...
/// event-name prefix (`sage-shell-*` vs `sage-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "sage-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "sage-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("sage-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("sage-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "sage-workspace-turn");
    }
