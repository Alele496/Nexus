...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "sage_code.session_start",
            Self::SessionEnd => "sage_code.session_end",
            Self::UserPrompt => "sage_code.user_prompt",
            Self::TurnCompleted => "sage_code.turn_completed",
            Self::ApiRequest => "sage_code.api_request",
            Self::ApiError => "sage_code.api_error",
            Self::ToolResult => "sage_code.tool_result",
            Self::ToolDecision => "sage_code.tool_decision",
            Self::McpServerConnection => "sage_code.mcp_server_connection",
            Self::PermissionModeChanged => "sage_code.permission_mode_changed",
            Self::SkillActivated => "sage_code.skill_activated",
            Self::PluginLoaded => "sage_code.plugin_loaded",
            Self::Compaction => "sage_code.compaction",
            Self::Subagent => "sage_code.subagent",
            Self::Auth => "sage_code.auth",
            Self::InternalError => "sage_code.internal_error",
            Self::ModelSwitched => "sage_code.model_switched",
            Self::ContextualTip => "sage_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `sage_code.session.count`.
    SessionCount,
    /// `sage_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `sage_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `sage_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `sage_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `sage_code.error.count`.
    ErrorCount {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "sage_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "sage_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "sage_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "sage_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "sage_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "sage_code.error.count";
...
fn tool_outcome_label(o: &sage_file_utils::events::types::ToolOutcome) -> &'static str {
    use sage_file_utils::events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `sage_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `sage_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `sage_code.session_end`.
...
/// `PromptSubmitted` → `sage_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `sage_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `sage_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `sage_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `sage_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `sage_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `sage_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `sage_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `sage_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `sage_code.permission_mode_changed`.
...
/// `ContextualTip` → `sage_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `sage_code.permission_mode_changed`.
...
/// `SkillDispatched` → `sage_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `sage_code.plugin_loaded`.
...
/// `PluginUsed` → `sage_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `sage_code.compaction`.
...
/// `CompactionTriggered` → `sage_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `sage_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `sage_code.subagent` (`phase=completed`).
...
/// `InternalError` → `sage_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `sage_code.model_switched`.
