...
fn event_names_are_pinned() {
    ...
    let expected: &[(E, &str)] = &[
        (E::SessionStart, "sage_code.session_start"),
        (E::SessionEnd, "sage_code.session_end"),
        (E::UserPrompt, "sage_code.user_prompt"),
        (E::TurnCompleted, "sage_code.turn_completed"),
        (E::ApiRequest, "sage_code.api_request"),
        (E::ApiError, "sage_code.api_error"),
        (E::ToolResult, "sage_code.tool_result"),
        (E::ToolDecision, "sage_code.tool_decision"),
        (E::McpServerConnection, "sage_code.mcp_server_connection"),
        (
            E::PermissionModeChanged,
            "sage_code.permission_mode_changed",
        ),
        (E::SkillActivated, "sage_code.skill_activated"),
        (E::PluginLoaded, "sage_code.plugin_loaded"),
        (E::Compaction, "sage_code.compaction"),
        (E::Subagent, "sage_code.subagent"),
        (E::Auth, "sage_code.auth"),
        (E::InternalError, "sage_code.internal_error"),
        (E::ModelSwitched, "sage_code.model_switched"),
        (E::ContextualTip, "sage_code.contextual_tip"),
    ];
...
fn session_start_snapshot_counts_not_names() {
    ...
    let ev = &events[0];
    assert_eq!(ev.0, "sage_code.session_start");
...
fn session_new_increments_session_count_only() {
    ...
    assert_eq!(names, vec!["sage_code.session.count".to_owned()]);
}
...
fn api_request_snapshot_and_token_usage() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "sage_code.api_request");
...
fn one_failed_turn_increments_error_count_exactly_once() {
    ...
    let names: Vec<String> = exported_events(&stream)
        ...
        .collect();
    assert_eq!(
        names.iter().filter(|n| *n == "sage_code.api_error").count(),
        2
    );
    ...
    let total: u64 = stream
        ...
        .filter(|m| m.name() == "sage_code.error.count")
        .map(|m| match m.data() {
...
fn turn_error_increments_error_count() {
    ...
    names.sort();
    assert_eq!(names, vec!["sage_code.error.count", "sage_code.turn.count"]);
}
...
fn tool_result_gates_off_collapses_and_reduces() {
    ...
    emit_event_into(
        ...
        &events::ToolCallCompleted {
            ...
            outcome: sage_file_utils::events::types::ToolOutcome::Success,
            duration_ms: 42,
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "sage_code.tool_result");
...
fn user_prompt_gates_off_drops_text() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "sage_code.user_prompt");
...
fn mcp_connection_collapses_server_name_by_default() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "sage_code.mcp_server_connection");
...
fn tool_decision_snapshot() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "sage_code.tool_decision");
...
fn contextual_tip_maps_every_tip_and_action() {
    ...
    for (tip, action, tip_label, action_label) in cases {
        ...
        let events = exported_events(&stream);
        let ev = &events[0];
        assert_eq!(ev.0, "sage_code.contextual_tip");
...
fn validating_metric_exporter_drops_export_on_bad_attr_key() {
    ...
    let rogue = meter.u64_counter("sage_code.session.count").build();
    rogue.add(
        1,
...
fn redacting_log_exporter_drops_record_with_closed_gate_key() {
    ...
    record.set_event_name("sage_code.user_prompt");
...
fn redacting_log_exporter_drops_record_with_unknown_key() {
    ...
    let logger = stream.ext.logger.as_ref().unwrap();
    let mut record = logger.create_log_record();
    record.set_event_name("sage_code.api_request");
...
fn metric_increments_pass_model_through_scrub_and_attach_session_id() {
    ...
    let exported = stream.metrics.get_finished_metrics().unwrap();
    let blob = format!("{exported:?}");
    assert!(blob.contains("sage_code.token.usage"));
    assert!(
