...
//! file logger that writes to `~/.sage/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `sage_hooks` or `sage_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! SAGE_HOOKS_LOG=1 sage              # enable, write to ~/.sage/logs/hooks.log
//! SAGE_HOOKS_LOG=/tmp/h.log sage     # write to custom path
//! SAGE_HOOKS_LOG=0 sage              # explicitly disable
//! tail -f ~/.sage/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "SAGE_HOOKS_LOG";
...
///
/// Writes to `~/.sage/logs/hooks.log` (or custom path via `SAGE_HOOKS_LOG`).
/// Filters to hooks (`sage_hooks`) and plugins (`sage_agent::plugins`) targets.
/// Set `SAGE_HOOKS_LOG=0` to disable, `SAGE_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "sage_hooks=debug,sage_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || sage_home().join("logs").join("hooks.log");
