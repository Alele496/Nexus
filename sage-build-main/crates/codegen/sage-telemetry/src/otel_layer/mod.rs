...
const ENV_OTEL_FILTER: &str = "SAGE_OTEL_FILTER";
...
/// `sage_shell::auth::credential_provider::build_otel_credential_provider`.
...
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL (e.g. `https://cli-chat-proxy.sage.local/v1/traces`).
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid SAGE_OTEL_FILTER '{}': {}. Using default '{}'.",
...
fn build_export_headers(
    ...
    snapshot: &sage_auth::CredentialSnapshot,
...
mod tests {
    ...
    use std::sync::Mutex;
    use sage_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
