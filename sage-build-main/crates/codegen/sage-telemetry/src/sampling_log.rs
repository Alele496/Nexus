//! Tracing layer for `target: "sampling_log"` → `~/.sage/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `SAGE_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "SAGE_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = sage_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
