...
use sage_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use sage_pager::client_identity::PAGER_CLIENT_VERSION;
use sage_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use sage_shell::agent::config::Config as AgentConfig;
use sage_shell::leader::{
...
use sage_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.sage_ws_origin {
        config.sage_com_config.sage_ws_origin = v.clone();
    }
    if let Some(v) = &args.sage_ws_url {
        config.sage_com_config.sage_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &sage_pager::app::AgentArgs,
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Sage agent server starting...");
    eprintln!();
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use sage_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(sage_telemetry::sampling_log::layer())
        .with(sage_telemetry::instrumentation::layer())
        .with(sage_telemetry::hooks_log::layer())
        .with(sage_telemetry::otel_layer::build_otel_layer(
            sage_telemetry::otel_layer::OtelClientInfo {
                client_name: "sage-pager",
                client_version: sage_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            sage_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    sage_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    sage_telemetry::external::init(
        sage_shell::agent::config::resolve_external_otel_config(
            sage_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: sage_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `sage setup`: rendering + exit codes only; fetch logic lives in `sage_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use sage_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        ...
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export SAGE_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:SAGE_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  sage setup");
        eprintln!();
        eprintln!("Or add the key to ~/.sage/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's Sage administrator."
        );
        std::process::exit(1);
    }
    if json {
        match managed_config::fetch_setup_report().await {
            Ok(report) => {
                ...
                if !report.configured {
                    eprintln!(
                        "Your team doesn't have a managed configuration yet. A team admin can set one up at console.sage.local."
                    );
                }
            }
            Err(e) => {
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = sage_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !sage_shell::util::is_sage_process(pid) {
        ...
        if let Err(e) = sage_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(sage_shell::env::SageBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, sage_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = sage_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = sage_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&sage_shell::leader::ControlPayload>,
...
const WORKSPACE_COMMAND_ENV: &str = "SAGE_WORKSPACE_COMMAND";
...
/// The `SAGE_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&sage_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
fn fetch_remote_settings() -> Option<sage_shell::util::config::RemoteSettings> {
    join_early_prefetch(sage_shell::agent::models::start_early_prefetch(None))
}
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.sage_com_config.sage_ws_url;
...
async fn workspace_control(
    ...
    let raw_config = sage_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use sage_shell::auth::ensure_authenticated;
    sage_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = sage_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`sage workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.sage/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.sage_com_config,
        false,
        Some("No cached credentials found. Run `sage login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.sage_com_config);
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    sage_telemetry::sentry::flush_on_shutdown();
    sage_telemetry::otel_layer::shutdown_otel();
    sage_telemetry::debug_log::flush();
...
async fn run_agent_command(
    agent_args: Box<sage_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use sage_pager::app::signal_handler::next_signal_code;
    ...
    let _otel_guard = sage_telemetry::otel_layer::otel_guard();
    sage_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => sage_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = sage_shell::agent::models::start_early_prefetch(None);
    sage_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Sage (pager) - v{}",
            sage_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                sage_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(sage_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = sage_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.default_auto_mode = sage_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&sage_shell::agent::config::RuntimeResolutionContext {
    ...
    let managed_install = is_managed_install(
        ...
        &sage_shell::util::sage_home::sage_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = sage_shell::leader::LeaderEnvUrls::from(&agent_config.sage_com_config);
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = sage_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = sage_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            sage_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(sage_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = sage_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = sage_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `SAGE_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
...
/// (`[dashboard].enabled = false` / `SAGE_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !sage_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.sage/config.toml and \
             unsetting SAGE_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("SAGE_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<sage_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(sage_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<sage_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(sage_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    sage_shell::heap_profile::install(sage_shell::heap_profile::HeapProfileHooks {
...
fn main() {
    // First-run setup wizard: must run BEFORE any sage_home() calls so
    // SAGE_HOME is set before the OnceLock in sage-config caches the path.
    if crate::setup_wizard::needs_setup_wizard() {
        if let Err(e) = crate::setup_wizard::run_setup_wizard() {
            eprintln!("Setup wizard error: {e}");
        }
    }
    sage_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    sage_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        sage_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        sage_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = sage_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    sage_pager::memory_trace::start(
        sage_shell::util::sage_home::sage_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = sage_config::validate_requirements() {
        eprintln!("Couldn't start Sage: {e}");
        eprintln!();
        eprintln!(
            "Update Sage to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = sage_telemetry::sentry::init(sage_telemetry::sentry::Config {
        client: "sage-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: sage_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    sage_pager::docs::extract_user_guide_docs(&sage_shell::util::sage_home::sage_home());
    sage_crash_handler::install_terminal_restore_only();
    if sage_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = sage_shell::util::sage_home::sage_home().join("crash");
        if let Some(report) = sage_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("Sage crashed during your last session.");
            eprintln!("  Signal:  {}", report.signal_name);
            eprintln!("  Version: {}", report.app_version);
            eprintln!("  Report:  {}", report.report_path.display());
            eprintln!();
        }
        if !sage_crash_handler::install(sage_crash_handler::CrashHandlerConfig {
    ...
    let crashed = sage_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    sage_telemetry::debug_log::flush();
    if let Err(e) = result {
        sage_tty_utils::restore_native_stderr();
        eprintln!("Error: {e:#}");
        drop(_sentry_guard);
        std::process::exit(1);
    }
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("SAGE_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("SAGE_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(sage_shell::agent::chat_modes::SAGE_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(sage_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("SAGE_DEBUG_LOG", path);
            std::env::remove_var("SAGE_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("SAGE_DEBUG_LOG", "1");
        set_if_unset("SAGE_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        sage_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return sage_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        sage_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        sage_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    sage_shell::config::apply_sandbox(
        None,
    ...
    sage_shell::http::set_client_name(if is_interactive {
        sage_workspace::permission::ClientType::GrokPager
    } else {
        sage_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        sage_update::channel_name().unwrap_or("unknown"), }
                    );
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    println!(
            ...
            Command::Inspect { json } => {
                ...
                sage_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return sage_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = sage_telemetry::otel_layer::otel_guard();
                return sage_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = sage_telemetry::otel_layer::otel_guard();
                let config = sage_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return sage_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return sage_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return sage_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return sage_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return sage_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return sage_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return sage_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                sage_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                sage_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = sage_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                sage_shell::auth::run_cli_logout(&config)?;
                sage_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = sage_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(sage_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == sage_pager::headless::OutputFormat::Plain {
                args.output_format = sage_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return sage_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            sage_pager::headless::HeadlessOptions {
    ...
    let result = sage_pager::app::run(args, bg_update_rx).await;
    sage_sandbox::flush();
    match result {
        Ok(true) => {
...
fn build_update_config() -> UpdateConfig {
    let environment = sage_shell::env::SageBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                sage_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("SAGE_NPM_REGISTRY"))
        .ok()
        .or_else(sage_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = sage_shell::config::load_effective_config_disk_only()
        && let Some(ch) = sage_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("SAGE_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<sage_home>/bin/sage` resolves to, the
...
/// npm shim hardcodes `~/.sage`, so a custom `SAGE_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, sage_home: &std::path::Path) -> bool {
    if sage_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = sage_config::sage_application_in(sage_home);
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in sage_shell::leader::discover_leaders().await {
        if d.classification != sage_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match sage_shell::leader::LeaderClient::connect(
            socket_path,
            "sage-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(sage_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(sage_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            sage_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: sage_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            sage_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!sage_shell::heap_profile::prof_available());
            return;
        }
        assert!(sage_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(sage_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(sage_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        sage_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_sage_target() {
        let home =
        ...
        std::os::unix::fs::symlink(&target, home.join("bin").join("sage")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("sage")),
            &home
        ));
    ...
    #[serial_test::serial(SAGE_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        let mut args = PagerArgs::try_parse_from(["sage", "dashboard"]).unwrap();
        ...
        assert_eq!(
            std::env::var("SAGE_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("SAGE_OPEN_DASHBOARD_AT_STARTUP") };
    }
    ...
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["sage", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(SAGE_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("SAGE_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("SAGE_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("SAGE_WORKSPACE_COMMAND", "off") };
