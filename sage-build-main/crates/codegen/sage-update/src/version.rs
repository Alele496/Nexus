...
use sage_shell::env::SageBuildEnvironment;
...
pub(crate) const CLI_BASE_URL_FALLBACK: &str =
    "https://storage.googleapis.com/sage-build-public-artifacts/cli";
...
///
/// Constructed once from `SageBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `SageBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://cli-chat-proxy.sage.local/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.sage/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (SAGE_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &SageBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: sage_shell::auth::SageComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = sage_home().join("version.json");
...
pub use sage_version::installed as get_installed_sage_version;

/// Version of the managed sage binary currently on disk, read from the
/// `~/.sage/bin/sage` symlink target (`../downloads/sage-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `sage update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.sage/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = sage_shell::util::sage_home::sage_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "sage")
    }
    #[cfg(not(unix))]
    {
        None
...
///
/// Handles the internal layout (`sage-0.1.150-macos-aarch64`, including
/// pre-releases: `sage-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`sage-0.1.150`,
/// `sage-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`sage-latest`, `sage-pager-*` when `bin_prefix` is
/// `sage`) return `None` instead of garbage.
///
...
/// Read the cached stable version from `~/.sage/version.json` (sync, for display).
///
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(sage_version::VERSION, &stable)
    })
}
...
/// `~/.sage/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(sage_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    /// symlink-target file name (`sage-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("sage-0.2.46-darwin-arm64", Some("0.2.46")),
            ("sage-0.1.220-linux-x86_64", Some("0.1.220")),
            ("sage-0.2.5-windows-x86_64.exe", Some("0.2.5")),
            ...
            ("sage-0.1.220-alpha.4-linux-x86_64", Some("0.1.220-alpha.4")),
            ("sage-0.1.220-alpha.4", Some("0.1.220-alpha.4")), // npm layout
            ("sage-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
            ("sage-garbage-darwin-arm64", None),               // unparseable version
            ("sage-0.2.46", Some("0.2.46")),                   // no platform suffix
            ("other-0.2.46-darwin-arm64", None),               // wrong prefix
            ("sage-latest", None),                             // symlink alias, not a version
            ("sage", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "sage").as_deref(),
                *expected,
        ...
        // prefix but not under "sage".
        assert_eq!(
            version_from_versioned_binary_name("sage-pager-0.1.5-darwin-arm64", "sage-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    fn test_update_config_default_channel_is_stable() {
        use sage_shell::env::SageBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&SageBuildEnvironment::Production);
