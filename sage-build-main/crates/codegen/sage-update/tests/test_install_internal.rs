...
//!
//! Wires together a wiremock-mocked GCS bucket + an isolated `SAGE_HOME`
...
//!
//! The function reads `sage_home()` (a process-wide `OnceLock`), so all
//! tests in this binary share a single `SAGE_HOME` and run serially via
//! `#[serial]`.
...
use sage_update::UpdateConfig;
use sage_update::auto_update::{install_internal_from_base, install_internal_from_bases};
...
#[tokio::test]
#[serial]
async fn install_internal_rolls_back_sage_when_agent_swap_fails() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    let sage_target = std::fs::read_link(bin_dir.join("sage")).unwrap();
    assert_eq!(
        sage_target.file_name().unwrap(),
...
#[tokio::test]
#[serial]
async fn install_internal_rollback_removes_absent_prior_sage_link() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    let sage_path = bin_dir.join("sage");
    assert!(
        !sage_path.is_symlink() && !sage_path.exists(),
...
async fn install_internal_cleans_up_stale_pager_symlink() {
    ...
    // update removes the stale symlink from ~/.sage/bin/.
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
...
#[tokio::test]
#[serial]
async fn install_internal_fails_on_sage_binary_404() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = MockServer::start().await;
...
#[tokio::test]
#[serial]
async fn install_internal_creates_sage_home_subdirs_if_missing() {
    let _ = test_home();
    reset_home();
