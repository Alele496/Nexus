...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".sage").join("downloads");
    let bin = home.join(".sage").join("bin");
...
fn assert_active_sage_runs(home: &Path) {
    let link = home.join(".sage").join("bin").join("sage");
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("SAGE_BIN_DIR", home.join(".sage").join("bin"))
        .env("SAGE_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
...
fn run_shell_rc_case(case: &ShellRcCase) {
    ...
    assert_active_sage_runs(&home_path);
}

#[test]
fn install_sh_blitz_keeps_sage_runnable_under_corruption() {
    ...
    for (mode, expect_ok) in cases {
        ...
        // rejection).
        assert_active_sage_runs(home.path());
    }
}
