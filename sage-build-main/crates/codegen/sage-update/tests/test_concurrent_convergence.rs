...
use sage_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use sage_update::version::installed_on_disk_version;

/// Assert the active `~/.sage/bin/sage` resolves to the expected versioned
...
/// Lay down a managed-install layout in the test SAGE_HOME:
...
// installers that actually maintain the managed `~/.sage/bin/sage` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.sage/downloads cleanup), the probe must report None — otherwise
