...
//!
//! `nexus_hooks` pulls in `git2`/`reqwest`/`sage-tools`, too heavy for
...
/// Wire mirror of `nexus_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `nexus_hooks::config::HookSpec`.
///
...
/// `sage-hooks`, so a server-side test (`sage-workspace`'s
...
/// Wire mirror of `nexus_hooks::event::HookEventName`.
///
