//! Wire types for the `sage-workspace` API.
//!
...
//! `sage-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `sage-workspace-types` (instead of `sage-workspace` or
/// `sage-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `nexus_workspace::permission`
