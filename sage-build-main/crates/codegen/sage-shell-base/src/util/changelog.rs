...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$SAGE_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `sage_home()` OnceLock). A seeded `$SAGE_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("SAGE_HOME")
            .map(std::path::PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .unwrap_or_else(crate::util::sage_home::sage_home);
        Self {
    ...
    ///
    /// When `SAGE_CHANGELOG_OFFLINE` is set (PTY / integration tests), skip
    ...
    /// `$SAGE_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`SAGE_HOME` /
    /// `SAGE_CHANGELOG_OFFLINE`), which races across the parallel test
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$SAGE_HOME` even when offline mode was not
...
fn changelog_offline() -> bool {
    std::env::var_os("SAGE_CHANGELOG_OFFLINE").is_some_and(|v| !v.is_empty() && v != "0")
}
...
mod tests {
    ...
    /// `$SAGE_HOME` env so tests never race the parallel harness.
