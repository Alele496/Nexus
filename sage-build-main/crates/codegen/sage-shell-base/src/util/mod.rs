...
pub mod tips;
pub mod uname;
pub use sage_shared::clipboard;
pub use sage_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// True if `pid` is a sage process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_sage_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_accepts_proxy_subpath() {
        assert!(is_cli_chat_proxy_url(
            "https://cli-chat-proxy.sage.local/v1/chat/completions"
        ));
    }
    #[test]
    ...
    fn test_is_cli_chat_proxy_url_rejects_spoofed_hostname() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.sage.local.evil.example/v1"
        ));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_v11_prefix_confusion() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.sage.local/v11/chat/completions"
        ));
    }
    #[test]
    ...
    #[test]
    fn is_sage_process_self_true_impossible_pid_false() {
        assert!(is_sage_process(std::process::id()));
        assert!(!is_sage_process(u32::MAX));
    }
}
