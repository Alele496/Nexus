...
//! `sage_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `SAGE_POOL_*` /
//! `SAGE_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `SAGE_SAMPLER_SHARED_CLIENT=0` falls back to
...
// `OriginClientInfo` is owned by `sage-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `sage-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `sage-telemetry` consumes
// the same type via `sage_sampler::OriginClientInfo`. The shell-specific
// constructors that depended on `ClientType` (a shell-only type) are free
// functions below.
pub use sage_sampler::OriginClientInfo;

/// Construct an [`OriginClientInfo`] from `SAGE_CLIENT_NAME` /
/// `SAGE_CLIENT_VERSION` env vars. Returns `None` when
/// `SAGE_CLIENT_NAME` is unset.
pub fn origin_client_info_from_env() -> Option<OriginClientInfo> {
    std::env::var("SAGE_CLIENT_NAME")
        .ok()
        .map(|product| OriginClientInfo {
            product,
            version: std::env::var("SAGE_CLIENT_VERSION").ok(),
        })
}
...
/// Process-level client identifier (`SAGE_CLIENT_NAME` env var, default `"sage-shell"`).
...
/// (`sage -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-sage-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `sage -p`, `run_headless_inner` for
/// `sage agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn sage_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(sage_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        ...
        assert_eq!(
            ...
            Some(OriginClientInfo {
                product: "sage-desktop".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("sage-desktop/1.2.3 sage-shell/"));
        ...
        assert!(without_version.starts_with("sage-web sage-shell/"));
        assert!(!without_version.starts_with("sage-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        ...
        assert_eq!(ua.render(), "sage-shell/0.1.171 (macos; aarch64)");
    }
}
