...
//! terminal's *native* scrollback (via `sage_ratatui_inline::Terminal::insert_before`,
...
//! seam ([`sage_pager::minimal_hook`]) of function pointers, and the
...
/// subsequent calls are ignored (see [`sage_pager::minimal_hook`]).
pub fn install() {
    sage_pager::minimal_hook::install(sage_pager::minimal_hook::MinimalHooks { draw });
}
