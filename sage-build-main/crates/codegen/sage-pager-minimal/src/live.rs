...
use sage_pager::app::PagerTerminal;
use sage_pager::app::app_view::{ActiveView, AppView};
use sage_pager::minimal_api;
use sage_pager::render::Renderable;
use sage_pager::scrollback::state::ScrollbackState;
use sage_pager::scrollback::wrappers::EntryRenderer;
use sage_pager::theme::Theme;
use sage_pager::views::prompt_widget::PromptStyle;
use sage_pager::views::turn_status;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &sage_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
fn clear_btw_geometry(agent: &mut sage_pager::app::agent_view::AgentView) {
    agent.last_btw_selection_model =
        sage_pager::scrollback::text_selection::ResolvedSelectionModel::default();
...
pub(super) fn prompt_style(
    appearance: &sage_pager::appearance::AppearanceConfig,
) -> PromptStyle {
    PromptStyle {
        focused: true,
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    sage_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        agent.active_pane = sage_pager::app::agent_view::AgentPane::Prompt;
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                sage_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        let raw_btw = if minimal_api::minimal_btw_surface_available(agent) {
            sage_pager::views::btw_overlay::btw_panel_height(
        ...
        if let (Some(btw), Some(btw_area)) = (agent.btw_state.as_ref(), btw_area) {
            ...
            sage_pager::views::btw_overlay::render_btw_panel(
                frame.buffer_mut(),
                btw,
                btw_area,
                tick,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(sage_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a sage_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &sage_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_advance_phase_timer(
    agent: &mut sage_pager::app::agent_view::AgentView,
) -> Option<sage_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &sage_pager::app::agent_view::AgentView,
    activity: &Option<sage_pager::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = sage_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = sage_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use sage_pager::views::context_bar::fmt_tokens;
    ...
    if let (Some(used), Some(total)) = (used, total)
        && total > 0
    {
        let pct = sage_token_estimation::usage_percentage(used, total);
        segs.push((
...
fn minimal_pending_hint(
    pending: &Option<sage_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> sage_pager::app::agent_view::AgentView {
    ...
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use sage_pager::app::agent::AgentState;
        use sage_pager::scrollback::RenderBlock;
        use sage_pager::scrollback::entry::ScrollbackEntry;
        use sage_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &sage_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use sage_pager::acp::tracker::TurnActivity;
        use sage_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        sage_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        ...
        sage_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            sage_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(sage_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &sage_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use sage_pager::app::app_view::PendingAction;
        use sage_pager::input::key::KeyShortcut;
