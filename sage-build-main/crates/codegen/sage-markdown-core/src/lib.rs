//! Headless markdown analysis sharing Sage's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `sage-markdown` so analysis matches what Sage
//! Build actually renders 1:1.
//!
//! After parsing, Sage applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Sage uses to render markdown.
///
...
/// Offset event stream from Sage's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Sage's options; count elements and flag structural issues.
