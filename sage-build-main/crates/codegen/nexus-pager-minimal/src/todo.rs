...
use nexus_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (nexus_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    nexus_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use nexus_shell::tools::{TodoItem, TodoPriority};
