...
//!
//! `sage-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "NEXUS_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `SageComConfig`).
///
/// Built by the binaries (`sage-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `nexus_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-XAI-Token-Auth` header (typically `"sage-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`sage-pager`) -> `client.name`.
...
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL (e.g. `https://cli-chat-proxy.nexus.local/v1/traces`).
...
///
/// - `client_name`: binary name (e.g. `"sage-tui"`, `"sage-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-sage-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("sage-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid NEXUS_OTEL_FILTER '{}': {}. Using default '{}'.",
...
fn build_export_headers(
    ...
    snapshot: &nexus_auth::CredentialSnapshot,
...
fn build_server_provider(client: OtelClientInfo, config: OtelLayerConfig) -> SdkTracerProvider {
    ...
    if config.exporter.enabled {
        ...
        static_headers.insert(
            "x-sage-client-version".to_string(),
...
mod tests {
    ...
    use std::sync::Mutex;
    use nexus_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("sage-cli"),
