...
/// Wire schema version, exported as resource attr `nexus_code.schema.version`.
/// Additive changes (new events/attrs) do not bump it; renames/removals do.
pub const SCHEMA_VERSION: &str = "v1";

/// Meter/logger instrumentation scope name.
pub const SCOPE_NAME: &str = "ai.xai.nexus_code";
...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "nexus_code.session_start",
            Self::SessionEnd => "nexus_code.session_end",
            Self::UserPrompt => "nexus_code.user_prompt",
            Self::TurnCompleted => "nexus_code.turn_completed",
            Self::ApiRequest => "nexus_code.api_request",
            Self::ApiError => "nexus_code.api_error",
            Self::ToolResult => "nexus_code.tool_result",
            Self::ToolDecision => "nexus_code.tool_decision",
            Self::McpServerConnection => "nexus_code.mcp_server_connection",
            Self::PermissionModeChanged => "nexus_code.permission_mode_changed",
            Self::SkillActivated => "nexus_code.skill_activated",
            Self::PluginLoaded => "nexus_code.plugin_loaded",
            Self::Compaction => "nexus_code.compaction",
            Self::Subagent => "nexus_code.subagent",
            Self::Auth => "nexus_code.auth",
            Self::InternalError => "nexus_code.internal_error",
            Self::ModelSwitched => "nexus_code.model_switched",
            Self::ContextualTip => "nexus_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `nexus_code.session.count`.
    SessionCount,
    /// `nexus_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `nexus_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `nexus_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `nexus_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `nexus_code.error.count`.
    ErrorCount {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "nexus_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "nexus_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "nexus_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "nexus_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "nexus_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "nexus_code.error.count";
...
/// Built-in tool names (closed enum surface in `sage-tools`) that pass
...
fn tool_outcome_label(o: &nexus_file_utils::events::types::ToolOutcome) -> &'static str {
    use nexus_file_utils::events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `nexus_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `nexus_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `nexus_code.session_end`.
...
/// `PromptSubmitted` → `nexus_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `nexus_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `nexus_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `nexus_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `nexus_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `nexus_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `nexus_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `nexus_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `nexus_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `nexus_code.permission_mode_changed`.
...
/// `ContextualTip` → `nexus_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `nexus_code.permission_mode_changed`.
...
/// `SkillDispatched` → `nexus_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `nexus_code.plugin_loaded`.
...
/// `PluginUsed` → `nexus_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `nexus_code.compaction`.
...
/// `CompactionTriggered` → `nexus_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `nexus_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `nexus_code.subagent` (`phase=completed`).
...
/// `InternalError` → `nexus_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `nexus_code.model_switched`.
