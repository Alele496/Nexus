...
//!
//! Enterprise customers point the Sage CLI at *their own* OpenTelemetry
//! collector (standard `OTEL_*` env vars + the `NEXUS_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `NEXUS_EXTERNAL_OTEL` unset (or no exporter
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &nexus_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             NEXUS_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }
