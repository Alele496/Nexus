...
//! file logger that writes to `~/.nexus/logs/memory.log`.
//!
//! ## When to use
//!
...
//! NEXUS_MEMORY_LOG=0 sage                # disable even when enabled
//! tail -f ~/.nexus/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "NEXUS_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.nexus/logs/memory.log`. Filters to `xai_memory=trace`.
    /// Set `NEXUS_MEMORY_LOG=0` to disable, `NEXUS_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || nexus_home().join("logs").join("memory.log");
