...
//! `sage-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use nexus_telemetry::client;
use nexus_telemetry::config::{TelemetryConfig, TelemetryMode};
use nexus_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn manual_auth_posts_to_events_endpoint_as_nexus_shell_manual_auth() {
    ...
    nexus_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "sage-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no sage-shell-manual_auth POST received"
        );
