...
fn external_stream_end_to_end() {
    ...
    let mut cfg = nexus_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
    ...
    cfg.client = nexus_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    nexus_telemetry::external::init(Some(cfg));
    assert!(nexus_telemetry::external::is_active());
    ...
    nexus_telemetry::log_event(nexus_telemetry::events::SessionNew {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("sage-cli")),
        "service.name=sage-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "nexus_code.session_start",
        "nexus_code.user_prompt",
        "nexus_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "nexus_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "nexus_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "nexus_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "nexus_code.token.usage"));
    assert_eq!(
    ...
    nexus_telemetry::external::shutdown();
    assert!(
