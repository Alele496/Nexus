...
//!
//! Credentials are stored in `$SAGE_HOME/mcp_credentials.json`, keyed by a
...
/// File name for the credential store inside `$SAGE_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$SAGE_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$SAGE_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user sage home (set $SAGE_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$SAGE_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(sage_config::user_sage_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$SAGE_HOME/mcp_credentials.json`. Existing credential files must keep
