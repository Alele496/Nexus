...
const MCP_OAUTH_CLIENT_NAME: &str = "Sage";
...
//
// Layer 1 (cross-process): filesystem lock at $SAGE_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    sage_config::sage_home().join(format!("mcp_auth_{safe}.lock"))
}
