//! Agent definition types — parsed from `.sage/agents/*.md` files.
...
use sage_tools::implementations::codex;
use sage_tools::implementations::sage_build;
use sage_tools::implementations::sage_build_concise;
use sage_tools::implementations::memory;
use sage_tools::implementations::opencode;
use sage_tools::implementations::search_tool;
use sage_tools::implementations::use_tool;
use sage_tools::registry::types::{ToolConfig, ToolServerConfig};
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&sage_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&sage_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&sage_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&sage_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&sage_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_sage_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_sage_build_toolset() -> ToolServerConfig {
    let mut tools = default_sage_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&sage_build::EnterPlanModeTool).into());
    tools.push((&sage_build::ExitPlanModeTool).into());
    tools.push((&sage_build::AskUserQuestionTool).into());
    tools.push((&sage_build::WebSearchTool).into());
    tools.push((&sage_build::ImageGenTool).into());
    tools.push((&sage_build::ImageToVideoTool).into());
    tools.push((&sage_build::ReferenceToVideoTool).into());
    tools.push((&sage_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&sage_build::LspTool).into());
    ToolServerConfig {
        tools,
...
fn sage_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&sage_build::ReadFileTool).into(),
        (&sage_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&sage_build::ListDirTool).into(),
        (&sage_build::GrepTool).into(),
        (&sage_build::KillTerminalCommandTool).into(),
        (&sage_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("sage-build", workspace_sage_build_toolset()),
        ("sage-build-concise", sage_build_concise_toolset()),
        ("sage-build-plan", sage_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("sage-computer", sage_computer_toolset()),
    ]
}
...
fn default_sage_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&sage_build::ReadFileTool).into(),
            ...
            (&sage_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&sage_build::SchedulerCreateTool).into(),
            (&sage_build::SchedulerDeleteTool).into(),
            (&sage_build::SchedulerListTool).into(),
            (&sage_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&sage_build::UpdateGoalTool).into(),
        ],
        behavior_preset: None,
    }
}
fn sage_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&sage_build_concise::BashConciseTool).into(),
            (&sage_build_concise::ReadFileConciseTool).into(),
            (&sage_build_concise::SearchReplaceConciseTool).into(),
            (&sage_build::ListDirTool).into(),
            (&sage_build::GrepTool).into(),
            kill_task_tool_config(),
            (&sage_build::TodoWriteTool).into(),
...
pub fn sage_build_hashline_toolset(
    hashline_tools: Vec<sage_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<sage_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
...
fn sage_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&sage_build::ReadFileTool).into(),
            ...
            (&use_tool::UseTool).into(),
            (&sage_build::UpdateGoalTool).into(),
            (&sage_build::EnterPlanModeTool).into(),
            (&sage_build::ExitPlanModeTool).into(),
            (&sage_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            (&sage_build::MonitorTool).into(),
            (&sage_build::WebSearchTool).into(),
            (&sage_build::WebFetchTool).into(),
            (&sage_build::ImageGenTool).into(),
            (&sage_build::ImageToVideoTool).into(),
            (&sage_build::ReferenceToVideoTool).into(),
...
///
/// Same as `sage_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn sage_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&sage_build::ReadFileTool).into(),
...
///
/// Same as `default_sage_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn sage_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&sage_build::ReadFileTool).into(),
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::SageBuild => AgentDefinition::default_sage_build(),
            Self::SageBuildConcise => AgentDefinition::sage_build_concise(),
            Self::SageBuildPlan => AgentDefinition::sage_build_plan(),
            Self::SageBuildPlanNoSubagents => AgentDefinition::sage_build_plan_no_subagents(),
            Self::SageBuildAskUser => AgentDefinition::sage_build_ask_user(),
            ...
            Self::SageBuildOrchestrator => AgentDefinition::sage_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .sage/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_sage_build_toolset")]
    ...
    pub capability_mode: Option<sage_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
...
pub enum AgentScope {
    /// .sage/agents/ (project-level, highest priority)
    Project,
    /// ~/.sage/agents/ (user-level)
    User,
    /// ~/.sage/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_sage_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.sage/agent-memory/<name>/`
    User,
    /// `<project>/.sage/agent-memory/<name>/`
    Project,
    /// `<project>/.sage/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: sage_config::sage_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".sage/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".sage/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `sage_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let sage = sage_config::user_sage_home();
        ...
        if path_str.contains(".sage/agents/") || path_str.contains(".sage\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".sage/bundled/agents/")
            || path_str.contains(".sage\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<sage_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["SageBuild:read_file", "SageBuildHashline:hashline_read"],
            [
                "SageBuild:search_replace",
                "SageBuildHashline:hashline_edit",
            ],
            ["SageBuild:grep", "SageBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_sage_build_toolset(),
    ...
    pub fn default_sage_build() -> Self {
        Self::base(
            BuiltinAgentName::SageBuild,
            "Sage agent for software engineering tasks.",
        )
    }
    /// Sage Concise agent definition — concise output format for SFT/RL.
    pub fn sage_build_concise() -> Self {
        Self {
            tool_config: sage_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::SageBuildConcise,
                "Sage agent with concise output format.",
            )
        }
    }
    /// Sage agent with plan mode tools.
    pub fn sage_build_plan() -> Self {
        Self {
            tool_config: sage_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::SageBuildPlan,
                "Sage agent with plan mode support.",
            )
        }
    }
    /// Sage + plan mode WITHOUT subagent tools.
    pub fn sage_build_plan_no_subagents() -> Self {
        Self {
            tool_config: sage_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::SageBuildPlanNoSubagents,
                "Sage agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Sage agent with the `ask_user_question` tool.
    pub fn sage_build_ask_user() -> Self {
        Self {
            tool_config: sage_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::SageBuildAskUser,
                "Sage agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: sage_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: sage_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: sage_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    pub fn sage_build_orchestrator() -> Self {
        Self {
            ...
            ..Self::base(
                ...
                "SageBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_sage_build_toolset();
        }
...
mod tests {
    ...
    fn sage_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&sage_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&sage_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn sage_computer_preset_is_curated_sage_build_subset() {
        ...
        let exclusive_ids = sage_computer_exclusive_ids();
    ...
    #[test]
    fn sage_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("sage-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&sage_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&sage_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&sage_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&sage_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&sage_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&sage_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&sage_build::KillTerminalCommandTool).id
            {
    ...
    #[test]
    fn sage_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("sage-computer").unwrap();
    ...
    #[test]
    fn sage_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("sage-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&sage_build::LspTool).id,
            ToolConfig::from(&sage_build::EnterPlanModeTool).id,
            ToolConfig::from(&sage_build::ExitPlanModeTool).id,
        ] {
            assert!(
        ...
        let full = workspace_sage_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_sage_build_toolset must ship `{present}`"
            );
        }
    }
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.sage/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.sage/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_sage_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have sage_build tools"
        );
    }
    #[test]
    ...
    fn test_from_json_has_default_toolset_with_task_tool() {
        ...
        let def = AgentDefinition::from_json(&json).unwrap();
        let task_tool_id = "SageBuild:task";
        assert!(
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_sage_build();
