...
//!
//! Plugins from project directories (`.sage/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.sage/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.sage/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.sage/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_sage_home() so a project's `.sage/trusted-plugins` is never
        // read as the user trust store when neither SAGE_HOME nor a home dir resolves.
        let Some(sage) = sage_config::user_sage_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = sage.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.sage/trusted-plugins`.
    ...
    /// rewrites `~/.sage/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.sage/trusted-plugins`.
