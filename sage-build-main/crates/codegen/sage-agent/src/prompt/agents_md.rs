...
//!
//! Searches from cwd to repo root, plus `~/.sage/`. Also discovers
//! `*.md` files in rules directories: vendor-prefixed `.sage/rules/`,
...
//! (`~/.sage/rules/`, `~/.claude/rules/`, `~/.cursor/rules/`).
...
/// Find `*.md` files in `.sage/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.sage/, git repo root, and session cwd.
...
async fn read_agents_config_with_roots(
    ...
    add_discovery_root(&mut home_roots, sage_home, true, HOME_RULES_DIRS);
    ...
    candidates
        ...
        .filter_map(|candidate| {
            ...
            let content = if candidate.is_rule {
                sage_tools::implementations::skills::skill::extract_skill_body(&content)
            } else {
                content
            };
...
mod tests {
    ...
    async fn home_and_project_rules_have_stable_order_without_doubled_paths() {
        ...
        for path in [
            sage_home.join(".sage/rules/doubled.md"),
    ...
    async fn vendor_home_agents_and_rules_cells_are_independent() {
        let tmp = tempfile::tempdir().unwrap();
        let sage_home = tmp.path().join("sage-home");
        let home = tmp.path().join("home");
        let cwd = tmp.path().join("project");
        fs::create_dir_all(&sage_home).unwrap();
    ...
    #[tokio::test]
    async fn nested_sage_home_keeps_project_role_in_repo_order() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        let nested = repo.join("nested");
        fs::create_dir_all(nested.join("rules")).unwrap();
        fs::create_dir_all(nested.join(".sage/rules")).unwrap();
        init_git_repo(&repo);
        ...
        fs::write(nested.join(".sage/rules/project.md"), "nested-project-rule").unwrap();
    ...
    #[tokio::test]
    async fn overlapping_sage_home_and_project_root_merges_roles() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        ...
        fs::write(repo.join(".sage/rules/project.md"), "project-sage-rule").unwrap();
        fs::write(repo.join(".claude/rules/project.md"), "project-claude-rule").unwrap();
        fs::create_dir_all(repo.join(".sage/.sage/rules")).unwrap();
        fs::write(repo.join(".sage/.sage/rules/doubled.md"), "doubled").unwrap();
    ...
    async fn rule_frontmatter_is_stripped_but_named_frontmatter_is_preserved() {
        ...
        for (path, body) in [
            (sage_home.join("rules/global.md"), "custom-home-body"),
            (home.join(".claude/rules/global.md"), "claude-body"),
            (home.join(".cursor/rules/global.md"), "cursor-body"),
            (repo.join(".sage/rules/project.md"), "sage-project-body"),
