...
use sage_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
/// One discovered rule file (AGENTS.md / Claude.md / .sage/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `sage_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.sage/, ~/.claude/).
    ...
    /// (`~/.sage/projects/<encoded-cwd>/mcps`). Surfaced in
