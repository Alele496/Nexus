use crate::{
    ...
    implementations::{
        codex, sage_build, sage_build_concise, sage_build_hashline, opencode,
...
/// this crate: instead of `sage-tools` referencing an out-of-tree tool
/// pack, the pack calls
...
impl ToolConfig {
    ...
    /// `sage_tool_runtime::Tool::id()`. Use this for built-in tools known
    ...
    where
        T: crate::types::tool_metadata::ToolMetadata + sage_tool_runtime::Tool + Default,
    {
        Self::from(&T::default())
    }
...
impl<T: crate::types::tool_metadata::ToolMetadata + sage_tool_runtime::Tool> From<&T>
    for ToolConfig
{
    fn from(tool: &T) -> Self {
        Self {
            id: format!(
                "{}:{}",
                tool.tool_namespace(),
                sage_tool_runtime::Tool::id(tool).as_str()
            ),
            params: None,
            name_override: None,
            params_name_overrides: None,
...
pub struct SessionContext {
    ...
    pub parent_scheduler_handle:
        Option<crate::implementations::sage_build::scheduler::types::SchedulerHandle>,
    ...
    pub web_fetch_config: crate::implementations::sage_build::web_fetch::WebFetchConfig,
    ...
    /// call the Sage Imagine API. When `Disabled` (default), the tool is not
    /// registered and image generation is unavailable.
    pub image_gen_config: crate::implementations::sage_build::image_gen::ImageGenConfig,
    ...
    /// call the Sage Video Generation API. When `Disabled` (default), the tool is not
    /// registered and video generation is unavailable.
    pub video_gen_config: crate::implementations::sage_build::video_gen::VideoGenConfig,
    ...
    pub app_builder_deployer_config:
        crate::implementations::sage_build::deploy_app::AppBuilderDeployerConfig,
    ...
    /// Auth provider which returns a sage_computer_hub_sdk::AuthCredential. Can be used by
    ...
    pub auth_provider: Option<sage_computer_hub_sdk::SharedAuthProvider>,
...
pub async fn drain_value_stream(
    mut stream: sage_tool_runtime::ToolStream<sage_tool_runtime::TypedToolOutput>,
) -> Result<serde_json::Value, sage_tool_runtime::ToolError> {
    use futures::StreamExt;
    while let Some(item) = stream.next().await {
        match item {
            sage_tool_runtime::ToolStreamItem::Progress(_) => continue,
            sage_tool_runtime::ToolStreamItem::Terminal(result) => {
...
fn stream_no_terminal_error() -> sage_tool_runtime::ToolError {
    sage_tool_runtime::ToolError::custom(
        "stream_no_terminal",
...
struct DispatchParts {
    ...
    lr_handle: Arc<dyn sage_computer_hub_core::ToolHandle>,
...
struct ToolEntry {
    ...
    parse_input: Box<
        dyn Fn(serde_json::Value) -> Result<ToolInput, sage_tool_runtime::ToolError> + Send + Sync,
    >,
    /// Registers this tool into a `LocalRegistry` using the concrete type.
    /// Captured at `register::<T>()` time when T is known.
    register_in_local: Box<dyn Fn(&sage_computer_hub_sdk::LocalRegistry) + Send + Sync>,
}
...
pub struct FinalizedToolset {
    ...
    local_registry: sage_computer_hub_sdk::LocalRegistry,
    ...
    workspace_viewer_ctx: Option<sage_tool_runtime::WorkspaceViewerContext>,
}
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub struct ToolRegistryBuilder {
    ...
    shared_local_registry: Option<sage_computer_hub_sdk::LocalRegistry>,
}
...
impl ToolRegistryBuilder {
            ...
            + 'static,
    {
        ...
        let id = sage_tool_runtime::Tool::id(&tool).as_str().to_string();
        let kind = tool.kind();
        ...
        self.tools.insert(
            ...
            ToolEntry {
                ...
                register_in_local: Box::new(|lr: &sage_computer_hub_sdk::LocalRegistry| {
    ...
    /// Fully-qualified tool id (`"SageBuild:read_file"`) → declared
    ...
    pub fn new() -> Self {
        ...
        b.register_with_params::<sage_build::BashTool, sage_build::bash::BashParams>();
        b.register_with_params::<sage_build::ReadFileTool, sage_build::read_file::ReadFileParams>();
        b.register_with_params::<
                sage_build::SearchReplaceTool,
                sage_build::search_replace::SearchReplaceParams,
            >();
        b.register_with_params::<sage_build::ListDirTool, sage_build::list_dir::ListDirParams>();
        b.register_with_params::<sage_build::GrepTool, sage_build::grep::GrepParams>();
        b.register::<sage_build::KillTaskTool>();
        b.register::<sage_build::KillTerminalCommandTool>();
        b.register::<sage_build::TodoWriteTool>();
        b.register::<sage_build::UpdateGoalTool>();
        b.register::<sage_build::TaskOutputTool>();
        b.register::<sage_build::GetTerminalCommandOutputTool>();
        b.register::<sage_build::WaitTasksTool>();
        b.register::<sage_build::TaskTool>();
        b.register::<sage_build::WebSearchTool>();
        b.register_with_params::<sage_build::WebFetchTool, sage_build::web_fetch::WebFetchParams>();
        b.register::<sage_build::LspTool>();
        b.register::<sage_build::ImageGenTool>();
        b.register::<sage_build::ImageEditTool>();
        b.register::<sage_build::ImageToVideoTool>();
        b.register::<sage_build::ReferenceToVideoTool>();
        b.register::<sage_build::EnterPlanModeTool>();
        b.register::<sage_build::ExitPlanModeTool>();
        b.register_with_params::<
                sage_build::AskUserQuestionTool,
                sage_build::ask_user_question::AskUserQuestionParams,
            >();
        b.register::<sage_build::MonitorTool>();
        b.register::<sage_build::SchedulerCreateTool>();
        b.register::<sage_build::SchedulerDeleteTool>();
        b.register::<sage_build::SchedulerListTool>();
        ...
        b.register_with_params::<
            ...
            >();
        b.register_with_params::<
                sage_build_concise::ReadFileConciseTool,
                sage_build::read_file::ReadFileParams,
            >();
        b.register_with_params::<
                sage_build_concise::SearchReplaceConciseTool,
                sage_build::search_replace::SearchReplaceParams,
            >();
        b.register_with_params::<
                sage_build_concise::BashConciseTool,
                sage_build::bash::BashParams,
            >();
        b.register_with_params::<
                sage_build_hashline::HashlineReadTool,
                sage_build_hashline::config::HashlineSchemeParams,
            >();
        b.register_with_params::<
                sage_build_hashline::HashlineEditTool,
                sage_build_hashline::config::HashlineSchemeParams,
            >();
        b.register_with_params::<
                sage_build_hashline::HashlineGrepTool,
                sage_build_hashline::config::HashlineSchemeParams,
            >();
    ...
    pub fn with_local_registry(mut self, registry: sage_computer_hub_sdk::LocalRegistry) -> Self {
    ...
    pub fn validate_config(&self, config: &ToolServerConfig) -> Vec<RequirementError> {
        ...
        }
        {
            let standard_file_ids: &[&str] = &[
                "SageBuild:read_file",
                "SageBuild:search_replace",
                "SageBuild:grep",
            ];
            let hashline_file_ids: &[&str] = &[
                "SageBuildHashline:hashline_read",
                "SageBuildHashline:hashline_edit",
                "SageBuildHashline:hashline_grep",
            ];
    ...
    pub fn finalize_with_trunc_config(
        ...
        if ctx.image_gen_config.has_credentials() {
            match crate::implementations::sage_build::image_gen::ImageGenClient::new(
        ...
        if ctx.video_gen_config.is_enabled() {
            match crate::implementations::sage_build::video_gen::VideoGenClient::new(
        ...
        if let crate::implementations::sage_build::web_fetch::WebFetchConfig::Enabled { params } =
            &ctx.web_fetch_config
        {
            match crate::implementations::sage_build::web_fetch::WebFetchClient::new(params) {
                Ok(client) => {
                    resources.insert(client);
                }
                Err(e) => {
        ...
        let concise_ns = crate::types::tool::ToolNamespace::SageBuildConcise.to_string();
        ...
        resources.register_state::<crate::implementations::sage_build::todo::TodoState>();
        ...
        resources
            ...
            >();
        resources
            .register_state::<crate::implementations::sage_build::scheduler::types::SchedulerState>(
            );
        ...
        let (scheduler_cmd_rx, scheduler_cancel_token) =
            if let Some(parent_handle) = ctx.parent_scheduler_handle {
                ...
                resources.insert(
                    crate::implementations::sage_build::scheduler::types::SchedulerHandle(
                        scheduler_cmd_tx,
                    ),
                );
        ...
        if let (Some(cmd_rx), Some(cancel_token)) = (scheduler_cmd_rx, &scheduler_cancel_token) {
            let actor = crate::implementations::sage_build::scheduler::actor::SchedulerActor {
...
///
/// Implements the canonical `sage_tool_runtime::ToolDispatch` trait so the
...
#[async_trait::async_trait]
impl sage_tool_runtime::ToolDispatch for InnerDispatchForToolset {
    async fn call(
        &self,
        ...
        let result = self
            ...
            .and_then(|output| {
                let value = serde_json::to_value(&output).map_err(|e| {
                    sage_tool_runtime::ToolError::custom("output_encoding", e.to_string())
                })?;
                Ok(sage_tool_runtime::TypedToolOutput::from_value(
                    tool_id.clone(),
                    value,
                ))
            });
        sage_tool_runtime::terminal_only(result)
    }
}
impl FinalizedToolset {
    ...
    pub fn empty_for_test() -> Self {
        Self {
            ...
            local_registry: sage_computer_hub_sdk::LocalRegistry::new(),
    ...
    pub fn local_registry(&self) -> &sage_computer_hub_sdk::LocalRegistry {
        &self.local_registry
    }
    ...
    fn tool_not_found_error(tool_name: &str) -> sage_tool_runtime::ToolError {
        let tid = sage_tool_protocol::ToolId::new(tool_name)
            .unwrap_or_else(|_| sage_tool_protocol::ToolId::new("unknown").expect("valid"));
        sage_tool_runtime::ToolError::not_found(tid, format!("Tool not found: {tool_name}"))
    }
    pub async fn try_parse(
        &self,
        tool_name: &str,
        tool_params: &serde_json::Value,
    ) -> Result<ToolInput, sage_tool_runtime::ToolError> {
    ...
    async fn call_raw(
        ...
        parent_ctx: sage_tool_runtime::ToolCallContext,
    ) -> Result<crate::types::output::ToolOutput, sage_tool_runtime::ToolError> {
        ...
        let mut ctx = sage_tool_runtime::ToolCallContext::new(parent_ctx.call_id.clone());
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cwd) = parent_ctx.extensions.get::<sage_tool_runtime::Cwd>() {
            ctx.extensions.insert((*cwd).clone());
        }
        let tool_id = sage_tool_protocol::ToolId::new(&registry_id)
            .unwrap_or_else(|_| sage_tool_protocol::ToolId::new("unknown").expect("valid"));
        let lr_handle = self.local_registry.find(&tool_id).ok_or_else(|| {
            sage_tool_runtime::ToolError::not_found(
                tool_id,
        ...
        (output_converter)(value)
            .map_err(|e| sage_tool_runtime::ToolError::custom("output_decoding", e.to_string()))
    }
    ...
    ) -> Result<ToolRunResult, sage_tool_runtime::ToolError> {
        use futures::StreamExt;
        let mut stream = self.call_streaming(tool_name, tool_args, tool_call_id, cwd_override);
        while let Some(item) = stream.next().await {
            match item {
                sage_tool_runtime::ToolStreamItem::Progress(_) => continue,
                sage_tool_runtime::ToolStreamItem::Terminal(result) => return result,
            }
        }
        Err(stream_no_terminal_error())
    }
    ...
    ///
    /// [`ToolStream`]: sage_tool_runtime::ToolStream
    /// [`ToolStreamItem::Progress`]: sage_tool_runtime::ToolStreamItem::Progress
    pub fn call_streaming(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> sage_tool_runtime::ToolStream<ToolRunResult> {
        use futures::StreamExt;
        ...
        Box::pin(async_stream::stream! {
            ...
            sage_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; } }; let
            ...
            match item { sage_tool_runtime::ToolStreamItem::Progress(p) => { yield
            sage_tool_runtime::ToolStreamItem::Progress(p); }
            sage_tool_runtime::ToolStreamItem::Terminal(Err(e)) => { yield
            sage_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; }
            sage_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => { let run_result
            = this.finalize_output(typed.value, & output_converter,
            effective_tool_name). await; yield
            sage_tool_runtime::ToolStreamItem::Terminal(run_result); return; } } }
            yield
            sage_tool_runtime::ToolStreamItem::Terminal(Err(stream_no_terminal_error()));
        })
    }
    ...
    fn prepare_dispatch(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> Result<DispatchParts, sage_tool_runtime::ToolError> {
        let (registry_id, output_converter, reverse_params) = {
            let tools = self.tools.read();
            let entry = tools
                .iter()
        ...
        let rt_call_id = sage_tool_protocol::ToolCallId::new(tool_call_id)
            .unwrap_or_else(|_| sage_tool_protocol::ToolCallId::new_v7());
        let mut ctx = sage_tool_runtime::ToolCallContext::new(rt_call_id);
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cwd) = cwd_override {
            ctx.extensions.insert(sage_tool_runtime::Cwd(cwd));
        }
        if let Some(ref version) = contract_version {
            ctx.extensions
                .insert(sage_tool_runtime::BehaviorVersion(version.clone()));
        }
    ...
    ///
    /// The tool must implement `sage_tool_runtime::Tool + ToolMetadata`.
    ...
    ) -> Result<(), sage_tool_runtime::ToolError>
    where
        T: sage_tool_runtime::Tool + ToolMetadata + std::fmt::Debug + Send + Sync + 'static,
        T::Output: serde::Serialize,
    {
        let mut tools = self.tools.write();
        if tools.iter().any(|t| t.client_name == name) {
            return Err(sage_tool_runtime::ToolError::invalid_arguments(format!(
                "Tool already registered: {name}"
            )));
        }
        let description = tool.description_template().to_string();
        let kind = tool.kind();
        let registry_id = sage_tool_runtime::Tool::id(&tool).as_str().to_owned();
    ...
    pub fn unregister_tools_by_prefix(&self, prefix: &str) -> usize {
        ...
        let to_remove: Vec<_> = tools
            ...
            .filter_map(|t| sage_tool_protocol::ToolId::new(&t.registry_id).ok())
            .collect();
    ...
    pub fn unregister_tool_by_name(&self, name: &str) -> bool {
        ...
        let tool_id = tools
            ...
            .and_then(|t| sage_tool_protocol::ToolId::new(&t.registry_id).ok());
        let before = tools.len();
...
fn explain_requirement_failure(
    ...
    match fq_tool_id.as_str() {
        ...
        "SageBuild:get_task_output" => {
            let has_sage_build_bash = has_tool_with_bool_param(
                proposed,
                "SageBuild",
                "run_terminal_cmd",
                "enabled_background",
            ...
            let has_sage_build_concise_bash = has_tool_with_bool_param(
                proposed,
                "SageBuildConcise",
                "run_terminal_cmd",
                "enabled_background",
            ...
            let has_task = has_tool(proposed, "SageBuild", "task");
            let mut notes = vec![];
            if has_tool(proposed, "SageBuild", "run_terminal_cmd")
                && !has_sage_build_bash
            {
                notes
                    .push(
                        "SageBuild:run_terminal_cmd is present but enabled_background=false",
                    );
            }
            if has_tool(proposed, "SageBuildConcise", "run_terminal_cmd")
                && !has_sage_build_concise_bash
            {
                notes
                    .push(
                        "SageBuildConcise:run_terminal_cmd is present but enabled_background=false",
                    );
            }
            let mut message = "get_task_output requires a background-capable bash tool (SageBuild:run_terminal_cmd or SageBuildConcise:run_terminal_cmd with enabled_background=true), OpenCode:bash, or SageBuild:task"
                .to_string();
            let has_provider = has_sage_build_bash || has_sage_build_concise_bash
        ...
        "SageBuild:search_replace" if !params
...
mod tests {
    ...
    fn test_session_context(tmp: &TempDir) -> SessionContext {
        SessionContext {
            ...
            web_fetch_config:
                crate::implementations::sage_build::web_fetch::WebFetchConfig::default(),
            lsp: None,
            image_gen_config:
                crate::implementations::sage_build::image_gen::ImageGenConfig::default(),
            video_gen_config:
                crate::implementations::sage_build::video_gen::VideoGenConfig::default(),
            app_builder_deployer_config:
                crate::implementations::sage_build::deploy_app::AppBuilderDeployerConfig::default(),
    ...
    /// appeared before `sage_build:search_replace` (`ToolKind::Edit`, input:
    ...
    async fn kind_params_merged_across_multiple_tools_of_same_kind() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "SageBuild:search_replace".to_string(),
                    params: Some(
                        serde_json::json!({
    ...
    async fn search_replace_description_renders_minimum_anchor_guidance() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "SageBuild:read_file".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
    ...
    /// Smoke test: finalize the full SageBuild toolset and verify every
    ...
    async fn full_toolset_descriptions_render_cleanly() {
        use crate::implementations::sage_build::{
        ...
        let config = ToolServerConfig {
            ...
            .into_iter()
            .map(|id| ToolConfig::from_id(format!("SageBuild:{id}")))
    ...
    /// name: `run_terminal_cmd` (sage).
    #[tokio::test]
    async fn tool_name_for_kind_resolves_execute() {
        use crate::types::tool::ToolKind;
        let tmp = TempDir::new().unwrap();
        let sage = ToolRegistryBuilder::new()
            .finalize(
                ToolServerConfig {
                    tools: vec![
                        ToolConfig::from_id("SageBuild:run_terminal_cmd".to_string()),
                        ToolConfig::from_id("SageBuild:get_task_output".to_string()),
                        ToolConfig::from_id("SageBuild:kill_task".to_string()),
                    ],
                    behavior_preset: None,
                },
                test_session_context(&tmp),
            )
            .expect("sage toolset should finalize");
        assert_eq!(
            sage.tool_name_for_kind(ToolKind::Execute).as_deref(),
    ...
    fn tool_config_kind_sinks_unknown_to_other() {
        ...
        let known = parse(serde_json::json!({ "id" : "SageBuild:read_file", "kind" : "read" }));
        assert_eq!(known.kind, Some(ToolKind::Read));
        let typo = parse(serde_json::json!({ "id" : "SageBuild:read_file", "kind" : "raed" }));
        assert_eq!(typo.kind, Some(ToolKind::Other));
        let absent = parse(serde_json::json!({ "id" : "SageBuild:read_file" }));
    ...
    /// Verify SageBuildConcise tools can be finalized and produce concise output.
    #[tokio::test]
    async fn test_concise_namespace_tools() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "SageBuildConcise:read_file".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
                ...
                ToolConfig {
                    id: "SageBuildConcise:search_replace".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
                ...
                ToolConfig {
                    id: "SageBuildConcise:run_terminal_cmd".to_string(),
                    params: Some(
                ...
                ToolConfig::for_tool::<sage_build::GrepTool>(),
                ToolConfig::for_tool::<sage_build::KillTaskTool>(),
                ToolConfig::for_tool::<sage_build::TaskOutputTool>(),
                ToolConfig {
                    id: "SageBuild:list_dir".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
    ...
    fn has_tool_id_knows_pinned_tool_config_ids() {
        let builder = ToolRegistryBuilder::new();
        for id in [
            "SageBuild:run_terminal_cmd",
            "SageBuild:read_file",
            "SageBuild:search_replace",
            "SageBuild:list_dir",
            "SageBuild:grep",
            "SageBuild:get_terminal_command_output",
            "SageBuild:kill_terminal_command",
        ] {
            assert!(
        ...
        assert!(
            !builder.has_tool_id("SageBuild:does_not_exist"),
    ...
    fn known_tool_kinds_maps_pinned_tool_config_ids() {
        ...
        for (id, expected) in [
            ("SageBuild:run_terminal_cmd", ToolKind::Execute),
            ("SageBuild:read_file", ToolKind::Read),
            ("SageBuild:search_replace", ToolKind::Edit),
            ("SageBuild:grep", ToolKind::Search),
            ("SageBuild:list_dir", ToolKind::List),
        ] {
            assert_eq!(
                kinds.get(id),
                Some(&expected),
        ...
        assert!(
            !kinds.contains_key("SageBuild:does_not_exist"),
    ...
    /// (e.g. `"read_file"`). If both `SageBuild:read_file` and
    ...
    fn validate_config_reports_param_field_path_and_value() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "SageBuild:run_terminal_cmd".to_string(),
                params: Some(
        ...
        let error = &errors[0];
        assert_eq!(error.tool, "SageBuild:run_terminal_cmd");
        assert_eq!(
    ...
    fn validate_config_reports_semantic_hashline_param_error() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "SageBuildHashline:hashline_read".to_string(),
                params: Some(
    ...
    impl sage_tool_runtime::Tool for FakeMcpTool {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> sage_tool_protocol::ToolId {
            sage_tool_protocol::ToolId::new("fake_mcp").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::sage_tool_runtime::ListToolsContext,
        ) -> sage_tool_types::ToolDescription {
            sage_tool_types::ToolDescription::new("fake_mcp", &self.description)
        }
        async fn run(
            &self,
            _ctx: sage_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> Result<String, sage_tool_runtime::ToolError> {
            Ok("ok".into())
        }
    }
    #[tokio::test]
    ...
    impl sage_tool_runtime::Tool for NonStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> sage_tool_protocol::ToolId {
            sage_tool_protocol::ToolId::new("non_streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::sage_tool_runtime::ListToolsContext,
        ) -> sage_tool_types::ToolDescription {
            sage_tool_types::ToolDescription::new("non_streaming_stub", "non-streaming stub")
        }
        async fn run(
            &self,
            _ctx: sage_tool_runtime::ToolCallContext,
    ...
    impl sage_tool_runtime::Tool for StreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> sage_tool_protocol::ToolId {
            sage_tool_protocol::ToolId::new("streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::sage_tool_runtime::ListToolsContext,
        ) -> sage_tool_types::ToolDescription {
            sage_tool_types::ToolDescription::new("streaming_stub", "streaming stub")
        }
        async fn run(
            &self,
            _ctx: sage_tool_runtime::ToolCallContext,
        ...
        async fn execute(
            ...
            _ctx: sage_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> sage_tool_runtime::ToolStream<String> {
            Box::pin(futures::stream::iter(vec![
                sage_tool_runtime::ToolStreamItem::Progress(sage_tool_runtime::ToolProgress::Text {
                    text: "progress-1".into(),
                }),
                sage_tool_runtime::ToolStreamItem::Terminal(Ok("terminal-value".to_string())),
            ]))
        }
    }
    ...
    async fn call_non_streaming_stub_back_compat() {
        ...
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![ToolConfig::for_tool::<sage_build::ReadFileTool>()],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
        let toolset = Arc::new(builder.finalize(config, ctx).unwrap());
        ...
        while let Some(item) = stream.next().await {
            match item {
                sage_tool_runtime::ToolStreamItem::Progress(p) => {
                    ...
                    assert!(matches!(p, sage_tool_runtime::ToolProgress::Text { .. }));
                    progress_count += 1;
                }
                sage_tool_runtime::ToolStreamItem::Terminal(result) => {
    ...
    impl sage_tool_runtime::Tool for NoTerminalStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> sage_tool_protocol::ToolId {
            sage_tool_protocol::ToolId::new("no_terminal_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::sage_tool_runtime::ListToolsContext,
        ) -> sage_tool_types::ToolDescription {
            sage_tool_types::ToolDescription::new("no_terminal_stub", "no-terminal stub")
        }
        async fn run(
            &self,
            _ctx: sage_tool_runtime::ToolCallContext,
    ...
    async fn tool_definitions_builtins_only_hides_mcp_tools() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<sage_build::ReadFileTool>(),
                ToolConfig::for_tool::<sage_build::GrepTool>(),
            ],
            behavior_preset: None,
        };
    ...
    fn task_tool_rejected_without_get_task_output_and_kill_task() {
        ...
        assert!(
            errors.iter().any(|e| e.tool == "SageBuild:task"
                && e.message.contains("SageBuild:get_task_output")
                && e.message.contains("SageBuild:kill_task")),
    ...
    fn task_tool_rejected_with_only_get_task_output() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "SageBuild:get_task_output".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.tool == "SageBuild:task" && e.message.contains("SageBuild:kill_task")),
    ...
    fn task_tool_rejected_with_only_kill_task() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "SageBuild:kill_task".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
    ...
    /// sage-build agent config (all tools present) and that the new examples
    ...
    async fn background_param_templates_reference_real_schema_keys() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                tool("SageBuild:run_terminal_cmd"),
                tool("SageBuild:task"),
                tool("SageBuild:get_task_output"),
                tool("SageBuild:wait_tasks"),
                tool("SageBuild:kill_task"),
            ],
            behavior_preset: None,
        };
        let tmp = TempDir::new().unwrap();
    ...
    fn bash_with_enabled_background_reports_missing_task_tools() {
        ...
        assert!(error.message.contains("SageBuild:get_task_output"));
        assert!(error.message.contains("SageBuild:kill_task"));
        assert!(
            error
                .expected
                .as_deref()
    ...
    fn task_requirement_error_lists_missing_background_tools() {
        ...
        let error = &errors[0];
        assert_eq!(error.tool, "SageBuild:task");
        assert!(error.message.contains("SageBuild:get_task_output"));
        assert!(error.message.contains("SageBuild:kill_task"));
    ...
    fn search_replace_requirement_error_mentions_read_tool() {
        ...
        let error = errors
            .iter()
            .find(|error| error.tool == "SageBuild:search_replace")
    ...
    fn ask_user_question_validates_standalone() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "SageBuild:ask_user_question".to_string(),
                params: None,
                name_override: None,
                params_name_overrides: None,
                description_override: None,
    ...
    async fn task_description_renders_with_default_agent_config() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "SageBuild:web_search".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
        ...
        let toolset = builder
            .finalize(config, ctx)
            .expect("finalize should succeed with default sage-build tools");
        let defs = toolset.tool_definitions();
        let task_def = defs
            .iter()
    ...
    fn hashline_tools_registered_in_builder() {
        ...
        assert!(
            builder
                .tools
                .contains_key("SageBuildHashline:hashline_read"),
            "hashline_read should be registered"
        );
        assert!(
            builder
                .tools
                .contains_key("SageBuildHashline:hashline_edit"),
            "hashline_edit should be registered"
        );
        assert!(
            builder
                .tools
                .contains_key("SageBuildHashline:hashline_grep"),
    ...
    async fn hashline_config_finalizes_successfully() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                hashline_tool_config("SageBuildHashline:hashline_read"),
                hashline_tool_config("SageBuildHashline:hashline_edit"),
                hashline_tool_config("SageBuildHashline:hashline_grep"),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn standard_and_hashline_each_finalize_independently() {
        ...
        let standard_config = ToolServerConfig {
            tools: vec![
                hashline_tool_config("SageBuild:read_file"),
                hashline_tool_config("SageBuild:search_replace"),
                hashline_tool_config("SageBuild:grep"),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn empty_struct_schema_has_properties_and_required() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<sage_build::EnterPlanModeTool>(),
                ToolConfig::for_tool::<sage_build::ExitPlanModeTool>(),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn hashline_toolset_e2e_with_utilities() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "SageBuildHashline:hashline_read".to_owned(),
                    params: Some(
                ...
                ToolConfig::for_tool::<sage_build_hashline::HashlineEditTool>(),
                ToolConfig::for_tool::<sage_build_hashline::HashlineGrepTool>(),
                ToolConfig::for_tool::<sage_build::ListDirTool>(),
            ],
            behavior_preset: None,
        };
        let errors = builder.validate_config(&config);
    ...
    fn bash_config_with_background() -> ToolConfig {
        ToolConfig {
            id: "SageBuild:run_terminal_cmd".to_owned(),
            params: Some(
                serde_json::json!({ "enabled_background" : true })
                    .as_object()
                    .unwrap()
    ...
    async fn sage_build_bridge(tmp: &TempDir) -> crate::bridge::ToolBridge {
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<sage_build::ListDirTool>(),
                ToolConfig::for_tool::<sage_build::ReadFileTool>(),
                ToolConfig::for_tool::<sage_build::SearchReplaceTool>(),
                bash_config_with_background(),
                ToolConfig::for_tool::<sage_build::TaskOutputTool>(),
                ToolConfig::for_tool::<sage_build::KillTaskTool>(),
                ToolConfig::for_tool::<sage_build::GrepTool>(),
                ToolConfig::for_tool::<sage_build::TodoWriteTool>(),
            ],
            behavior_preset: None,
        };
    ...
    async fn hub_dispatch_list_dir() {
        ...
        let bridge = sage_build_bridge(&tmp).await;
        let result = bridge
            .call(
                "list_dir",
    ...
    async fn hub_dispatch_parity_list_dir() {
        ...
        let hub_bridge = sage_build_bridge(&tmp).await;
        ...
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![ToolConfig::for_tool::<sage_build::ListDirTool>()],
            behavior_preset: None,
        };
    ...
    async fn prepare_dispatch_stamps_workspace_viewer_ctx_when_present() {
        let (toolset, _tmp) =
            toolset_with_viewer_ctx(Some(sage_tool_runtime::WorkspaceViewerContext {
        ...
        let wvc = parts
            .ctx
            .extensions
            .get::<sage_tool_runtime::WorkspaceViewerContext>()
    ...
    async fn bash_streaming_progress_emitted_via_finalized_toolset_when_gate_on() {
        ...
        while let Some(item) = stream.next().await {
            match item {
                sage_tool_runtime::ToolStreamItem::Progress(_) => progress_count += 1,
                sage_tool_runtime::ToolStreamItem::Terminal(r) => {
