...
struct ClaudeTool {
    ...
    /// Sage tool names this Claude tool maps to (empty when there is no direct
    /// Sage tool — the entry then only contributes a `kind`).
    sage: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, sage: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        sage,
...
const fn match_only(claude: &'static str, sage: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        sage,
...
pub fn sage_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.sage)
...
/// The Claude names that map to `sage_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(sage_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.sage.contains(&sage_name))
        .map(|t| t.claude)
}
...
pub fn sage_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.sage.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        ...
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.sage.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
