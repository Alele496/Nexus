...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.sage/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.sage/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.sage/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.sage/ or ~/.claude/.
    User { path: PathBuf },
