...
//!
//! The tool runtime contract (`Tool` trait) lives in `sage_tool_runtime`.
...
///
/// Serializes to snake_case (`sage_build`, `mcp`, …) for the
...
/// (e.g. `"SageBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "SageBuild")]
    SageBuild,
    #[serde(alias = "SageBuildConcise")]
    SageBuildConcise,
    #[serde(alias = "SageBuildHashline")]
    SageBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
