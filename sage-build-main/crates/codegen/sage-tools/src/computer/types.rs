...
/// (`sage-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (sage-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in sage-tools, spawns processes)
/// - `AcpTerminalBackend` (in sage-shell, calls ACP protocol)
#[async_trait::async_trait]
