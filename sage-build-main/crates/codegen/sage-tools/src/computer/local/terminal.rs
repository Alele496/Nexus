...
impl LocalTerminalActor {
    ...
    async fn spawn_static_command(
        ...
        crate::util::apply_sage_agent_marker(&mut cmd);
        ...
        #[cfg(target_os = "linux")]
        if sage_sandbox::should_restrict_child_network() {
            unsafe {
                cmd.pre_exec(|| sage_sandbox::child_net::install_child_network_filter());
            }
        }
    ...
    async fn spawn_persistent_command(
        ...
        // Apply SHELL_ENV_OVERRIDES (TERM=dumb, NO_COLOR, SAGE_AGENT=1, etc.)
                ...
                && process.exit_status.is_none()
            {
                ...
                if process.kind == crate::computer::types::TaskKind::Monitor {
                    ...
                    let start_offset = std::fs::metadata(&pipeline_output_file)
                        ...
                        .unwrap_or(0);
                    tokio::spawn(async move {
                        crate::implementations::sage_build::monitor::tool::run_monitor_pipeline(
                            &pipeline_task_id,
...
#[cfg(unix)]
const ENV_LOGIN_ENV: &str = "SAGE_LOGIN_ENV";
...
fn spawn_shell_command(
    ...
    let mut build_cmd = |with_breakaway: bool| {
        ...
        let inv = sage_config::shell::shell_command_argv(command);
...
mod tests {
    ...
    async fn test_persistent_shell_env_var_persists() {
        ...
        let result = backend
            .run(make_request("export SAGE_PERSIST_TEST=hello123"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));

        let result = backend
            .run(make_request("echo $SAGE_PERSIST_TEST"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(
