...
pub use sage_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__SAGE_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    sage_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = sage_config::shell::detect_unix_shell_kind();
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias sage_alias_probe='echo ALIAS_OK'\nsage_fn_probe() { echo FN_OK; }\n",
            "sage_alias_probe && sage_fn_probe",
        )
        .await;
