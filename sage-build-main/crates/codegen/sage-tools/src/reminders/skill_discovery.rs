...
//! after every tool call to check for SKILL.md files in `.sage/skills/`,
...
/// Directories that contain skill definitions (`.sage/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".sage", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.sage/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.sage/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.sage/) are outside the git root so the walk breaks early.
