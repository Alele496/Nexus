...
//! `LspTool` (Tool trait impl) under the `SageBuild` namespace.
...
impl sage_tool_runtime::ToolOutput for LspToolOutput {}
...
impl sage_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> sage_tool_protocol::ToolId {
        sage_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, sage_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    sage_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.sage/lsp.json or <cwd>/.sage/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
