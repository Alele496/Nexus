//! `image_gen` tool — generates images via the Sage Imagine API and saves
...
const XAI_IMAGINE_MODEL: &str = "sage-imagine-image-quality";
// Some Imagine models (e.g. `sage-imagine-image`, selectable via `model_override`)
...
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a SuperGrok feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperGrok: https://sage.local/supergrok?referrer=sage-build. Do not retry this tool.";

/// HTTP client for Sage Imagine API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
...
impl sage_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> sage_tool_protocol::ToolId {
        sage_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(sage_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
        assert!(
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("sage-imagine-image".into()),
            tier_restricted: false,
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("sage-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("sage-imagine-image")), None)
                .unwrap()
                .model,
            "sage-imagine-image"
        );
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        ...
        match result {
            ToolOutput::Text(t) => {
                ...
                assert!(t.text.contains("supergrok?referrer=sage-build"));
            }
