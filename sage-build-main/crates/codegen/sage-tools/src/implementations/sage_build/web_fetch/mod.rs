...
impl sage_tool_runtime::Tool for WebFetchTool {
    type Args = WebFetchInput;
    type Output = WebFetchOutput;

    fn id(&self) -> sage_tool_protocol::ToolId {
        sage_tool_protocol::ToolId::new("web_fetch").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: WebFetchInput,
    ) -> Result<WebFetchOutput, sage_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(sage_tool_runtime::Tool::id(&tool).as_str(), "web_fetch");
        assert_eq!(
