...
use crate::implementations::sage_build::task_output::{
...
use sage_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for WaitTasksTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            sage_tool_types::build_wait_tasks_description(&sage_tool_types::WaitTasksToolNaming {
...
/// [`sage_tool_types::build_wait_tasks_description`] builder so the CLI and
...
impl sage_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> sage_tool_protocol::ToolId {
        sage_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let timeout =
            crate::implementations::sage_build::task_output::capped_wait_timeout(input.timeout_ms);
