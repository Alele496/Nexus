...
//! (in `sage-tools`) and the subagent coordinator (in `sage-shell`).
//!
//! ## Resource types
//!
...
use sage_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"sage-build-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`sage_tool_types::is_not_sentinel`] for existing call sites.
pub use sage_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `sage-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "SageBuild:run_terminal_cmd" | "SageBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `sage-shell` relies on
...
register_resource!("sage_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-sage-build
...
pub type MonitorEventBuffer = sage_interjection_core::EventQueue<MonitorEventNotification>;

register_resource!("sage_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// sage-shell produces it, the channel protocol carries it, and the
/// compaction pipeline in sage-chat-state (via `RunningSubagentSummary`)
...
register_resource!("sage_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("sage_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("sage_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by sage-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by sage-shell at turn start.
...
register_resource!("sage_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("SageBuild:run_terminal_cmd", ToolKind::Execute),
                tc("SageBuild:read_file", ToolKind::Read),
                tc("SageBuild:list_dir", ToolKind::List),
                tc("SageBuild:grep", ToolKind::Search),
                tc("SageBuild:kill_task", ToolKind::KillTaskAction),
                tc("SageBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("SageBuild:kill_task", ToolKind::KillTaskAction),
                tc("SageBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("SageBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("SageBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
