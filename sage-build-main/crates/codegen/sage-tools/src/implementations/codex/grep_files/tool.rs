...
use crate::implementations::sage_build::grep::ripgrep::rg_path;
...
///
/// Shares `ToolKind::Search` with the sage-build `GrepTool`. These tools are
/// namespace-exclusive — consumers enable either `SageBuild` or `Codex` search,
...
impl sage_tool_runtime::Tool for CodexGrepFilesTool {
    type Args = CodexGrepFilesInput;
    type Output = CodexGrepFilesOutput;

    fn id(&self) -> sage_tool_protocol::ToolId {
        sage_tool_protocol::ToolId::new("grep_files").expect("valid tool id")
    }
    ...
    ) -> Result<CodexGrepFilesOutput, sage_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn test_ctx(cwd: &Path) -> sage_tool_runtime::ToolCallContext {
    ...
    async fn tool_reports_empty_pattern_error() {
        ...
        let result = sage_tool_runtime::Tool::run(&tool, test_ctx(tmp.path()), input)
            .await
            .unwrap();
        match result {
