...
crate::register_resource!("sage_build", "UseTool", UseToolParams);
...
async fn dispatch_local_mcp(
    ...
    let tool_id = sage_tool_protocol::ToolId::new(tool_name).map_err(|_| {
        sage_tool_runtime::ToolError::invalid_arguments(format!("invalid tool name: '{tool_name}'"))
    })?;
...
fn is_local_tool_id_rejection(err: &sage_tool_runtime::ToolError, tool_name: &str) -> bool {
    err.kind == sage_tool_runtime::ToolErrorKind::InvalidArguments
    ...
    if let Some(source) = gateway_source {
            ...
            && let Some(dispatch) = dispatch.clone()
        {
            match dispatch_local_mcp(dispatch, tool_name, tool_input.clone(), ctx.clone()).await {
                ...
                Err(err)
                    if err.kind != sage_tool_runtime::ToolErrorKind::NotFound
...
impl sage_tool_runtime::Tool for UseTool {
    type Args = UseToolInput;
    type Output = ToolOutput;

    fn id(&self) -> sage_tool_protocol::ToolId {
        sage_tool_protocol::ToolId::new("use_tool").expect("valid tool id")
    }
...
mod tests {
    ...
    impl sage_tool_runtime::ToolDispatch for MockToolDispatch {
        async fn call(
            &self,
            ...
            sage_tool_runtime::terminal_only(Ok(sage_tool_runtime::TypedToolOutput::from_value(
                tool_id, value,
            )))
        }
    }
    ...
    #[async_trait::async_trait]
    impl sage_tool_runtime::ToolDispatch for CapturingDispatch {
        async fn call(
            &self,
            tool_id: sage_tool_protocol::ToolId,
            ...
            if !matches!(
                ...
                return sage_tool_runtime::terminal_only(Err(
                    sage_tool_runtime::ToolError::not_found(tool_id, "Tool not found"),
                ));
            }
    ...
    fn ctx_capturing() -> (sage_tool_runtime::ToolCallContext, SharedArgs) {
    ...
    #[async_trait::async_trait]
    impl sage_tool_runtime::ToolDispatch for NotFoundDispatch {
        async fn call(
            &self,
            tool_id: sage_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            sage_tool_runtime::terminal_only(Err(sage_tool_runtime::ToolError::not_found(
                tool_id,
                "Tool not found",
            )))
        }
    ...
    #[async_trait::async_trait]
    impl sage_tool_runtime::ToolDispatch for InvalidArgumentsDispatch {
        async fn call(
            &self,
            _tool_id: sage_tool_protocol::ToolId,
            _args: serde_json::Value,
            _ctx: sage_tool_runtime::ToolCallContext,
        ) -> sage_tool_runtime::ToolStream<sage_tool_runtime::TypedToolOutput> {
            sage_tool_runtime::terminal_only(Err(sage_tool_runtime::ToolError::invalid_arguments(
    ...
    #[async_trait::async_trait]
    impl sage_tool_runtime::ToolDispatch for ErrorToolDispatch {
        async fn call(
            &self,
            _tool_id: sage_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            let tid = sage_tool_protocol::ToolId::new(&self.error)
    ...
    fn new_ctx() -> sage_tool_runtime::ToolCallContext {
        let call_id = sage_tool_protocol::ToolCallId::new_v7();
        sage_tool_runtime::ToolCallContext::new(call_id)
    }

    fn ctx_with_dispatch(
        dispatch: impl sage_tool_runtime::ToolDispatch + 'static,
    ...
    async fn rejects_builtin_tool_names() {
        ...
        assert_eq!(err.kind, sage_tool_runtime::ToolErrorKind::InvalidArguments);
    ...
    fn ctx_capturing_with_resources(
        resources: crate::types::resources::SharedResources,
    ) -> (sage_tool_runtime::ToolCallContext, SharedArgs) {
        let args: SharedArgs = Arc::new(std::sync::Mutex::new(None));
        let mut ctx = new_ctx();
        ctx.extensions
