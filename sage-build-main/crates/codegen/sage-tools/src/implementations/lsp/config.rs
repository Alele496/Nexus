//! LSP server configuration from `.sage/lsp.json`.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::sage_home::sage_home().join("lsp.json");
    let project_path = cwd.join(".sage").join("lsp.json");
...
/// Load LSP server configs from `~/.sage/lsp.json` and `<cwd>/.sage/lsp.json`.
