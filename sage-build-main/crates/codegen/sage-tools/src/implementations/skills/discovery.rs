...
///
/// `skills` is the standard layout (`.sage/skills/`, `.claude/skills/`,
...
/// name (e.g. `~/.sage/skills/shell`) is NOT dropped — only skills physically
...
/// `cwd` (exclusive). At each directory, checks for `.sage/skills/`,
...
/// visits known config roots (`.sage`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.sage` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".sage", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.sage is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.sage/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.sage/ → kept (user content).
        let sage_shell = tmp.path().join(".sage").join("skills").join("shell");
        std::fs::create_dir_all(&sage_shell).unwrap();
        std::fs::write(
            sage_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (sage_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.sage/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .sage skill in an intermediate dir.
        ...
        .unwrap();
        let sage_skill = sub.join(".sage").join("skills").join("sage-dyn");
        std::fs::create_dir_all(&sage_skill).unwrap();
        std::fs::write(sage_skill.join("SKILL.md"), "---\nname: sage-dyn\n---\n").unwrap();
