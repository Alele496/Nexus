...
impl sage_tool_runtime::Tool for BashTool {
    ...
    fn id(&self) -> sage_tool_protocol::ToolId {
        sage_tool_protocol::ToolId::new("bash").expect("valid tool id")
    }
        ...
        let result = match backend.run(request).await {
            ...
            Err(e) => {
                ...
                return Err(sage_tool_runtime::ToolError::execution(
                    sage_tool_protocol::ToolId::new("run_terminal_command").expect("valid"),
                    e.to_string(),
                ));
            }
        };
...
mod tests {
    ...
    fn rt_ctx_with_call_id(
        ...
        let id = sage_tool_protocol::ToolCallId::new(call_id).unwrap();
    ...
    async fn tool_metadata() {
        use crate::types::tool_metadata::ToolMetadata;
        let tool = BashTool;
        assert_eq!(sage_tool_runtime::Tool::id(&tool).as_str(), "bash");
    ...
    //
    // Same shape as sage_build/bash: the opencode tool inherits the same
