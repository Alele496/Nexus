...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.sage/skills/ or .sage/skills/)
${%- endif %}
</available_skills>"#;
...
impl sage_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> sage_tool_protocol::ToolId {
        sage_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, sage_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(sage_tool_runtime::Tool::id(&tool).as_str(), "skill");
