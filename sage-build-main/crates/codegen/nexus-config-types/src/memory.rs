//! Memory-system configuration value types, extracted from sage-shell
...
//! `sage-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.nexus/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.nexus/memory/` are scanned:
