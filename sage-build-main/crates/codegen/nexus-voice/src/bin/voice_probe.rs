...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,nexus_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                nexus_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("NEXUS_XAI_API_BASE_URL").ok();
...
Environment:
  ...
  RUST_LOG        optional (default info,nexus_voice=debug)

Reads [voice] from ~/.nexus/config.toml unless --config PATH is set.
"#
    );
}
