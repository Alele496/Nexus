//! Config file loading for Sage.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/sage/managed_config.toml`
//! 2. `$SAGE_HOME/managed_config.toml`
//! 3. `$SAGE_HOME/config.toml`
//! 4. `$SAGE_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/sage/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.sage`, admin-forced) — macOS only
//!
...
pub use paths::{
    ...
    default_sage_home, encode_cwd_dirname, ensure_sessions_cwd_dir, sage_application,
    sage_application_in, sage_home, sessions_cwd_dir, system_config_dir, user_sage_home,
};
pub use validation::{
