...
use crate::paths::{system_config_dir, user_sage_home};
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_sage_home().as_deref(), "config.toml")
}
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_sage_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.sage/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.sage` to the user tier).
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_sage_home().as_deref())
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `SAGE_CAMPAIGNS_OVERRIDE` env layer.
...
/// `SAGE_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("SAGE_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`sage_shell::util::config::load_effective_config`) when remote campaigns
/// or `SAGE_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// can't be parsed (broken `SAGE_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match sage_version::installed_semver() {
...
mod tests {
    ...
    /// `SAGE_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `SAGE_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("SAGE_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads SAGE_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("SAGE_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("SAGE_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("SAGE_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("SAGE_CAMPAIGNS") },
        }
    }
}
