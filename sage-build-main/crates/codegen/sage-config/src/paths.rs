...
/// The default user sage directory (`~/.sage`, canonicalized) used when
/// `SAGE_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`sage_home()`] is the default without duplicating the computation.
///
...
/// `~/.sage/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `sage_fast_worktree::db::resolve_sage_home` (deliberately standalone crate).
...
pub fn sage_home() -> PathBuf {
    SAGE_HOME
        .get_or_init(|| {
            let sage_home = if let Ok(v) = std::env::var("SAGE_HOME") {
                PathBuf::from(v)
            } else {
                default_sage_home()
            };
...
/// `$SAGE_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`sage_home()`], this never falls back to a cwd-relative `.sage`, so callers
/// that *scan* user-global sage resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.sage` tree for the user-global one when no home resolves.
pub fn user_sage_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("SAGE_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(sage_home)
}

/// Canonical sage application path: `$SAGE_HOME/bin/sage` (Unix) or `sage.exe` (Windows).
pub fn sage_application() -> PathBuf {
    sage_application_in(&sage_home())
}

/// [`sage_application`] under an explicit home instead of `$SAGE_HOME`.
pub fn sage_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "sage.exe" } else { "sage" };
...
/// `sage_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    sage_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = sage_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_sage_home_has_no_verbatim_prefix() {
        ...
        let home = default_sage_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".sage"));
    }
