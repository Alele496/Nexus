...
//! [`MockInferenceServer`] from `sage-test-support` and provides the
...
pub use nexus_test_support::mock_server::LogEntry;
pub use nexus_test_support::mock_server::MockModelEntry as MockModel;
pub use nexus_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use nexus_test_support::sse;
pub use nexus_test_support::{
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.nexus
    ...
    ///
    /// Mirrors `nexus_test_support::env::test_env_cmd_tokio`.
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit NEXUS_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // NEXUS_HOME is set in the test runner's env).
            ("NEXUS_HOME".into(), nexus_home),
            ("NEXUS_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("NEXUS_XAI_API_BASE_URL".into(), self.url()),
            ...
            ("NEXUS_PROMPT_SUGGESTIONS".into(), "false".into()),
            // Compatibility set_turns remains request-FIFO, so retries stay off.
            ("NEXUS_MAX_RETRIES".into(), "0".into()),
        ]
    }
...
mod tests {
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("NEXUS_HOME").as_deref(),
            content.home().join(".nexus").to_str()
        );
        assert_eq!(get("NEXUS_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("NEXUS_XAI_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("XAI_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("NEXUS_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("NEXUS_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("NEXUS_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("NEXUS_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("NEXUS_MAX_RETRIES").as_deref(), Some("0"));
