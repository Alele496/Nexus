...
//! elected leader) plus a fixed leader socket beneath its `NEXUS_HOME`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `NEXUS_HOME`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared NEXUS_HOME => one leader; the socket lives beneath it so
        // every client (sharing the same env) elects/attaches to the same one.
        let nexus_home = content.home().join(".nexus");
        std::fs::create_dir_all(&nexus_home).context("create sage home")?;
        let socket = nexus_home.join("leader-e2e.sock");
    ...
    /// The cluster's sessions root: `NEXUS_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".nexus").join("sessions")
    }
