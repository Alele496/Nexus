...
//! Backend environment presets for the Sage CLI crate family: endpoint URL
//! defaults, environment selection, and env-var test support.
//!
//! Public builds expose production endpoints. Values resolve as a `NEXUS_*`
//! env-var override when set, else the compiled production default.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct NexusBuildEndpoints {
...
const PRODUCTION_ENDPOINTS: NexusBuildEndpoints = NexusBuildEndpoints {
    cli_chat_proxy_base_url: "https://cli-chat-proxy.nexus.local/v1",
    asset_server_url: "https://assets.nexus.local",
    relay_ws_url: "wss://code.nexus.local/ws/code-agent",
    gateway_ws_url: "wss://sage.local/ws/gw/",
    ws_origin: "https://sage.local",
};
...
pub enum NexusBuildEnvironment {
    #[default]
    Production,
}
impl NexusBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        NexusBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            NexusBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, NexusBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            NexusBuildEnvironment::Production => "NEXUS_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> NexusBuildEndpoints {
        match self {
            NexusBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// The relay WebSocket URL (Web Frontend at `sage.local/code` driving a
    ...
    /// `NEXUS_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for NexusBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            NexusBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            NexusBuildEnvironment::Production.env_prefix(),
            "NEXUS_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "XAI_NEXUS_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    /// loop mistakenly connecting to `wss://sage.local/ws/gw/`).
    #[test]
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            NexusBuildEnvironment::Production.relay_ws_url(),
            NexusBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            NexusBuildEnvironment::from_flags(false, false),
            NexusBuildEnvironment::Production
        );
    }
}
