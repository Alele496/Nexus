...
use nexus_tools::types::memory_backend::{MemoryBackend, MemorySearchResult};
...
pub struct EndpointScopedCredentials {
    ...
    auth_credentials: Option<Arc<dyn nexus_auth::AuthCredentialProvider>>,
...
impl EndpointScopedCredentials {
    ...
    fn auth_credentials(&self) -> Option<&Arc<dyn nexus_auth::AuthCredentialProvider>> {
        self.auth_credentials.as_ref()
    }

    fn api_key_provider(&self) -> Option<&nexus_tools::types::SharedApiKeyProvider> {
...
pub struct MemoryBackendParams {
    ...
    pub embed_config: Option<nexus_config_types::MemoryEmbeddingConfig>,
    ...
    pub search_config: nexus_config_types::MemorySearchConfig,
...
async fn build_embedding_provider(
    config: Option<&nexus_config_types::MemoryEmbeddingConfig>,
...
pub struct MemoryBackendImpl {
    ...
    embed_config: Option<nexus_config_types::MemoryEmbeddingConfig>,
    ...
    search_config: nexus_config_types::MemorySearchConfig,
...
impl MemoryBackendImpl {
    ...
    pub fn new(db_path: PathBuf, storage: MemoryStorage) -> Self {
        Self {
            ...
            embed_api_key: None,
            search_config: nexus_config_types::MemorySearchConfig::default(),
            watcher: None,
            stale_claim_secs: 60,
    ...
    pub fn with_embedding(
        mut self,
        config: nexus_config_types::MemoryEmbeddingConfig,
        base_url: String,
    ...
    pub fn with_search_config(mut self, config: nexus_config_types::MemorySearchConfig) -> Self {
    ...
    fn open_readonly(&self) -> Result<rusqlite::Connection, rusqlite::Error> {
        ...
        nexus_sqlite_journal::JournalMode::for_db_path(&self.db_path).open_readonly(&self.db_path)
    }
...
impl MemoryBackendImpl {
    ...
    pub fn search_config_for_test(&self) -> &nexus_config_types::MemorySearchConfig {
        &self.search_config
    }
}
...
impl MemoryBackend for MemoryBackendImpl {
    ...
    async fn search(
        ...
        let mut index = super::index::MemoryIndex::open_or_create(
            &self.db_path,
            self.storage.clone(),
            nexus_config_types::MemoryIndexConfig::default(),
            embed_dims,
        )
        ...
        if needs_release {
            ...
            if let Some((dirty_count, reindexed_count, sync_start)) = watcher_sync_stats {
                nexus_telemetry::session_ctx::log_event(
                    nexus_telemetry::memory_telemetry::MemoryWatcherSync {
        ...
        if results.is_empty() {
            nexus_telemetry::session_ctx::log_event(
                nexus_telemetry::memory_telemetry::MemorySearchEmpty {
                    session_id: self.session_id.clone(),
            ...
            nexus_telemetry::session_ctx::log_event(
                nexus_telemetry::memory_telemetry::MemorySearch {
                    session_id: self.session_id.clone(),
                    query_length: query.len(),
                    keyword_count,
...
mod factory_tests {
    ...
    use nexus_config_types::{MemoryEmbeddingConfig, MemorySearchConfig};
    ...
    fn test_factory_wires_non_overridable_search_config_fields() {
        ...
        let custom_search = MemorySearchConfig {
            max_results: 7,
            mmr: nexus_config_types::MmrConfig {
                enabled: true,
                lambda: 0.42,
            },
            temporal_decay: nexus_config_types::TemporalDecayConfig {
                enabled: true,
                half_life_days: 14.0,
            },
            ..Default::default()
    ...
    async fn make_embedding_provider_uses_async_api_key_resolution() {
        ...
        use nexus_tools::types::ApiKeyProvider;
        ...
        let probe: nexus_tools::types::SharedApiKeyProvider = Arc::new(AsyncProbe {
...
mod tests {
    ...
    async fn test_build_drops_credentials_when_request_url_differs() {
        let session: nexus_tools::types::SharedApiKeyProvider = Arc::new(PanicKey);
        ...
        let config = nexus_config_types::MemoryEmbeddingConfig {
    ...
    async fn test_trusted_endpoint_prefers_session_credential() {
        struct StubAuth;
        impl nexus_auth::HttpAuth for StubAuth {
            fn apply(
                &self,
        ...
        #[async_trait::async_trait]
        impl nexus_auth::AuthCredentialProvider for StubAuth {
            fn snapshot(&self) -> nexus_auth::CredentialSnapshot {
                nexus_auth::CredentialSnapshot::default()
            }
        ...
        let auth: Arc<dyn nexus_auth::AuthCredentialProvider> = Arc::new(StubAuth);
        let api_key: nexus_tools::types::SharedApiKeyProvider = Arc::new(PanicKey);
        let scoped = EndpointScopedCredentials::for_endpoint(
            "https://api.deepseek.com/v1",
            |_| true,
            Some(auth),
        ...
        let provider =
            build_embedding_provider(Some(&config), &scoped, None, "https://api.deepseek.com/v1").await;
        assert!(
            provider.is_some(),
    ...
    fn endpoint_scoped_credentials_trust_gate_and_url_match() {
        struct AnyKey;
        impl nexus_tools::types::ApiKeyProvider for AnyKey {
            fn current_api_key(&self) -> Option<String> {
                None
            }
        }
        let key = || Arc::new(AnyKey) as nexus_tools::types::SharedApiKeyProvider;
