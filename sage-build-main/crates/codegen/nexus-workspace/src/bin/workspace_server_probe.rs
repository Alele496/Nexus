...
//! access token from `~/.nexus/auth.json` does (same identity).
...
use url::Url;
use uuid::Uuid;
use nexus_computer_hub_sdk::pool::HubConnectionPool;
use nexus_computer_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &nexus_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<nexus_computer_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "NexusBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
            {"id": "NexusBuild:read_file"},
        ],
    });
    let tools = harness
