//! Hub [`AuthProvider`] from `~/.nexus/auth.json` for the standalone
...
//!
//! The in-leader `sage workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let sage = nexus_config::user_nexus_home()
        .ok_or_else(|| anyhow::anyhow!("no user sage home (set $NEXUS_HOME or $HOME)"))?;
    Ok(sage.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `sage login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `sage login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.nexus/auth.json`).
pub fn provider(
    hub_url: &Url,
