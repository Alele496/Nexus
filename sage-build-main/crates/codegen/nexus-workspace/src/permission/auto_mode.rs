...
//!
//! Port of common agent auto-permission classifier semantics adapted to Sage's
...
/// mapped to Sage access kinds + known names).
...
pub fn mcp_access_detail(name: &str, input: &serde_json::Value) -> String {
    ...
    nexus_tools::util::truncate_line(&format!("{name} {compact}"), MCP_ACCESS_DETAIL_MAX_LEN)
        .into_owned()
}
...
pub fn permission_decision_args(access: &AccessKind, access_detail: Option<&str>) -> String {
    ...
    nexus_tools::util::truncate_str_with_marker(&raw, CLASSIFIER_TURN_MAX_LEN).into_owned()
}
