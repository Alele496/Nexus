...
use std::sync::Arc;
use nexus_computer_hub_mcp_adapter::McpBridgeHandle;
use nexus_mcp::servers::McpState;
use nexus_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use nexus_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
use nexus_hunk_tracker::HunkTrackerHandle;
use nexus_tool_protocol::ToolId;
use nexus_tool_runtime::WorkspaceViewerContext;
...
pub struct WorkspaceSession {
    ...
    ///
    /// [`HunkTrackerActor`]: nexus_hunk_tracker::HunkTrackerActor
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, nexus_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<nexus_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<nexus_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<nexus_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<nexus_hooks::error::HookError>,
    ...
    pub(crate) auth_provider: Option<nexus_computer_hub_sdk::SharedAuthProvider>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<nexus_tools::notification::types::ToolNotificationHandle>>,
    ...
    pub(crate) local_registry: nexus_computer_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn nexus_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `NEXUS_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$NEXUS_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<nexus_home>/workspace` by default). The upload queue spills here.
    pub(crate) workspace_home: std::path::PathBuf,
    pub(crate) upload_queue: Option<std::sync::Arc<nexus_file_utils::queue::UploadQueue>>,
    ...
    /// (`NEXUS_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](nexus_file_utils::events::EventWriter::noop) and
    ...
    /// enabled (`NEXUS_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, nexus_file_utils::events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<nexus_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<nexus_file_utils::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](nexus_file_utils::events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> nexus_file_utils::events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<nexus_file_utils::events::EventWriter> {
    ...
    pub fn auth_provider(&self) -> Option<&nexus_computer_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](nexus_computer_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<nexus_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, nexus_file_utils::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> nexus_file_utils::events::EventWriter {
...
mod tests {
    ...
    use dashmap::DashMap;
    use nexus_file_utils::events::{Event, EventWriter};
