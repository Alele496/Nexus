...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `sage-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `sage-shell::session::events`.
    ...
    /// discriminator constants in `sage-shell::session::events`.
    ...
    /// `sage-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `sage-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Telemetry mirror of `sage-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `sage-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `sage-shell`'s `GoalPauseReason`. The two types
