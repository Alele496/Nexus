...
use sage_tools_api::ToolConfigEntry;

fn full_entry() -> ToolConfigEntry {
    ToolConfigEntry {
        id: "SageBuild:grep".to_owned(),
...
fn minimal_entry_deserializes_from_id_only() {
    ...
    let back: ToolConfigEntry =
        serde_json::from_value(serde_json::json!({"id": "SageBuild:read_file"}))
            .expect("deserialize minimal");
    assert_eq!(back.id, "SageBuild:read_file");
