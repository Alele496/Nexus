...
//! [`crate::trust::TrustStore`] (`~/.sage/trusted_folders.toml`). The
...
use sage_config_types::{BoolFlag, RemoteSettings};
...
pub fn decide_inputs_with_interactive(
    ...
    DecideInputs {
        ...
        // can't persist (Case 2: cwd IS $HOME, incl. the default `~/.sage`).
...
/// this binary — true on a local/dev build (no `SAGE_VERSION` release stamp).
///
...
/// (`SAGE_VERSION` unset at compile time) — i.e. a local/dev build.
///
...
fn is_local_build() -> bool {
    // Runtime escape hatch: a pinned SAGE_TEST_VERSION simulates a release build,
    // so tests/CI (which run unstamped, i.e. local-looking) can exercise the gate.
    if std::env::var(sage_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
    option_env!("SAGE_VERSION").is_none()
}

/// Resolve whether the folder-trust gate is enabled.
///
/// On a local/dev build (no `SAGE_VERSION` release stamp) the feature is OFF
...
/// env `SAGE_FOLDER_TRUST` > `[folder_trust] enabled` (user) > managed >
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    // shipped, release-stamped binaries. Even an explicit SAGE_FOLDER_TRUST/config
    ...
    let user = sage_config::load_from_disk().ok();
    let managed = sage_config::load_managed_config().ok();
    BoolFlag::env("SAGE_FOLDER_TRUST")
...
/// same `~/.sage/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
/// `[plugins].paths` / `.claude` / `.sage/agents` / subdir-launch gaps. NOT
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = sage_agent::repo::RepoDirChain::resolve(cwd);
    ...
    // Project `.sage/config.toml` declaring repo-controlled code-exec: a
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = sage_config::load_config_file(&path) else {
            continue;
        };
    ...
    // Project `.sage/lsp.json`.
    if cwd.join(".sage").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    // must be gated — else a hooks-only clone (e.g. `.sage/hooks/evil.json`) would
    ...
    if path_present_or_uncertain(&hook_root.join(".sage").join("hooks"))
    ...
    // that must be gated — else a plugin clone (e.g. `.sage/plugins/evil/`, even
    ...
    if !sage_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    // Project AGENT dirs (`.sage/agents` / `.claude/agents`): a project agent
    ...
    if !sage_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .sage/lsp.json / hooks) \
...
mod tests {
    ...
    #[test]
    fn repo_configs_present_detects_sage_config_mcp_servers() {
        let tmp = repo_tmp();
    ...
    #[test]
    fn repo_configs_present_detects_sage_lsp_json() {
        let tmp = repo_tmp();
        let sage = tmp.path().join(".sage");
        std::fs::create_dir_all(&sage).unwrap();
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.sage/agents`-only clone must be gated: a project agent definition
        // can carry an inline `hooks:` block (code-exec) and can shadow a built-in
        // subagent by name.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".sage").join("agents")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_roles() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".sage").join("roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_personas() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".sage").join("personas")).unwrap();
    ...
    fn project_subagent_marker_at_repo_root_is_absent_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".sage/roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".sage").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".sage").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    #[test]
    fn repo_configs_present_detects_sage_config_plugins_paths() {
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.sage/agents` — even when launched from a SUBDIR (the cwd→git-root walk
    ...
    // SAGE_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(sage_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn local_build_ignores_remote_rollout() {
        ...
        let _home = EnvVarGuard::set("SAGE_HOME", home.path());
        let _flag = EnvVarGuard::unset("SAGE_FOLDER_TRUST");
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty SAGE_HOME (no
        // config.toml/managed_config.toml) + SAGE_FOLDER_TRUST unset. nextest's
        // process-per-test makes sage_home()'s OnceLock pick up the temp dir.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("SAGE_HOME", home.path());
        let _flag = EnvVarGuard::unset("SAGE_FOLDER_TRUST");
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // SAGE_FOLDER_TRUST=1 does NOT enable the feature (so a self-built sage
        // never prompts). SAGE_HOME isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("SAGE_HOME", home.path());
        let _flag = EnvVarGuard::set("SAGE_FOLDER_TRUST", Path::new("1"));
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty SAGE_HOME (no config.toml/managed config) +
        // SAGE_FOLDER_TRUST unset so only the default applies.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("SAGE_HOME", home.path());
        let _flag = EnvVarGuard::unset("SAGE_FOLDER_TRUST");
    ...
    fn is_local_build_honors_test_version_override() {
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // A pinned SAGE_TEST_VERSION simulates a release build => not a local build.
        {
            let _sim = EnvVarGuard::set(sage_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
            assert!(!is_local_build());
        }
        // With it unset, an unstamped build (no SAGE_VERSION) is a local build.
        // Guard to the unstamped case so a release-stamped test binary (CI release)
        // doesn't spuriously fail this arm.
        let _unset = EnvVarGuard::unset(sage_version::TEST_VERSION_ENV);
        if option_env!("SAGE_VERSION").is_none() {
            assert!(is_local_build());
        }
    }
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); SAGE_HOME-isolated
        // and ENV_LOCK-serialized so toggling SAGE_TEST_VERSION is race-safe.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("SAGE_HOME", home.path());
        let _unset = EnvVarGuard::unset(sage_version::TEST_VERSION_ENV);
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // SAGE_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("SAGE_HOME", home.path());
        let _sim = simulate_release_build();
        let tmp = repo_tmp();
        let key = workspace_key(tmp.path());
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // SAGE_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("SAGE_HOME", home.path());
        let _sim = simulate_release_build();
