//! Permission resolution engine: merges native `.sage/config.toml`,
...
/// user `~/.sage` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    sage_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Find every `<dir>/.sage/config.toml` from `cwd` upward to the git repo
/// root (or just `<cwd>/.sage/config.toml` when there is no git repo).
///
...
fn find_project_sage_configs(cwd: &Path) -> Vec<PathBuf> {
    ...
    if let Some(ref root) = git_root {
        ...
        while let Some(dir) = current {
            let p = dir.join(".sage").join("config.toml");
            if p.is_file() {
                configs.push(p);
            }
            if dir == *root {
        ...
        let p = cwd.join(".sage").join("config.toml");
        if p.is_file() {
            configs.push(p);
        }
    }
...
///
///   * `~/.sage/config.toml` (lowest priority)
///   * Each `.sage/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.sage/config.toml` first (lowest priority within this layer).
    // Gated on user_sage_home() so a project's .sage/config.toml is never read as
    // global permissions when neither SAGE_HOME nor a home dir resolves.
    if let Some(global_path) = sage_config::user_sage_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match sage_config::load_config_file(&global_path) {
    ...
    for path in find_project_sage_configs(cwd) {
        match sage_config::load_config_file(&path) {
            Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[sage_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
    ...
    fn live() -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &sage_config::managed_config_layers(),
            ),
        }
    }
}
...
/// [`sage_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = sage_config::claude_managed_settings_path() else {
...
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = sage_config::requirements_layers();
...
/// without `~/.sage`); `path` only names the layer in a non-bool warning.
...
pub const MANAGED_MCP_PREFIX: &str = "sage_com_";

/// Max `char` length of a managed runtime name (`sage_com_` + normalized display
...
///
/// Both sides reduce to one key (strip `sage_com_`, [`normalize_managed_name`],
...
mod tests {
    ...
    // Shared so `SAGE_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // SAGE_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("SAGE_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_SAGE_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate SAGE_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate SAGE_HOME so the claude-import marker reads clean (an
    ...
    fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
        ...
        assert!(mcp_name_matches("foo", "sage_com_foo"));
        assert!(mcp_name_matches("sage_com_foo", "foo"));
        assert!(mcp_name_matches("sage_com_foo", "sage_com_foo"));
        assert!(!mcp_name_matches("foo", "foobar"));
        assert!(!mcp_name_matches("foo", "sage_com_foobar"));
    ...
    fn mcp_name_matches_is_case_and_space_insensitive() {
        ...
        assert!(mcp_name_matches("Slack", "sage_com_slack"));
        assert!(mcp_name_matches("My Server", "sage_com_my_server"));
        assert!(mcp_name_matches("sage_com_my_server", "My Server"));
        ...
        assert!(!mcp_name_matches("sage_com_", "sage_com_anything"));
    }
    ...
    fn denied_by_server_name_matches_bare_and_managed_prefix() {
        ...
        for unrelated in ["foobar", "sage_com_foobar", "barfoo", "bar"] {
    ...
    fn allowed_by_server_name_restricts_across_transports() {
        ...
        assert!(al.is_server_allowed(&http_named("sage_com_foo", "https://evil.example.com/x")));
        assert!(al.is_server_allowed(&stdio_named("sage_com_foo", "/usr/bin/whatever")));
    ...
    fn server_name_deny_beats_allow() {
        ...
        for s in [
            ...
            http_named("sage_com_foo", "https://foo.example.com/x"),
        ] {
    ...
    fn server_name_prefix_edge_cases_vice_versa() {
        ...
        let al = allowlist_from(serde_json::json!({
            "deniedMcpServers": [ { "serverName": "sage_com_foo" } ]
        }));

        assert!(al.is_server_denied(&http_named("foo", "https://x.example.com/mcp")));
        assert!(al.is_server_denied(&http_named("sage_com_foo", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("foobar", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("sage_com_foobar", "https://x.example.com/mcp")));
    }
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.sage/requirements.toml` is not, despite its path.
        ...
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.sage/requirements.toml".into(),
        }));
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            sage_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            sage_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
