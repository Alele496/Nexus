...
use sage_tools::implementations::sage_build::web_fetch::{
    DomainMatcher, domain::normalize_domain,
};
use sage_tools::types::resources::resolve_model_path;
...
fn permission_mode_artifact_str(mode: sage_telemetry::enums::PermissionMode) -> &'static str {
    use sage_telemetry::enums::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.sage/sessions/<cwd>/permission.toml. This caused
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && sandbox_may_auto_allow_bash(
                            bash_evaluation.as_ref(),
                            sage_sandbox::should_auto_allow_bash(),
                        )
                        && !policy_forced_prompt
                        && !auto_forced_prompt
                    {
...
mod tests {
    ...
    fn manager_with_recording_client_remember(
        ...
        let (gateway, receiver) = sage_acp_lib::acp_gateway::<acp::AgentSide, _>(client);
