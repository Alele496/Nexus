...
use sage_file_utils::events::{Event, EventWriter, PermissionDecision};
use sage_mcp::servers::parse_mcp_qualified_name;
use sage_tools::implementations::sage_build::web_fetch::domain_from_url;
...
///        `~/.sage/config.toml` via the `Effect::PersistPermissionMode` effect
...
/// Canonical definition lives in `sage_workspace_types` (so both the
...
/// reached `sage_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
...
/// `"list_issues"` → `"List Issues"`, `"sage_com_notion"` →
/// `"Sage Com Notion"`, and `"getMyTaskList"` → `"GetMyTaskList"`.
...
pub struct McpToolPermission {
    ...
    /// (e.g. `"sage_com_notion__notion-fetch"`).
    pub tool_name: String,
    /// Server component of a valid qualified MCP ID (e.g. `"sage_com_notion"`).
...
mod tests {
    ...
    fn mcp_titleize_segment_handles_snake_camel_kebab() {
        ...
        assert_eq!(mcp_titleize_segment("sage_com_notion"), "Sage Com Notion");
