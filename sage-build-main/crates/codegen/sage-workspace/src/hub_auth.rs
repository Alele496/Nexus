//! Hub [`AuthProvider`] from `~/.sage/auth.json` for the standalone
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let sage = sage_config::user_sage_home()
        .ok_or_else(|| anyhow::anyhow!("no user sage home (set $SAGE_HOME or $HOME)"))?;
...
/// the default credential path (`~/.sage/auth.json`).
pub fn provider(
    hub_url: &Url,
