...
/// `sage_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "sage_workspace_preview_proxy_restart_total",
...
/// Build the unspawned proxy command. Secrets (`SAGE_SERVER_KEY` /
/// `SAGE_SESSION_ID`) reach the proxy by env inheritance — never argv.
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT sage_tty_utils::detach_command: the proxy must stay in
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "SAGE_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
