...
//!
//! Reads OIDC credentials from `~/.sage/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use std::path::PathBuf;
use url::Url;
use sage_workspace::config::WorkspaceServerMetadata;
use sage_workspace::daemonize;
use sage_workspace::diag_server;
use sage_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_sage_workspace";
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<sage_tool_protocol::ServerId>()
        .err()
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.sage.local/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `SAGE_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "SAGE_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Trust project-scoped LSP servers from `<repo>/.sage/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "SAGE_WORKSPACE_PROJECT_LSP_TRUSTED",
    ...
    /// Override with `SAGE_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "SAGE_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = sage_computer_hub_sdk::DonatingLogLayer::new_inert();
    tracing_subscriber::registry()
        .with(env_filter)
        .with(tracing_subscriber::fmt::layer())
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("SAGE_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match sage_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use sage_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("SAGE_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized SAGE_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if sage_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            tracing::info!(
                ...
                "Sandbox explicitly disabled via SAGE_SANDBOX_PROFILE=off"
            );
        } else {
            ...
            sandbox.install();
            let active = sage_sandbox::is_active();
            ...
            tracing::info!(
                ...
                sage_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = sage_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("SAGE_SESSION_ID").ok();
    ...
    sage_workspace::init_metrics();
    let ws_handle = sage_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = sage_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            sage_workspace::handle::DrainReason::Sigterm,
        )
        .await;
    ...
    sage_computer_hub_sdk::flush_log_layer();
...
mod tests {
    ...
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("SAGE_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["xai-workspace-server"]).unwrap();
