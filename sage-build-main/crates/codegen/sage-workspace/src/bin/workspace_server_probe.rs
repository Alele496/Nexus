...
//! access token from `~/.sage/auth.json` does (same identity).
...
use url::Url;
use uuid::Uuid;
use sage_computer_hub_sdk::pool::HubConnectionPool;
use sage_computer_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &sage_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<sage_computer_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "SageBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
            {"id": "SageBuild:read_file"},
        ],
    });
    let tools = harness
