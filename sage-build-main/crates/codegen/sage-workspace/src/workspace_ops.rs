...
use sage_computer_hub_sdk::ToolHarness;
use sage_tools::types::output::ToolRunResult;
use sage_workspace_client::{WorkspaceClient, is_transport_fatal};
pub use sage_workspace_types::rpc::WorkspaceRpc;
pub use sage_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use sage_workspace_types::rpc::code_nav::{
...
pub use sage_workspace_types::rpc::fs::{
...
pub use sage_workspace_types::rpc::git::{
...
pub use sage_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use sage_workspace_types::rpc::hunks::{
...
pub use sage_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use sage_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use sage_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use sage_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use sage_workspace_types::rpc::worktree::{
...
fn hunk_line_info_to_wire(info: &sage_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: sage_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use sage_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &sage_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: sage_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use sage_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: sage_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(entry: sage_hunk_tracker::FileContentEntry) -> FileContentEntryWire {
...
fn session_stats_to_wire(stats: &sage_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: sage_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: sage_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> sage_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => sage_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => sage_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<sage_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = sage_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                sage_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &sage_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<sage_hooks::discovery::HookRegistry> {
    let value = serde_json::to_value(wire).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
    let mut registry: sage_hooks::discovery::HookRegistry =
        serde_json::from_value(value).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<sage_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<sage_codebase_graph::QueryResult, sage_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<sage_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](sage_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: sage_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<sage_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](sage_tools::registry::types::FinalizedToolset)
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(sage_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = sage_tool_protocol::ToolId::new(name).map_err(|e| {
                    sage_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                        format!("invalid tool name: {e}"),
                    )
                })?;
                let mut ctx = sage_tool_runtime::ToolCallContext::default();
                ctx.call_id =
                    sage_tool_protocol::ToolCallId::new(call_id.to_owned()).unwrap_or(ctx.call_id);
...
mod tests {
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset = std::sync::Arc::new(
            sage_tools::registry::types::FinalizedToolset::empty_for_test(),
        );
    ...
    fn hunk_to_wire_serializes_identically() {
        use sage_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use sage_hunk_tracker::FileContentEntry;
        use sage_hunk_tracker::types::FileContentView;
    ...
    fn session_summary_to_wire_serializes_identically() {
        use std::sync::Arc;
        use sage_hunk_tracker::SessionSummary;
        use sage_hunk_tracker::types::{Hunk, HunkSource, TurnSummary};
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = sage_hooks::config::HookSpec {
            ...
            event: sage_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.sage/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
        };
        let mut registry = sage_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use sage_hooks::event::HookEventName as E;
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use sage_hooks::config::HookSpec;
