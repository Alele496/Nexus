...
/// Directory (under `<cwd>/.sage`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.sage/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
