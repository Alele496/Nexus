...
use std::sync::Arc;
use sage_computer_hub_mcp_adapter::McpBridgeHandle;
use sage_mcp::servers::McpState;
use sage_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use sage_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
use sage_hunk_tracker::HunkTrackerHandle;
use sage_tool_protocol::ToolId;
use sage_tool_runtime::WorkspaceViewerContext;
...
pub struct WorkspaceSession {
    ...
    ///
    /// [`HunkTrackerActor`]: sage_hunk_tracker::HunkTrackerActor
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, sage_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<sage_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<sage_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<sage_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<sage_hooks::error::HookError>,
    ...
    pub(crate) auth_provider: Option<sage_computer_hub_sdk::SharedAuthProvider>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<sage_tools::notification::types::ToolNotificationHandle>>,
    ...
    pub(crate) local_registry: sage_computer_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn sage_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `SAGE_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$SAGE_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<sage_home>/workspace` by default). The upload queue spills here.
    pub(crate) workspace_home: std::path::PathBuf,
    pub(crate) upload_queue: Option<std::sync::Arc<sage_file_utils::queue::UploadQueue>>,
    ...
    /// (`SAGE_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](sage_file_utils::events::EventWriter::noop) and
    ...
    /// enabled (`SAGE_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, sage_file_utils::events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<sage_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<sage_file_utils::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](sage_file_utils::events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> sage_file_utils::events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<sage_file_utils::events::EventWriter> {
    ...
    pub fn auth_provider(&self) -> Option<&sage_computer_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](sage_computer_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<sage_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, sage_file_utils::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> sage_file_utils::events::EventWriter {
...
mod tests {
    ...
    use dashmap::DashMap;
    use sage_file_utils::events::{Event, EventWriter};
