...
//!
//! Persists per-folder trust decisions to `~/.sage/trusted_folders.toml`.
...
//!
//! The store is rooted at [`sage_config::user_sage_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.sage`) when
//! neither `$SAGE_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.sage/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.sage/`.
...
impl TrustStore {
    /// Load the trust store from `<user_sage_home>/trusted_folders.toml`.
    ///
    ...
    /// Default on-disk path: `<user_sage_home>/trusted_folders.toml`, or `None`
    /// when no user home resolves.
    ///
    /// Resolves via [`sage_config::user_sage_home`], never
    /// [`sage_config::sage_home`], so it never falls back to a cwd-relative
    /// `./.sage` — that fallback would let an untrusted cloned repo's `.sage`
    /// masquerade as the user-global store and self-trust the checkout.
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(sage_config::user_sage_home())
    }
    ...
    fn default_path_in(user_sage_home: Option<PathBuf>) -> Option<PathBuf> {
        Some(user_sage_home?.join(TRUST_FILE_NAME))
    }
...
/// ROOT (via the `~/.sage/worktrees.db` registry), so every `sage -w` worktree
...
/// sharing `~/.sage/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.sage/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = sage_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match sage_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.sage");
        assert_eq!(
        ...
        // cwd-relative `./.sage` that sage_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.sage/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
        assert_eq!(TrustStore::default_path_in(None), None);
    }

    #[test]
    fn default_path_sources_from_user_sage_home() {
        // Thin source-pin: the production accessor reads user_sage_home()
        // (Option, no cwd fallback), not sage_home(). The real regression guard
        // is the seam test above (default_path_in(None) == None).
        assert_eq!(
            TrustStore::default_path(),
            sage_config::user_sage_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    /// Point `SAGE_HOME` at an isolated tempdir and register one sage-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `SAGE_HOME` on
    /// drop (before releasing the lock), so the caller may bind it any way.
    fn register_sage_worktree(
        temp: &tempfile::TempDir,
        name: &str,
        source_repo: &Path,
        creation_mode: &str,
    ) -> (LockedTestEnv, PathBuf) {
        use sage_fast_worktree::{WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
    ...
    #[test]
    fn workspace_key_collapses_standalone_sage_worktree_onto_source_repo() {
        ...
        let (_env, wt) = register_sage_worktree(&temp, "wt", &source_repo, "standalone");
    ...
    fn workspace_key_collapses_worktree_onto_source_repo_git_root() {
        ...
        let (_env, wt) = register_sage_worktree(&temp, "wt", &subdir, "standalone");
    ...
    fn workspace_key_ignores_registry_for_cwd_outside_worktrees_dir() {
        ...
        // `<sage_home>/worktrees`: `worktree_record_for_cwd` skips the registry
        ...
        let (_env, _wt) = register_sage_worktree(&temp, "wt", &source_repo, "standalone");
        ...
        assert_eq!(
            ...
            "a cwd outside <sage_home>/worktrees keys on its own repo root"
        );
        assert_ne!(
            key,
