...
use sage_fast_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};
...
pub(crate) fn to_creation_mode(t: WorktreeType) -> sage_fast_worktree::CreationMode {
    match t {
        WorktreeType::Linked => sage_fast_worktree::CreationMode::Linked,
        WorktreeType::Standalone => sage_fast_worktree::CreationMode::Standalone,
        WorktreeType::Git => sage_fast_worktree::CreationMode::GitCheckout,
    }
}
...
/// `worktrees.db` (`sage_fast_worktree::resolve_sage_home`), so checkout dirs and
/// the metadata DB always live under the same `.sage` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `sage_config::sage_home()`,
/// so worktree paths also agree with trust/hooks and other sage-home paths.
fn sage_home() -> std::path::PathBuf {
    sage_fast_worktree::resolve_sage_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.sage/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    sage_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.sage/worktrees/<repo_name>`)
/// for a given source path, correctly handling sage-managed worktrees.
///
/// When `source_path` is already under `~/.sage/worktrees/<repo>/...`, the
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = sage_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.sage/worktrees/`) to its registered
/// worktree record.
///
...
/// returns `None` (without DB I/O) for paths outside `~/.sage/worktrees/`.
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Map WorktreeCopyMode to sage_fast_worktree::WorkingTreeMode
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(sage_fast_worktree::WorktreeKind::Session)
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        sage_fast_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        sage_fast_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        sage_fast_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        sage_fast_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        sage_fast_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
///
/// When the source path is already inside `~/.sage/worktrees/<repo>/`, the
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        sage_fast_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
    ...
    let builder_result = {
        ...
        tokio::task::spawn_blocking(move || {
            let mut builder = WorktreeBuilder::new(&source_worktree_path, &dest_path)
                ...
                .ignored_files_mode(IgnoredFilesMode::Skip)
                .creation_mode(to_creation_mode(creation_mode))
                .worktree_kind(sage_fast_worktree::WorktreeKind::Fork)
                .session_id(session_id_for_builder)
                .metadata(label_metadata);
...
use sage_fast_worktree::{
    DbStats, GcOptions, GcReport, ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord,
    gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_sage_home,
};
...
pub fn worktree_db_rebuild() -> Result<sage_fast_worktree::RebuildReport> {
    let home = resolve_sage_home()?;
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = sage_fast_worktree::ListFilter {
...
/// Scan `~/.sage/worktrees/<repo_name>/` for subdirectories not tracked
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[sage_fast_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
...
mod tests {
    ...
    /// Point `SAGE_HOME` at an isolated tempdir (`resolve_sage_home` re-reads
    ...
    /// and restores `SAGE_HOME` on drop (before releasing the lock), so the
    ...
    fn worktree_db_fixture(
        ...
        let env = LockedTestEnv::lock().set("SAGE_HOME", &home);
        ...
        let record = WorktreeRecord {
            ...
            status: sage_fast_worktree::WorktreeStatus::Alive,
