...
pub struct AnimationConfig {
    ...
    /// Also enabled by the `SAGE_FPS=1` env var. Default: false.
    pub show_fps: bool,
}
...
pub struct RawAnimationConfig {
    ...
    /// Requires a debug build. Also enabled by SAGE_FPS=1 env var. Default: false.
    pub show_fps: bool,
}
...
impl RawAppearanceConfig {
    pub fn to_toml_with_comments() -> String {
        ...
        let pager_path = crate::util::display_user_sage_path("pager.toml");
        let header = format!(
            "\
...
pub fn persist_respect_manual_folds(enabled: bool) -> std::io::Result<()> {
    ...
    if sage_config::user_sage_home().is_none() {
