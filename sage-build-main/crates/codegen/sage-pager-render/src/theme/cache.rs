...
//! lookup must be cheaper than re-loading from `~/.sage/config.toml`.
...
//! `sage_shell::util::config::set_theme()` (and friends), invoked
...
///
/// On the first call, reads from `~/.sage/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    sage_markdown::set_color_level_cap(if locked {
        sage_markdown::ColorLevel::Basic
    } else {
        sage_markdown::ColorLevel::TrueColor
    });
}
...
///
/// Precedence:
/// 1. Environment variable (`SAGE_THEME`)
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `sage_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = sage_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = sage_config::load_effective_config_disk_only() else {
