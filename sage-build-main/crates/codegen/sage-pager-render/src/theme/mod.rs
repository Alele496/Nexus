...
/// `"sagenight"` → `"Sage Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "sagenight" => "Sage Night",
        "sageday" => "Sage Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.sage/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `SAGE_FORCE_COLOR_LEVEL=basic`),
...
pub fn apply_cursor_color() {
    ...
    sage_shared::stderr::with_locked_stderr(|stderr| {
