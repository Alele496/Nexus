...
pub use sage_config::sage_home;

/// Path to `$SAGE_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    sage_home().join("pager.toml")
}

/// User-facing label for the user sage directory (``~/.sage`` or ``$SAGE_HOME``).
///
/// Derived from resolved [`sage_home()`] vs `sage_config::default_sage_home()`,
/// not from whether `SAGE_HOME` is set in the environment.
pub fn display_sage_home_prefix() -> String {
    if sage_home() == sage_config::default_sage_home() {
        "~/.sage".to_string()
    } else {
        "$SAGE_HOME".to_string()
    }
...
/// User-facing path under [`sage_home()`], e.g. ``~/.sage/config.toml``.
pub fn display_user_sage_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_sage_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`sage_home()`], then `$HOME`.
...
/// True when `path` is under user [`sage_home()`] (not project `{cwd}/.sage`).
pub fn is_under_user_sage_home(path: &Path) -> bool {
    path.starts_with(sage_home())
}
...
mod tests {
    ...
    #[test]
    fn display_sage_home_prefix_default_install() {
        if std::env::var("SAGE_HOME").is_ok() {
            return;
        }
        assert_eq!(display_sage_home_prefix(), "~/.sage");
    }

    #[test]
    fn display_user_sage_path_joins_relative() {
        let path = display_user_sage_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".sage") || path.contains("$SAGE_HOME"));
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.sage/memory/MEMORY.md");
