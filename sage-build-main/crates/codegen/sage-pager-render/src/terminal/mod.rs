...
pub enum TerminalName {
    ...
    JetBrains,
    /// Sage Desktop (Electron app).
    #[strum(to_string = "Sage Desktop")]
    GrokDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` sage is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches sage, which
    ...
    pub fn telemetry_snapshot(&self) -> sage_telemetry::events::TerminalTelemetry {
        ...
        sage_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: sage_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> sage_shared::session::FeedbackTerminalInfo {
        use sage_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(sage_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `sage ssh` can
    // forward env vars.
...
/// `sage-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
///
/// Parsed from `[terminal] alt_screen` in `~/.sage/pager.toml` and
