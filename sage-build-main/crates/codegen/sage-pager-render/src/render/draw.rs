...
//! [`sage_ratatui_inline::Terminal`] whose `flush()` returns `bool` indicating
...
use sage_ratatui_inline::LinkSpan;
...
pub type PagerTerminal = sage_ratatui_inline::Terminal<CrosstermBackend<TermWriter>>;
#[derive(Debug)]
...
pub fn spawn_writer_thread() -> (
    ...
    let test_delay = std::env::var("SAGE_TEST_FRAME_WRITE_DELAY_MS")
        .ok()
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || -> std::io::Result<()> {
            ...
            let mut writer: Box<dyn std::io::Write> = {
                let tui_out = sage_tty_utils::dup_tui_stderr().unwrap_or_else(|_| {
            ...
            while let Ok(payload) = rx.recv() {
                ...
                let result = {
                    let _guard = sage_shared::stderr::stderr_lock();
