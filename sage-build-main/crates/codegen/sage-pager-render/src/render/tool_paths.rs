...
fn resolve_tool_path_with_home(
    ...
    let display_path = target
        .as_deref()
        .map(sage_paths::normalize_lexically)
        .unwrap_or_else(|| PathBuf::from(path));
    let relative_to_cwd = target.as_ref().and_then(|_| {
        let cwd = sage_paths::normalize_lexically(cwd?);
...
mod tests {
    ...
    fn expanded_surface_uses_worktree_cwd() {
        let cwd = Path::new("/Users/me/.sage/worktrees/foo");
        let path = "/Users/me/.sage/worktrees/foo/crates/x/a.rs";
        assert_eq!(
    ...
    fn home_relative_target_preserves_filesystem_spelling_for_io() {
        ...
        assert_eq!(
            ...
            sage_paths::normalize_lexically(&home.join("notes.md"))
        );
    }
