...
//! `.nexus-plugin/plugin-index.json` (preferred), then
...
use nexus_hooks_plugins_types::PluginComponents;
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_nexus_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".nexus-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_nexus_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".nexus-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".nexus-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
    ...
    fn load_catalog_ignores_unknown_fields() {
        ...
        write_catalog(
            ...
            ".nexus-plugin",
            r#"{
                "$schema": "https://sage.local/plugin-index.schema.json",
                "version": 1,
