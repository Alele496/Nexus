...
//!
//! Catalog file lookup, in order: `.nexus-plugin/marketplace.json` (preferred),
//! `.nexus-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.nexus-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.nexus-plugin/plugin.json`
...
mod tests {
    ...
    fn load_index_valid_nexus_dir() {
        ...
        assert_eq!(result.unwrap().name, "sage");
    }

    #[test]
    fn load_index_nexus_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".nexus-plugin", "sage"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "sage");
    }
