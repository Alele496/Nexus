...
// (`~/.nexus/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_nexus_home>/trusted-hook-projects`), or `None` when no user sage home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(nexus_config::user_nexus_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user sage home (set $NEXUS_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$NEXUS_HOME/disabled-hooks`, or `None` when no user sage
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(nexus_config::user_nexus_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        ...
        nexus_dir.join("trusted-hook-projects")
    }
