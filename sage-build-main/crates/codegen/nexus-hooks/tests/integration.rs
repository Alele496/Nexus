//! Integration tests for sage-hooks.
//!
...
use nexus_hooks::discovery::load_hooks;
use nexus_hooks::dispatcher;
use nexus_hooks::event::*;
use nexus_hooks::result::HookDecision;
use nexus_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        nexus_hooks::result::HookRunResult::Success { .. }
    ));
}
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$NEXUS_HOOK_EVENT" > {f}; echo "NAME=$NEXUS_HOOK_NAME" >> {f}; echo "SESSION=$NEXUS_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`NEXUS_HOOK_EVENT`, `NEXUS_HOOK_NAME`,
/// `NEXUS_SESSION_ID`, `NEXUS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$NEXUS_HOOK_EVENT" > {f}; echo "NAME=$NEXUS_HOOK_NAME" >> {f}; echo "SESSION=$NEXUS_SESSION_ID" >> {f}; echo "ROOT=$NEXUS_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "NEXUS_HOOK_EVENT": "spoofed_event",
                                "NEXUS_HOOK_NAME": "spoofed_name",
                                "NEXUS_SESSION_ID": "spoofed_session",
                                "NEXUS_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "NEXUS_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed NEXUS_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "NEXUS_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed NEXUS_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "NEXUS_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed NEXUS_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use nexus_hooks::result::HookRunResult;
    let info = match run {
