//! Leader-mode (`sage agent --leader stdio`) test harness.
//!
...
use sage_acp_lib::LineBufferedRead;

use crate::env::sage_binary;
...
/// (version-skew) test. Falls back to [`sage_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "SAGE_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`sage_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "SAGE_BINARY_CLIENT";
...
/// (`SAGE_BINARY_LEADER`, else the shared [`sage_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`SAGE_BINARY_CLIENT`,
/// else the shared [`sage_binary`] resolution).
...
impl acp::Client for LeaderAcpClient {
    ...
    async fn ext_notification(&self, args: acp::ExtNotification) -> acp::Result<()> {
        if &*args.method == "sage.local/leader_reconnected" {
            self.capture
                .reconnected_count
                .fetch_add(1, Ordering::SeqCst);
        }
...
/// A `sage agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&sage_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export SAGE_* vars
            // (e.g. SAGE_LEADER_SOCKET pointing at a REAL leader on this
            // machine). env_clear + explicit allowlist guarantees the test
            // can never touch a leader outside its sandbox home.
            .env_clear()
            .env("PATH", std::env::var("PATH").unwrap_or_default())
            .env("HOME", home)
            .env("SAGE_HOME", home.join(".sage"))
            ...
            .env("SAGE_LEADER_SOCKET", home.join(".sage").join("leader.sock"))
            .env("SAGE_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("SAGE_XAI_API_BASE_URL", server.url())
            .env("XAI_API_KEY", "test-key-for-ci")
            .env("SAGE_TELEMETRY_ENABLED", "false")
            .env("SAGE_FEEDBACK_ENABLED", "false")
            .env("SAGE_TRACE_UPLOAD", "false")
            .env("SAGE_INSTRUMENTATION", "disabled")
            // Inherited by the spawned leader, whose stderr goes to
            // ~/.sage/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "sage_shell=debug");
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".sage").join("leader.lock")
}
...
///
/// The `sage.local/leader_reconnected` ext notification is dropped by the typed
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".sage").join("leader.log")).unwrap_or_default()
}
