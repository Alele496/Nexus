...
fn local_sage_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("sage-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_sage_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = Command::new(&cargo)
        ...
        .args(["build", "-p", "sage-pager", "--bin", "sage-pager"])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build sage-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build sage-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "sage-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve sage binary: `SAGE_BINARY` env (CI) or a locally built `sage-pager` binary.
pub fn sage_binary() -> PathBuf {
    if let Ok(path) = std::env::var("SAGE_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "SAGE_BINARY does not exist: {}", p.display());
        return p;
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_sage-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_sage_binary_path();
    ensure_local_sage_binary(&binary);
    binary
}
...
/// Point sage at the mock server with a fake API key and telemetry disabled.
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        // HOME alone does not sandbox sage on Windows: the product resolves
        // `~` via `USERPROFILE`/Known Folders (`std::env::home_dir()`), so
        // without an explicit SAGE_HOME every spawned child shares the real
        // `%USERPROFILE%\.sage` — test 1's models_cache.json (which embeds
        ...
        .env("SAGE_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("SAGE_XAI_API_BASE_URL", mock_url)
        ...
        .env("SAGE_DISABLE_AUTOUPDATER", "1");
}
