...
//! `.sage`, `.cursor`, and `.claude` vendor dirs, spawns a real `sage agent
//! stdio` process against the mock inference server (toggling the
//! `SAGE_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
...
const MARKER_SAGE_SKILL: &str = "ZZ_SAGE_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".sage", "sage-skill", MARKER_SAGE_SKILL);
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_SAGE_SKILL),
...
/// `SAGE_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; sage stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("SAGE_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_SAGE_SKILL),
...
/// `SAGE_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; sage stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("SAGE_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_SAGE_SKILL),
            "sage-skill always present"
        );
...
/// `SAGE_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("SAGE_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `SAGE_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("SAGE_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `SAGE_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("SAGE_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `SAGE_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("SAGE_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("SAGE_CURSOR_SKILLS_ENABLED", "false"),
            ("SAGE_CURSOR_RULES_ENABLED", "false"),
            ("SAGE_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_SAGE_SKILL), "sage always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("SAGE_CLAUDE_SKILLS_ENABLED", "false"),
            ("SAGE_CLAUDE_RULES_ENABLED", "false"),
            ("SAGE_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
