...
use sage_acp_lib::{AcpAgentGatewaySender, AcpClientMessage};
...
/// Shared `SAGE_HOME` for the entire test binary. The `OnceLock` in
...
async fn dont_ask_policy_denies_without_prompting() {
    use sage_workspace::permission::types::{PermissionConfig, PromptPolicy};
