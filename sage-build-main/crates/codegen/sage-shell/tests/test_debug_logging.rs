...
//! caller-owned `$SAGE_HOME`, then inspects `~/.sage/debug/`:
...
//!   `~/.sage/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `SAGE_LOG_SAMPLING`/`SAGE_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `SAGE_DEBUG_LOG=1`) writes a per-session
//!   `~/.sage/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.sage/debug/` files).
//! - `SAGE_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
/// The per-session firehose directory under a pinned `$SAGE_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".sage").join("debug")
}

/// List firehose `*.txt` files under `~/.sage/debug` (excluding the `latest.txt`
...
/// Build a headless `sage -p` command with a pinned `$SAGE_HOME` so the firehose
/// lands under `<home>/.sage/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    sage_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home);
    // Pin the home location and drop inherited firehose toggles for determinism.
    cmd.env("SAGE_HOME", home.join(".sage"));
    cmd.env_remove("SAGE_DEBUG_LOG");
    cmd.env_remove("SAGE_LOG_FILE");
    cmd.env_remove("SAGE_LOG_SAMPLING");
    cmd.env_remove("SAGE_HOOKS_LOG");
    cmd
}
...
/// A live `agent` session writes `~/.sage/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let home = TempDir::new().expect("create temp home");
        let sage_home = home.path().join(".sage");
        let sage_home_str = sage_home.to_string_lossy().into_owned();

        let client = GrokStdioClient::spawn_with_home_and_env(
            &server,
            workdir.path(),
            home,
            &[("SAGE_DEBUG_LOG", "1"), ("SAGE_HOME", &sage_home_str)],
        )
        .await;
        ...
        let session_file = sage_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.sage/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = sage_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `SAGE_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `SAGE_LOG_SAMPLING`/`SAGE_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // clears SAGE_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.sage/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // SAGE_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = sage_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.sage/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
...
/// `SAGE_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn sage_log_file_explicit_path_is_written() {
    let server = MockInferenceServer::start()
        .await
        .expect("start mock server");
    let workdir = git_workdir();
    ...
    cmd.env("SAGE_LOG_FILE", &custom);
    let result = run_headless_with_cmd(cmd).await;

    assert_headless_success(&result, "sage SAGE_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit SAGE_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "SAGE_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
