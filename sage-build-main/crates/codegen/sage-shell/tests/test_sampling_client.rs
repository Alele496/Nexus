...
use sage_shell::sampling::{
    ApiBackend, Client, ConversationItem, ConversationRequest, ConversationToolChoice,
    SamplingError, ToolCall, ToolSpec, rs,
};
use sage_shell::session::storage::JsonlStorageAdapter;
use sage_test_support::sse::responses_api_reasoning_and_text_events;
use sage_test_support::{MockInferenceServer, ScriptedResponse, SseEvent};
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use sage_shell::sampling::rs::ResponseStreamEvent;
        match event {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        if let ResponseStreamEvent::ResponseCompleted(completed) = event {
            for output in completed.response.output {
                use sage_shell::sampling::rs::OutputItem;
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use sage_shell::sampling::rs::OutputMessageContent;
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    use sage_sampling_types::doom_loop::{DOOM_LOOP_CHECK_EVENT_TYPE, SAMPLE_CHECK_EVENT_DATA};
