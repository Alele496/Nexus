...
//! 4. Smoke-validate session JSON under `$SAGE_HOME/sessions/` after exit.
//!
//! Requires a built `sage` binary (`SAGE_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p sage-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p sage-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use sage_agent::plugins::SharedPluginRegistryHandle;
use sage_agent::plugins::discovery::DiscoveryConfig;
use sage_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _sage_guard = EnvVarGuard::set("SAGE_HOME", &sage_home);
    ...
    let mut registry = InstallRegistry::empty(sage_home.join("installed-plugins"));
    ...
    // sage_home() caches the first SAGE_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        sage_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built sage binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    let home = dunce::canonicalize(home_tmp.path()).unwrap();
    let sage_home = home.join(".sage");
    std::fs::create_dir_all(&sage_home).unwrap();
    ...
    // The spawned binary gets HOME/SAGE_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via sage_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _sage_guard = EnvVarGuard::set("SAGE_HOME", &sage_home);
    ...
    // Smoke: session storage under SAGE_HOME/sessions has JSON artifacts.
