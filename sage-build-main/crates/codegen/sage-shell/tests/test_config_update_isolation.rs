...
/// All tests share this as SAGE_HOME (the `OnceLock` in sage-config
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("SAGE_HOME", &path) };
        path
    })
}
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = sage_shell::config::load_effective_config().unwrap();
    let effective_cfg = sage_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    sage_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = sage_shell::util::config::load_config_from_toml(&user_toml);
