...
fn invalidate_models_cache(home: &std::path::Path) {
    let cache = home.join(".sage").join("models_cache.json");
    if cache.exists() {
...
#[tokio::test]
#[ignore]
async fn test_default_model_uses_sage_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Sage") || sys_prompt.contains("sage"),
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_grok =
            original_sys_prompt.contains("Sage") || original_sys_prompt.contains("sage");
        let resumed_has_grok =
            resumed_sys_prompt.contains("Sage") || resumed_sys_prompt.contains("sage");
        assert_eq!(
            original_has_grok,
            resumed_has_grok,
...
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_sage_build() {
    with_local_set(|| async {
...
/// The `SAGE_AGENT` escape hatch should override the model's agent_type.
/// Setting `SAGE_AGENT=sage-build` with an alternate-agent model should use
/// sage-build harness.
#[tokio::test]
#[ignore]
async fn test_sage_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            sage_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("SAGE_AGENT", "sage-build");
            ...
            let incoming = sage_acp_lib::LineBufferedRead::spawn_local(incoming);
            ...
            assert!(
                sys_prompt.contains("Sage") || sys_prompt.contains("sage"),
                "SAGE_AGENT=sage-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
