...
//! - `SAGE_BINARY_LEADER` — the binary that elects the initial leader
...
//! - `SAGE_BINARY_CLIENT` — the second client (typically a freshly built main).
//!
...
//!
//! ```bash
//! SAGE_BINARY_LEADER=/path/to/sage-old SAGE_BINARY_CLIENT=/path/to/sage-new \
...
fn skew_binaries() -> Option<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    if old == new {
        eprintln!(
            "SKIP: SAGE_BINARY_LEADER/SAGE_BINARY_CLIENT resolve to the same binary ({})",
            old.display()
        );
        return None;
    }
...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".sage").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
...
#[tokio::test]
#[ignore = "two-binary version-skew test; set SAGE_BINARY_LEADER/SAGE_BINARY_CLIENT and run with --ignored"]
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let control = LeaderClient::connect(
                home.path().join(".sage").join("leader.sock"),
                "sage-pager-update",
                ClientMode::Stdio,
...
async fn eviction_leaves_single_leader_and_single_auth_owner() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let auth_path = home.path().join(".sage").join("auth.json");
