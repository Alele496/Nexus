...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // GrokStdioClient sets HOME=<temp>; the binary uses <HOME>/.sage as SAGE_HOME.
        let sage_home = shared_home.path().join(".sage");
        let session_dir = locate_session_dir(&sage_home, session_id.0.as_ref());
