...
//! ```
//!
//! In CI, set `SAGE_BINARY` to point at the release artifact:
//! ```bash
//! SAGE_BINARY=./artifacts/sage-0.1.159-linux-x86_64 \
//!   cargo test -p sage-shell --test test_built_binary_e2e -- --ignored
//! ```
...
use sage_test_support::env::test_env_cmd_tokio;
use sage_test_support::*;
...
async fn sage_build_server() -> MockInferenceServer {
    MockInferenceServer::start_with_models(vec![
...
async fn run_headless_with_env(
    ...
    let mut cmd = tokio::process::Command::new(sage_binary());
    cmd.args(args)
        .current_dir(cwd)
...
async fn test_version_with_crash_handler_exits_zero() {
    ...
    let output = Command::new(&binary)
        .arg("--version")
        .env("SAGE_CRASH_HANDLER", "1")
        .output()
        .unwrap_or_else(|e| panic!("failed to run {}: {e}", binary.display()));

    assert!(
        output.status.success(),
        "sage --version with SAGE_CRASH_HANDLER=1 failed (exit {:?}):\n{}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_headless_tools_allowlist_keeps_enabled_web_tools() {
    let server = sage_build_server().await;
...
async fn test_headless_json_reports_server_cost() {
    use sage_test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        sage_test_support::scripted::ScriptedResponse::sse(vec![
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        sage_test_support::scripted::ScriptedResponse::sse(
            sage_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
/// Isolated headless run with a custom `~/.sage/`. Clean env (no leaked
/// host credentials). Write config files into `sage_dir()` before `run()`.
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        std::fs::create_dir_all(home.path().join(".sage")).unwrap();
        Self {
            home,
            workdir: git_workdir(),
            env: vec![
                ("SAGE_CLI_CHAT_PROXY_BASE_URL".into(), server.url()),
                ("SAGE_TELEMETRY_ENABLED".into(), "false".into()),
                ("SAGE_FEEDBACK_ENABLED".into(), "false".into()),
                ("SAGE_TRACE_UPLOAD".into(), "false".into()),
                ("SAGE_INSTRUMENTATION".into(), "disabled".into()),
                ("SAGE_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }

    fn sage_dir(&self) -> std::path::PathBuf {
        self.home.path().join(".sage")
    }
...
async fn test_headless_managed_config_byok_sends_authorized_requests() {
    ...
    std::fs::write(
        h.sage_dir().join("managed_config.toml"),
        format!(
            r#"
[endpoints]
...
"#,
    ...
    .unwrap();
    h.env("SAGE_TEST_BYOK_TOKEN", "test-byok-secret-token");
