...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    sage_config::MANAGED_CONFIG_FILENAME,
    sage_config::REQUIREMENTS_FILENAME,
    sage_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    sage_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            sage_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", sage_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            sage_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
fn read_active_team_auth() -> Option<GrokAuth> {
    let home = crate::util::sage_home::sage_home();
...
/// Base is 1s; `SAGE_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("SAGE_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Override with `SAGE_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("SAGE_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// Resolve deployment key from `SAGE_DEPLOYMENT_KEY` env var, then config files.
...
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("SAGE_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.sage/`, trying the
...
fn apply_fetched(
    ...
    let verified = if sage_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(sage_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        sage_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(sage_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            sage_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(sage_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &sage_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<sage_config::signed_policy::SignatureEnvelope> {
    use sage_config::signed_policy::now_unix;
    ...
    let claim = match sage_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &sage_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
fn bump_managed_rollback_floor() {
    ...
    if !sage_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::sage_home::sage_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            sage_config::bump_rollback_floor(&home);
        }
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    if source.is_some()
        && sage_config::signed_policy::verification_active()
