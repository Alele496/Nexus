...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.sage/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** sage process
    /// sharing the same `~/.sage` (the writer may also be this process;
...
/// Watches `~/.sage/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.sage/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.sage/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let sage_home_buf = sage_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(sage_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(sage_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(sage_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(sage_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %sage_home.display(),
                    error = %e,
                    "failed to watch sage home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.sage/config.toml` so the loop above
        // watches `<cwd>/.sage/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.sage/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.sage/config.toml` trigger a
    ...
    /// `<cwd>/.sage/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.sage/` does not yet
/// exist at session-open time, the `.sage/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.sage/` followed by a
/// write to `<cwd>/.sage/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let sage_dir = cwd.join(".sage");
    if let Err(e) = debouncer
        .watcher()
        .watch(&sage_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .sage directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let sage_dir = cwd.join(".sage");
    if let Err(e) = debouncer.watcher().unwatch(&sage_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .sage directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.sage/` hasn't
...
/// Watches skill directories (`~/.sage/skills/`, `<repo>/.sage/skills/`, etc.)
...
/// recursively: `sage_home` (`~/.sage`, or `$SAGE_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.sage` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, sage_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, sage_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".sage", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, sage_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(sage_home) {
        return true;
    }
...
/// `~/.sage` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](sage_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = sage_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &sage_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &sage_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only sage_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_sage_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let sage_home = home.join(".sage");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &sage_home, Some(home));

        // sage_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&sage_home));
        ...
        assert!(!g(&home.join("repo").join(".sage")));
    }

    /// Regression for the ~/.sage inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.sage/worktrees/`) must not
    ...
    /// A write to `<sage_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another sage process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.sage/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let sage_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_grok = cwd.path().join(".sage");
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.sage/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.sage/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".sage");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
