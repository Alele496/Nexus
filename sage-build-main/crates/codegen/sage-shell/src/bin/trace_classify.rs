...
//!       [--api-key <key> | $XAI_API_KEY | <sage-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use sage_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// has access to.
    #[arg(long, default_value = "sage-4.5")]
    model: String,
    ...
    /// `$XAI_API_KEY`, then `<sage-home>/auth.json` (`xai::api_key`
    ...
    /// shell uses (`$SAGE_HOME` or `~/.sage`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    sage_home: Option<PathBuf>,
}
...
mod tests {
    ...
    #[test]
    fn cli_sage_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.sage_home, Some(PathBuf::from("/tmp/scratch-sage")));
    }
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["sage-4.5"]);
