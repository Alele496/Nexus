...
/// Resolved absolute path to bash. On Unix uses the `sage_config` shell
/// resolution cascade (`$SAGE_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        sage_config::shell::unix_shell_path(sage_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
pub use sage_tools::util::pager_env;
