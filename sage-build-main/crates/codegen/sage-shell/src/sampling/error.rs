...
//!
//! The canonical error types now live in `sage_sampling_types::error`.
...
pub use sage_sampling_types::error::*;
...
pub const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Sage usage limit for now. Get SuperSage for much higher limits, or try again later: https://sage.local/supergrok?referrer=sage-build";
    ...
    match err {
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                sage_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: sage_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != sage_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == sage_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = sage_chat_state::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &sage_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn format_rate_limited_surfaces_nonempty_server_detail() {
        ...
        let team = "resource-exhausted: Too many requests for team abc. See https://console.sage.local/team/default/rate-limits.";
    ...
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        ...
        let prev_legacy = std::env::var("SAGE_CODE_XAI_API_KEY").ok();
        // SAFETY: serial_test ensures no concurrent env mutation.
        unsafe {
            std::env::remove_var("XAI_API_KEY");
            std::env::remove_var("SAGE_CODE_XAI_API_KEY");
            if let Some(k) = key {
        ...
        unsafe {
            ...
            if let Some(v) = prev_legacy {
                std::env::set_var("SAGE_CODE_XAI_API_KEY", v);
            }
        }
        if let Err(e) = result {
