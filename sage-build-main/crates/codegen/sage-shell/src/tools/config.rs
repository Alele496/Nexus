...
use sage_sampler::SamplerConfig;
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `SAGE_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `SAGE_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> sage_tools::implementations::sage_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("SAGE_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| sage_config::env_bool("SAGE_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `sage_tools::registry::types::ToolsetConfig` which holds
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "SageBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "SageBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"SageBuild:read_file"));
        assert!(ids.contains(&"SageBuild:search_replace"));
        assert!(ids.contains(&"SageBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"SageBuildHashline:hashline_read"));
        assert!(ids.contains(&"SageBuildHashline:hashline_edit"));
        assert!(ids.contains(&"SageBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            sage_agent::config::AgentDefinition::plan(),
            sage_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "SageBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"SageBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"SageBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"SageBuildHashline:hashline_edit"),
