...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.sage/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use sage_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::SageBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = sage_version::VERSION;
...
pub struct LeaderEnvUrls {
    pub sage_ws_url: String,
    pub sage_ws_origin: String,
}
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<SageBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(SageBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<SageBuildEnvironment> {
    let environments: &[SageBuildEnvironment] = &[SageBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<SageBuildEnvironment> {
    let environments: &[SageBuildEnvironment] = &[SageBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::sage_home::sage_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : sage_version::VERSION,
                            }
                        )),
                    );
                }
...
impl From<&crate::auth::SageComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::SageComConfig) -> Self {
        Self {
            sage_ws_url: c.sage_ws_url.clone(),
            sage_ws_origin: c.sage_ws_origin.clone(),
        }
    }
}
...
///
/// The `env_urls.sage_ws_url` determines which leader instance to connect to.
...
pub async fn connect_or_spawn(
    ...
    let mut lock = LeaderLock::new(&env_urls.sage_ws_url);
...
///
/// For a **managed install** — the running binary lives under `sage_home`
/// (e.g. `~/.sage/...`) — prefer the managed `~/.sage/bin/sage` symlink. After an
...
/// `sage_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `sage_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.sage/bin/sage` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::sage_home::sage_home())
}
fn resolve_binary_with_home(sage_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(sage_home, std::env::current_exe().ok())
}
/// Binary file name for the managed sage install (`sage` / `sage.exe`).
fn managed_sage_bin_name() -> &'static str {
...
fn resolve_binary_impl(
    ...
    let managed_bin = sage_home.join("bin").join(managed_sage_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, sage_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    cmd.arg("--sage-ws-url").arg(&env_urls.sage_ws_url);
    cmd.arg("--sage-ws-origin").arg(&env_urls.sage_ws_origin);
    ...
    let log_path = crate::util::sage_home::sage_home().join("leader.log");
    ...
    let leader_log = std::env::var("SAGE_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "sage_shell=info,sage_acp_lib=warn,sage_mcp=warn".into());
...
mod tests {
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            sage_ws_url: "wss://test.invalid".into(),
            sage_ws_origin: "https://test.invalid".into(),
        };
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_sage_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_sage_bin_name());
