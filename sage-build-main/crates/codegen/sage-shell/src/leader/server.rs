...
use sage_computer_hub_sdk::{AuthCredential, AuthIdentity, AuthProvider};
use sage_workspace::WorkspaceHandle;
...
const PROD_COMPUTER_HUB_URL: &str = "wss://computer-hub.sage.local/v1/tools";
...
async fn handle_workspace_start(
    ...
    let status_config = sage_workspace::StatusConfig::from_env();
    ...
    let upload_queue_enabled =
        std::env::var("SAGE_WORKSPACE_UPLOAD_QUEUE_ENABLED").as_deref() != Ok("false");
    ...
    let handle = sage_workspace::connect_local_workspace(
        cwd_path.clone(),
        url,
        auth,
        Some(metadata),
        ...
        match poll {
            ...
            LeaderServerPoll::Event(event) => match event {
                ServerEvent::Registered(id, mode, capabilities, client_type) => {
                    if let Some(client) = clients.get_mut(&id) {
                        ...
                        sage_telemetry::unified_log::info(
            ...
            LeaderServerPoll::Response(payload) => {
                    ...
                    && !clients.contains_key(&orphan_client)
                {
                    ...
                    sage_telemetry::unified_log::warn(
...
mod tests {
    ...
    async fn models_update_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_x.ai/models/update","params":{"currentModelId":"sage-new","availableModels":[{"modelId":"sage-new","name":"Sage New"}]}}"#;
    ...
    async fn mcp_servers_updated_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_x.ai/mcp/servers_updated","params":{"method":"x.ai/mcp/servers_updated","params":{"mcpServers":[{"name":"sage_com_slack","source":"managed"}]}}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
            got_a
                .as_deref()
                .is_some_and(|p| p.contains("sage_com_slack")),
