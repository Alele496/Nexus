...
fn load_filtered_marketplace_sources() -> Vec<sage_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                sage_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                sage_plugin_marketplace::SourceKind::Local { path } => {
...
async fn handle_action(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            for source in &sources {
                if let Some(ref filter) = source_url_or_path {
                    let identity = match &source.kind {
                        ...
                        sage_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
                    };
                    if &identity != filter {
                        continue;
                    }
                }
                if let sage_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
                    let cache_root = sage_plugin_marketplace::git::default_cache_root();
                    if let Err(e) = sage_plugin_marketplace::git::force_sync_source_cache(
                        url,
                        branch.as_deref(),
                        &cache_root,
                    ) {
            ...
            sage_hooks_plugins_types::ActionOutcome {
                status: sage_hooks_plugins_types::OutcomeStatus::Success,
                message: msg,
...
async fn handle_update(
    ...
    use sage_plugin_marketplace::installer;
    use sage_hooks_plugins_types::{ActionOutcome, OutcomeStatus};

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &sage_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            sage_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            sage_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match sage_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        sage_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = sage_plugin_marketplace::git::default_cache_root();
            match sage_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                sage_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = sage_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = sage_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = sage_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, sage_hooks_plugins_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
        ...
        Err(sage_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            sage_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match sage_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    sage_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
fn scan_source(
    source: &sage_plugin_marketplace::MarketplaceSource,
    ...
    let scan = sage_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = sage_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match sage_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: sage_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.sage/config.toml`.
async fn handle_add_source(url: &str) -> sage_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &sage_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if sage_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        sage_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
...
/// Remove a marketplace source from `~/.sage/config.toml` and uninstall all
/// plugins that were installed from it.
async fn handle_remove_source(source_url_or_path: &str) -> sage_hooks_plugins_types::ActionOutcome {
    ...
    match tokio::task::spawn_blocking(move || remove_source_locked(&src)).await {
        Ok(outcome) => outcome,
        Err(e) => sage_hooks_plugins_types::ActionOutcome {
            status: sage_hooks_plugins_types::OutcomeStatus::InternalError,
            message: format!("Config write task failed: {e}"),
            requires_reload: false,
            requires_restart: false,
        },
...
fn remove_source_locked(source_url_or_path: &str) -> sage_hooks_plugins_types::ActionOutcome {
    use crate::plugin;
    use sage_hooks_plugins_types::{ActionOutcome, OutcomeStatus};
    ...
    let is_official = sage_plugin_marketplace::is_official_source_url(source_url_or_path);
...
/// Acquire an advisory exclusive `flock` on `<sage_home>/.config-init.lock`,
...
fn acquire_init_lock(sage_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(sage_home);
    let lock_path = sage_home.join(".config-init.lock");
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a sage_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
pub fn purge_default_skills_installs(sage_home: &std::path::Path) {
    purge_default_skills_installs_impl(sage_home, || {
        sage_agent::plugins::install_registry::InstallRegistry::try_load_from(
            sage_agent::plugins::install_registry::InstallRegistry::resolve_install_dir(),
        )
    });
}
...
fn purge_default_skills_installs_impl(
    ...
    load_registry: impl FnOnce() -> Result<
        sage_agent::plugins::install_registry::InstallRegistry,
        sage_agent::plugins::install_registry::InstallError,
    >,
) {
    let config_path = sage_home.join("config.toml");
    ...
    let _lock = match acquire_init_lock(sage_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %sage_home.join(".config-init.lock").display(),
...
pub fn ensure_official_marketplace_source(sage_home: &std::path::Path) {
    let config_path = sage_home.join("config.toml");
    ...
    // (settings.json / known_marketplaces.json) under sage_home. Scoped to
    // sage_home only (not ~/.claude) to keep tests hermetic; a user with the URL
    // solely in ~/.claude gets one duplicate entry that the UI dedupes by URL.
    let toml_sources = sage_plugin_marketplace::load_sources(&parsed);
    let json_sources = sage_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&sage_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, sage_plugin_marketplace::SourceKind::Git { url, .. }
            if sage_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            sage_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                sage_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = sage_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<sage_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        sage_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            sage_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                sage_plugin_marketplace::SourceKind::Git { url, .. }
                    if sage_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            sage_plugin_marketplace::SourceKind::Git { url, .. }
                if url == sage_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                sage_plugin_marketplace::OFFICIAL_SOURCE_NAME,
                sage_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod default_skills_purge_tests {
    use super::*;
    use sage_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = sage_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
            ...
            components: Some(sage_hooks_plugins_types::PluginComponents {
                skills: vec![sage_hooks_plugins_types::ComponentItem::new(
                    "code-review",
