...
fn discover_auto_sources(cwd: &str, skills: &[SkillInfo]) -> Vec<(String, usize)> {
    ...
    for subdir in &subdirs {
        try_add_source(sage_home.join(subdir), None);
    }
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "sage.local/skills/add" => {
            ...
            .await
            {
                sage_telemetry::session_ctx::log_event(
                    sage_telemetry::events::SkillAdded {
                        added_count: 0,
                        total_skills: 0,
                        success: false,
                    },
            ...
            sage_telemetry::session_ctx::log_event(sage_telemetry::events::SkillAdded {
        ...
        "sage.local/skills/remove" => {
            ...
            {
                sage_telemetry::session_ctx::log_event(
                    sage_telemetry::events::SkillRemoved { success: false },
                );
            ...
            sage_telemetry::session_ctx::log_event(sage_telemetry::events::SkillRemoved {
                success: true,
            });
