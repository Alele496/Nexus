//! `sage.local/session/repair` — out-of-band recovery for sessions bricked by
...
//!
//! Repairs via [`sage_chat_state::compaction_utils::repair_history`]. Resident
...
use sage_chat_state::compaction_utils::HistoryRepairReport;
...
/// Response payload for `sage.local/session/repair`.
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "sage.local/session/repair" => handle_session_repair(agent, args).await,
...
/// back atomically. `sage_root` is injectable for tests.
async fn repair_on_disk(sage_root: &std::path::Path, session_id: &str, dry_run: bool) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &sage_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(sage_root.to_path_buf());
    ...
    let report = sage_chat_state::compaction_utils::repair_history(&mut chat_history);
