...
use sage_telemetry::id::agent_id;
...
/// hot-reload watcher when `<cwd>/.sage/config.toml`,
...
/// Hot-reload the model catalog from `~/.sage/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .sage/plugins under it, plus
