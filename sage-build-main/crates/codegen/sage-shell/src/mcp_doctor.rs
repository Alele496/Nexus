//! `sage mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = sage_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.sage/config.toml".to_string(),
...
// ── Managed (sage.local) server discovery ─────────────────────────

const MANAGED_SOURCE_LABEL: &str = "sage.local";
...
/// Discover managed `sage_com_*` servers if the user has xAI auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    let sage_home = sage_tools::util::sage_home::sage_home();
    let sage_com_config = SageComConfig::default();
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&sage_home, sage_com_config));
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `sage login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use sage_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `sage mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `sage mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `sage mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `sage mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
