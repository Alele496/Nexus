...
const SAGE_CODE_BACKEND_URL: &str = "https://code.sage.local";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const SAGE_CODE_WEB_URL: &str = "https://sage.local";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("SAGE_CODE_WEB_URL").unwrap_or_else(|_| SAGE_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("x-sage-client-version", sage_version::VERSION);
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::sage_auth_credentials::GrokAuthCredentials::new(
    ...
    sage_file_utils::trace_context::inject_trace_context_into_request(builder)
}
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn sage_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("SAGE_CODE_BACKEND_URL")
                .unwrap_or_else(|_| SAGE_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn sage_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    /// Must include X-XAI-Token-Auth so nginx auth subrequest routes to authenticate_xai_sage_cli_token.
    ...
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_static(sage_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = sage_file_utils::trace_context::inject_trace_context_into_request(
...
/// Fetch `sage_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(sage_telemetry::id::agent_id)
        .await
        .ok()?;
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| sage_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(sage_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
    ...
    fn inference_url_defaults_to_proxy() {
        let ep = endpoints("https://proxy.sage.local/v1", None, None);
        assert_eq!(ep.resolve_inference_base_url(), "https://proxy.sage.local/v1");
    }
    #[test]
    fn inference_url_uses_models_base_url() {
        let ep = endpoints(
            "https://proxy.sage.local/v1",
    ...
    fn list_url_defaults_to_proxy_models() {
        ...
        assert_eq!(
            ...
            "https://proxy.sage.local/v1/models"
        );
    }
    #[test]
    ...
    fn models_fetch_endpoint_matches_auth_mode() {
        ...
        assert_eq!(session.url, "https://cli-chat-proxy.sage.local/v1/models");
        ...
        assert_eq!(deployment.url, "https://cli-chat-proxy.sage.local/v1/models");
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("SAGE_DEPLOYMENT_KEY", "xai-token-ENTERPRISE") };
        ...
        assert_eq!(url, "https://cli-chat-proxy.sage.local/v1/deployment/config");
        assert!(
        ...
        unsafe { std::env::remove_var("SAGE_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
