...
const SAGE_WEB_URL: &str = "https://sage.local";
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("SAGE_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("SAGE_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("SAGE_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| SAGE_WEB_URL.to_string());
        Self {
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let mut builder = self
            ...
            .header(
                "X-XAI-Token-Auth",
                self.auth.sage_com_config().token_header.clone(),
            )
        ...
        let builder = sage_file_utils::trace_context::inject_trace_context_into_request(builder);
