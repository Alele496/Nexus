...
use crate::auth::{AuthManager, GrokAuth, SageComConfig};
use crate::remote::{FetchModelsResult, fetch_models_blocking};
use crate::sampling::SamplerConfig as SamplingConfig;
use globset::{Glob, GlobSet, GlobSetBuilder};
use sage_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
struct Inner {
    ...
    gateway: RwLock<Option<sage_acp_lib::AcpAgentGatewaySender>>,
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&sage_home, SageComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: sage_acp_lib::AcpAgentGatewaySender) {
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<sage_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<sage_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    ///
    /// `model_id` may be a routing slug (`config.model`, e.g. `sage-4.5`)
    /// OR a catalog key; the catalog map is keyed by the config key, which can
    /// differ from the slug for custom/enterprise ids (e.g. key `enterprise-sage-build`
    /// → slug `sage-4.5`). Resolve to the catalog key first so a slug
    ...
    /// Hot-reload the catalog from `~/.sage/models_cache.json` after an
    ...
    /// When another sage process sharing `~/.sage` (a `--no-leader` run, a
    /// newer client, sage-desktop) fetches a fresher `/v1/models` catalog and
    ...
    /// `ModelsCacheManager` path is fixed to `sage_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::sage_home::sage_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.sage_version.as_deref() != Some(sage_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            sage_version: Some(sage_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "Sage" both route to `sage-build`)
...
pub(crate) fn prefetch_models_and_settings_blocking(
    ...
    // Settings need a sage.local session; skip for BYOK.
...
fn resolve_prefetch_env(sage_com_config: Option<SageComConfig>) -> Option<PrefetchEnv> {
    let sage_home = crate::util::sage_home::sage_home();
    let auth_manager = AuthManager::new(&sage_home, sage_com_config.unwrap_or_default());
...
pub fn start_early_prefetch(sage_com_config: Option<SageComConfig>) -> Option<EarlyPrefetchHandle> {
    let env = resolve_prefetch_env(sage_com_config)?;
    Some(spawn_prefetch_thread(env))
}
...
///
/// Sessions persist the routing slug (`[model.X].model`, e.g. `sage-4.5`);
/// the catalog and `/model` picker use config keys (e.g. `enterprise-sage-build`).
...
pub fn resolve_model_catalog(
    ...
    // must not stamp `none` onto sage-4.5, which only offers low/medium/high).
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let tmp = std::env::temp_dir().join("sage-test-models-manager");
        let auth_manager = Arc::new(AuthManager::new(&tmp, SageComConfig::default()));
        ModelsManager::new(
            None,
            IndexMap::new(),
            acp::ModelId::new("default"),
    ...
    async fn set_current_model_id_change_fires_watch_to_all_subscribers() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("sage-4"));
    ...
    async fn model_switch_generation_snapshot_reflects_current_state() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("sage-3"));
    ...
    fn current_reasoning_effort_seeded_from_config() {
        let tmp = std::env::temp_dir().join("sage-test-models-manager-seed");
        let auth_manager = Arc::new(AuthManager::new(&tmp, SageComConfig::default()));
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use sage_sampling_types::ReasoningEffortOption;
        ...
        let mut no_none = ModelEntry {
            info: config::ModelInfo::fallback("sage-4.5"),
            api_key: None,
            env_key: None,
            api_base_url: None,
        };
        ...
        prefetched.insert("sage-4.5".to_string(), no_none);
        ...
        let catalog = resolve_model_catalog(&cfg, Some(prefetched));
        assert_eq!(
            catalog["sage-4.5"].info.reasoning_effort,
            Some(ReasoningEffort::High),
    ...
    fn config_menu_only_model_derives_support_and_default() {
        ...
        let tmp = std::env::temp_dir().join("sage-test-models-manager-menu-only");
        let auth_manager = Arc::new(AuthManager::new(&tmp, SageComConfig::default()));
        let mgr = ModelsManager::new(
            None,
            catalog,
    ...
    fn first_apply_refresh_reselects_default_model() {
        let mgr = test_manager();
        let mut cfg = config::Config::default();
        cfg.models.default = Some("sage-3".to_string());

        assert!(!mgr.has_fetched_real_catalog());

        let prefetched = make_prefetched(&["sage-3", "sage-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);

        assert!(mgr.has_fetched_real_catalog());
        assert_eq!(mgr.current_model_id().0.as_ref(), "sage-3");
    }
    ...
    fn subsequent_refresh_reselects_when_model_removed() {
        ...
        // Second refresh with sage-4 removed.
        let prefetched = make_prefetched(&["sage-3", "sage-4.5"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
    ...
    fn apply_config_honors_new_preferred_model() {
        ...
        *mgr.inner.cfg.write() = stale_cfg;
        ...
        new_cfg.models.default = Some("sage-3".to_string());
    ...
    fn apply_config_falls_back_when_preferred_not_in_catalog() {
        ...
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("sage-nonexistent".to_string());
        mgr.apply_config(new_cfg);
    ...
    fn auth_refresh_then_config_reload_preserves_user_model() {
        ...
        *mgr.inner.etag.write() = None;
        ...
        let prefetched = make_prefetched(&["sage-3", "sage-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        assert_eq!(mgr.current_model_id().0.as_ref(), "sage-4");

        // Config reload with persisted preference.
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("sage-4".to_string());
        mgr.apply_config(new_cfg);
        assert_eq!(mgr.current_model_id().0.as_ref(), "sage-4");
    }
    ...
    fn reload_from_disk_cache_applies_external_catalog() {
        ...
        cache.persist(
            &make_prefetched(&["sage-4.5", "sage-4.3"]),
            Some("etag-ext"),
            auth_method,
            &mgr.cache_origin(),
        );
        ...
        assert!(mgr.has_fetched_real_catalog());
        assert!(mgr.models().contains_key("sage-4.5"));
        assert!(mgr.models().contains_key("sage-4.3"));
    ...
    fn reload_from_disk_cache_ignores_stale_cache() {
        ...
        let stale = ModelsCache {
            ...
            models: make_prefetched(&["sage-stale"]),
        };
        cache.atomic_write(&stale);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("sage-stale"));
    ...
    fn reload_from_disk_cache_ignores_auth_method_mismatch() {
        ...
        cache.persist(
            &make_prefetched(&["sage-other-auth"]),
            Some("etag-x"),
            other,
            &mgr.cache_origin(),
        );
        ...
        assert!(!mgr.models().contains_key("sage-other-auth"));
    }
    ...
    fn reload_from_disk_cache_ignores_origin_mismatch() {
        ...
        let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
        cache.persist(
            &make_prefetched(&["sage-other-origin"]),
            Some("etag-y"),
            auth_method,
        ...
        assert!(!mgr.models().contains_key("sage-other-origin"));
        assert!(mgr.inner.etag.read().is_none());
    }
    ...
    fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
        ...
        let legacy = ModelsCache {
            ...
            models: make_prefetched(&["sage-legacy"]),
        };
        cache.atomic_write(&legacy);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("sage-legacy"));
    }
    ...
    fn clear_resets_has_fetched_real_catalog() {
        ...
        let prefetched = make_prefetched(&["sage-4.5", "sage-4.3"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        let first_available = mgr.available().keys().next().unwrap().clone();
        assert_eq!(
            mgr.current_model_id().0.as_ref(),
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        ...
        let _unset_legacy = EnvGuard::unset("SAGE_CODE_XAI_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "sage-build", Some("Auto")),
            make_entry_config_with_id(Some("sage-build"), "sage-build", Some("Sage")),
            make_entry_config_with_id(
                Some("sage-composer-2.5-fast"),
                "sage-composer-2.5-fast",
                Some("Sage Fast"),
            ),
        ];
        ...
        assert!(map.contains_key("sage-build"));
        assert!(map.contains_key("sage-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "sage-build",
            "auto entry should still route to sage-build"
        );
        assert_eq!(map["sage-build"].info.model, "sage-build");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("sage-build"), "sage-build", Some("First")),
            make_entry_config_with_id(Some("sage-build"), "sage-build", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["sage-build"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        catalog.insert(
            "auto-sage-build".to_string(),
            make_model_entry("sage-build"),
        );
        catalog.insert("sage-build".to_string(), make_model_entry("sage-build"));

        let mut cfg = config::Config::default();
        cfg.models.default = Some("sage-build".to_string());

        let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
        assert_eq!(key, "sage-build", "must match id, not first slug hit");
    }
    ...
    fn resolve_catalog_key_maps_routing_slug_to_config_key() {
        ...
        models.insert(
            "enterprise-sage-build".to_string(),
            make_model_entry("sage-4.5"),
        );
        models.insert("sage-4.3".to_string(), make_model_entry("sage-4.3"));

        let persisted = acp::ModelId::new("sage-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "enterprise-sage-build");
    }

    #[test]
    fn resolve_catalog_key_prefers_exact_key_match() {
        let mut models = IndexMap::new();
        models.insert("sage-4.5".to_string(), make_model_entry("sage-4.5"));

        let persisted = acp::ModelId::new("sage-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("exact key must resolve");
        assert_eq!(key.0.as_ref(), "sage-4.5");
    }
    ...
    fn resolve_catalog_key_last_slug_match_wins() {
        let mut models = IndexMap::new();
        models.insert(
            "default-sage-build".to_string(),
            make_model_entry("sage-4.5"),
        );
        models.insert("user-sage-build".to_string(), make_model_entry("sage-4.5"));

        let persisted = acp::ModelId::new("sage-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "user-sage-build");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        let mut models = IndexMap::new();
        models.insert("sage-build".to_string(), make_model_entry("sage-build"));
        models.insert(
            "enterprise-sage-build".to_string(),
            make_model_entry("sage-build"),
        );
        ...
        let available = test_available_keys(&["enterprise-sage-build", "sage-4.3"]);

        let persisted = acp::ModelId::new("sage-build");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        let mut models = IndexMap::new();
        models.insert("sage-build".to_string(), make_model_entry("sage-4.5"));
        models.insert("other".to_string(), make_model_entry("sage-build"));

        let available = test_available_keys(&["sage-build", "other"]);

        let persisted = acp::ModelId::new("sage-build");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "sage-build");
    }
