...
pub const LEGACY_XAI_API_KEY_ENV_VAR: &str = "SAGE_CODE_XAI_API_KEY";

/// Read the API key from the environment.
///
/// Checks `XAI_API_KEY` first, then falls back to the legacy
/// `SAGE_CODE_XAI_API_KEY` for backward compatibility.
...
/// Returns `true` if either `XAI_API_KEY` or `SAGE_CODE_XAI_API_KEY` is set.
...
/// `SAGE_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
pub struct AuthMethodsBuildInputs<'a> {
    ...
    /// True if `sage_com_config.oauth2` is configured (e.g. `SAGE_OAUTH2_ISSUER`
    ...
    pub has_oauth2: bool,
    /// True if `sage_com_config.auth_provider_command` is configured (sets
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.sage_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.sage_com_config.oidc.as_ref().unwrap()`
        ...
        methods.push(sage_com_auth_method(login_label, has_auth_provider_command));
    }
}
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            ...
            SAGE_COM_METHOD_ID => Self::GrokCom,
...
pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `sage login`, set XAI_API_KEY, or add api_key to ~/.sage/config.toml.";
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.sage/auth.json".to_string())),
    )
}

pub const SAGE_COM_METHOD_ID: &str = "sage.local";

/// xAI OAuth2/OIDC auth. Method id `"sage.local"` kept for ACP wire-compat.
pub fn sage_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("Sage");
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(acp::AuthMethodId::new(SAGE_COM_METHOD_ID), name.to_string())
...
mod tests {
    ...
    #[test]
    fn after_cached_token_unavailable_falls_to_sage_com_without_api_key() {
        assert_eq!(
    ...
    #[test]
    fn fresh_user_only_advertises_sage_com_and_requires_login() {
    ...
    #[test]
    fn enterprise_oidc_replaces_sage_com_but_xai_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
    ...
    /// `~/.sage/config.toml` skeleton from the bug report, walks it through
    ...
    /// Legacy `SAGE_CODE_XAI_API_KEY` env var is accepted as a fallback
    ...
    /// When both `XAI_API_KEY` and `SAGE_CODE_XAI_API_KEY` are set,
    ...
    // When this token is present via the `SAGE_AUTH` env var (or via legacy
    ...
    // to-end. A regression in SAGE_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `SAGE_AUTH` env var, with no other auth
    ...
    fn sage_login_legacy_token_does_not_require_login() {
        ...
        // Provide it via SAGE_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("SAGE_AUTH", &legacy_json);
        ...
        assert!(
            current.is_some(),
            "legacy token in SAGE_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("SAGE_AUTH");
        let _g2 = EnvGuard::unset("SAGE_AUTH_PATH");
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, SAGE_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![SAGE_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
