...
impl RelayConfig {
    ...
    pub(crate) fn for_session(
        ...
        Some(Self {
            ws_url: ctx.sage_ws_url.clone(),
            ws_origin: ctx.sage_ws_origin.clone(),
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(sage_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                sage_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
