...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(sage_file_utils::trace_context::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = sage_agent::prompt::workspace_user::optional_workspace_user_dir();
...
/// ([`sage_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    sage_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
...
pub async fn run_stdio_agent(
    ...
    sage_telemetry::unified_log::set_version(sage_version::VERSION);
    ...
    sage_file_utils::queue::cleanup_orphaned_uploads(
        &sage_home::sage_home(),
        sage_file_utils::queue::DEFAULT_MAX_AGE,
    );
    ...
    if let Ok(version) = std::env::var("SAGE_CLIENT_VERSION") {
    ...
    // blocking stdin reads (see `sage_acp_lib::spawn_stdin_line_reader`): on
...
async fn run_headless_inner(
    ...
    let ctx = &agent_config.sage_com_config;
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        let auth_manager = Arc::new(AuthManager::new(&sage_home::sage_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
    ...
    let sage_code_url = format!("{}/build", ctx.sage_ws_origin);
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            let url_for_open = sage_code_url.clone();
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: sage_version::VERSION.to_string(),
    })
    ...
    local_set
        .run_until(async move {
            ...
            let auth_scope = agent_config.sage_com_config.auth_scope();
            // Gated on user_sage_home() so a cwd-relative .sage/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = sage_config::user_sage_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.sage/models_cache.json
...
mod tests {
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::SageComConfig {
            sage_ws_url: format!("ws://{addr}"),
            sage_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
