...
use sage_telemetry::id::{agent_id, agent_instance_id};
use sage_telemetry::session_ctx::log_event;
use sage_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use sage_workspace::permission::{ClientType, PermissionEvent};
...
use sage_workspace::session::git::GitDiscoveryResult;
use sage_hunk_tracker::HunkTrackerActor;
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        ...
        "SuperGrokPro" => jwt_claim == "supersage_heavy",
        "SuperGrokLite" => jwt_claim == "supersage_lite",
        _ => false,
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a sage_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[sage_announcements::RemoteAnnouncement]>,
    last_emitted: &[sage_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<sage_announcements::RemoteAnnouncement>> {
    let current = sage_announcements::filter_expired_at(
...
/// Override with `SAGE_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("SAGE_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
pub struct MvpAgent {
    ...
    /// list. Set by `enforce_sage_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.sage/`).
    ///
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<sage_codebase_graph::IndexManagerHandle>>,
    >,
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        sage_tools::implementations::sage_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: sage_tools::implementations::sage_build::task::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: sage_agent::plugins::SharedPluginRegistryHandle,
    ...
    workspace_ops: RefCell<Option<sage_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<sage_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &sage_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<sage_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !sage_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = sage_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use sage_agent::config::is_strict_harness_agent_type;
    match (
...
use sage_chat_state::conversation_util::replace_or_insert_system_head;
...
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<sage_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(sage_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<sage_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<sage_acp_lib::AcpResult<()>>,
        >,
        mark_replay: bool,
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<sage_acp_lib::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = sage_tools::types::TaskSnapshot {
    ...
    pub(super) async fn enforce_sage_code_access(&self, auth: &crate::auth::GrokAuth) {
        if !auth.is_xai_auth() {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings sage_build_access_gate)"
            );
    ...
    ///    against `~/.sage/bundled/` and the manifest.
...
/// absent — matching the `sage_build_access_gate` flag's server-side default.
///
/// Used by both `enforce_sage_code_access` (initial login gate) and
