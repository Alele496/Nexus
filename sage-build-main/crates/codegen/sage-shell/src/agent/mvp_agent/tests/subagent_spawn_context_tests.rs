...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    use sage_workspace::permission::types::{
    ...
    local
        .run_until(async {
            ...
            let cwd = sage_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                sage_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    sage_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                sage_workspace::permission::AccessKind::Read(Some(".env".into())),
                sage_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        sage_workspace::permission::Decision::PolicyDeny(_)
                    ),
