...
fn jwt_claim_matches_user_subscription_tier_known_pairs() {
    let cases = [
        ...
        ("supersage_heavy", "SuperGrokPro"),
        ("supersage_lite", "SuperGrokLite"),
    ];
...
mod hunk_tracking_mode {
    ...
    use sage_hunk_tracker::TrackingMode;
    #[test]
...
fn harness_pair(id: &str) -> Vec<sage_sampling_types::conversation::ConversationItem> {
    use sage_sampling_types::ToolCall;
    use sage_sampling_types::conversation::ConversationItem;
    vec![
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_sage_build() {
    let prev = std::env::var("SAGE_AGENT").ok();
    unsafe {
        std::env::remove_var("SAGE_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("SAGE_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return sage-build.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = sage_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `sage-build-plan`, models in the catalog that still declare
/// `agent_type = "sage-build"` explicitly must NOT preempt an ACP
/// profile. Any value in the `sage-build*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_sage_build_family() {
    let prev = std::env::var("SAGE_AGENT").ok();
    unsafe {
        std::env::remove_var("SAGE_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["sage-build", "sage-build-plan", "sage-build-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for sage-build family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("SAGE_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use sage_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".sage").join("agents");
    ...
    assert_eq!(
        def.model,
        sage_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("SAGE_AGENT", v) },
        None => unsafe { std::env::remove_var("SAGE_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `sage-build` (a client profile name) to
/// `sage-build-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("sage-build", "sage-build-plan"));
    assert!(harnesses_are_compatible("sage-build-plan", "sage-build"));
    assert!(harnesses_are_compatible("sage-build", "sage-build"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("sage-build-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "sage-build-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_sage_build_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "sage-build-plan"),
        "sage-build-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_sage_build() {
    assert_eq!(
        resolve_required_agent_type(None, "sage-build"),
        "sage-build"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "sage-build-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "sage-build-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "sage-build-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use sage_tools::computer::local::{LocalFs, LocalTerminalBackend};
    use sage_tools::notification::ToolNotificationHandle;
    use sage_tools::registry::types::SessionContext;
    let tmp = tempfile::tempdir().unwrap();
    ...
    let builder = sage_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: sage_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: sage_tools::implementations::sage_build::image_gen::ImageGenConfig::default(),
        video_gen_config: sage_tools::implementations::sage_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: sage_tools::implementations::sage_build::deploy_app::AppBuilderDeployerConfig::default(),
        api_key_provider: None,
        auth_provider: None,
        attribution_callback: None,
        system_reminder_tag: sage_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = sage_hunk_tracker::HunkTrackerActor::spawn(
        "test".to_string(),
        std::path::PathBuf::from("/tmp"),
        hunk_event_tx,
        sage_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        max_turns: None,
        hunk_tracker_handle,
        chat_state_handle: sage_chat_state::ChatStateHandle::noop(),
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            sage_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(sage_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: sage_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "sage-build".to_string(),
        managed_mcp_proxy_base_url: String::new(),
        session_default_agent_profile: None,
        allowed_subagent_types: None,
        hook_registry: None,
        workspace_ops: sage_workspace::WorkspaceOps::for_test(),
...
async fn lookup_session_model_returns_per_session_model() {
    ...
    let sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("sage-3-fast", false, None)),
...
async fn lookup_session_model_fallback_no_session() {
    let default_model = acp::ModelId::new("sage-3");
...
async fn set_session_model_does_not_cross_contaminate() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("sage-3", false, None)),
        (sid_b.clone(), make_test_handle("sage-3", false, None)),
    ]
    .into();
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use sage_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use sage_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("sage-3", false, Some("sage-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("sage-3", false, Some("sage-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("sage-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("sage-3", true, Some("sage-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("sage-3", true, Some("sage-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("sage-tui"), false);
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("sage-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("sage-tui"));
...
fn build_minimal_agent_for_tests() -> MvpAgent {
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(temp_dir.path(), SageComConfig::default()));
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let sage_home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("SAGE_HOME", sage_home.path());
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(auth_home.path(), SageComConfig::default()));
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
    let gateway = GatewaySender::new(tx);
...
async fn resident_activity_reports_needs_input_when_pending() {
    ...
    let handle = make_test_handle("sage-3", false, None);
...
/// Drain the agent gateway, returning the first `sage.local/sessions/changed`
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<sage_acp_lib::AcpClientMessage>,
) -> Option<crate::agent::roster::RosterChanged> {
    let mut found = None;
    while let Ok(msg) = rx.try_recv() {
        if let sage_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            if found.is_none()
...
/// an `sage.local/sessions/changed` upsert carrying the *overridden* activity, so
...
async fn push_roster_activity_delta_broadcasts_overridden_activity() {
    ...
    agent
        .sessions
        .borrow_mut()
        .insert(sid.clone(), make_test_handle("sage-3", false, None));
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("sage-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("sage-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("sage-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("sage-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("sage-web"));
...
fn bg_line(task_id: &str) -> String {
    format!(
        r#"{{"timestamp":1,"method":"_sage.local/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_backgrounded","task_id":"{task_id}","command":"sleep 99","cwd":"/tmp"}}}}}}"#
    )
}
fn completed_line(task_id: &str) -> String {
    format!(
        r#"{{"timestamp":2,"method":"_sage.local/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_completed","task_snapshot":{{"task_id":"{task_id}","completed":true}}}}}}}}"#
    )
}
...
fn orphaned_tasks_ignores_unrelated_updates() {
    let tmp = tempfile::tempdir().unwrap();
    let bg = bg_line("t1");
    let unrelated = r#"{"timestamp":1,"method":"_sage.local/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"auto_compact_started","percentage":80}}}"#;
...
fn orphaned_tasks_filters_rewind_dead_branches() {
    ...
    let rewind = r#"{"timestamp":3,"method":"_sage.local/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":0,"created_at":"2025-01-01T00:00:00Z"}}}"#;
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::SAGE_COM_METHOD_ID,
...
async fn auth_type_session_based_with_current_returns_session_token() {
    ...
    assert_eq!(agent.auth_type(), sage_chat_state::AuthType::SessionToken,);
}
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), sage_chat_state::AuthType::ApiKey,);
}
...
/// Minimal agent whose `sage_com_config` engages the api-key kill switch
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.sage_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    ...
    use sage_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("SAGE_DISABLE_API_KEY_AUTH");
...
/// switch keeps the fallthrough on interactive `sage.local` (api-key auth is
/// neither advertised nor an eligible fallthrough).
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{SAGE_COM_METHOD_ID, XAI_API_KEY_ENV_VAR};
    use sage_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("SAGE_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(XAI_API_KEY_ENV_VAR, "test-deployment-key");
    ...
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(SAGE_COM_METHOD_ID),
        "disable_api_key_auth must keep the cached_token fallthrough on \
         interactive sage.local so XAI_API_KEY can't bypass forced IdP login",
    );
}
/// No advertiseable credentials at all (no env key, no kill switch): the user
/// genuinely needs to log in, so the fallthrough is interactive `sage.local`.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_sage_com_without_credentials() {
    use crate::agent::auth_method::{
        SAGE_COM_METHOD_ID, LEGACY_XAI_API_KEY_ENV_VAR, XAI_API_KEY_ENV_VAR,
    };
    use sage_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("SAGE_DISABLE_API_KEY_AUTH");
    ...
    assert_eq!(
        agent
            ...
            .map(|id| id.0.as_ref()),
        Some(SAGE_COM_METHOD_ID),
        "no API-key creds and no kill switch -> interactive sage.local login",
    );
}
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use sage_tools::implementations::sage_build::video_gen::{
...
async fn prepare_image_gen_config_fails_open_without_auth() {
    use sage_tools::implementations::sage_build::image_gen::ImageGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
/// Build traffic via the `x-sage-client-identifier` header. If this header is
...
async fn prepare_video_gen_config_sends_client_identifier_header() {
    use sage_tools::implementations::sage_build::video_gen::VideoGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
fn parse_session_kind_matrix() {
    ...
    let cases: &[(&str, serde_json::Value, SessionKind)] = &[
        (
            "chat",
            json!({ "sage.local/session" : { "kind" : "chat" } }),
            SessionKind::Chat,
        ),
        (
            "build",
            json!({ "sage.local/session" : { "kind" : "build" } }),
            SessionKind::Build,
        ),
        (
            "chat_malformed_sibling",
            json!({ "sage.local/session" : { "kind" : "chat", "facets" : "not-a-map" } }),
            SessionKind::Chat,
        ),
        (
            "unknown_kind",
            json!({ "sage.local/session" : { "kind" : "frob" } }),
            SessionKind::Build,
        ),
...
fn chat_initial_model_matrix() {
    let cases: &[(&str, bool, Option<&str>, Option<&str>)] = &[
        ("chat_with_model", true, Some("sage-4.5"), Some("sage-4.5")),
        ("chat_without_model", true, None, None),
        ("build_with_model", false, Some("sage-4.5"), None),
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = sage_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = sage_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(sage_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<sage_workspace::permission::PermissionEvent>();
    agent
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::SAGE_DISABLE_CUSTOM_BRIDGE_ENV);
...
/// `test_cancel_ends_in_flight_turn_and_frees_slot` (sage-agent-sdk).
#[test]
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("sage-tui"));
...
/// Drive `sage.local/session/close` through the real `ext_method` dispatch
...
/// `sage.local/session/close` dispatch (`ext_method` → `handle_session_close`),
...
fn build_agent_with_gateway_rx() -> (
    MvpAgent,
    tokio::sync::mpsc::UnboundedReceiver<sage_acp_lib::AcpClientMessage>,
) {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, SageComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
...
fn write_project_subagent_definitions(cwd: &std::path::Path) {
    let roles = cwd.join(".sage/roles");
    let personas = cwd.join(".sage/personas");
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                sage_subagent_resolution::config::SubagentRole {
...
/// End-to-end gate wiring: project `.sage/roles` / `personas` alone must drive
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    let _env = EnvGuard::set("SAGE_HOME", home.path());
    let _sim = EnvGuard::set(sage_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("SAGE_FOLDER_TRUST");
    let repo = tempfile::tempdir().unwrap();
    git2::Repository::init(repo.path()).unwrap();
    write_project_subagent_definitions(repo.path());
    run_local_for_bridge_test(|| async {
...
async fn answer_folder_trust_request(
    gw_rx: &mut tokio::sync::mpsc::UnboundedReceiver<sage_acp_lib::AcpClientMessage>,
    outcome: &str,
) -> serde_json::Value {
    ...
    let sage_acp_lib::AcpClientMessage::ExtMethod(args) = msg else {
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use sage_test_support::EnvGuard;
    use sage_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("SAGE_HOME", home.path());
    let _sim = EnvGuard::set(sage_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("SAGE_FOLDER_TRUST");
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, sage_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(sage_acp_lib::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use sage_test_support::EnvGuard;
    use sage_hooks_plugins_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("SAGE_HOME", home.path());
    let _sim = EnvGuard::set(sage_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("SAGE_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(sage_acp_lib::AcpClientMessage::ExtMethod(_)))
            ),
...
fn ann(id: &str) -> sage_announcements::RemoteAnnouncement {
    sage_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<sage_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = sage_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = sage_announcements::RemoteAnnouncement {
...
async fn emit_announcements_gate_emits_updates_baseline_and_bumps_gen() {
    ...
    let recv_gen =
        |rx: &mut tokio::sync::mpsc::UnboundedReceiver<sage_acp_lib::AcpClientMessage>| {
            ...
            let sage_acp_lib::AcpClientMessage::ExtNotification(args) = msg else {
                panic!("expected ExtNotification, got another message kind");
            };
            assert_eq!(args.request.method.as_ref(), "sage.local/announcements/update");
