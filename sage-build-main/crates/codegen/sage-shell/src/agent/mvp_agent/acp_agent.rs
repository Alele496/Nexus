...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        sage_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::sage_home::sage_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    sage_workspace::permission::cleanup_stale_permission_state(
        ...
        sage_workspace::trust::migrate_legacy_hook_trust();
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_sage_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("sage-web") => client_type = ClientType::GrokWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("sage-code-extension") => client_type = ClientType::Extension,
                Some("sage-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.sage_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.sage_com_config.auth_provider_label.clone(),
                cfg.sage_com_config.auth_provider_command.is_some(),
                cfg.sage_com_config.oidc.is_some(),
                issuer,
                cfg.sage_com_config.oauth2.is_some(),
            )
        };
        if has_enterprise_oidc {
            ...
            tracing::info!(
                ...
                "auth: advertising sage.local auth method",
            );
        }
        let preferred_method = self.cfg.borrow().sage_com_config.preferred_method;
        ...
        sage_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!(
                    { "sage_home" : crate ::util::sage_home::sage_home().display()
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("SAGE_AGENT_METADATA");
                    serde_json::json!(
                        { "grokShell" : true, "defaultAuthMethodId" :
                        default_auth_method_id_wire, (sage_mcp::wire::MCP_SDK) :
                        ...
                        sage_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().sage_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                log_event(sage_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().sage_com_config.preferred_method, Some(crate
            ...
            auth_method::SAGE_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let sage_ctx = self.auth_manager.sage_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_sage_oauth(cli_oauth);
                tracing::debug!(
                ...
                let auth_result = if !auth_meta.headless {
                    ...
                    tokio::select! {
                        ...
                        sage_ctx, crate ::auth::AuthChannels { url_tx : Some(url_tx),
                    ...
                    tokio::select! {
                        biased; _ = cancel.cancelled() => { cancelled = true;
                        Err(anyhow::anyhow!("Authentication cancelled")) } r = crate
                        ::auth::run_auth_flow(& self.auth_manager, sage_ctx, auth_meta
                    ...
                    })?;
                {
                    ...
                    tracing::debug!(
                        "auth: sage.local/oidc handler set api_key (SessionToken)"
                    );
                    sage_telemetry::unified_log::debug(
                        "auth: sage.local/oidc handler set api_key (SessionToken)",
                        None,
    ...
    async fn new_session(
        ...
        sage_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || sage_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = sage_telemetry::context::collect_git_context(&cwd_str);
                let ev = sage_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                sage_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match sage_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
        ...
        if let Some(obj) = meta.as_object_mut() {
            obj.insert("sage.local/sessionConfig".to_string(), session_config_value);
            obj.insert("sage.local/sessionDetail".to_string(), session_detail_value);
        }
        Ok(
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        sage_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        sage_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        sage_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => sage_telemetry::events::PlanModeState::Inactive,
        };
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            sage_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = sage_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : sage_workspace::session::git::RESTORE_CODE_LOG, session_id =
                % session_id.0, supplied_cwd = % cwd.as_str(), persisted_cwd = % summary
                .info.cwd, target_sha = % target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a sage worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            sage_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use sage_workspace::session::git::RestoreKind;
            let outcome = sage_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let preloaded_envrc = sage_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let is_sage_build = persisted_model.0.starts_with("sage-build");
        let same_family_fallback = if is_sage_build {
            available.keys().find(|id| id.0.starts_with("sage-build")).cloned()
        } else {
            available.keys().find(|id| !id.0.starts_with("sage-build")).cloned()
        };
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    sage_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = sage_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = sage_workspace::session::git::detect_head_divergence(
        ...
        response_meta_map.insert("sage.local/sessionConfig".to_string(), session_config_value);
        response_meta_map.insert("sage.local/sessionDetail".to_string(), session_detail_value);
        ...
        if self.product_analytics_enabled() {
            log_event(sage_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            sage_file_utils::trace_context::link_current_span_to_meta(
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: sage_file_utils::trace_context::current_traceparent(),
                json_schema,
                send_now,
                admission: None,
                respond_to: tx,
            ...
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new(
                        "sage.local/session/prompt_complete",
                        params.into(),
                    ),
                );
        }
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<sage_chat_state::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = sage_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = sage_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = sage_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            sage_file_utils::trace_context::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            ...
            "sage.local/session/info" | "sage.local/session/close" | "sage.local/session/list"
            | "sage.local/sessions/list" => {
            ...
            "sage.local/session/load_history" => {
                crate::extensions::chat_conversation_history::handle(self, &args).await
            }
            "sage.local/session/search" => {
                crate::extensions::session_search::handle(&args).await
            }
            "sage.local/session/resolve_local_for_worktree_resume"
            | "sage.local/session/rehydrate" => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            "sage.local/session/rename" | "sage.local/session/delete"
            | "sage.local/session/update_mcp_servers" | "sage.local/session/fork"
            ...
            "sage.local/session/repair" => crate::extensions::repair::handle(self, &args).await,
            "x.ai/memory/flush" | "x.ai/memory/rewrite" => {
                crate::extensions::memory::handle(self, &args).await
            }
            "sage.local/skills/refresh-baseline" => {
            ...
            "x.ai/cloud/terminate" => {
                crate::extensions::auth_gate::require_xai_auth(
                    ...
                    "Run `sage login` to authenticate.",
                )?;
            ...
            s if s.starts_with("sage.local/session_summaries/") => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            ...
            s if s.starts_with("sage.local/skills/") => {
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "_sage.local/session/update" {
        ...
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                sage_telemetry::session_ctx::log_event(sage_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                sage_telemetry::session_ctx::log_event(sage_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| sage_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                sage_telemetry::session_ctx::log_event(sage_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                sage_telemetry::session_ctx::log_event(sage_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| sage_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == sage_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                sage_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            sage_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
