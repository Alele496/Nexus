...
impl MvpAgent {
    ...
    pub(crate) fn sync_process_static_api_key(&self, preferred_model_id: Option<&str>) {
        if self.cfg.borrow().sage_com_config.api_key_auth_disabled() {
    ...
    /// must not block the ACP response (sage-desktop sends `initialize` immediately).
    ...
    /// plus user/marketplace walks stalled sage-desktop's first `initialize`),
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            ...
            sage_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &sage_agent::plugins::SharedPluginRegistryHandle {
    ...
    /// discovery on sage-desktop Windows).
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<sage_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    sage_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    sage_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match sage_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => sage_workspace::WorkspaceOps::local(handle),
            Err(e) => {
    ...
    ///     `sage.local`, `oidc`) -- even if the in-memory token is currently
    ///     expired or missing.
    ///
    ...
    pub(crate) fn auth_type(&self) -> sage_chat_state::AuthType {
    ...
    /// iff `should_advertise_xai_api_key`; otherwise `sage.local`. Returns `None`
    ...
    pub(super) fn cached_token_fallthrough_method_id(
        ...
        let preferred = self.cfg.borrow().sage_com_config.preferred_method;
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_xai_api_key(
                self.cfg.borrow().sage_com_config.api_key_auth_disabled(),
    ...
    /// `use_oauth` only when the target is interactive `sage.local`. When
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let meta = if method_id.0.as_ref() == auth_method::SAGE_COM_METHOD_ID {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::GrokAuth) {
        ...
        let (
            ...
            let sage_user_id = is_xai.then(|| user_id.clone());
            let sage_team_id = is_xai.then(|| team_id.clone()).flatten();
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<sage_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    /// funnels through here. Emits `sage.local/announcements/update` and advances
    ...
    pub(super) fn emit_announcements(&self, mode: AnnouncementsPushMode) {
        ...
        let accepted = self
            ...
            .forward_fire_and_forget(
                acp::ExtNotification::new("sage.local/announcements/update", params.into()),
            );
        if !accepted {
            return;
        }
    ...
    /// Next generation for an `sage.local/announcements/update` push. Strictly
    ...
    pub(super) async fn send_model_auto_switched(
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&notification) {
            let _ = self
                .gateway
                .ext_notification(
                    acp::ExtNotification::new("sage.local/session_notification", params.into()),
                )
                .await;
        }
    }
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if matches!(preferred, Some(crate ::auth::PreferredAuthMethod::Oidc))
            ...
            && credentials.auth_type == sage_chat_state::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = sage_chat_state::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == sage_chat_state::AuthType::ApiKey
    ...
    /// Whether the current session is a personal sage.local account on a gated
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> sage_tools::implementations::sage_build::image_gen::ImageGenConfig {
        ...
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| sage_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("xai-sage-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        ...
        use sage_tools::implementations::sage_build::deploy_app::AppBuilderDeployerConfig;
    ...
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> sage_tools::implementations::sage_build::video_gen::VideoGenConfig {
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `SAGE_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `SAGE_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    /// - `allow_local`: `[toolset.web_fetch] allow_local` > `SAGE_WEB_FETCH_ALLOW_LOCAL` > false
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> sage_tools::implementations::sage_build::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        if !cfg.sage_com_config.api_key_auth_disabled() {
        ...
        if relay_sync_enabled {
            tracing::info!("[sage] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_xai_auth {
            tracing::info!(
                "[sage] Relay sync: DISABLED (no auth - run 'sage login' first)"
            );
        ...
        let instance = Self {
            ...
            plugin_registry_handle: sage_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: sage_tools::implementations::sage_build::task::types::MonitorEventBuffer::default(),
        ...
        instance
            .auth_manager
            .configure_refresher(
                instance.cfg.borrow().sage_com_config.auth_provider_command.clone(),
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<sage_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> sage_tools::implementations::sage_build::task::types::SubagentCancelOutcome {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<sage_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or SAGE_RELAY_SYNC_ENABLED)
            ...
            while state_rx.changed().await.is_ok() {
                ...
                if let Ok(params) = serde_json::value::to_raw_value(&notification) {
                    let ext_notification = acp::ExtNotification::new(
                        "sage.local/session_notification",
                        params.into(),
                    );
    ...
    pub async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<sage_hooks_plugins_types::HooksListResponse> {
    ...
    pub async fn execute_hooks_action(
        &self,
        session_id: &acp::SessionId,
        action: sage_hooks_plugins_types::HooksAction,
    ) -> Option<sage_hooks_plugins_types::ActionOutcome> {
        if matches!(action, sage_hooks_plugins_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&sage_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub async fn execute_plugins_action(
        ...
        let is_reload = matches!(action, sage_hooks_plugins_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(| o | & o.status),
            Some(sage_hooks_plugins_types::OutcomeStatus::Success)
        );
    ...
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<sage_agent::plugins::PluginRegistry>> {
    ...
    /// Build the `sage.local/sessionConfig` and `sage.local/sessionDetail` `_meta` values
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<sage_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<sage_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, sage_chat_state::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata {
                ...
                agent_type: None,
                shell_version: Some(sage_version::VERSION.to_string()),
                workspace_type: None,
                sandbox: local_sandbox_telemetry(),
            };
            let capture = sage_chat_state::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            ...
            None => {
                ...
                sage_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
        ...
        let queue = session_handle
            .upload_queue
            .get_or_init(|| {
                let sage_home = crate::util::sage_home::sage_home();
    ...
    /// 5. `SAGE_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `SAGE_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`sage_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<sage_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> sage_agent::AgentDefinition {
        use sage_agent::AgentDefinition;
        let sage_agent_env_set = std::env::var("SAGE_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(sage_agent::config::is_strict_harness_agent_type);
        if !sage_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = sage_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = sage_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = % name,
        ...
        let agent_name = std::env::var("SAGE_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("sage-build-concise") | Some("sage_build_concise") => {
                AgentDefinition::sage_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::sage_build_plan()
                    }
                }
            }
            Some(name) => {
                sage_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::sage_build_plan)
            }
            None => AgentDefinition::sage_build_plan(),
        };
        if !sage_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = sage_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs: Arc<dyn sage_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let (hunk_tracker_handle, hunk_event_rx) = match hunk_plan.actor_mode {
            ...
            None => (sage_hunk_tracker::HunkTrackerHandle::noop(), None),
        };
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                let loc_writer = sage_hunk_tracker::JsonlHunkRecordWriter::new(loc_path);
                let loc_ctx = sage_hunk_tracker::LocSinkContext {
                    session_id: session_info.id.0.to_string(),
                    agent_id: agent_id(),
                ...
                tokio::spawn(
                    sage_hunk_tracker::run_loc_sink(
                        hunk_event_rx,
                        loc_writer,
                        loc_ctx,
                        loc_cancel,
        ...
        let mut session_env = sage_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                sage_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            sage_agent::config::ModelOverride::Override(id) => {
            ...
            sage_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = sage_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = sage_tools::util::sage_home::sage_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".sage").join("lsp.json");
                tracing::warn!(
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let credentials = sage_chat_state::Credentials {
            ...
            let attribution_callback: Option<
                sage_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = sage_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                    ...
                    let git_root = sage_workspace::session::git::find_git_root_from_path(
                            cwd,
                        )
                        .ok();
            ...
            spawn_session_on_thread(
                    ...
                    subagent_toggle,
                    Vec::new(),
                    sage_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        sage_hunk_tracker::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                            ...
                            let is_agent = author_type
                                == sage_hunk_tracker::AuthorType::Agent;
                            signals
                                .record_loc_change(
                                    is_agent,
                                    lines_added,
                        ...
                        sage_hunk_tracker::LocAggregate::LinesReverted {
