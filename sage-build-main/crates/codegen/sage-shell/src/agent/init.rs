...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    if cfg.remote_settings.is_none()
        && let Some(handle) =
            crate::agent::models::start_early_prefetch(Some(cfg.sage_com_config.clone()))
    {
        match handle.join() {
            Ok(result) => {
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (sage -p, subagents).
    ...
    // Writeback talks to the code backend; requires sage.local auth.
    if cfg.storage_mode == StorageMode::Writeback
        && !auth_manager.current().is_some_and(|a| a.is_xai_auth())
    {
        tracing::info!("Writeback is disabled: requires auth with sage.local");
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        crate::builtin::extract_bundled_files(&sage_home);

        crate::extensions::marketplace::purge_default_skills_installs(&sage_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&sage_home);
        }
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    let sage_auth = auth_manager.current().filter(|a| a.is_xai_auth());
    let user_id = sage_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = sage_auth.as_ref().and_then(|a| a.team_id.clone());
