...
/// session.
fn is_cached_credential_compatible(auth: &GrokAuth, sage_com_config: &SageComConfig) -> bool {
    let expected_issuer = sage_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| sage_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(sage_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("SAGE_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `SAGE_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `sage_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("SAGE_LOGIN_DEVICE_FLOW");
...
async fn run_external_auth_provider(
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.sage_com_config());
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = sage_com_config.oidc.is_some(),
        has_oauth2 = sage_com_config.oauth2.is_some(),
        has_external_auth = sage_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, sage_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, sage_com_config)
        }) {
            sage_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = sage_com_config.auth_provider_command {
    ...
    if !sage_com_config.blocks_automatic_oidc()
    ...
    // --device-auth / SAGE_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(sage_com_config) {
        return crate::auth::oidc::run_login_flow(sage_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = sage_com_config.oauth2 {
    ...
    match try_api_key_auth(auth_manager).await {
        ...
        Err(e) => {
            ...
            anyhow::bail!(
                "No authentication method available. Set SAGE_API_KEY environment variable \
...
async fn try_api_key_auth(
    ...
    // Try XAI_API_KEY env var first, then fall back to auth.json.
    let api_key = match crate::agent::auth_method::read_xai_api_key_env() {
        ...
        Err(_) => {
            ...
            crate::auth::storage::read_api_key(&sage_home)
        }
    };
...
pub async fn try_ensure_fresh_auth(sage_com_config: &SageComConfig) -> Option<GrokAuth> {
    let sage_home = sage_home::sage_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&sage_home, sage_com_config.clone()));

    // auth() handles cached-valid (fast path), OIDC refresh, external
    // binary -- all through refresh_chain (single mutation point).
    auth_manager.configure_refresher(sage_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    if let Some(auth) = try_ensure_fresh_auth(sage_com_config).await {
        return Some(auth);
    }
    let sage_home = sage_home::sage_home();
    let auth_manager = Arc::new(AuthManager::new(&sage_home, sage_com_config.clone()));
    ...
    mint_session_noninteractive(&auth_manager, sage_com_config).await
}
...
async fn mint_session_noninteractive(
    ...
    if let Some(cmd) = sage_com_config.auth_provider_command.as_deref() {
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(sage_com_config).await)
    } else {
        ensure_authenticated(sage_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}
...
/// otherwise `SAGE_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.sage_com_config, login_override).await {
        if config.sage_com_config.oauth2.is_none() {
        ...
        let sage_home = sage_home::sage_home();
        let auth_manager = Arc::new(AuthManager::new(&sage_home, config.sage_com_config.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.sage_com_config) {
            eprintln!(
...
pub struct LogoutResult {
    ...
    /// `true` if `XAI_API_KEY` / `SAGE_CODE_XAI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        sage_telemetry::external::set_identity(
            sage_telemetry::external::IdentityAttrs::default(),
        );
        sage_telemetry::external::flush();
...
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let sage_home = sage_home::sage_home();
    let auth_manager = AuthManager::new(&sage_home, config.sage_com_config.clone());
...
mod tests {
    ...
    /// Run `f` with `SAGE_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("SAGE_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("SAGE_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("SAGE_LOGIN_DEVICE_FLOW"),
        };
        f()
    }
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only sage_home: reading a missing auth.json succeeds (empty), but
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("SAGE_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("SAGE_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        // SAGE_LOGIN_DEVICE_FLOW / the real config file (the CLI override
