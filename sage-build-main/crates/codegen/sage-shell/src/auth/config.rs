...
pub struct SageComConfig {
    ...
    /// Login button label (env: `SAGE_AUTH_PROVIDER_LABEL`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// refresh works. Env: `SAGE_AUTH_TOKEN_TTL`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// can't bypass the deployment's IdP login. Env: `SAGE_DISABLE_API_KEY_AUTH`.
...
/// Customer OIDC Identity Provider configuration (`[sage_com_config.oidc]`).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// OAuth2 provider configuration (`SAGE_OAUTH2_ISSUER` / `SAGE_OAUTH2_CLIENT_ID`).
///
...
impl SageComConfig {
    ...
    /// `SAGE_DISABLE_API_KEY_AUTH` env lockdown is sticky: because the env value
...
impl OAuth2ProviderConfig {
    ...
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("SAGE_OAUTH2_ISSUER").ok()?;
        let client_id = std::env::var("SAGE_OAUTH2_CLIENT_ID").ok()?;
        let principal_type = std::env::var("SAGE_OAUTH2_PRINCIPAL_TYPE").ok();
        let principal_id = std::env::var("SAGE_OAUTH2_PRINCIPAL_ID").ok();
        ...
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("SAGE_OAUTH2_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or(default_scopes),
            principal_type,
            principal_id,
            referrer: Some(
                std::env::var("SAGE_OAUTH2_REFERRER")
...
impl Default for SageComConfig {
    fn default() -> Self {
        ...
        // an org explicitly wires in their own IdP through SAGE_OIDC_* or
        // SAGE_OAUTH2_* environment variables.
        ...
        Self {
            sage_ws_origin: std::env::var("SAGE_WS_ORIGIN")
                .unwrap_or_else(|_| PROD_WS_ORIGIN.to_owned()),
            sage_ws_url: std::env::var("SAGE_WS_URL")
                .unwrap_or_else(|_| PROD_RELAY_WS_URL.to_owned()),
            token_header: "sage-cli".to_owned(),
            oidc,
            oauth2,
            auth_provider_command: std::env::var("SAGE_AUTH_PROVIDER_COMMAND").ok(),
            auth_provider_label: std::env::var("SAGE_AUTH_PROVIDER_LABEL").ok(),
            auth_token_ttl: std::env::var("SAGE_AUTH_TOKEN_TTL")
                .ok()
                .and_then(|v| v.parse().ok()),
            disable_api_key_auth: std::env::var("SAGE_DISABLE_API_KEY_AUTH")
                .ok()
...
/// `no`, empty) count as disabled — so e.g. `SAGE_DISABLE_API_KEY_AUTH=false`
...
/// True when the admin has set `SAGE_DISABLE_API_KEY_AUTH` to a truthy value in
...
fn env_lockdown_forced() -> bool {
    std::env::var("SAGE_DISABLE_API_KEY_AUTH")
        .ok()
        .is_some_and(|v| env_flag_enabled(&v))
}
impl OidcAuthConfig {
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("SAGE_OIDC_ISSUER").ok()?;
        let client_id = std::env::var("SAGE_OIDC_CLIENT_ID").ok()?;
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("SAGE_OIDC_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or_else(|_| default_oidc_scopes()),
            audience: std::env::var("SAGE_OIDC_AUDIENCE").ok(),
        })
    }
}
#[cfg(test)]
