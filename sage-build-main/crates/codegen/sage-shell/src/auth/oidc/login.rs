...
use super::protocol::{
    OidcError, build_authorize_url, build_sage_auth, discover, enforce_login_principal,
...
pub async fn run_login_flow_with_config(
    ...
    let oauth2 = auth_manager.sage_com_config().oauth2.as_ref();
    ...
    let principal_policy = login_principal_policy(auth_manager.sage_com_config());
    ...
    let mut auth = build_sage_auth(tokens, user_info, &oidc.issuer, &oidc.client_id);
...
mod tests {
    ...
    async fn full_login_flow_via_race() {
        ...
        .await
        .unwrap();
        let auth = build_sage_auth(tokens, user_info, &oidc_cfg.issuer, &oidc_cfg.client_id);
