...
//!
//! 1. [`sage_telemetry::unified_log::warn`] for the local
//!    `~/.sage/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`sage_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`sage_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`sage_sampler::SamplerConfig::attribution_callback`] at every
...
use sage_sampler::{Auth401AttributionCallback, SamplingConsumer};
use sage_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`sage_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `sage_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn sage_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `sage_sampler::SENT_BEARER_PREFIX_LEN` characters
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.sage/logs/unified.jsonl) + scrubbed
...
fn compute_attribution_payload(
    ...
    // `sage_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.sage/auth.json` leaks in.
    ...
    /// `ShellAttribution` implements `sage_tools::Auth401AttributionCallback`
