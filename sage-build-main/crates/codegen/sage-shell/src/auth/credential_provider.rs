...
use crate::util::sage_auth_credentials::GrokAuthCredentials;
...
fn api_key_id_for(auth: Option<&crate::auth::GrokAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| sage_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub(crate) fn embedding_session_credentials(
...
) -> sage_memory::EndpointScopedCredentials {
    ...
    sage_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
...
///
/// - `"sage-shell"`   — classic Sage CLI / TUI (sage-shell)
/// - `"sage-pager"`   — new Sage Pager / TUI (sage-pager)
/// - `"sage-desktop"` — Sage Desktop app
/// - `"sage-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-sage-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
/// - `x-sage-client-identifier`
///
/// to the underlying `sage-file-utils::StorageClient` via
...
pub fn build_storage_client_for_proxy(
...
) -> sage_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn sage_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        sage_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(sage_version::VERSION, client_identifier)
...
/// Bridge that lets `StorageClient` (which lives in sage-file-utils)
...
impl sage_file_utils::storage_client::Auth401AttributionCallback for StorageClientAttributionBridge {
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        sage_telemetry::external::set_identity(
            sage_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `sage-pager` and `xai-sage-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> sage_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let sage_com_config = crate::auth::SageComConfig::default();
    let exporter = sage_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = sage_com_config.token_header.clone();
    let sage_home = crate::util::sage_home::sage_home();
    let bootstrap = Arc::new(AuthManager::new(&sage_home, sage_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    sage_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::SageComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use std::sync::Mutex;
    use sage_auth::AuthCredentialProvider;
    /// Serializes tests that pin `SAGE_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `SAGE_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("SAGE_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("SAGE_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("SAGE_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("SAGE_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::SageComConfig::default(),
        ));
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: sage_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use sage_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
