...
fn legacy_scope_fallback_reads_old_auth_json() {
    ...
    // a machine that was authenticated with an older sage version).
...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    // funnel the user back through `sage login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `sage logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `sage login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
/// `has_sage_code_access`. The `&mut GrokAuth` signature already
...
fn apply_user_info_enrichment_preserves_token_fields() {
    let mut disk = GrokAuth {
        ...
        has_sage_code_access: Some(true),
    ...
    assert_eq!(disk.has_sage_code_access, snapshot.has_sage_code_access);
...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    let _legacy = EnvGuard::unset("SAGE_CODE_XAI_API_KEY");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), SageComConfig::default()));
    mgr.hot_swap(GrokAuth {
...
async fn enrich_auth_inline_populates_zdr_flags() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), SageComConfig::default()).with_proxy_base_url(&base);
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    ...
    let mgr2 = Arc::new(AuthManager::new(dir2.path(), SageComConfig::default()));
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use sage_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use sage_telemetry::events::{AuthTokenKind, ManualAuthSurface};
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    fn mgr_with(dir: &std::path::Path, key: &str, mode: AuthMode) -> Arc<AuthManager> {
        let mgr = Arc::new(AuthManager::new(dir, SageComConfig::default()));
