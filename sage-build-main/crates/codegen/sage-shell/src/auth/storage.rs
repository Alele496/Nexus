...
pub fn read_token_by_scope(sage_home: &Path, scope: &str) -> anyhow::Result<String> {
    let path = sage_home.join("auth.json");
    let store =
...
pub fn read_api_key(sage_home: &Path) -> Option<String> {
    let path = sage_home.join("auth.json");
...
pub fn store_api_key(sage_home: &Path, api_key: &str) -> std::io::Result<()> {
    let path = sage_home.join("auth.json");
...
pub fn clear_api_key(sage_home: &Path) -> std::io::Result<()> {
    let path = sage_home.join("auth.json");
