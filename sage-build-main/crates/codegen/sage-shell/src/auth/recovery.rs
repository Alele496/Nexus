...
use sage_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        sage_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
        ...
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.sage_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<GrokAuth> {
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            sage_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
...
mod tests {
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.sage_com_config().auth_scope();
        let fresh = GrokAuth {
