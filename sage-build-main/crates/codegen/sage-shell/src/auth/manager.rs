...
use sage_telemetry::events::ManualAuthSurface;
...
pub struct AuthManager {
    ...
    power_listener: parking_lot::Mutex<Option<sage_system_power::SystemPowerListener>>,
...
impl AuthManager {
    pub fn new(sage_home: &Path, sage_com_config: SageComConfig) -> Self {
        ...
        sage_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "sage_home": sage_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "SAGE_HOME": std::env::var("SAGE_HOME").unwrap_or_else(|_| "(unset)".into()),
                "SAGE_AUTH_PATH": std::env::var("SAGE_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "SAGE_AUTH": std::env::var("SAGE_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // SAGE_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("SAGE_AUTH") {
            ...
            tracing::warn!("SAGE_AUTH set but failed to parse as JSON, falling back to file");
        }

        // SAGE_AUTH_PATH: custom file path (overrides default $SAGE_HOME/auth.json).
        let path = std::env::var("SAGE_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| sage_home.join("auth.json"));
    ...
    /// (inline `SAGE_AUTH` vs. on-disk `auth.json`), which differ only in the
    ...
    pub(crate) fn cached_token_policy_error(&self, auth: &GrokAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.sage_com_config)?;
    ...
    fn token_expired_with_buffer(&self, auth: &GrokAuth, buffer: Duration) -> bool {
        ...
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.sage_com_config.auth_token_ttl
        {
    ...
    pub(crate) async fn update(self: &Arc<Self>, auth: GrokAuth) -> std::io::Result<GrokAuth> {
        ...
        match &write_result {
            Ok(()) => sage_telemetry::unified_log::info(
            ...
            Err(e) => sage_telemetry::unified_log::error(
    ...
    pub(crate) fn sage_com_config(&self) -> &SageComConfig {
        &self.sage_com_config
    }
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in SAGE_AUTH mode).
            DiskAuthState::Ok => {
                sage_telemetry::unified_log::info(
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.sage_com_config.blocks_automatic_oidc()
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.sage_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        sage_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `sage_system_power::PowerState` for the canonical
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<sage_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[sage_com] auth_token_ttl` to drive a real schedule.
...
impl sage_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.sage_com_config.preferred_method,
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.sage_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.sage_com_config.preferred_method,
...
) -> sage_tools::types::SharedApiKeyProvider {
