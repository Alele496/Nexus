...
#[non_exhaustive]
pub enum AuthError {
    #[error("Not logged in. Run `sage login`.")]
    NotLoggedIn,

    /// Token expired and no refresh authority available.
    #[error("Token expired. Run `sage login` to re-authenticate.")]
    TokenExpiredNoRefresh,

    /// Server rejected the token (401) with no recovery path.
    #[error("Authentication rejected by server. Run `sage login` to re-authenticate.")]
    ...
    #[error("{message} Run `sage login` to sign in with the required team.")]
    PinnedTeamMismatch { message: String },

    /// Cached API-key session rejected because API-key auth is disabled.
    #[error("API-key auth is disabled by your administrator. Run `sage login` to authenticate.")]
    ApiKeyAuthDisabled,
    ...
    pub(crate) fn user_message(self) -> &'static str {
        match self {
            Self::RefreshTokenRejected => {
                "Your session has expired. Run `sage login` to sign in again."
            }
            Self::ClientRejected => {
                "Authentication is temporarily unavailable. Run `sage login` if this persists."
            }
            Self::Other => {
                "Authentication could not be refreshed. Run `sage login` to sign in again."
            }
        }
    }
}
