...
use sage_agent::plugins::discovery::PluginScope;
use sage_agent::plugins::git_install::{self, UpdateStatus};
use sage_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use sage_plugin_marketplace::git::{self, SourceCacheLease};
use sage_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.sage/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &sage_agent::plugins::install_registry::MarketplaceProvenance,
...
pub fn marketplace_require_sha() -> bool {
    sage_config::load_effective_config_disk_only()
        .map(|c| sage_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| sage_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &sage_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.sage/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user sage via user_sage_home() (None when no home resolves) and
    // home separately, so removal still runs from $SAGE_HOME when no home dir
    // exists, and never touches a cwd-relative .sage.
...
mod tests {
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> sage_workspace::permission::resolution::MarketplaceAllowlist {
        sage_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = sage_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
