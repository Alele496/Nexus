//! Shim — see `sage_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `sage_shell::instrumentation_timer!`). Keeping the macro here
...
pub use sage_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `sage_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `sage_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `sage_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `sage_telemetry::instrumentation`.
#[macro_export]
