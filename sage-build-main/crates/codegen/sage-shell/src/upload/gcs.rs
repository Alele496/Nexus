...
//! `StorageClient` constructed inside `sage_file_utils::gcs::*` helpers.
//!
...
//! to an `sage_file_utils::gcs::*` helper.
...
/// shell-side adapter that lets `sage_file_utils::gcs::*` helpers wire
...
///
/// ```ignore
/// sage_file_utils::gcs::upload_bytes(
...
/// `SAGE_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("SAGE_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match sage_file_utils::gcs::upload_bytes(
