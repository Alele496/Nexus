...
use sage_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .sage/hooks/ directories are still scanned (they hold
    ...
    // user_sage_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .sage.
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".sage").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    sage_hooks::discovery::HookRegistry,
    Vec<sage_hooks::error::HookError>,
) {
