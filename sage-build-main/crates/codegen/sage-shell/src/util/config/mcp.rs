...
use sage_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use sage_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
...
pub use sage_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use sage_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.sage/config.toml` at each level.
/// Project-scoped `.sage/config.toml` entries override global `~/.sage/config.toml`
...
/// `.sage/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.sage/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.sage/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.sage/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .sage/config.toml"
                );
...
pub fn mcp_preferences_path() -> PathBuf {
    sage_config::sage_home().join("mcp_preferences.json")
}
...
pub fn collect_mcp_setup_configs(
    cwd: &std::path::Path,
    plugin_registry: Option<&sage_agent::plugins::PluginRegistry>,
    compat: &CompatConfig,
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    sage_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.sage/config.toml`.
...
/// Upsert an MCP server entry in `~/.sage/config.toml`.
///
...
/// config file, e.g. a project-scoped `.sage/config.toml`.
...
/// Delete an MCP server entry from `~/.sage/config.toml`.
///
...
/// config file, e.g. a project-scoped `.sage/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = sage_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.sage/config.toml` files.
...
pub use sage_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.sage/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::sage_home::sage_home().join("config.toml")
}

/// Path to the user-level config file (`~/.sage/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.sage/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".sage").join("config.toml")
}
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.sage/skills", "~/.sage/skills/special/SKILL.md"]
ignore = ["~/.sage/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.sage/skills", "~/.sage/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.sage/skills/noisy/SKILL.md"]);
    }
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${SAGE_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
