...
pub fn load_relay_sync_enabled_sync() -> bool {
    toml_bool_sync(Some("SAGE_RELAY_SYNC_ENABLED"), "relay", "enabled")
}
...
/// 1. Nearest project `.sage/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.sage/config.toml` `[permission]` section
///
