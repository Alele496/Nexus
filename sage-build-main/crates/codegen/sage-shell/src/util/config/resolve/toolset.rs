...
/// [`sage_config::env_bool`] parser (`SAGE_TOOLS_FIND_BFS` /
/// `SAGE_TOOLS_GREP_UGREP`, plus the `SAGE_FIND_BFS` / `SAGE_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.sage/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`sage_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = sage_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = sage_config::env_bool(primary).or_else(|| sage_config::env_bool(alias));
    ...
    (
        resolve("SAGE_TOOLS_FIND_BFS", "SAGE_FIND_BFS", "find_bfs"),
        resolve("SAGE_TOOLS_GREP_UGREP", "SAGE_GREP_UGREP", "grep_ugrep"),
    )
}
...
const ENV_LOGIN_SHELL_CAPTURE: &str = "SAGE_LOGIN_ENV";
...
/// the shared [`sage_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "SAGE_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`SAGE_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            sage_tools::implementations::sage_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`SAGE_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        sage_tools::implementations::sage_build::ask_user_question::response_timeout_env_secs(),
...
) -> sage_tools::implementations::sage_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use sage_tools::implementations::sage_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = sage_tools::implementations::sage_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes SAGE_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
