...
pub(crate) const ENV_AUTO_PERMISSION_MODE: &str = "SAGE_AUTO_PERMISSION_MODE";

/// Crate-wide serialization lock for tests that mutate
/// `SAGE_AUTO_PERMISSION_MODE`. Every test reading the gate (here and in
...
/// two can't drift. Precedence: requirement > env (`SAGE_AUTO_PERMISSION_MODE`)
...
///
/// requirements > env (`SAGE_AUTO_PERMISSION_MODE`) > `[auto_mode] enabled` in
...
pub fn auto_mode_classifier_defaults(
    ...
    sage_workspace::permission::ClassifierPromptType,
    Option<sage_sampling_types::ReasoningEffort>,
) {
    let prompt_type = cfg
        .prompt_type
        .unwrap_or(sage_workspace::permission::ClassifierPromptType::Full);
    let reasoning_effort = cfg.reasoning_effort.or_else(|| {
        effective_supports_reasoning_effort.then_some(sage_sampling_types::ReasoningEffort::Low)
    });
...
mod auto_permission_mode_gate_tests {
    ...
    // `SAGE_AUTO_PERMISSION_MODE` is process-global; serialize every test that
