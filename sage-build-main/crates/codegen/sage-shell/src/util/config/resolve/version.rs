...
///
/// Reads `stable_version` from `~/.sage/version.json` (written by the
...
///
/// This is a lightweight duplicate of `sage_update::channel_name()` for
...
pub fn channel_name_from_cache() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        let version_path = crate::util::sage_home::sage_home().join("version.json");
        let content = std::fs::read_to_string(&version_path).ok()?;
        let parsed: serde_json::Value = serde_json::from_str(&content).ok()?;
        let stable = parsed.get("stable_version")?.as_str()?;
        let current = semver::Version::parse(sage_version::VERSION).ok()?;
