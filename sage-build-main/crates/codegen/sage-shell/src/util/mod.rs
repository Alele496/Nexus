...
// existing `crate::util::…` / `sage_shell::util::…` users compile
// unchanged.
pub use sage_shell_base::util::*;
...
pub(crate) fn is_user_instruction_path(
    ...
    let sage_rules = sage_home.join("rules");
    let is_exact_home_surface = parent
        .is_some_and(|parent| parent == sage_home || parent == sage_rules)
...
mod is_user_instruction_path_tests {
    ...
    #[test]
    fn sage_home_named_file_nested_in_workspace_is_user_scoped() {
    ...
    #[test]
    fn workspace_descendants_under_sage_home_stay_project_scoped() {
        assert!(!is_user_instruction_path(
