...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that SAGE_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.sage. Make sure the directory exists and is writable.\n  ({0})"
    )]
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<sage_config::signed_policy::SignatureEnvelope>>,
    ...
    pub(super) managed_identity_signatures:
        Option<Vec<sage_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        ...
        pick_trusted_envelope(
            ...
            sage_config::signed_policy::embedded_key_id_trusted,
        )
    }
...
fn pick_trusted_envelope(
    envelopes: Option<&[sage_config::signed_policy::SignatureEnvelope]>,
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: sage_config::signed_policy::SignatureEnvelope,
    pub(super) payload: sage_config::signed_policy::SignedPayload,
}
...
mod tests {
    ...
    fn pick_trusted_envelope_prefers_trusted_then_falls_back() {
        use sage_config::signed_policy::SignatureEnvelope;
