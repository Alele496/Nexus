//! `sage inspect` — configuration introspection.
//!
//! Shows everything Sage discovers in the current directory: project
...
use sage_tools::util::truncate::estimate_tokens;
...
/// Enterprise login-hardening policy resolved from `[sage_com_config]`
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `SAGE_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .sage/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `sage inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = sage_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = sage_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = sage_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        sage_version: sage_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if path
        .parent()
        .is_some_and(|parent| parent == sage_home.join("rules"))
        || has_rules_directory(file_path, ".sage")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = sage_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        sage_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `sage_agent::prompt::agents_md::read_agents_config_with_paths`
    ...
    configs
        ...
        .map(|c| {
            let file_type =
                instruction_file_type(&c.file_path, &sage_home, imported, &extra_rule_prefixes);
            let scope = instruction_scope(&c.file_path, &sage_home, &vendor_homes, &workspace_root);
...
/// which handles both Sage TOML and vendor settings fallback in one codepath.
async fn list_permissions(cwd: &Path) -> PermissionsReport {
    use sage_workspace::permission::resolution;
...
/// (`[sage_com_config]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let sage_com_config = config
        .map(|c| c.sage_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: sage_com_config.api_key_auth_disabled(),
        disable_api_key_auth: sage_com_config.disable_api_key_auth,
        force_login_team_uuid: sage_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[sage_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = sage_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        sage_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);

    let home_dir = dirs::home_dir();
    let sage_home = sage_config::sage_home();

    let mut entries: Vec<HookEntry> = registry
        .all_hooks()
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&sage_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &sage_agent::plugins::PluginRegistry,
    skills_config: &sage_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = sage_agent::prompt::skills::list_skills_with_plugins(
    ...
    skills
        .into_iter()
        .map(|s| {
            let source = skill_entry_source(&s, &sage_home);
...
/// `<sage_home>/skills/<name>/SKILL.md` and discovered as user skills, so a
...
/// discovery: deployed pagers parse `sage.local/skills/list` into a typed
...
fn skill_entry_source(
    s: &sage_agent::prompt::skills::SkillInfo,
    sage_home: &Path,
) -> ConfigSource {
    use sage_tools::implementations::skills::types::SkillScope;
    ...
    match s.scope {
        ...
        SkillScope::User => {
            if crate::builtin::is_extracted_bundled_skill(&s.name, &path, sage_home) {
...
fn list_agents(
    ...
    let agents = sage_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[sage_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                sage_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                sage_agent::plugins::PluginScope::Project => Scope::Project,
                sage_agent::plugins::PluginScope::User => Scope::User,
                sage_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: sage_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    sage_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = sage_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `sage mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// `.sage/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_sage_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.sage_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `sage mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use sage_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use sage_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for (vendor, path) in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.sage"), false, &[]);
        ...
        for path in ["/repo/.sage/rules/team.md", r"C:\repo\.sage\rules\team.md"] {
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.sage"), false, &[]),
                "rules"
            );
        }
        for path in [
            ...
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.sage"), true, &[]),
                "rules"
            );
        }
        for path in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.sage"), true, &[]);
    ...
    #[test]
    fn sage_home_nested_in_workspace_keeps_direct_surfaces_global() {
        let sage_home = Path::new("/repo/config");
        let workspace = Path::new("/repo");
        for path in ["/repo/config/AGENTS.md", "/repo/config/rules/global.md"] {
            assert!(matches!(
                instruction_scope(path, sage_home, &[], workspace),
                Scope::Global
            ));
        }
        for path in [
            "/repo/config/.sage/rules/project.md",
    ...
    fn vendor_home_nested_in_workspace_keeps_direct_surfaces_global() {
        ...
        for path in ["/repo/.claude/rules/global.md", "/repo/.claude/CLAUDE.md"] {
            assert!(matches!(
                instruction_scope(path, Path::new("/other/sage"), &vendor_homes, workspace),
                Scope::Global
            ));
        }
        for path in [
    ...
    #[test]
    fn workspace_scope_wins_inside_sage_home() {
        let sage_home = Path::new("/custom/sage");
        let workspace = Path::new("/custom/sage/worktrees/repo");
        for path in [
            "/custom/sage/worktrees/repo/.cursor/rules/project.md",
            "/custom/sage/worktrees/repo/src/AGENTS.md",
        ] {
            assert!(matches!(
                instruction_scope(path, sage_home, &[], workspace),
                Scope::Project
        ...
        assert!(matches!(
            instruction_scope("/custom/sage/rules/global.md", sage_home, &[], workspace,),
            Scope::Global
        ));
    }

    #[test]
    fn custom_sage_home_rules_are_classified_as_rules() {
        assert_eq!(
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.sage/requirements.toml";
    ...
    fn model_override_warnings_inspect_smoke() {
        ...
        assert!(cfg.config_models.contains_key("sage-4.5"));

        let human = render_model_override_warnings(&warnings);
        assert!(human.contains("Model Overrides"), "{human}");
        assert!(
            human.contains("[model.\"sage-4.5\"] send_compactions_remaining"),
            "{human}"
        );
        assert!(
            human.contains("[model.\"sage-4.5\"] reasoning_effort"),
            "{human}"
        );
        ...
        assert_eq!(alias_warning["modelKey"], "sage-4.5");
    ...
    fn skill_entry_source_maps_scopes() {
        let home = Path::new("/home/u/.sage");

        let s = skill_fixture("a", "/repo/.sage/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.sage/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.sage/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::User { .. }
        ));
        ...
        let s = skill_fixture(
            "d",
            "/home/u/.sage/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("e", "/home/u/.sage/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    /// path `<sage_home>/skills/<name>/SKILL.md`; a same-named skill anywhere
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.sage/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("help", "/repo/.sage/skills/help/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        // Bundled name in a user dir outside <sage_home>/skills: stays user.
        let s = skill_fixture(
            "help",
        ...
        // Bundled frontmatter name in a different dir under <sage_home>/skills:
        // not the extracted copy — stays user.
        let s = skill_fixture(
            "help",
            "/home/u/.sage/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Non-bundled name under <sage_home>/skills: stays user.
        let s = skill_fixture(
            "my-skill",
            "/home/u/.sage/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.sage dirs.
        ...
        let registry = sage_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
