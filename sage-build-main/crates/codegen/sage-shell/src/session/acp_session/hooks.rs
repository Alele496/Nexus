...
//! both matched by the agent ([`sage_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use sage_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use sage_telemetry::events::ClientHookGateOutcome;
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        sage_telemetry::session_ctx::log_event(sage_telemetry::events::HookBlocked {
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = sage_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    sage_telemetry::session_ctx::log_event(
                        sage_telemetry::events::ClientHookGate {
...
mod tests {
    ...
    async fn hook_event_active_inert_vs_active() {
        use sage_hooks::event::HookEventName;
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    sage_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
    ...
    fn matching_callback_ids_filters_by_matcher() {
        use sage_hooks::matcher::HookMatcher;
