...
//! [`sage_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`sage_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`sage_agent::Agent`] owns an [`sage_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`sage_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use sage_agent::config::AgentDefinition;
use sage_agent::error::AgentBuildError;
use sage_agent::prompt::context::PromptAudience;
use sage_agent::prompt::skills::SkillsConfig;
use sage_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use sage_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use sage_tools::implementations::sage_build::ask_user_question::types::UserQuestionRequest;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<sage_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<sage_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use sage_tools::implementations::sage_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(sage_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(sage_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use sage_tools::implementations::sage_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            sage_tools::computer::local::LocalTerminalBackend::new_local(
                sage_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: sage_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(sage_tools::types::tool::ToolKind::Task)
            .expect("SageBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("SageBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_sage_build())
                    .await
