...
use super::support::*;
use super::*;
/// Build an actor whose toolset parses sage `search_replace` plus the plan
...
async fn build_gate_actor() -> SessionActor {
    ...
    *actor.agent.borrow_mut() = test_agent_with_tools(vec![
        ToolConfig::from_id("SageBuild:read_file"),
        ToolConfig::from_id("SageBuild:search_replace"),
...
async fn tool_result_text(actor: &SessionActor, call_id: &str) -> String {
    ...
    conv.iter()
        ...
        .find_map(|item| match item {
            sage_sampling_types::ConversationItem::ToolResult(tr)
...
async fn plan_mode_rejects_sage_edit_outside_plan_file_despite_allow_all_permissions() {
