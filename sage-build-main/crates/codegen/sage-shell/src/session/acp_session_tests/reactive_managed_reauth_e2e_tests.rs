...
use sage_mcp::servers::{ClientStateKind, HttpConfig, McpClient};

const MANAGED: &str = "sage_com_testconnector";
...
async fn actor_with_proxy(
    proxy_base: &str,
    gw_tx: tokio::sync::mpsc::UnboundedSender<sage_acp_lib::AcpClientMessage>,
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use sage_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

    // Managed-prefix gate: only `sage_com_*` tools enter entry-B.
