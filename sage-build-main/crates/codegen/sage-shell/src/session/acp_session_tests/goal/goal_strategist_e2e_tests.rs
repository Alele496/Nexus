...
//!
//! Tests mutate `SAGE_GOAL_CLASSIFIER` so they carry `serial`.
...
use sage_tools::implementations::sage_build::update_goal::UpdateGoalInput;
...
async fn answer_strategist(
    ...
    req: Box<sage_tools::implementations::sage_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
async fn make_actor(
    ...
    actor.tool_context.cwd =
        sage_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
