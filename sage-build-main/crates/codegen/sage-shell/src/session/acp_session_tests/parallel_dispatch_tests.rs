...
//!
//! These tests verify the parallel dispatch path (SAGE_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and sage_build's
...
#[test]
fn lock_path_for_args_matches_sage_build_file_path() {
    // sage_build search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_sage_build_target_file() {
    // sage_build read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_sage_build_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. sage_build search_replace + StrReplace
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use sage_sampling_types::{ConversationItem, SyntheticReason};
