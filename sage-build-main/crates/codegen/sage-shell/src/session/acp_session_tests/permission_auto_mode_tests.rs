...
use sage_acp_lib::AcpAgentGatewaySender;
use sage_paths::AbsPathBuf;
use sage_workspace::permission::{AccessKind, ClientType, spawn_permission_manager};
...
async fn set_auto_mode_path_wires_live_side_query_via_session_actor() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(d, sage_workspace::permission::Decision::Allow),
            ...
            let d2 = session
                ...
                .await;
            assert!(
                !matches!(d2, sage_workspace::permission::Decision::Allow),
    ...
    local
        .run_until(async {
            ...
            actor.permissions.set_classifier_transcript(vec![
                sage_workspace::permission::ClassifierTurn::UserText("please run tests".into()),
            ]);
...
fn build_classifier_turns_captures_tool_use_excludes_text_and_results() {
    use sage_workspace::permission::ClassifierTurn;
    let conv = vec![
        ...
        super::ConversationItem::assistant_tool_calls(vec![
            sage_sampling_types::conversation::ToolCall {
