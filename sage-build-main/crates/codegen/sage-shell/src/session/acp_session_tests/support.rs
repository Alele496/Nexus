...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> sage_computer_hub_sdk::ObservabilityBridge {
    sage_computer_hub_sdk::ObservabilityBridge::new(
        None,
        sage_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> sage_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> sage_agent::Agent {
    use sage_tools::implementations::sage_build::update_goal::UpdateGoalTool;
    use sage_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// Sage-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_sage_build_agent_with_todo() -> sage_agent::Agent {
    use sage_tools::implementations::sage_build::todo::TodoWriteTool;
    use sage_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> sage_agent::Agent {
    use sage_tools::implementations::sage_build::enter_plan_mode::EnterPlanModeTool;
    use sage_tools::implementations::sage_build::exit_plan_mode::ExitPlanModeTool;
    use sage_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<sage_tools::registry::types::ToolConfig>,
) -> sage_agent::Agent {
    test_agent_from_config(
        sage_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        sage_agent::AgentDefinition::default_sage_build(),
        std::sync::Arc::new(sage_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: sage_agent::AgentDefinition,
    backend: std::sync::Arc<dyn sage_tools::computer::types::TerminalBackend>,
) -> sage_agent::Agent {
    use sage_tools::computer::local::LocalFs;
    use sage_tools::computer::types::AsyncFileSystem;
    ...
    sage_agent::Agent::new(
        definition,
        sage_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        sage_agent::ReminderPolicy::default(),
        sage_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<sage_acp_lib::AcpClientMessage>,
    persistence_tx: tokio::sync::mpsc::UnboundedSender<PersistenceMsg>,
) -> (
    SessionActor,
    tokio::sync::mpsc::UnboundedReceiver<SessionEvent>,
) {
    let cwd = sage_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(sage_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    tool_context.task_completion_reservations =
        Some(sage_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(sage_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        permissions: sage_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
...
pub(crate) async fn build_actor() -> (
    ...
    let (gateway_tx, gateway_rx) =
        tokio::sync::mpsc::unbounded_channel::<sage_acp_lib::AcpClientMessage>();
