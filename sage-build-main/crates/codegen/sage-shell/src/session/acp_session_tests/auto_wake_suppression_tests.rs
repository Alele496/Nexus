use super::support::*;
use super::*;
use sage_tools::reminders::task_completion::consumed_completion_ids;
use sage_tools::types::output::{BashOutput, TextOutput, ToolOutput};
use sage_tool_types::{
...
async fn cancel_barrier_rejects_task_completion_wake_without_reporting_it() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                res.get::<sage_tools::types::resources::State<
                    sage_tools::reminders::task_completion::ReportedTaskCompletions,
                >>()
                .is_none(),
                "declined admission must not report before user re-engagement"
            );
            drop(res);
            let reminder = sage_tools::reminders::TaskCompletionReminder;
            let reminders = sage_tools::types::tool::Reminder::collect_reminders(
                &reminder,
                resources,
        ...
        .run_until(async {
            ...
            assert!(
                resources
                    .lock()
                    .await
                    .get::<sage_tools::types::resources::State<
                        sage_tools::reminders::task_completion::ReportedTaskCompletions,
                    >>()
                    .is_none(),
...
async fn genuine_user_start_consumes_deferred_completions_without_notification_turn() {
    ...
    local
        .run_until(async {
            ...
            let body = sage_tools::reminders::task_completion::format_monitor_completion(
                &sage_tools::types::TaskSnapshot {
                    ...
                    signal: None,
                    completed: true,
                    kind: sage_tools::computer::types::TaskKind::Monitor,
                    block_waited: false,
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use sage_tools::reminders::task_completion::ReportedTaskCompletions;
...
struct OneTaskTerminal {
    tasks: Vec<sage_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl sage_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: sage_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        sage_tools::computer::types::TerminalRunResult,
        sage_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<sage_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> sage_tools::computer::types::KillOutcome {
        sage_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<sage_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<sage_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> sage_tools::computer::types::TaskSnapshot {
    sage_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
