...
fn test_json_round_trip() {
    let ctx = sage_agent::PromptContext {
        ..Default::default()
    };

    let json = serde_json::to_string_pretty(&ctx).unwrap();
    let loaded: sage_agent::PromptContext = serde_json::from_str(&json).unwrap();
...
/// depend on `sage_home()` and `SessionInfo` path encoding).
#[test]
fn test_json_round_trip_via_filesystem() {
    ...
    let ctx = sage_agent::PromptContext::default();
    ...
    let loaded: sage_agent::PromptContext = serde_json::from_str(&read_json).unwrap();
...
fn test_canonical_artifacts_coexist() {
    ...
    let read_ctx: sage_agent::PromptContext = serde_json::from_str(
...
fn test_corrupt_json_returns_error() {
    ...
    let result: Result<sage_agent::PromptContext, _> = serde_json::from_str(&json);
...
//
// `sage_home()` is a process-wide `OnceLock`, so the real async method is
