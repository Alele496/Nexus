...
/// Env lane (desktop `SAGE_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`SAGE_CHAT_MODE`); hard-off in release builds.
...
mod tests {
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = sage_test_support::EnvGuard::set(
            "SAGE_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = sage_test_support::EnvGuard::unset("SAGE_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = sage_test_support::EnvGuard::set("SAGE_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = sage_test_support::EnvGuard::set("SAGE_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::SAGE_CHAT_MODE_ENV;
        let _chat_off = sage_test_support::EnvGuard::unset(SAGE_CHAT_MODE_ENV);
        let _desktop_off =
            sage_test_support::EnvGuard::unset("SAGE_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                sage_test_support::EnvGuard::set("SAGE_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = sage_test_support::EnvGuard::set(SAGE_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = sage_test_support::EnvGuard::unset(SAGE_CHAT_MODE_ENV);
        ...
        {
            let _on = sage_test_support::EnvGuard::set(SAGE_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
