...
use std::sync::Arc;
use sage_chat_state::compaction_utils::{
...
use sage_sampling_types::{ApiBackend, ConversationItem};
...
/// Override with `SAGE_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("SAGE_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    use sage_sampling_types::ConversationItem;
    #[test]
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("SAGE_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        sage_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("SAGE_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG SAGE_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(sage_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
/// (`sage-chat-state` `replace_conversation`, the authority): it matches the reseed's
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                sage_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            sage_telemetry::session_ctx::log_event(
                sage_telemetry::events::AutoCompactSuppressed {
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    sage_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    sage_chat_state::estimate_conversation_tokens(&full_conv),
                );
                if sage_token_estimation::exceeds_threshold(
                    projected_preserved,
                    context_window,
                    self.compaction.threshold_percent.get(),
                ) {
    ...
    async fn run_compact_inner(
        ...
        trigger: sage_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        ...
        let trigger_str = match trigger {
            sage_telemetry::events::CompactionTrigger::Manual => "manual",
            sage_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = sage_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            sage_hooks::event::HookEventName::PreCompact,
            sage_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            sage_chat_state::compaction_utils::prepare_conversation_for_segment(
        ...
        let simplified_messages = if verbatim_input_enabled {
            sage_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
                summary_strips_reasoning,
            )
        } else {
            sage_chat_state::compaction_utils::prepare_conversation_for_summarization(
                full_conversation,
            )
        };
        if conv_len == 0 {
        ...
        let effective_tool_defs: Vec<sage_sampling_types::ToolDefinition> = self
        ...
        let compaction_tool_tokens =
            sage_chat_state::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<sage_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(sage_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<sage_sampling_types::HostedTool> =
            if use_backend_search {
        ...
        let estimated_input_tokens =
            sage_chat_state::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, sage_telemetry::events::CompactionTrigger::Auto);
        ...
        let fr_config = sage_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match sage_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(sage_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(sage_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(sage_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            sage_telemetry::session_ctx::log_event(
                                sage_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = sage_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    sage_chat_state::compaction_utils::fit_conversation_to_budget(
                                        verbatim, budget,
                                    )
                                }
                                InputStage::Lossy => {
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                sage_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                sage_tools::computer::types::TaskKind::Bash => {
                    ...
                    let todos = {
                        ...
                        use sage_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn sage_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn sage_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : sage_telemetry::memory_log::TARGET, count = n,
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        sage_sampling_types::ContentPart::Text { text } => {
        ...
        self.dispatch_hook(
            sage_hooks::event::HookEventName::PostCompact,
            sage_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) fn should_auto_compact(
        ...
        if sage_token_estimation::exceeds_threshold(
            ...
            let percentage = sage_token_estimation::usage_percentage_u8(total_tokens, cw);
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &sage_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = sage_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
    ...
    pub(crate) async fn run_compact_only(
        ...
        sage_telemetry::session_ctx::log_event(sage_telemetry::events::AutoCompactFired {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                sage_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<sage_sampling_types::ToolSpec>,
        user_context: Option<&str>,
...
mod inline_auto_compact_flow_tests {
    ...
    use tokio::sync::mpsc;
    use sage_paths::AbsPathBuf;
    use sage_workspace::file_system::MockFs;
    use sage_workspace::permission::PermissionHandle;
    #[derive(Debug)]
    ...
    async fn create_test_actor(
        ...
        gateway_tx: mpsc::UnboundedSender<sage_acp_lib::AcpClientMessage>,
        ...
        let hunk_tracker_handle = sage_hunk_tracker::HunkTrackerActor::spawn(
            ...
            cwd.to_path_buf(),
            hunk_tx,
            sage_hunk_tracker::TrackingMode::AgentOnly,
        ...
        let chat_state_handle = sage_chat_state::ChatStateActor::spawn(
            vec![],
            sage_sampling_types::SamplingConfig {
            ...
            Box::new(sage_chat_state::NullChatPersistence),
            chat_event_tx,
            tokio_util::sync::CancellationToken::new(),
        );
        ...
        SessionActor {
            ...
            compaction: crate::session::compaction_config::CompactionConfig {
                ...
                compaction_mode: sage_chat_state::CompactionMode::Transcript,
                verbatim_input: true,
            ...
            extension_registry: sage_agent_lifecycle::LocalExtensionRegistry::default(),
            ...
            vcs_kind: sage_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: sage_sampler::SamplerHandle::noop(),
        ...
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    mpsc::unbounded_channel::<sage_acp_lib::AcpClientMessage>();
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use std::sync::atomic::Ordering::Relaxed;
        use sage_test_support::MockInferenceServer;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) = mpsc::unbounded_channel();
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("API error (status 402 Payment Required): Sage usage balance exhausted"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
            classify("Sage usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> sage_sampler::SamplingErrorInfo {
        sage_sampler::SamplingErrorInfo {
            kind: sage_sampler::SamplingErrorKind::Api,
    ...
    async fn test_compact_on_error_triggers_when_tokens_exceed_new_window() {
        ...
        local
            .run_until(async {
                let (gateway_tx, _) = mpsc::unbounded_channel::<sage_acp_lib::AcpClientMessage>();
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = sage_sampler::SamplingErrorInfo {
                    kind: sage_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
    ...
    async fn get_transcript_path_returns_some_when_file_exists() {
        ...
        local
            .run_until(async {
                ...
                let mut actor =
                    create_test_actor(50_000, 200_000, 85, gateway_tx, persistence_tx).await;
                actor.compaction.compaction_mode = sage_chat_state::CompactionMode::Transcript;
                ...
                actor.compaction.compaction_mode = sage_chat_state::CompactionMode::Summary;
