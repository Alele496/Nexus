...
//!
//! Receives [`sage_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`sage_mcp::liveness`]),
//! - the [`sage_mcp::servers::GrokClientHandler`] (server-pushed
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`sage_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `sage_com_` prefix)
    /// or `local` (user `.sage/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`sage_mcp::servers::ClientStateKind::Ready`]
...
/// [`sage_mcp::servers::McpClientEventKind`]). Used as the
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && sage_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///
/// `gateway` is a [`sage_acp_lib::AcpAgentGatewaySender`] (forwarded
...
mod tests {
    ...
    fn managed_handshake_auth_rejection_maps_to_needs_auth() {
        let key = (
            "sage_com_notion".to_string(),
            McpClientEventKind::HandshakeFailed,
        );
        let ev = McpClientEvent::HandshakeFailed {
            server: "sage_com_notion".to_string(),
    ...
    fn managed_handshake_non_auth_stays_unavailable() {
        for reason in ["403 Forbidden", "cli-chat-proxy returned 502"] {
            ...
            let ev = McpClientEvent::HandshakeFailed {
                server: "sage_com_slack".to_string(),
    ...
    /// Contract: managed server names (starting with `sage_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(classify_source("sage_com_linear"), McpServerSource::Managed);
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use sage_mcp::servers::{McpClient, McpState};
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("sage_com_slack"), // managed
    ...
    fn discard_gateway() -> sage_acp_lib::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        sage_acp_lib::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = sage_acp_lib::AcpAgentGatewaySender::new(gw_tx);
