...
use sage_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use sage_agent::prompt::skills::SkillsConfig;
use sage_sampler::SamplerConfig as SamplingConfig;
use sage_sampling_types::truncate_bytes;
...
use sage_tools::types::compat::CompatConfig;
use sage_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use sage_workspace::file_system::CodebaseIndexManager;
use sage_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use sage_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
impl sage_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        sage_tools::types::resources::ManagedGatewayToolCallResponse,
        sage_tool_runtime::ToolError,
    > {
        let auth_key = self
            .auth_manager
            .get_valid_token()
            ...
            .ok_or_else(|| sage_tool_runtime::ToolError::unauthorized("no auth token available"))?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                sage_tool_runtime::ToolError::unauthorized(detail)
            } else if status == reqwest::StatusCode::FORBIDDEN {
                sage_tool_runtime::ToolError::permission_denied(detail)
            } else {
                let tool_id = sage_tool_protocol::ToolId::new(caller)
                    .unwrap_or_else(|_| sage_tool_protocol::ToolId::new("use_tool").expect("valid"));
                sage_tool_runtime::ToolError::execution(tool_id, detail)
            };
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            sage_tool_runtime::ToolError::network_error(format!(
                "Managed MCP gateway tool call failed: {}",
                e.without_url()
            ))
        }
        crate::session::managed_mcp::ManagedMcpFetchError::NoAuth => {
            sage_tool_runtime::ToolError::unauthorized("no auth token available")
        }
    }
}
#[cfg(test)]
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, sage_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(err.kind, sage_tool_runtime::ToolErrorKind::PermissionDenied);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, sage_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, sage_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// `sage-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<sage_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: sage_chat_state::ChatStateHandle,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<sage_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<sage_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-sage-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<sage_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<sage_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<sage_agent::Agent>,
    ...
    /// Kill-switch (`SAGE_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<sage_tools::implementations::sage_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, sage_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: sage_agent_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<sage_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<sage_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<sage_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: sage_computer_hub_sdk::ObservabilityBridge,
    ...
    /// Handle to the per-session `sage-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: sage_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`sage_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`sage_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: sage_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    /// Fire-and-forget — failures are logged but do not interrupt the turn.
    async fn send_after_turn_event(&self, payload: sage_tool_protocol::turn_hook::AfterTurnPayload) {
        self.workspace_ops
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == sage_tools::implementations::sage_build::SCHEDULER_CREATE_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<sage_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `sage prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &sage_agent::PromptContext) {
...
) -> Option<sage_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use sage_tools::types::output::{MCPOutput, ToolOutput};
    use sage_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl sage_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl sage_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> sage_tool_protocol::ToolId {
            sage_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::sage_tool_runtime::ListToolsContext,
        ) -> sage_tool_types::ToolDescription {
            sage_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
            _ctx: sage_tool_runtime::ToolCallContext,
            _args: serde_json::Value,
        ) -> Result<ToolOutput, sage_tool_runtime::ToolError> {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use sage_tools::registry::types::ToolConfig;
    use sage_tools::tool_taxonomy::TOOL_META_KEY;
    use sage_workspace::permission::PermissionCommand;
...
mod managed_gateway_tool_tests {
    ...
    use sage_tools::types::output::{MCPOutput, ToolOutput};
    use sage_tools::types::tool::{ToolKind, ToolNamespace};
    use sage_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<sage_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
