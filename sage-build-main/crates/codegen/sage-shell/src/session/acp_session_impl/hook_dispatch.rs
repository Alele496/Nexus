...
) -> sage_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> sage_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use sage_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> sage_hooks::runner::RunContext<'_> {
        sage_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[sage_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, sage_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: sage_hooks::event::HookEventName,
        payload: sage_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            sage_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                sage_hooks::result::HookRunResult::Success {
                    ...
                    hook_name,
                    elapsed,
                    sage_telemetry::events::HookOutcome::Success,
                ),
                sage_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    sage_telemetry::events::HookOutcome::Error,
                ),
                sage_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            sage_telemetry::session_ctx::log_event(sage_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: sage_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
