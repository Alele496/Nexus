...
pub(super) fn try_send_ack(
    ack_tx: Option<
        ...
        >,
    >,
    ack: sage_tools::implementations::sage_build::update_goal::UpdateGoalAck,
) {
...
/// `Some` only when the planner is enabled (`SAGE_GOAL_PLANNER`) and a plan
...
/// Override with `SAGE_GOAL_REVERIFY_AFTER` (floored at 1).
...
pub(crate) fn goal_slash_and_harness_available(goal_enabled: bool, tool_names: &[String]) -> bool {
    use sage_tools::implementations::sage_build::UPDATE_GOAL_TOOL_NAME;
...
impl SessionActor {
    ...
    pub(super) async fn set_goal_loop_active_resource(&self, active: bool) {
        ...
        self.agent
            ...
            .tool_bridge()
            .update_resource(
                sage_tools::implementations::sage_build::task::types::GoalLoopActive(active),
            )
            .await;
    }
}
