...
fn partition_rules_by_scope(
    files: Vec<sage_agent::prompt::agents_md::AgentConfigFile>,
    sage_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_root: Option<&std::path::Path>,
) -> (
    Vec<sage_agent::prompt::user_message::RuleEntry>,
    Vec<sage_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = sage_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use sage_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[sage_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
        entries.iter().map(|entry| entry.content.as_str()).collect()
    }
    #[test]
    fn partitions_custom_sage_and_vendor_home_rules_as_user_scope() {
        let files = vec![
    ...
    #[test]
    fn sage_home_nested_in_workspace_keeps_direct_surfaces_user_scoped() {
        let files = vec![
            file("/repo/config/AGENTS.md"),
            file("/repo/config/rules/global.md"),
            file("/repo/config/.sage/rules/project.md"),
    ...
    #[test]
    fn nested_sage_home_workspace_files_stay_workspace_scoped() {
        let files = vec![
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(sage_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: sage_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use sage_agent::prompt::agents_md::read_agents_config_with_paths;
        use sage_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use sage_workspace::file_system::{git_status_short, jj_status};
        use sage_workspace::session::git::VcsKind;
    ...
    ) -> Vec<sage_agent::prompt::user_message::McpServerEntry> {
        use sage_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &sage_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
        ...
        let client = sage_sampler::SamplingClient::new(sampler_config).map_err(|e| {
