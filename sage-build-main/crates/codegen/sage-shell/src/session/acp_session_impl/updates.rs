...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, sage_chat_state::UsageTotals)],
        ...
        match &notification.update {
            XaiSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    sage_hooks::event::HookEventName::SubagentStart,
                    None,
                    sage_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = sage_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        sage_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
            ...
            XaiSessionUpdate::SubagentFinished {
                ...
                let envelope = self.fire_hook(
                    sage_hooks::event::HookEventName::SubagentEnd,
                    None,
                    sage_hooks::event::HookPayload::SubagentStop {
                        subagent_id: subagent_id.clone(),
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            sage_hooks::event::HookEventName::Notification,
            None,
            sage_hooks::event::HookPayload::Notification {
