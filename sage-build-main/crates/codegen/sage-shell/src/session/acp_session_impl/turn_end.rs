...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            sage_workspace::session::git::get_worktree_info(cwd),
            sage_workspace::session::git::get_branch(cwd),
        );
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<sage_tools::implementations::sage_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) async fn handle_completion(&self, prompt_id: String, result: PromptTurnResult) {
        ...
        // replayed `_sage.local/session/update` rail so a viewer that re-attaches
    ...
    pub(super) fn classify_install_error(
        err: &sage_agent::plugins::install_registry::InstallError,
    ) -> String {
