...
/// - `file_path` — sage_build (`search_replace`), opencode (`EditTool`,
///   `WriteTool`, `ReadTool`), codex (`read_file`), sage_build_hashline
///   (`hashline_edit`)
/// - `path` — alternate edit/read tools
/// - `target_file` — sage_build (`read_file`, via `#[serde(rename)]`)
///
...
/// `list_dir`=`target_directory`. Grammar lives in `sage_chat_state`.
pub(super) fn compaction_artifact_read(
    args: &serde_json::Value,
) -> Option<sage_chat_state::compaction_transcript::CompactionArtifact> {
    let path = str_arg(
        args,
        &["target_file", "file_path", "path", "target_directory"],
    )?;
    sage_chat_state::compaction_transcript::classify_compaction_path(path)
}
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = sage_tools::util::strip_redundant_session_cd(
            &command,
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(sage_tools::types::tool::ToolKind::Execute)
        };
