...
//! (`sage_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](sage_compaction::sample_full_replace_summary);
...
//! [`FullReplaceError`](sage_compaction::FullReplaceError).
...
use sage_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use sage_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    sage_telemetry::session_ctx::log_event(CompactionRetryDegraded {
