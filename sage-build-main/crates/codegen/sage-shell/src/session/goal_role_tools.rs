...
pub(crate) struct RoleToolNames {
    ...
    /// (the default sage-build host's `search_replace` mutator) when `Write`
...
impl RoleToolNames {
    ...
    /// `search_replace` on the default sage-build host) instead of the literal
    ...
    pub(crate) fn from_summary(
        ...
        Self::from_parts(
            ...
            get(ToolKind::Search),
            // The default sage-build host's pre-spawn describe probe exposes only
...
pub(crate) mod tests {
    ...
    #[test]
    fn from_summary_uses_sage_build_names() {
        // A sage-build toolset: client names match the literal defaults.
    ...
    fn web_tools_fall_back_when_absent_from_the_toolset() {
        ...
        // on the default sage-build host (the stock `web_search`/`web_fetch`).
    ...
    #[test]
    fn from_summary_write_falls_back_to_edit_on_default_sage_build_host() {
        // Default sage-build host: the pre-spawn describe probe exposes only
    ...
    fn from_parent_write_falls_back_to_edit_when_bridge_has_no_write() {
        // Default sage-build parent bridge: no `Write`, only `Edit`
    ...
    #[test]
    fn parent_and_summary_renders_agree_on_default_sage_build_mutator() {
        ...
        // default sage-build host (Edit-only toolset), so a fail-open retry
