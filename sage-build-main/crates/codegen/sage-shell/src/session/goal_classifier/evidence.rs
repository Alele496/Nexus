    ...
    builder
        ...
        .filter_entry(|entry| {
            ...
            entry
                ...
                .map(|name| {
                    name != ".git"
                        && !sage_file_utils::skip_dir_set().contains(name.to_lowercase().as_str())
                })
                .unwrap_or(true)
        });
...
mod tests {
    use super::*;
    use sage_sampling_types::{AssistantItem, UserItem};
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![sage_sampling_types::ContentPart::Text { text: text.into() }],
    ...
    fn evidence_packet_with_file_path_renders_path_not_content() {
        let packet = build_classifier_evidence_packet(
            ...
            Some(Path::new("/home/u/.sage/sessions/s1/goal/plan.md")),
            None,
            "I did it.",
        );
        assert_eq!(
            ...
            "OBJECTIVE:\ndo X\n\n\
             ...
             CHANGED_FILES:\n- js/main.js\n\n\
             PLAN_FILE: /home/u/.sage/sessions/s1/goal/plan.md\n\n\
             PLAN_CHANGES: (none)\n\n\
             FINAL_RESPONSE:\nI did it.\n",
        );
    }
    ...
    async fn capture_changes_diff_walkdir_skips_all_well_known_directories() {
        let tmp = tempfile::tempdir().unwrap();
        let goal_created_at = now_unix_seconds() - 60;
        for sub in sage_file_utils::SKIP_DIR_NAMES {
