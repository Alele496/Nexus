//! Data APIs for `sage models`. Clients own display.
...
/// Status for the `sage models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `sage_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.sage_com_config.sage_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_xai_api_key(
            agent_config.sage_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    ...
    ///
    /// Uses `SAGE_AUTH_PATH` (not `SAGE_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("SAGE_AUTH"),
            EnvGuard::set("SAGE_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("SAGE_DEPLOYMENT_KEY"),
            EnvGuard::unset("SAGE_WS_ORIGIN"),
            EnvGuard::unset("SAGE_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("SAGE_AUTH", &json);

        assert_eq!(
            AuthStatus::resolve(&Config::default()),
            AuthStatus::LoggedIn("sage.local".to_owned())
        );
    }
