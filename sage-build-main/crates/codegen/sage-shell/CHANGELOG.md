...
- **/btw** now works inside `sage --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **Global rules** from ~/.sage/rules and compatible vendor homes are now discovered correctly.
...
- **SSH sessions** now show a one-time tip recommending `sage wrap ssh <host>` for clipboard and terminal restore.
...
- **sage** and the agent binary now stay in sync even when no update is installed.
- **Copying** a multiline queued prompt now copies the complete text instead of a collapsed summary.
- **sage wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `sage login`.
...
- **sage login** now requests Grok Projects scopes so workspace listing works after consent.
...
- **`sage plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using sage -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **sage inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct sage --minimal --resume command.
...
- **sage setup --json** prints your team's managed configuration without installing it.
...
- **`sage worktree ls`** now works as a short alias for `sage worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.sage/config.toml`.
...
- **Resumed sage.local chats** now use the conversation's last model instead of the gateway default.
...
- **File links and media** now work for worktree sessions under ~/.sage/worktrees/.
...
- **Terminal command cards** on sage.local now correctly settle after foreground bash tasks.
...
- **sage inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain sage launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive sage agent stdio without silent parse drops on session/* calls.
...
- **sage --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- The "Managed by sage.local" link in the Extensions modal is now clickable and underlined.
...
- **sage --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **sage update** on Windows no longer fails when a previous .old executable is still running.
...
- **sage models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `sage plugin marketplace add`.
...
- **Plan files** now default to .sage/plan.md to match Grok conventions.
...
- **--minimal** flag now shows in `sage --help`.
...
- **Tool result previews** no longer paint opaque panels in `sage --minimal`.
- **sage wrap** now correctly handles quoted strings and shell aliases.
...
- **Chat sessions show the sage.local model catalog** in the picker.
...
- **`sage update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `sage wrap`.
...
- **sage agent stdio** now checks for updates in the background like other modes.
...
- **sage wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `sage agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `sage wrap`** to run any command with local clipboard support.
...
- **Fixed `sage agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or sage-desktop.
...
- You can now pass --json-schema to sage -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `SAGE_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **sage -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in sage -p and sage agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `sage` after running `sage update`.
- Headless (`sage -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`sage plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **sage sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **sage quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, sage sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, SAGE_HUNK_TRACKER, or config.
...
- **`sage dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`sage mcp add`** now accepts positional arguments (e.g. `sage mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`sage update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.sage.
...
- **MCP servers menu** and slash commands now work when starting sage outside a project directory.
...
- **sage --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `sage --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `sage update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`sage --debug`** now produces per-session log files under ~/.sage/debug/ even with a leader process.
...
- **`sage update`** now restarts any older running leader so all clients get the new binary.
...
- **`sage login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `sage agent --leader` without forced interactive login or timeouts.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the SAGE_IMAGE_GEN_HARNESS env var.
...
- **sage inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via SAGE_CURSOR_MCPS_ENABLED / SAGE_CURSOR_HOOKS_ENABLED.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the sage-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality sage-imagine-image-quality model.
