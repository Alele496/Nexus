...
use sage_announcements::RemoteAnnouncement;
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `sage_build_settings`
    ...
    #[serde(default)]
    pub sage_oauth_enabled: Option<bool>,
    #[serde(default)]
    ...
    /// default (on). Sits below env `SAGE_FOLDER_TRUST`, user
    ...
    /// remote settings `sage_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `sage_build_settings` remote settings key) to enable it. See
    ...
    /// `sage_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `SAGE_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `sage_build_settings`).
    ...
    /// ghost text), from the `sage_build_settings` remote settings flag. Sits below
    /// env (`SAGE_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    ...
    /// Overridable locally via `SAGE_VOICE_MODE`. Free-tier SuperGrok upsell
    ...
    /// User's subscription tier from remote settings `sage_build_access_gate`.
    /// E.g. "free", "premium", "supergrok", "supersage_heavy".
    ...
    /// `sage_build_access_gate` targeting rules. `None` = no server response
    ...
    /// Server-controlled via the remote settings `sage_build_usage_redirect_url`
    ...
    /// `sage_build_settings`. Per-model override on `ModelInfo`
    /// (`sage_build_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`SAGE_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`SAGE_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`SAGE_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`SAGE_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// workspace exposure), from `sage_build_settings.workspace_command_enabled`.
