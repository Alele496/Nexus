...
//!
//! TODO: Move from sage-shell/src/sampling/error.rs
...
/// Flat error from the Sage proxy/gateway: `{"code": "...", "error": "..."}`.
#[derive(Debug, Deserialize)]
...
pub fn status_user_message(status: StatusCode) -> String {
    match status.as_u16() {
        code @ 502..=504 => {
            format!("Nexus is temporarily unavailable. Please try again in a moment. (HTTP {code}).")
        }
        // Cloudflare edge codes (origin down / connect fail / timeout / …).
        code @ 520..=524 => {
            format!(
                "Connection to Sage timed out or was interrupted. Please try again. (HTTP {code})."
            )
        }
...
<html lang="en-US">
<head><title>sage.local | 524: A timeout occurred</title></head>
