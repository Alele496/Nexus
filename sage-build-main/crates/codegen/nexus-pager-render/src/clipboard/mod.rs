...
pub use nexus_ratatui_textarea::{ClipboardProvider, InternalClipboard};
...
fn is_remote() -> bool {
    ...
    *REMOTE.get_or_init(nexus_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(nexus_shared::clipboard::is_containerized_without_display)
}
...
///
/// Two names are accepted: the canonical `NEXUS_OSC52_SINK` (inherited by local
/// children) and the `LC_`-prefixed `LC_NEXUS_OSC52_SINK`, which the default
...
pub fn osc52_sink_active() -> bool {
    ...
    *SINK.get_or_init(|| {
        std::env::var_os("NEXUS_OSC52_SINK").is_some()
            || std::env::var_os("LC_NEXUS_OSC52_SINK").is_some()
    })
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if nexus_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = nexus_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = nexus_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`nexus_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        nexus_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = nexus_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match nexus_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
fn log_clipboard_copy_event(
    ...
    if !nexus_telemetry::client::is_enabled() {
        return;
    }
    nexus_telemetry::session_ctx::log_event(nexus_telemetry::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    nexus_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !nexus_shared::clipboard::x11_display_env_present() {
        return None;
    }
    nexus_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && nexus_shared::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    nexus_telemetry::session_ctx::log_event(
        nexus_telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn log_clipboard_paste_event(
    ...
    nexus_telemetry::session_ctx::log_event(nexus_telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match nexus_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use nexus_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    nexus_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    nexus_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    nexus_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(nexus_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match nexus_shared::clipboard::get_image() {
        Ok(img) => {
