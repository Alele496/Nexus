...
//! The default theme is SageNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! SageNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::SageNight, ThemeKind::SageDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::SageNight => "sagenight",
            Self::TokyoNight => "tokyonight",
            Self::SageDay => "sageday",
    ...
    /// when quantized to 256 or 16 colors. SageNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::SageNight => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "sagenight" | "sage-night" | "dark" => Some(Self::SageNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "sageday" | "sage-day" | "light" | "day" => Some(Self::SageDay),
...
/// `"sagenight"` → `"Nexus Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "sagenight" => "Nexus Night",
        "sageday" => "Nexus Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.nexus/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::SageNight => Self::sagenight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::SageDay => Self::sageday(),
            ...
            // if reached, fall back to SageNight.
            ThemeKind::Auto => Self::sagenight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `NEXUS_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on sagenight, erasing every state signal). We pin each
    ...
    /// theme (e.g., SageNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // SageNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    nexus_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::SageNight.is_auto());
        assert!(!ThemeKind::SageDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::sageday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::sagenight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::sageday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. SageNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::SageNight => Theme::sagenight(),
                ThemeKind::SageDay => Theme::sageday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_sagenight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark SageNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::sagenight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("sagenight"),
            Some(ThemeKind::SageNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::SageNight));
        assert_eq!(ThemeKind::from_name("sageday"), Some(ThemeKind::SageDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::SageDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("sagenight", ThemeKind::SageNight),
            ("sage-night", ThemeKind::SageNight),
            ("dark", ThemeKind::SageNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("sageday", ThemeKind::SageDay),
            ("sage-day", ThemeKind::SageDay),
            ("light", ThemeKind::SageDay),
            ("day", ThemeKind::SageDay),
