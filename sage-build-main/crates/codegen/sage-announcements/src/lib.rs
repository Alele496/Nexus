...
/// Read hidden announcement ids from `~/.sage/announcements.json`.
...
/// Write hidden announcement ids to `~/.sage/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    sage_tools::util::sage_home::sage_home().join("announcements.json")
}
...
///
/// Precedence: `SAGE_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("SAGE_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid SAGE_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("SAGE_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("SAGE_ANNOUNCEMENTS_OVERRIDE");
        }
    }
