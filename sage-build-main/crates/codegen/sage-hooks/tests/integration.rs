...
use sage_hooks::discovery::load_hooks;
use sage_hooks::dispatcher;
use sage_hooks::event::*;
use sage_hooks::result::HookDecision;
use sage_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        sage_hooks::result::HookRunResult::Success { .. }
    ));
}
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$SAGE_HOOK_EVENT" > {f}; echo "NAME=$SAGE_HOOK_NAME" >> {f}; echo "SESSION=$SAGE_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`SAGE_HOOK_EVENT`, `SAGE_HOOK_NAME`,
/// `SAGE_SESSION_ID`, `SAGE_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$SAGE_HOOK_EVENT" > {f}; echo "NAME=$SAGE_HOOK_NAME" >> {f}; echo "SESSION=$SAGE_SESSION_ID" >> {f}; echo "ROOT=$SAGE_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "SAGE_HOOK_EVENT": "spoofed_event",
                                "SAGE_HOOK_NAME": "spoofed_name",
                                "SAGE_SESSION_ID": "spoofed_session",
                                "SAGE_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "SAGE_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed SAGE_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "SAGE_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed SAGE_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "SAGE_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed SAGE_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use sage_hooks::result::HookRunResult;
    let info = match run {
