...
pub struct RawHandler {
    ...
    /// keys (`SAGE_HOOK_EVENT`, `SAGE_HOOK_NAME`, `SAGE_SESSION_ID`,
    /// `SAGE_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at load
...
pub struct HookSpec {
    ...
    ///    (`SAGE_HOOK_EVENT`, `SAGE_HOOK_NAME`, `SAGE_SESSION_ID`,
    ///    `SAGE_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at
    ...
    ///    `SAGE_PLUGIN_ROOT`, `CLAUDE_PLUGIN_ROOT`, `SAGE_PLUGIN_DATA`,
    ...
    ///    (`SAGE_HOOK_EVENT`, `SAGE_HOOK_NAME`, `SAGE_SESSION_ID`,
    ///    `SAGE_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`). These are
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.sage/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.sage/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "SAGE_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "SAGE_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "SAGE_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "SAGE_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "SAGE_HOOKS_PARSE_TEST_MATCHER_VAR";
    ...
    fn parse_hook_file_matcher_braced_var_is_not_env_expanded() {
        ...
        let key = "SAGE_HOOKS_PARSE_TEST_MATCHER_BRACED";
    ...
    /// User attempts to set runner-reserved keys (SAGE_HOOK_*,
    /// SAGE_SESSION_ID, SAGE_WORKSPACE_ROOT, CLAUDE_PROJECT_DIR) via
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "SAGE_HOOK_EVENT": "spoofed",
                                    "SAGE_HOOK_NAME": "spoofed",
                                    "SAGE_SESSION_ID": "spoofed",
                                    "SAGE_WORKSPACE_ROOT": "/etc",
