...
//! `nexus_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `NEXUS_POOL_*` /
//! `NEXUS_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `NEXUS_SAMPLER_SHARED_CLIENT=0` falls back to
...
// the same type via `nexus_sampler::OriginClientInfo`. The shell-specific
...
/// `NEXUS_CLIENT_VERSION` env vars. Returns `None` when
/// `NEXUS_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`NEXUS_CLIENT_NAME` env var, default `"sage-shell"`).
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn nexus_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(nexus_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
