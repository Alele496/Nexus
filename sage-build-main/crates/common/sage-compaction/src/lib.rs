...
//! stay in each product host (for example `sage-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `sage-sampling-types`. It is decoupled from both Sage chat and
//! sage-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — sage-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Sage chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Sage chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Sage chat and
//! sage-build; hosts still own snapshotting and host-only sections).
...
// sage-build's full-replace subsystem now lives under `code_compaction`;
