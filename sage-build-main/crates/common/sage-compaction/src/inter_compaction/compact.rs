...
//! extraction, chunk summarisation, and the final `<sage_user_queries>`
...
pub struct ChunkedCompactionOutput {
    /// `<sage_user_queries>` preamble + `<chunk_summary index="i">` blocks.
...
/// 2. [`separate_prior_user_queries`] — split prior `<sage_user_queries>`
...
pub async fn sample_compaction_chunked<T: CompactionItemBuilder + Send + Sync>(
    ...
    // Step 2 — split prior `<sage_user_queries>` out of every prior
