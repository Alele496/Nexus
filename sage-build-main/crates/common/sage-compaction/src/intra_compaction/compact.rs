...
/// `<sage_user_queries>` preamble: the shared `code_compaction` summarizer
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<sage_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<sage_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<sage_user_queries>` preamble below — stays intra's.
    ...
    // 3c. For `History` target, prepend a `<sage_user_queries>` preamble so
