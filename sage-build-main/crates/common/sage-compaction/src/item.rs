...
//! sage-build implements it for `sage_sampling_types::ConversationItem`.
//!
...
//! `<sage_user_queries>` blocks, drop tool content from assistant turns,
...
/// extraction (`<sage_file id=".." name=".." />` lines in the
/// `<sage_user_queries>` preamble).
...
pub trait CompactionItem {
    ...
    /// strips their `<sage_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<sage_file>` lines in the
    /// `<sage_user_queries>` preamble. Empty for items without attachments.
    ///
