...
//!
//! The canonical `sample → classify → retry` loop, used by **both** sage-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Sage chat's intra `Shared` summarizer
...
//! the winning summary (sage-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the sage-build host uses to step down its input size).
