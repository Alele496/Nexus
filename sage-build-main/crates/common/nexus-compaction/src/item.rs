...
//! sage-build implements it for `nexus_sampling_types::ConversationItem`.
//!
...
//! `<nexus_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `GrokRole` (Sage chat) and the
/// `ConversationItem` variants (sage-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Sage chat) — maps to System on harnesses without a
...
/// extraction (`<nexus_file id=".." name=".." />` lines in the
/// `<nexus_user_queries>` preamble).
...
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Sage chat's
    ...
    /// Whether this item carries a *prior compaction summary* (Sage chat: a
    ...
    /// strips their `<nexus_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<nexus_file>` lines in the
    /// `<nexus_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Sage chat: a `Developer` turn with category
    ...
    /// requests/results (Sage chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up sage-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Sage chat's
/// `GrokTurn`; adding these constructors to it as required methods would break
/// that impl. They are also sage-build-specific (Sage chat's tail-keep path
...
///
/// The sage-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Sage chat stores turns as `Arc<GrokTurn>`).
