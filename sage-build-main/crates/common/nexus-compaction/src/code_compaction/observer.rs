//! Observability seam for the full-replace (sage-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (sage-build:
...
//!
//! Emission points are part of the behavior contract: the sage-build observer
