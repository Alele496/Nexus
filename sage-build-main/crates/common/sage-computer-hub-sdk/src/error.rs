...
//!
//! Wire-level [`sage_tool_protocol::ToolErrorWire`] variants and JSON-RPC
...
use thiserror::Error;
use url::Url;
use sage_tool_protocol::{IdError, JsonRpcError, ToolCallId, ToolErrorWire};
...
pub enum ClientError {
    ...
    /// [`sage_tool_runtime::ToolCallContext::default`], which does so)
...
mod tests {
    ...
    fn sdk_reexported_recognizer_matches_decoded_error() {
        ...
        let err = sage_computer_hub_core::error_from_envelope(workspace_gone_envelope());
    ...
    fn sdk_reexported_recognizer_rejects_unrelated_custom_error() {
        ...
        let err = sage_computer_hub_core::error_from_envelope(env);
        assert!(!crate::is_workspace_unavailable(&err));
    }
}
