...
/// A built-in subagent type shared by the CLI (`sage-agent`) and other
...
mod tests {
    ...
    fn task_tool_input_model_parses_explicit() {
        let input: TaskToolInput =
            serde_json::from_str(r#"{"description": "d", "prompt": "p", "model": "sage-3"}"#)
                .unwrap();
        assert_eq!(input.model.as_deref(), Some("sage-3"));
    }
    ...
    fn sanitize_optional_arg_moves_when_no_trim() {
        assert_eq!(
            sanitize_optional_arg(Some("sage-3".into())).as_deref(),
            Some("sage-3")
        );
        assert_eq!(
            sanitize_optional_arg(Some("  sage-3  ".into())).as_deref(),
    ...
    // match what the sage-shell MiniJinja templates render for the default
    // sage-build toolset (monitor + task + bash + read present, POSIX). The
